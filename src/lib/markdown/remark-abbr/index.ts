        ...
        while ((index = text.indexOf(abbr, index)) !== -1) {
          ...
          if (beforeOk && afterOk) {
            ...
            const overlaps = replacements.some(
              (r) => (index >= r.start && index < r.end) || (index + abbr.length > r.start && index < r.end)
            );
...
const remarkAbbr: RemarkAbbr = function (this: Processor, options = {}) {
  ...
  const toMarkdownExtensions = (processorData['toMarkdownExtensions'] as Array<Record<string, unknown>>) || [];
