...
describe('remark-captions (Layer 1: Isolated)', () => {
  it('should wrap captioned elements in figure/figcaption', async () => {
    ...
    const html = await processIsolated({
      ...
      pluginOptions: {
        external: { table: 'Table:', code: 'Code:' },
        internal: { blockquote: 'Source:', image: 'Figure:' },
      },
      gfm: true,
      remarkRehypeOptions: remarkRehypeConfig,
    })

    // At least one <figure> per captioned element
