...
describe('detectEmbedPlatform', () => {
  it('detects platforms by hostname (not substring)', () => {
    ...
    expect(detectEmbedPlatform('https://www.reddit.com/r/webdev/comments/abc123/test')).toBe('reddit')
  ...
  it('does not treat arbitrary URLs containing provider strings as trusted', () => {
    expect(detectEmbedPlatform('https://evil.example/?next=https://codepen.io/user/pen/abc')).toBe('x')
  ...
  it('detects Mastodon by path shape across instances', () => {
    ...
    expect(detectEmbedPlatform('https://example.instance/users/user/statuses/123456789')).toBe('mastodon')
  })
})
