...
describe('Consent checkbox selectors', () => {
  it('locates the checkbox and error nodes within the component scope', async () => {
    await renderSelectors(({ element }) => {
      ...
      expect(checkbox, 'ConsentCheckbox should include a checkbox input').toBeInstanceOf(
        HTMLInputElement
      )
      expect(checkbox.id, 'ConsentCheckbox checkbox id should match default fixture id').toBe(
        'gdpr-consent'
      )
      ...
      expect(
        error,
        'ConsentCheckbox should include an error element for the checkbox'
      ).toBeInstanceOf(HTMLDivElement)
      expect(error.id, 'ConsentCheckbox error element id should be derived from checkbox id').toBe(
        'gdpr-consent-error'
      )
    })
  })
  ...
  it('throws ClientScriptError when required component-scoped nodes are missing', async () => {
    await renderSelectors(({ element }) => {
      ...
      expect(
        () => getConsentCheckboxInput(element),
        'getConsentCheckboxInput should throw when input is missing'
  ...
  it('locates the default checkbox, container, description, and error nodes', async () => {
    await renderSelectors(() => {
      ...
      expect(checkbox, 'getConsentCheckbox should return the default input').toBeInstanceOf(
        HTMLInputElement
      )
      expect(
        container,
        'getConsentContainer should return the default container div'
      ).toBeInstanceOf(HTMLDivElement)
      expect(
        description,
        'getConsentDescription should return the default description span'
      ).toBeInstanceOf(HTMLSpanElement)
      expect(error, 'getConsentError should return the default error div').toBeInstanceOf(
        HTMLDivElement
      )
      expect(
        container.contains(checkbox),
        'Consent container should contain the checkbox input'
      ).toBe(true)
      expect(
        container.contains(description),
        'Consent container should contain the description span'
      ).toBe(true)
      expect(container.contains(error), 'Consent container should contain the error element').toBe(
        true
      )
    })
  })
  ...
  it('supports custom checkbox identifiers', async () => {
    await renderSelectors(
      () => {
        ...
        expect(checkbox.id, 'Custom consent checkbox id should be preserved').toBe(
          'newsletter-consent'
        )
        expect(container.id, 'Custom consent container id should be derived').toBe(
          'newsletter-consent-container'
        )
        expect(error.id, 'Custom consent error id should be derived').toBe(
          'newsletter-consent-error'
        )
        expect(description.id, 'Custom consent description id should be derived').toBe(
  ...
  it('throws ClientScriptError when required nodes are missing', async () => {
    await renderSelectors(() => {
      ...
      expect(
        () => getConsentCheckbox(),
        'getConsentCheckbox should throw when checkbox input is missing'
      ).toThrow(ClientScriptError)

      document.getElementById('gdpr-consent-error')?.remove()
      expect(
        () => getConsentError(),
        'getConsentError should throw when error element is missing'
      ).toThrow(ClientScriptError)

      document.getElementById('gdpr-consent-description')?.remove()
      expect(
        () => getConsentDescription(),
        'getConsentDescription should throw when description is missing'
      ).toThrow(ClientScriptError)

      document.getElementById('gdpr-consent-container')?.remove()
      expect(
        () => getConsentContainer(),
        'getConsentContainer should throw when container is missing'
      ).toThrow(ClientScriptError)
    })
  })
})
