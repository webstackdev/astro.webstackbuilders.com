...
tags: ["observability-and-telemetry", "typescript", "python", "go", "dotnet", "ruby", "grafana"]
featured: true
---
...
<Callout type="info">
Think of sampling as editorial judgment, not data loss. A newspaper doesn't print every press release it receives - it selects for newsworthiness. Your sampling strategy selects for operational value.
</Callout>

The challenge is building that editorial judgment into an automated system. The strategies that follow - head sampling, tail sampling, and hybrid approaches - each make different tradeoffs between simplicity, completeness, and operational cost that are worth understanding before you pick one.
...
<Table
  ...
  content={{
    figure: "Problems with 100% sampling at scale.",
    thead: { th: ["Problem", "Impact", "Scale Factor"] },
    tbody: {
      tr: [
        {
          th: "Storage costs",
          td: ["APM vendors charge per GB ingested", "Linear with traffic"],
        },
        {
          th: "Network overhead",
          td: ["Exporter bandwidth, collector capacity", "Linear with traffic"],
        },
        {
          th: "Query performance",
          td: ["Searching billions of spans is slow", "Superlinear with data"],
        },
        {
          th: "Signal-to-noise",
          td: ["Finding interesting traces in sea of identical ones", "Exponential difficulty"],
        },
        {
          th: "Cardinality explosion",
          td: ["High-cardinality attributes multiply storage", "Multiplicative"],
        },
        {
          th: "Processing latency",
          td: ["Collectors become bottleneck", "Throughput ceiling"],
        },
      ],
    },
  }}
...
<Diagram
  src={tradeOffsDiagram}
  title="Trade-offs between full and sampled tracing."
  description="Flowchart comparing three grouped states: 100% Sampling, Sampled Tracing, and What You Lose. In the 100% Sampling group are Every trace captured, Complete latency distribution, and All error traces. These map respectively to Representative traces, Statistical latency estimates, and Most error traces in the Sampled Tracing group. Dotted arrows from the sampled group then point to three losses: Specific trace lookup by ID, Exact percentile calculations, and Rare edge cases. The diagram shows that sampling preserves broad representativeness and most important outcomes, but gives up exact lookup, exact tail-percentile math, and visibility into very rare events."
/>
...
<List
  ...
  items={[
    {
      lead: "What sampling preserves:",
      text: "You still get representative traces for understanding request flow. You still get statistical estimates of latency distributions - with enough samples, your P50 and P95 estimates are accurate. You still get most error traces, especially if you use intelligent sampling that prioritizes errors.",
    },
    {
      lead: "What sampling loses:",
      text: "You can't guarantee you'll find a specific trace by ID. If a customer gives you a trace ID from their error message, that trace may not have been sampled. You lose exact percentile calculations - your P99 estimate has confidence intervals. You lose rare edge cases that happen less frequently than your sample rate.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Jaeger",
      text: "The most widely deployed open-source tracing backend. Originally built by Uber and now a CNCF graduated project, it provides storage, querying, and a web UI for exploring traces. You can run Jaeger with various storage backends - Cassandra, Elasticsearch, or in-memory for development. Its UI lets you search traces by service, operation, tags, and time range, then drill into individual traces to see the span waterfall.",
    },
    {
      lead: "Zipkin",
      text: "Predates Jaeger and was originally built by Twitter. It's simpler to deploy (single binary, supports SQLite for small deployments) and has good ecosystem support. Many tracing libraries support Zipkin's wire format natively. The UI is functional if less polished than Jaeger's.",
    },
    {
      lead: "Grafana Tempo",
      text: "Takes a different approach - it's a traces-only backend designed to be cheap to operate. Tempo stores traces in object storage (S3, GCS, Azure Blob) without indexing, which dramatically reduces cost. The trade-off is that you can't search by arbitrary attributes; you need a trace ID to retrieve a trace. The intended workflow is to discover trace IDs through other signals - metrics with exemplars, logs with trace correlation - then retrieve the full trace from Tempo. It integrates tightly with Grafana for visualization.",
    },
    {
      lead: "Commercial APM vendors (Datadog, New Relic, Honeycomb, Lightstep)",
      text: "Provide fully managed backends with sophisticated UIs, alerting, and analytics built in. They charge per GB ingested, which is why sampling matters so much - the economics section in the introduction assumed vendor pricing.",
    },
  ]}
/>
...
<Diagram
  src={headBasedDiagram}
  size="full"
  title="Head-based sampling decision flow."
  description="Sequence diagram showing how a head-sampling decision made at the start of a request propagates across services. A Client sends a Request to an API Gateway. At the gateway, a note indicates a sample decision of YES with a 10% chance. The gateway generates a trace ID, sets sampled=true, and forwards the request plus trace context to Service A. Service A creates spans that will be exported, forwards the request plus the same sampled=true trace context to Service B, and Service B also creates exportable spans. Service B, Service A, and the Gateway each export spans to the Collector. A final note emphasizes that all services respect the original head decision. The diagram shows that head sampling makes one early decision and then keeps the whole trace consistent across downstream services."
/>
...
<Table
  ...
  content={{
    figure: "Head sampling strategy comparison.",
    thead: { th: ["Strategy", "How It Works", "Best For", "Limitation"] },
    tbody: {
      tr: [
        {
          th: "Fixed probability",
          td: [
            "Sample N% of all traces uniformly",
            "Baseline visibility, predictable costs",
            "Misses rare important traces",
          ],
        },
        {
          th: "Rate limiting",
          td: [
            ...
            "Lower effective rate during spikes",
          ],
        },
        {
          th: "Attribute-based",
          td: [
            "Different rates based on request properties",
        ...
        },
        {
          th: "Parent-based",
          td: [
            "Inherit decision from incoming trace context",
            "Downstream services",
...
<List
  ...
  items={[
    {
      lead: "Fixed probability",
      text: "What we implemented above. It's simple, statistically representative, and gives you predictable costs. The downside is that it treats all requests equally. A payment transaction gets the same 10% chance as a health check.",
    },
    {
      lead: "Rate limiting",
      text: "Uses a token bucket or leaky bucket to cap traces per second. You configure \"sample at most 100 traces per second,\" and the sampler keeps a counter. This guarantees a cost ceiling regardless of traffic volume. During a traffic spike, your effective sampling rate drops (100 TPS out of 10,000 RPS is 1%), but you never exceed budget. The risk is missing important traces during exactly the periods when you most want visibility.",
    },
    {
      lead: "Attribute-based sampling",
      text: "Applies different rates to different request types. Sample 100% of errors, 50% of enterprise customer requests, 1% of health checks. This requires knowing which attributes matter upfront and configuring rules accordingly. It's more complex but captures more signal per dollar spent.",
    },
    {
      lead: "Parent-based sampling",
      text: "The default in OpenTelemetry. When a service receives a request with existing trace context, it respects whatever sampling decision the parent made. This ensures trace completeness - you never have partial traces. For root spans (no parent), it falls back to another strategy like probability sampling.",
    },
  ]}
/>
...
<Diagram
  src={tailBasedDiagram}
  title="Tail-based sampling decision flow."
  description="Sequence diagram showing how tail sampling buffers complete traces before deciding what to keep. Application Services send all spans, representing 100% of traffic, to an OTel Collector. The collector forwards buffered spans grouped by trace ID to a Tail Sampler. A note indicates that the tail sampler waits for the trace to complete. The tail sampler then evaluates the complete trace. If the trace is interesting, such as containing an error or slowness, the full trace is exported to the Trace Backend. If the trace is routine, it is discarded by the tail sampler. The diagram highlights that tail sampling makes decisions with full outcome information rather than guessing at trace start."
/>
...
<List
  ...
  items={[
    {
      lead: "OpenTelemetry Collector",
      text: "The industry standard. It includes a Tail Sampling Processor that handles buffering, completion detection, and policy evaluation out of the box. You configure it in YAML rather than writing code. The Collector runs as a standalone service - typically a Kubernetes Deployment or DaemonSet - that receives spans from all your microservices.",
    },
    {
      lead: "Grafana Alloy (formerly Grafana Agent)",
      text: "A high-performance distribution of the OTel Collector, widely used in environments that send data to Grafana Tempo. It supports the same tail-sampling processor with identical configuration.",
    },
    {
      lead: "Jaeger Collector v2",
      text: "Built directly on the OpenTelemetry Collector core, so it can use the same tail-sampling processors natively.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Tier 1 (Agents)",
      text: "Local collectors or sidecars receive spans from applications and forward them to a central tier",
    },
    {
      lead: "Tier 2 (Tail-Sampling Cluster)",
      text: "A central cluster of collectors uses a load-balancing exporter that routes by trace ID, ensuring all spans with the same trace ID reach the same collector instance",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Memory pressure",
      text: "The biggest operational concern. The collector must hold all spans in RAM while waiting for traces to complete. At 10,000 RPS with 20 spans per trace and a 10-second decision wait, you're buffering 2 million spans. Each span might be 1-2KB, so you need gigabytes of RAM just for the buffer - and that's before accounting for the overhead of the data structures holding them. If you run out of memory, the collector either drops traces or crashes.",
    },
    {
      lead: "Trace completion detection",
      text: "Inherently heuristic. There's no \"trace complete\" signal in distributed tracing - you just stop seeing new spans. The collector uses a timeout (`decision_wait`) to guess when a trace is done. Set it too short and you'll miss late-arriving spans from slow downstream services. Set it too long and your buffer grows, your memory pressure increases, and your traces take longer to appear in your backend.",
    },
    {
      lead: "Distributed coordination",
      text: "Adds complexity when you scale beyond a single collector instance. All spans for a trace must reach the same collector for tail sampling to work. This requires trace-ID-based routing at your load balancer, which means your collectors can't be fully stateless. If a collector handling certain trace IDs goes down, those in-flight traces are lost.",
    },
    {
      lead: "Latency addition",
      text: "Affects how quickly traces appear in your backend. With head sampling, spans are exported immediately after creation. With tail sampling, spans wait in the buffer for `decision_wait` seconds before the decision is made and the trace is exported. This delay affects real-time debugging - if you're actively investigating an incident, your most recent traces won't be visible yet.",
    },
  ]}
/>
...
<Diagram
  src={samplingStrategyDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Sampling strategy decision tree."
  description="Flowchart showing how to choose a sampling approach based on traffic volume and operational constraints. The process starts at Choose Sampling Strategy and asks about Traffic Volume. For less than 1,000 RPS, the recommendation is 100% or Simple Head, which leads to Simple Implementation. For 1,000 to 10,000 RPS, the next question is Need Error Capture? If yes, the result is Tail Sampling; if statistical error rate is acceptable, the result is Head Sampling. For more than 10,000 RPS, the next question is Budget for Tail Infrastructure? If yes, choose Tail Sampling; if no, choose Aggressive Head Sampling. Tail Sampling leads to OTel Collector + Tail Processor, while Head Sampling and Aggressive Head Sampling lead to SDK-level Sampler. The diagram emphasizes that the right choice depends on both scale and the team's ability to operate sampling infrastructure."
/>
...
<Table
  variant="numbered-review-table"
  content={{
    figure: "Head vs tail sampling comparison.",
...
<List
  ...
  classes={{
    wrapper: "sm:ml-6 sm:mr-12 mt-12 mb-6",
  }}
  items={[
    {
      lead: "Choose head sampling when:",
      text: "You have high traffic volume and need predictable costs. Your error rate is high enough that probabilistic sampling catches most errors. You don't have the ops capacity for stateful collector infrastructure. You need traces to appear in your backend immediately.",
    },
    {
      lead: "Choose tail sampling when:",
      text: "You must capture every error trace for compliance or debugging. You're tracking SLAs and need to sample all requests that violated them. You have moderate traffic where the collector infrastructure is manageable. You can tolerate the `decision_wait` delay before traces appear.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Error rate estimation",
      text: "Follows the standard sample size formula from statistics. To estimate a 1% error rate with ±0.5% margin of error at 95% confidence, you need about 1,500 samples. At 10,000 RPS with 1% sampling, you get 6,000 samples per minute - more than enough. But if your error rate is 0.01% and you want ±0.005% precision, you need 150,000 samples. That changes the math significantly.",
    },
    {
      lead: "Latency percentiles",
      text: "Need more samples than you might expect. The rule of thumb is that you need roughly 100/p samples to estimate the Pth percentile, where p is the tail probability. For P99 (p = 0.01), you need about 10,000 samples. For P99.9, you need 100,000 samples. If you're sampling at 1% and need P99.9 estimates, you need 10 million total requests in your analysis window to have enough samples.",
    },
    {
      lead: "Rare event detection",
      text: "Where sampling hurts most. If an event occurs once per 10,000 requests and you're sampling at 1%, you need 1 million total requests just to expect to see one occurrence. Rare bugs, edge cases, and unusual failure modes may never appear in your sampled data at all.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: "Sample requirements for statistically valid metrics.",
...
<List
  ...
  items={[
    {
      lead: "Always log trace IDs, even for unsampled requests",
      text: "Your application creates a trace ID for every request, regardless of whether that request is sampled. Include the trace ID in your structured logs. When a trace isn't sampled, you can still find the logs for that request - and logs often contain enough context to diagnose the issue without the trace.",
    },
    {
      lead: "Correlate trace IDs with request IDs and user IDs",
      text: 'If you can\'t find trace <span class="font-mono">abc123</span>, but you know it was for user <span class="font-mono">user456</span> at <span class="font-mono">14:32 UTC</span>, you can search for other traces from that user around that time. Sampled traces are statistically representative - a similar trace from the same user, endpoint, and time window often shows the same issue.',
    },
    {
      lead: "Use your trace backend's attribute search",
      text: 'Jaeger, Tempo, and commercial APM tools let you search traces by attributes, not just trace ID. Search for traces with the same <span class="font-mono">http.route</span>, same <span class="font-mono">error.type</span>, same <span class="font-mono">user.id</span>, same time window. You may not have <span class="italic">that</span> trace, but you\'ll have similar ones.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Structure your logs for trace correlation.",
      text: "Every log entry should include the trace ID and span ID. When you can't find a trace, you can still query your log aggregator (Loki, Elasticsearch, CloudWatch Logs) for entries with that trace ID. The logs won't show you the span waterfall, but they'll show you what happened.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Build dashboards that connect the three signals.",
      text: "Your Grafana dashboard should let you go from a metric anomaly → to logs from that time window → to traces matching those logs. Tempo's integration with Loki is designed for exactly this workflow: search logs, find trace IDs in log entries, click through to traces.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Feature flags for sample rates.",
      text: "If your sampler reads its rate from a feature flag service (LaunchDarkly, Split, ConfigCat, or even a simple Redis key), you can adjust rates in real-time. During an incident, flip the flag to 100%. After investigation, flip it back. No deployment required.",
    },
    {
      lead: "OTel Collector hot reload.",
      text: "If you're using tail sampling in the OTel Collector, the Collector supports configuration reload via SIGHUP. Update your ConfigMap, trigger a reload, and your sampling policies change without collector restart. This is slower than a feature flag but doesn't require application changes.",
    },
    {
      lead: "Time-bounded sample rate increases.",
      text: "If you're increasing rates during an incident, set a reminder to decrease them afterward. Better yet, build automation that resets rates after a fixed period. A Lambda function that checks a \"debug mode\" flag and resets it after 30 minutes prevents the \"we left it on 100% for a week\" budget surprise.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "In Jaeger:",
      text: 'Use the search panel to filter by service, operation (endpoint), tags, and time range. If you\'re looking for traces where <span class="font-mono">/api/checkout</span> returned a 500 error, search for <span class="font-mono">service=checkout-service</span>, <span class="font-mono">http.status_code=500</span>, <span class="font-mono">operation=POST /api/checkout</span>, time range around when the error occurred. You won\'t find the exact trace the customer hit, but you\'ll find recent traces with the same characteristics.',
    },
    {
      lead: "In Grafana Tempo:",
      text: 'Tempo\'s TraceQL query language lets you search by attributes. The query <span class="font-mono">{ resource.service.name="checkout-service" && span.http.status_code=500 && span.http.route="/api/checkout" }</span> finds error traces from checkout. Combine with Tempo\'s integration with <span class="font-mono">Loki: search</span> logs for the trace ID, find related log entries, discover other trace IDs in those logs that <span class="italic">were</span> sampled.',
    },
    {
      lead: "In commercial APM (Datadog, New Relic, Honeycomb):",
      text: "These tools have sophisticated search and correlation features. Honeycomb's BubbleUp can automatically identify which attributes are correlated with errors. Datadog's trace search lets you pivot from one trace to others with similar attributes. Use these features - they're designed for exactly this problem.",
    },
  ]}
/>
...
<Download
  resource="distributed-tracing-sampling-strategies-head-tail"
  title="Download the Tracing Sampling Guide"
  description="Get the complete sampling playbook for cost control, error capture, and practical trace-debug workflows at scale."
  listItems={[
    {
      text: "Head sampler configuration guide",
    },
    {
      text: "Tail policy tuning matrix",
    },
    {
      text: "Hybrid rollout decision framework",
    },
    {
      text: "Missing trace debugging runbook",
    },
  ]}
/>
