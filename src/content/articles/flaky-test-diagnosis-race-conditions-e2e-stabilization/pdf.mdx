...
<List
  ...
  items={[
    { text: "_Categorizing flakiness_ by root cause—race conditions, environment issues, and test design flaws each have distinct symptoms and fixes" },
    { text: "_Building detection infrastructure_ to track flake rates and identify patterns over time" },
    { text: "_Diagnosing race conditions_ with concrete debugging techniques and stable code patterns" },
    { text: "_Stabilizing test environments_ through isolation, time mocking, and animation handling" },
    { text: "_Managing flaky tests at scale_ with quarantine systems, triage frameworks, and CI debugging workflows" },
    { text: "_Choosing the right tools_ for flake detection and tracking" },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "Run the test many times in a loop. With Playwright, use `npx playwright test specific.spec.ts --repeat-each=50`. If it passes all 50 times, you may have fixed it accidentally, or the flake rate is very low. If it fails even once, you've confirmed the flakiness and can proceed.", text: "Step 1: Confirm it's actually flaky." },
    { lead: "Two complementary techniques help here. First, _slow things down_—run with `PWTEST_SLOW_MO=1000` to add delays between actions. If the test becomes stable when slowed, you've confirmed a timing issue. Second, _speed things up_—run under CPU pressure (with something like `stress-ng --cpu 4 &`) to make the race more likely to trigger. Add timestamps to key events: test step start/end, network requests, DOM mutations.", text: "Step 2: Narrow down the race." },
    { lead: "What's racing? The usual suspects are: test assertion vs async state update, click handler vs animation completing, network response vs render, or test cleanup vs next test setup. Look at the timestamps to see what's happening out of order.", text: "Step 3: Identify the competitors." },
    { lead: "The fix is almost always the same principle: wait for a specific condition, not an arbitrary amount of time.", text: "Step 4: Fix it." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "_Transaction rollback_", lead: "Wrap each test in a database transaction and roll back after. Fast and clean, but doesn't work if your test needs to verify committed data, test triggers, or observe behavior across transaction boundaries. Best for: unit-style integration tests where you're testing application logic, not database behavior." },
    { text: "_Database per test_", lead: "Create an isolated database for each test with a unique name. Slower but provides complete isolation. Works well with containerized databases (spin up a fresh Postgres container per test run). Best for: tests that need to verify committed data or test database-level features." },
    { text: "_Seeded snapshots_", lead: "Reset to a known state before each test using database snapshots or seed scripts. Good balance of isolation and speed. Best for: E2E tests where you need realistic data but can't afford the overhead of creating fresh databases." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "_Flake rate_ (30% weight): How often does it fail? Higher rates mean more disruption." },
    { text: "_Test importance_ (25% weight): Is this a critical path like checkout or authentication? Or an edge case?" },
    { text: "_Fix complexity_ (20% weight): Quick fixes get higher priority—they're easy wins." },
    { text: "_CI impact_ (25% weight): How many people does this flake block when it triggers?" },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "Explicit test hooks that don't change with styling or structure", text: "data-testid attributes" },
    { lead: "Semantic selectors that align with accessibility, like `getByRole('button', { name: 'Submit' })`", text: "ARIA roles and labels" },
    { lead: "Stable attributes like `[name=\"email\"]`", text: "Form element names" },
    { lead: "If they're semantic and unlikely to change", text: "Stable IDs" },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "_Timing differences_: Do failures take longer? That suggests timeouts or resource contention." },
    { text: "_Error message patterns_: Are failures consistent (same error) or varied? Consistent errors point to a specific race; varied errors suggest environmental instability." },
    { text: "_Screenshot differences_: What did the page look like at failure? Was an element missing, or in the wrong state?" },
  ]}
/>
...
<List
  ...
  items={[
    ...
    { text: "Limit CPU and memory to match CI constraints: `docker run --cpus=2 --memory=4g`" },
...
<List
  variant="check-icons-list"
  items={[
    { text: "_BuildPulse_ integrates with your CI pipeline and automatically detects flaky tests based on pass/fail patterns across runs.", lead: "It calculates flake rates, identifies root causes, and prioritizes which flakes to fix first. The service tracks whether fixes actually work by monitoring the test after your change." },
    { text: "_Trunk Flaky Tests_ provides similar functionality with automatic quarantining.", lead: "When a test is detected as flaky, Trunk can automatically retry it or move it to a quarantine suite, then notify you via Slack or create a tracking issue." },
    { text: "_Datadog CI Visibility_ takes a broader approach, correlating test performance with traces and logs.", lead: "If a test is flaky because of slow database queries or API timeouts, you can see the full request traces alongside the test results." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "_GitHub Actions_ supports automatic retries and tracks retry history", lead: "though the analytics are limited." },
    { text: "_CircleCI_ has test insights that show flaky test trends over time", lead: "and can automatically rerun failed jobs." },
    { text: "_GitLab CI_ has a \"retry flaky tests\" feature", lead: "that reruns only the failed tests rather than the whole job." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "Categorize", text: "Is it a race condition, environment issue, or test design flaw? The symptoms tell you where to look." },
    { lead: "Detect", text: "Track flake rates over time. A test that was stable and becomes flaky is different from a test that's always been flaky." },
    { lead: "Quarantine", text: "Isolate flaky tests so they don't block CI, but hold them accountable with owners, tickets, and deadlines." },
    { lead: "Diagnose", text: "Reproduce the flake, slow things down, add logging, identify what's racing." },
    { lead: "Fix", text: "Wait for specific conditions instead of time, isolate test state, use resilient selectors." },
  ]}
/>
