...
<List
  ...
  items={[
    {
      lead: "Replace them with waits for specific conditions — network responses, DOM elements, URL changes.",
      text: "For race conditions, find every place your test uses `waitForTimeout()` or assumes immediate state changes after actions.",
    },
    {
      lead: "Clear browser state between tests. Isolate database state through transactions, per-test databases, or seed scripts. Freeze time or use timezone-agnostic assertions. Disable animations.",
      text: "For environment issues, audit your test setup.",
    },
  ]}
/>
