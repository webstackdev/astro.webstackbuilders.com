...
tags: ["reliability-and-testing", "typescript"]
featured: true
---
...
<List
  ...
  items={[
    {
      lead: "Categorizing flakiness by root cause",
      text: "Race conditions, environment issues, and test design flaws each have distinct symptoms and fixes",
    },
    {
      lead: "Building detection infrastructure",
      text: "to track flake rates and identify patterns over time",
    },
    {
      lead: "Diagnosing race conditions",
      text: "with concrete debugging techniques and stable code patterns",
    },
    {
      lead: "Stabilizing test environments",
      text: "through isolation, time mocking, and animation handling",
    },
    {
      lead: "Managing flaky tests at scale",
      text: "with quarantine systems, triage frameworks, and CI debugging workflows",
    },
    {
      lead: "Choosing the right tools",
      text: "for flake detection and tracking",
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Flakiness categories and resolution characteristics.",
    thead: {
      th: ["Category", "Typical Fix Time", "CI Impact", "Detection Difficulty"],
    },
    tbody: {
      tr: [
        {
          th: "Race condition - simple",
          td: ["15-30 min", "High", "Medium"],
        },
        {
          th: "Race condition - complex",
          td: ["2-8 hours", "High", "Hard"],
        },
        {
          th: "Environment - resource",
          td: ["30-60 min", "Medium", "Easy"],
        },
        {
          th: "Environment - timing",
          td: ["1-4 hours", "Low", "Hard"],
        },
        {
          th: "Test design - state",
          td: ["1-2 hours", "High", "Medium"],
        },
        {
          th: "Test design - selector",
          td: ["15 min", "Low", "Easy"],
        },
        {
          th: "External dependency",
          td: ["Varies", "High", "Medium"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      title: "Step 1:",
      lead: "Confirm it's actually flaky.",
      text: 'Run the test many times in a loop. With Playwright, use <span class="text-mono">npx playwright test specific.spec.ts --repeat-each=50</span>. If it passes all 50 times, you may have fixed it accidentally, or the flake rate is very low. If it fails even once, you\'ve confirmed the flakiness and can proceed.',
    },
    {
      title: "Step 2:",
      lead: "Narrow down the race.",
      text: 'Two complementary techniques help here. First, <em>slow things down</em> — run with <span class="text-mono">PWTEST_SLOW_MO=1000</span> to add delays between actions. If the test becomes stable when slowed, you\'ve confirmed a timing issue. Second, <em>speed things up</em> — run under CPU pressure (with something like <span class="text-mono">stress-ng --cpu 4 &amp;</span>) to make the race more likely to trigger. Add timestamps to key events: test step start/end, network requests, DOM mutations.',
    },
    {
      title: "Step 3:",
      lead: "Identify the competitors.",
...
<Diagram
  src={timelineDiagram}
  size="full"
  title="Timeline showing race condition."
  description="Sequence diagram with four participants: Test, Browser, App, and Network. The test clicks submit,
  the browser delivers the click event to the app, and the test immediately checks for a success state before
  the application has finished its asynchronous work. After that early assertion, the app sends an API request
  to the network, the network returns a response, and only then does the app update the browser DOM. The key
  point is that the assertion happens before the response and DOM update, which is the race condition that makes
  the test flaky."
/>
...
<List
  ...
  items={[
    {
      lead: "Transaction rollback",
      text: "Wrap each test in a database transaction and roll back after. Fast and clean, but doesn't work if your test needs to verify committed data, test triggers, or observe behavior across transaction boundaries. Best for: unit-style integration tests where you're testing application logic, not database behavior.",
    },
    {
      lead: "Database per test",
      text: "Create an isolated database for each test with a unique name. Slower but provides complete isolation. Works well with containerized databases (spin up a fresh Postgres container per test run). Best for: tests that need to verify committed data or test database-level features.",
    },
    {
      lead: "Seeded snapshots",
      text: "Reset to a known state before each test using database snapshots or seed scripts. Good balance of isolation and speed. Best for: E2E tests where you need realistic data but can't afford the overhead of creating fresh databases.",
    },
  ]}
/>
...
<Diagram
  src={flakyTestDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Flaky test quarantine workflow."
  description="Flowchart showing the decision process after a test failure. It begins with Test Fails and asks
  whether the failure is a known flake. If yes, the team logs the occurrence, then checks whether the flake
  rate is greater than 10 percent. A high flake rate leads to Escalate priority, while a lower rate goes to
  Normal backlog. If the failure is not a known flake, the next decision asks whether a retry passes. If the
  retry does not pass, the test is treated as a real failure and the merge is blocked. If the retry does pass,
  the test is marked as a potential flake and monitored. Monitoring then asks whether the test fails two or
  more times within 24 hours. If not, it stays in Monitor. If yes, the test is auto-quarantined, a tracking
  issue is created, the test is added to the quarantine list, and it is run separately from the main suite."
/>
...
<List
  ...
  items={[
    {
      lead: 'Flake rate (30% weight):',
      text: "How often does it fail? Higher rates mean more disruption.",
    },
    {
      lead: 'Test importance (25% weight):',
      text: "Is this a critical path like checkout or authentication? Or an edge case?",
    },
    {
      lead: 'Fix complexity (20% weight):',
      text: "Quick fixes get higher priority — they're easy wins.",
    },
    {
      lead: 'CI impact (25% weight):',
      text: "How many people does this flake block when it triggers?",
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Example triage priority calculation (priority scored 1-10, higher = more urgent).",
    thead: {
      th: ["Test", "Flake Rate", "Importance", "Fix Est.", "CI Impact", "Priority (1-10)"],
    },
    tbody: {
      tr: [
        {
          th: "checkout.payment",
          td: ["8%", "Critical (10)", "4h (6)", "Daily (10)", "8"],
        },
        {
          th: "dashboard.analytics",
          td: ["3%", "Core (7)", "1h (10)", "Occasional (4)", "6"],
        },
        {
          th: "settings.timezone",
          td: ["12%", "Secondary (4)", "2d (3)", "Rare (1)", "5"],
        },
        {
          th: "onboarding.tour",
          td: ["2%", "Edge (1)", "Quick (10)", "Rare (1)", "3"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: '<span class="font-mono">data-testid</span> attributes',
      text: "Explicit test hooks that don't change with styling or structure",
    },
    {
      lead: "ARIA roles and labels",
      text: "Semantic selectors that align with accessibility, like `getByRole('button', { name: 'Submit' })`",
    },
    {
      lead: "Form element names",
      text: "Stable attributes like `[name=\"email\"]`",
    },
    {
      lead: "Stable IDs",
      text: "If they're semantic and unlikely to change",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Timing differences:",
      text: "Do failures take longer? That suggests timeouts or resource contention.",
    },
    {
      lead: "Error message patterns:",
      text: "Are failures consistent (same error) or varied? Consistent errors point to a specific race; varied errors suggest environmental instability.",
    },
    {
      lead: "Screenshot differences:",
      text: "What did the page look like at failure? Was an element missing, or in the wrong state?",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Use Docker with the same base image as your CI runners",
    },
    {
      lead: 'Limit CPU and memory to match CI constraints: <span class="font-mono">docker run --cpus=2 --memory=4g</span>',
    },
    {
      lead: "Run tests in parallel with the same worker count as CI",
    },
    {
      lead: 'Add artificial load with <span class="font-mono">stress-ng</span> to simulate resource contention',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "BuildPulse integrates with your CI pipeline and automatically detects flaky tests based on pass/fail patterns across runs.",
      text: "It calculates flake rates, identifies root causes, and prioritizes which flakes to fix first. The service tracks whether fixes actually work by monitoring the test after your change.",
    },
    {
      lead: "Trunk Flaky Tests provides similar functionality with automatic quarantining.",
      text: "When a test is detected as flaky, Trunk can automatically retry it or move it to a quarantine suite, then notify you via Slack or create a tracking issue.",
    },
    {
      lead: "Datadog CI Visibility takes a broader approach, correlating test performance with traces and logs.",
      text: "If a test is flaky because of slow database queries or API timeouts, you can see the full request traces alongside the test results.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "GitHub Actions",
      text: "Supports automatic retries and tracks retry history though the analytics are limited.",
    },
    {
      lead: "CircleCI",
      text: "Has test insights that show flaky test trends over time and can automatically rerun failed jobs.",
    },
    {
      lead: "GitLab CI",
      text: "Has a \"retry flaky tests\" feature that reruns only the failed tests rather than the whole job.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      title: "Categorize",
      lead: "Is it a race condition, environment issue, or test design flaw? The symptoms tell you where to look.",
    },
    {
      title: "Detect",
      lead: "Track flake rates over time. A test that was stable and becomes flaky is different from a test that's always been flaky.",
    },
    {
      title: "Quarantine",
      lead: "Isolate flaky tests so they don't block CI, but hold them accountable with owners, tickets, and deadlines.",
    },
    {
      title: "Diagnose",
      lead: "Reproduce the flake, slow things down, add logging, identify what's racing.",
    },
    {
      title: "Fix",
      lead: "Wait for specific conditions instead of time, isolate test state, use resilient selectors.",
    },
  ]}
/>

<Download
  resource="flaky-test-diagnosis-race-conditions-e2e-stabilization"
  title="Download the Flaky Test Diagnosis Guide"
  description="Get the complete stabilization playbook for race conditions, environment isolation, and reliable CI test execution."
  listItems={[
    {
      text: "Race condition detection checklist",
    },
    {
      text: "Environment isolation setup guide",
    },
    {
      text: "Flake triage prioritization matrix",
    },
    {
      text: "CI reproduction workflow runbook",
    },
  ]}
/>
