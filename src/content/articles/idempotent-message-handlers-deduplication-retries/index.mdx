...
<List
  ...
  items={[
    {
      lead: "At-most-once",
      text: "delivery is the simplest: acknowledge the message immediately upon receipt, before processing. If your handler crashes mid-processing, the message is gone. This works for telemetry, cache invalidation, and anything where losing the occasional message is acceptable. You'll never see duplicates, but you will lose data.",
    },
    {
      lead: "At-least-once",
      text: "delivery flips the tradeoff: acknowledge only after successful processing. If your handler crashes, the message gets redelivered. You won't lose messages, but you will see duplicates. A handler can also explicitly reject a message with a NACK (negative acknowledgment) to trigger immediate redelivery - useful when you detect a transient failure and want to retry without waiting for a timeout. This is the right choice for business events, payments, order processing - anything where losing a message is worse than processing it twice.",
    },
    {
      lead: "Exactly-once",
      text: "delivery is what everyone wants and nobody truly gets. What systems actually provide is \"effectively once\" - at-least-once delivery combined with idempotent handlers. The message may arrive multiple times, but idempotent processing ensures the result is the same as if it arrived once.",
    },
  ]}
/>

<Diagram
  src={messageDeliveryDiagram}
  title="Message delivery semantics."
  description="Flowchart comparing three message delivery semantics. The first subgraph, At-Most-Once, shows a producer sending to a queue, the queue delivering once to a consumer, and a dotted path indicating the message may be lost. The second subgraph, At-Least-Once, shows a producer sending to a queue, the queue delivering to a consumer, the consumer acknowledging back to the queue, and the queue redelivering if no acknowledgment arrives. The third subgraph, Exactly-Once, shows a producer sending to a queue, the queue delivering to a consumer, and the consumer processing plus acknowledging atomically to produce a result."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Delivery guarantee comparison.",
    thead: {
      th: ["Guarantee", "Message Loss", "Duplicates", "Complexity", "When to Use"],
    },
    tbody: {
      tr: [
        {
          th: "At-most-once",
          td: ["Possible", "Never", "Low", "Metrics, logs, non-critical"],
        },
        {
          th: "At-least-once",
          td: ["Never", "Possible", "Medium", "Most business events"],
        },
        {
          th: "Exactly-once",
          td: ["Never", "Never", "High", "Financial, requires idempotency"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Combines entity identifiers with operation context: <span class="font-mono">order:12345:payment:v3</span>. This approach has semantic meaning, making debugging easier, but requires careful thought about what constitutes a unique operation. Does a payment retry get a new version, or should it reuse the same key?',
    },
    {
      lead: "Queue message ID",
      text: "Don't use this. SQS assigns a new MessageId on each redelivery. RabbitMQ's delivery tag changes. This approach only deduplicates within a single delivery attempt, which defeats the purpose entirely.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Global scope",
      text: "Means any service can check whether a message ID was already processed. This requires a centralized idempotency store (typically Redis or a shared database) and makes sense when the same message might be consumed by multiple services.",
    },
    {
      lead: "Entity scope",
      text: "Limits deduplication to a specific business entity. The key `order:12345:payment` only needs to be unique within the context of order 12345. This allows storing idempotency state alongside the entity itself, eliminating the need for a separate store.",
    },
    {
      lead: "Time-windowed scope",
      text: "Partitions keys by time period: `metric:cpu:host-789:2024-01-15T10:00`. This bounds storage growth for high-volume systems where eventual consistency is acceptable.",
    },
  ]}
/>
...
<Diagram
  src={idempotencyFlowDiagram}
  size="full"
  title="Idempotency flow - second delivery skips processing entirely."
  description="Sequence diagram showing idempotent message handling across two deliveries. A queue delivers a message to the handler for the first time. The handler checks the idempotency store using key order:123:pay, the store replies that the key is not found and the lock is acquired, the handler processes the payment through the database, the database returns success, the handler marks the key as completed with a cached result, and then acknowledges the message to the queue. A note indicates the consumer crashes and the message is redelivered. On the second delivery, the handler checks the same key again, the idempotency store reports the key is already completed with a cached result, and the handler immediately acknowledges the message without processing the payment again."
/>
...
<Table
  variant="criteria-highlight-table"
  content={{
    figure: "Idempotency store options comparison.",
    thead: {
      th: ["Storage", "Latency", "Durability", "Complexity", "Best For"],
    },
    tbody: {
      tr: [
        {
          th: "Redis",
          td: ["~1ms", "Configurable (AOF)", "Low", "High-throughput, short TTL"],
        },
        {
          th: "PostgreSQL",
          td: ["~5ms", "High (ACID)", "Medium", "Transactional consistency"],
        },
        {
          th: "DynamoDB",
          td: ["~10ms", "High", "Low", "Serverless, global scale"],
        },
        {
          th: "In-memory",
          td: ["&lt;1ms", "None", "Very low", "Single instance, testing"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: "Set operations",
      text: 'Naturally idempotent. <span class="font-mono">user.email = \'new@example.com\'</span> produces the same state whether you run it once or ten times. Same with <span class="font-mono">UPDATE users SET email = $1 WHERE id = $2</span>.',
    },
    {
      lead: "Upserts",
      text: 'Naturally idempotent. <span class="font-mono">INSERT ... ON CONFLICT UPDATE</span> converges to the same final state regardless of how many times you execute it.',
    },
    {
      lead: "Deletes by ID",
      text: 'Naturally idempotent. <span class="font-mono">DELETE FROM orders WHERE id = 123</span> succeeds once and becomes a no-op on subsequent executions.',
    },
    {
      lead: "Absolute state sets",
      text: 'Naturally idempotent. <span class="font-mono">inventory.setQuantity(sku, 5)</span> sets the quantity to 5, not "adds 5." Running it twice still leaves the quantity at 5.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Increments",
      text: 'Compound on each execution. <span class="font-mono">balance += 100</span> adds 100 every time. The fix is to track which operations have been applied: <span class="font-mono">applyDeposit(depositId, 100)</span> can check whether <span class="font-mono">depositId</span> was already applied.',
    },
    {
      lead: "Appends",
      text: 'Grow the collection. <span class="font-mono">items.push(newItem)</span> adds another entry each time. The fix is to include an item ID and check before appending: <span class="font-mono">if (!items.some(i => i.id === newItem.id)) items.push(newItem)</span>.',
    },
    {
      lead: "External side effects",
      text: "Like sending emails or notifications aren't naturally idempotent. The user receives the email on each execution. Track sent notifications by key to prevent duplicates.",
    },
  ]}
/>
...
<Diagram
  src={transactionalOutboxDiagram}
  size="xl"
  title="Transactional outbox."
  description="Flowchart showing the transactional outbox pattern. Inside a Single Transaction subgraph, a handler writes both an Update Business State action and an Insert Outbox Record action. Both outputs point to the same database, indicating they are committed together. Separately, an Outbox Publisher reads from the database and sends messages to a Message Queue. A Downstream Consumer receives messages from that queue. The diagram shows that business state and outbox event are written atomically first, and event publication happens afterward by polling the outbox."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Queue deduplication capabilities - all require handler-side idempotency.",
    thead: {
      th: ["Queue", "Built-in Deduplication", "Scope", "Handler Idempotency Needed?"],
    },
    tbody: {
      tr: [
        {
          th: { label: "SQS Standard", color: "bg-note" },
          td: ["None", "N/A", "Yes"],
        },
        {
          th: { label: "SQS FIFO", color: "bg-success" },
          td: ["MessageDeduplicationId", "5 minutes", "Yes (for longer)"],
        },
        {
          th: { label: "Kafka", color: "bg-accent" },
          td: ["Idempotent producer", "Producer session", "Yes (consumer side)"],
        },
        {
          th: { label: "RabbitMQ", color: "bg-warning" },
          td: ["None", "N/A", "Yes"],
        },
        {
          th: { label: "Azure Service Bus", color: "bg-info" },
          td: ["MessageId", "Session/window", "Yes"],
        },
      ],
    },
  }}
...
<Download
  resource="idempotent-message-handlers-deduplication-retries"
  title="Download the Idempotency Playbook"
  description="Get the complete message-consumer guide for deduplication, retry safety, and side-effect-safe processing in distributed systems."
  listItems={[
    {
      text: "Idempotency key design templates",
    },
    {
      text: "Deduplication store decision matrix",
    },
    {
      text: "Handler implementation pattern catalog",
    },
    {
      text: "Retry and replay test plan",
    },
  ]}
/>
