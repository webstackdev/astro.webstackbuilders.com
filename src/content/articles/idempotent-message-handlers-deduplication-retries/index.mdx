---
title: "Why At-Least-Once Delivery Means Your Handler Is Broken"
...
tags: ["systems-and-development", "python", "aws", "typescript"]
featured: true
---
...
Every production queue operates on at-least-once delivery - "_probably more than once_." The question isn't _if_ duplicates arrive, it's when.
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Combines entity identifiers with operation context: <span class="font-mono">order:12345:payment:v3</span>. This approach has semantic meaning, making debugging easier, but requires careful thought about what constitutes a unique operation.',
    },
    {
      lead: "Queue message ID",
      text: 'Avoid this approach. SQS assigns a new <span class="font-mono">MessageId</span> on each redelivery. RabbitMQ\'s delivery tag changes. This approach only deduplicates within a single delivery attempt, which defeats the purpose entirely.',
    },
  ]}
/>
...
<Diagram
  ...
  description="Sequence diagram showing how an idempotent message handler processes a first delivery and short-circuits a duplicate. A queue delivers a message to the handler, which checks the idempotency store for key order:123:pay. The store reports the key is not found and grants the lock. The handler processes the payment through the database, receives success, marks the idempotency key as completed with a cached result, and acknowledges the message back to the queue. Later, after a note indicating the consumer crashed and the message was redelivered, the queue sends the same message again. The handler checks the same idempotency key, the store reports it is already completed with a cached result, and the handler immediately acknowledges the message without processing the payment again."
/>
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: "Redis",
          td: ["~1ms", "Configurable", "Low", "High-throughput, short TTL"],
        },
        {
          th: "PostgreSQL",
