...
<Diagram
  title="Request flow through reverse proxy showing the six phases where timeouts and buffers apply."
  description={
    `Sequence diagram with three participants: Client, Proxy, and Backend. Phase 1 is Client Connection, where the client performs a TCP handshake with the proxy and then a TLS handshake. Phase 2 is Request Receipt, where the client sends HTTP request headers followed by the HTTP request body to the proxy. Phase 3 is Backend Connection, where the proxy opens or reuses a TCP connection to the backend. Phase 4 is Request Forward, where the proxy sends the HTTP request to the backend. Phase 5 is Response Receipt, where the backend returns the HTTP response to the proxy. Phase 6 is Response Delivery, where the proxy sends the HTTP response back to the client. A final note indicates that the client connection may remain open for keep-alive reuse. The diagram maps the full path of a proxied request and highlights where connection, timeout, and buffering settings influence behavior.`
  }
>
```mermaid
sequenceDiagram
...
<Diagram
  title="Nginx timeout chain showing where each timeout applies in the request lifecycle."
  description={
  `Left-to-right flowchart divided into three grouped sections. The first group, Nginx Client Side, starts with client_header_timeout set to 10 seconds and flows to client_body_timeout set to 30 seconds. The second group, Nginx Proxy Processing, continues from there to proxy_connect_timeout at 5 seconds, then to proxy_send_timeout at 30 seconds, and then to proxy_read_timeout at 60 seconds. The third group, Nginx Response, ends with send_timeout at 30 seconds. The proxy_connect_timeout node is visually highlighted to show its importance in failing fast when a backend cannot be reached. The diagram shows the order in which Nginx timeout settings apply as a request moves from initial client reads through backend communication and back to the client response path.`
  }
>
```mermaid
flowchart LR
  subgraph nginxClientSide["Nginx Client Side"]
    A["client_header_timeout\n10s"] --> B["client_body_timeout\n30s"]
  end

  subgraph nginxProxyProcessing["Nginx Proxy Processing"]
    B --> C["proxy_connect_timeout\n5s"]
    C --> D["proxy_send_timeout\n30s"]
    D --> E["proxy_read_timeout\n60s"]
  end

  subgraph nginxResponse["Nginx Response"]
    E --> F["send_timeout\n30s"]
  end
```
</Diagram>
...
<Table
  ...
  content={{
    thead: {
      th: ['Timeout', 'Nginx', 'HAProxy', 'Recommended'],
    },
    tbody: {
      tr: [
        {
          th: 'Client connect',
          td: ['N/A (kernel)', 'N/A (kernel)', 'Kernel default'],
        },
        {
          th: 'Client request',
          td: ['client_header_timeout', 'timeout http-request', '10-30s'],
        },
        {
          th: 'Client body',
          td: ['client_body_timeout', 'timeout client', '30-120s'],
        },
        {
          th: 'Backend connect',
          td: ['proxy_connect_timeout', 'timeout connect', '3-10s'],
        },
        {
          th: 'Backend response',
          td: ['proxy_read_timeout', 'timeout server', '30-300s'],
        },
        {
          th: 'Keep-alive idle',
          td: ['keepalive_timeout', 'timeout http-keep-alive', '30-120s'],
        },
      ],
    },
    figure: 'Timeout comparison.',
  }}
/>
...
The `http-request deny if { req.body_size gt 104857600 }` line is worth noting: it rejects oversized requests __before__ buffering them. Without this, HAProxy would accept a 10GB upload attempt before rejecting it — wasting bandwidth and potentially filling disk.

<Table
  variant="vertical-column-delineation-table"
  content={{
    thead: {
      th: ['Scenario', 'Buffer Setting', 'Recommendation'],
    },
    tbody: {
      tr: [
        {
          th: 'Large cookies/headers',
          td: ['client_header_buffer_size', '4k-16k'],
        },
        {
          th: 'File uploads',
          td: ['client_body_buffer_size', '128k-1m'],
        },
        {
          th: 'Large API responses',
          td: ['proxy_buffers', '16-32 × 32k-64k'],
        },
        {
          th: 'Streaming/SSE',
          td: ['proxy_buffering', 'off'],
        },
        {
          th: 'Memory constrained',
          td: ['proxy_max_temp_file_size', 'Limit spillover'],
        },
      ],
    },
    figure: 'Buffer sizing recommendations by scenario. These are starting points — profile your actual traffic to refine.',
  }}
/>
...
<Diagram
  title="Connection pooling to backends. Multiple clients share a smaller pool of backend connections, reducing handshake overhead."
  description={
  `Top-down flowchart with three grouped sections. The first section, Client Connections, contains three separate clients labeled Client 1, Client 2, and Client 3. All three connect to a central proxy component labeled HAProxy or Nginx in the second section, Proxy keepalive pool. That proxy feeds into a highlighted Keep-alive Pool node labeled 32 connections. The keep-alive pool then connects to two backend servers in the third section, Backend Servers, labeled Backend 1 and Backend 2, each with maxconn 500. The diagram shows that many client connections can be funneled through a smaller reusable pool of backend connections, which reduces repeated TCP or TLS handshake overhead and lowers backend connection churn.`
  }
>
```mermaid
flowchart TD
  subgraph clientConnections["Client Connections"]
    C1["Client 1"]
    C2["Client 2"]
    C3["Client 3"]
  end

  subgraph proxyKeepalivePool["Proxy (keepalive pool)"]
    P["HAProxy/Nginx"]
    KA["Keep-alive Pool\n32 connections"]
  end
...
<Table
  ...
  content={{
    thead: {
      th: ['Algorithm', 'Use Case', 'Session Affinity', 'Even Distribution'],
    },
    tbody: {
      tr: [
        {
          th: 'Round-robin',
          td: ['Stateless services', 'No', 'Yes'],
        },
        {
          th: 'Least-conn',
          td: ['Varying request duration', 'No', 'Adaptive'],
        },
        {
          th: 'IP hash',
          td: ['Simple session affinity', 'Yes (IP-based)', 'Depends on IPs'],
        },
        {
          th: 'Cookie',
          td: ['Reliable session affinity', 'Yes (cookie)', 'Yes'],
        },
        {
          th: 'URI hash',
          td: ['Caching proxies', 'Yes (URL-based)', 'Depends on URLs'],
        },
      ],
    },
    figure: 'Load balancing algorithm selection guide. Start with round-robin and change only if you have a specific reason.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Optimization', 'Impact', 'Trade-off'],
    },
    tbody: {
      tr: [
        {
          th: 'Session cache',
          td: ['Reduces handshakes', 'Memory usage'],
        },
        {
          th: 'OCSP stapling',
          td: ['Faster validation', 'Requires resolver'],
        },
        {
          th: 'TLS 1.3',
          td: ['Faster handshake, better security', 'Client support'],
        },
        {
          th: 'HTTP/2',
          td: ['Multiplexing, fewer connections', 'Complexity'],
        },
        {
          th: 'Early data (0-RTT)',
          td: ['Faster first request', 'Replay risk'],
        },
      ],
    },
    figure: 'SSL optimization options. Session caching and TLS 1.3 provide the biggest wins with minimal downside.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Category', 'Default', 'Production', 'Why'],
    },
    tbody: {
      tr: [
        {
          th: 'worker_connections',
          td: ['1024', '4096+', 'Handle more concurrent connections'],
        },
        {
          th: 'keepalive_timeout',
          td: ['75s', '65s', 'Balance reuse vs. resource usage'],
        },
        {
          th: 'proxy_connect_timeout',
          td: ['60s', '5s', 'Fail fast on unreachable backends'],
        },
        {
          th: 'proxy_buffers',
          td: ['8 4k', '16 32k', 'Handle larger responses'],
        },
        {
          th: 'ssl_session_cache',
          td: ['none', 'shared:50m', 'Reduce TLS handshakes'],
        },
      ],
    },
    figure: 'Default vs. production values. These are starting points — profile your actual traffic to determine optimal values.',
  }}
/>
