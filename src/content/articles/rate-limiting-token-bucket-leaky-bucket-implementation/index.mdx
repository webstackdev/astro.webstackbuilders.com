...
tags: ["apis-and-gateways","aws","azure","terraform","typescript","python"]
featured: true
---
  ...
  items={[
    {
      ...
      text: "Prevents individual clients from monopolizing shared resources. Without limits, one customer's bulk export can exhaust your database connection pool, causing errors for everyone else. CPU-intensive endpoints, memory-heavy operations, and database-bound queries all need protection from unbounded consumption.",
    },
    {
      lead: 'Fairness',
      text: "Ensures equitable access across clients. Large customers shouldn't crowd out small ones. Free-tier users shouldn't impact paying customers. Rate limits enforce the allocation you've decided is appropriate for each tier.",
    },
    {
      lead: 'Abuse prevention',
      text: 'Stops malicious or buggy clients. Credential stuffing attacks, scraping attempts, and runaway retry loops all look like excessive request volume. Rate limiting slows these down enough to make attacks expensive and gives you time to respond.',
    },
    {
      lead: 'Cost control',
      text: 'Caps expensive operations. Third-party API calls, ML inference requests, and storage operations have real costs. Without limits, a compromised API key or buggy client can generate surprise bills within hours.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Algorithm', 'Mechanism', 'Burst Handling', 'Memory', 'Best For'],
    },
    tbody: {
      tr: [
        {
    ...
    figure: 'Rate limiting algorithm comparison.',
  }}
/>
  ...
  items={[
    {
      ...
      text: 'This is simple: count requests in minute-long (or hour-long) windows, reject when the count exceeds the limit, reset at the boundary. The problem is boundary bursts. A client can make 95 requests at 11:59:59 and 95 more at 12:00:01 - 190 requests in two seconds while staying under a "100 per minute" limit.',
    },
    {
...
<Table
  ...
  content={{
    ...
    thead: {
      th: ['Layer', 'Latency', 'Tools', 'Granularity', 'Best For'],
    },
    tbody: {
      tr: [
        {
          th: 'Edge/CDN',
          td: ['Lowest', 'Cloudflare, CloudFront, Fastly', 'IP, geography', 'DDoS, bot protection'],
        },
        {
          th: 'API Gateway',
          td: ['Low', 'AWS API Gateway, Kong, nginx', 'API key, route', 'API quotas, tiered plans'],
        },
        {
          th: 'Service Mesh',
          td: ['Low', 'Istio, Envoy, Linkerd', 'Service identity', 'Service-to-service limits'],
        },
        {
          th: 'Application',
          td: ['Higher', 'Custom middleware', 'User, action, context', 'Fine-grained business logic'],
        },
      ],
    },
  }}
  ...
  content={{
    ...
    tbody: {
      tr: [
        ...
        {
          th: 'Sustained 2 RPS',
          td: ['First ~15 allowed (burst + refill), then 50% rejected'],
        },
        {
...
<List
  ...
  items={[
    {
      lead: 'Token bucket (10 tokens, 1/s refill): All 10 served instantly, ~0ms latency each',
      color: "bg-page-inverse",
    },
    {
      lead: 'Leaky bucket (10 queue, 1/s leak): Queued, served over 10 seconds—0ms, 1s, 2s, ... 9s latency',
      color: "bg-page-inverse",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Characteristic', 'Token Bucket', 'Leaky Bucket'],
    },
    tbody: {
      tr: [
        {
          th: 'Burst handling',
          td: ['Allows bursts up to capacity', 'Smooths all traffic'],
        },
        {
          th: 'Output pattern',
          td: ['Bursty (matches input)', 'Constant rate'],
        },
        {
          th: 'Latency added',
          td: ['None (immediate decision)', 'Queue wait time'],
        },
        {
          th: 'Best for',
          td: ['API rate limiting', 'Traffic shaping'],
        },
        {
          th: 'Rejection behavior',
          td: ['Immediate 429', 'Queue, then overflow'],
        },
      ],
    },
    figure: 'Token bucket vs leaky bucket comparison.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Strategy', 'Best For', 'Watch Out For'],
    },
    tbody: {
      tr: [
        {
          th: 'IP Address',
          td: ['Anonymous, anti-bot', 'NAT, proxies, shared IPs'],
        },
        {
          th: 'API Key',
          td: ['B2B, developer APIs', 'Key sharing, stolen keys'],
        },
        {
          th: 'User ID',
          td: ['Consumer apps', 'Auth overhead'],
        },
        {
          th: 'IP + Endpoint',
          td: ['Mixed anonymous/auth', 'Configuration complexity'],
        },
        {
          th: 'User + Action',
          td: ['Fine-grained control', 'Key explosion'],
        },
      ],
    },
    figure: 'Rate limit key strategies.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'IP address',
      text: "Requires no authentication and blocks distributed attacks, but corporate NAT can share one IP across thousands of users. I've seen legitimate customers hit rate limits because 500 employees shared a single public IP — each got 0.2 requests per minute.",
    },
    {
      lead: 'API key',
      text: 'The standard for B2B APIs. Per-customer limits, usage tracking, revocation — all straightforward. The risk is key sharing: customers distributing their key to multiple applications, or keys getting stolen and abused.',
    },
    {
      lead: 'User ID',
    ...
    },
    {
      ...
      text: 'Gives you fine-grained control at the cost of complexity and key explosion. Useful when different operations need different limits.',
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'IETF rate limit headers.',
    thead: {
      th: ['Header', 'Meaning', 'Example'],
    },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">RateLimit-Limit</span>',
          td: ['Maximum requests allowed', '<span class="font-mono">100</span>'],
        },
        {
          th: '<span class="font-mono">RateLimit-Remaining</span>',
          td: ['Requests left in window', '<span class="font-mono">47</span>'],
        },
        {
          th: '<span class="font-mono">RateLimit-Reset</span>',
          td: ['Seconds until reset', '<span class="font-mono">30</span>'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: '429 Too Many Requests',
      text: 'Client exceeded their rate limit. Include `Retry-After` and rate limit headers.',
    },
    {
      lead: '503 Service Unavailable',
      text: 'System-wide overload, not per-client limiting. Use sparingly.',
    },
    {
      lead: '200 with Warning',
      text: 'Optional. Add `Warning: 299 - "Rate limit 80% consumed"` to help clients self-regulate.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Burst handling',
      text: 'Send requests up to capacity simultaneously. Exactly N should succeed.',
    },
    {
      lead: 'Refill behavior',
      text: 'Drain the bucket, wait for refill period, verify new requests succeed.',
    },
    {
    ...
    },
    {
      ...
      text: 'The header value should match when requests actually start succeeding.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Rejection rate',
      text: "High sustained rejection for legitimate clients means misconfiguration. High during an attack means it's working.",
    },
    {
      lead: 'Rate limiter latency',
      text: 'Redis calls should be under 5ms P99. If slower, check network or Redis load.',
    },
    {
      lead: 'Redis errors',
      text: "Any failures here mean rate limiting isn't happening. Alert immediately.",
    },
    {
...
<List
  ...
  items={[
    {
      lead: 'Fail open',
      text: "allows all requests when the rate limiter can't be reached. This preserves availability but removes protection. One compromised API key during an outage can generate unbounded requests.",
    },
    {
      lead: 'Fail closed',
      text: 'rejects all requests when state is unavailable. This maintains protection but sacrifices availability. A Redis blip becomes a full outage.',
    },
    {
      lead: 'Fail with local fallback',
      text: "uses in-memory rate limiting per instance when Redis is unavailable. It's not perfectly accurate — each pod limits independently — but it provides some protection while maintaining availability.",
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: 'Choose algorithms by burst tolerance',
      text: 'Token bucket allows bursts, leaky bucket smooths traffic, sliding window prevents boundary exploits',
    },
    {
      lead: 'Identify clients carefully',
      text: 'IP for anonymous endpoints, API key for B2B, user ID for authenticated actions',
    },
    {
      lead: 'Communicate clearly',
      text: 'Always return `Retry-After` and rate limit headers, even on successful responses',
    },
    {
      lead: 'Plan for failures',
      text: 'Decide your failure policy before Redis goes down, not during the incident',
    },
  ]}
/>
