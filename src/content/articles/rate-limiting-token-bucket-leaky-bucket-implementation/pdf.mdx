...
<Diagram
  title="Same burst traffic, different outcomes — fixed window allows 2x limit while token bucket enforces capacity."
  description={
...
```mermaid
flowchart TD
  subgraph sameBurst[Same 190 Requests in 2 Seconds]
    INPUT[Client sends 95 at :59<br/>then 95 at :00]
  end

  subgraph fixedWindowResult[Fixed Window Result]
    FW[All 190 allowed]
  ...
  subgraph tokenBucketResult[Token Bucket Result]
...
```mermaid
flowchart TD
    subgraph slidingWindowWeightedCalculation[Sliding Window Weighted Calculation]
