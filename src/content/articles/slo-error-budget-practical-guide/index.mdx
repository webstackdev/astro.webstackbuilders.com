...
tags: ["reliability-and-testing","prometheus","typescript"]
featured: true
---
...
<Table
  ...
  content={{
    figure: 'Common SLO objections and what they really mean.',
    thead: {
      th: ['Objection', 'What They Really Mean', 'How to Respond'],
    },
    tbody: {
      tr: [
        {
          th: '"We don\'t have time for this"',
          td: ['SLOs seem like extra work on top of everything else', 'SLOs reduce fire-fighting time; show them how much time they spend on incidents now'],
        },
        {
          th: '"Our service is simple"',
          td: ['Reliability engineering seems like overkill', 'Simple services still have consumers waiting when they fail'],
        },
        {
          th: '"Nobody has complained"',
          td: ['No complaints means no problem', 'Consumers have given up complaining; ask them directly'],
        },
        {
          th: '"We\'d need to hire SREs"',
          td: ['SLOs require special skills we don\'t have', 'Basic SLOs need basic math, not specialists'],
        },
        {
          th: '"Management won\'t care"',
          td: ['No executive sponsorship visible', 'Frame as developer productivity, not reliability jargon'],
        },
      ],
    },
  }}
  ...
  items={[
    {
      ...
      text: "Engineers spend 8 hours per week debugging integration issues with flaky dependencies. Here's the Jira query.",
    },
    {
      lead: 'Product managers',
      text: "We can't give customers reliable ETAs on features because deployments fail unpredictably.",
    },
    {
      lead: 'Finance',
      text: "CI pipelines are slow because internal tools timeout randomly-here's the compute cost of those retries.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: 'SLI evaluation criteria.',
    thead: {
      th: ['Criterion', 'Question', 'Why It Matters'],
    },
    tbody: {
      tr: [
        {
          th: 'User-facing',
          td: ['Does it measure what users actually experience?', 'Internal metrics often hide user pain'],
        },
        {
          th: 'Measurable',
          td: ['Can we collect this data reliably at all times?', "Can't set objectives without consistent data"],
        },
        {
          th: 'Specific',
          td: ['Does it target a single failure mode?', 'Composite metrics hide root causes'],
        },
        {
          th: 'Actionable',
          td: ['Can we improve it when it degrades?', 'Unactionable metrics cause learned helplessness'],
        },
        {
          th: 'Understandable',
          td: ['Can non-engineers grasp what it means?', 'Stakeholder buy-in requires clarity'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'CPU utilization as availability proxy:',
      text: "High CPU doesn't mean users are affected. A service can return errors at 20% CPU (bad config) or run fine at 95% CPU (efficient under load).",
    },
    {
      lead: 'Internal health check success:',
      text: 'Health checks often test whether the process is running, not whether it can serve user requests. A service with a working health endpoint can still fail to connect to its database.',
    },
    {
      lead: 'Average latency:',
      text: 'Averages hide tail latency. If 1% of users wait 10 seconds while 99% get responses in 100ms, the average is ~200ms. That 1% is invisible in the number but very visible in support tickets.',
    },
    {
      lead: 'Uptime percentage without error rate:',
      text: 'The process running does not mean it is serving correctly. A service can be up while returning 500 errors to every request.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Search service:',
      text: 'Results returned within expected count range (not zero when matches exist)',
    },
    {
      lead: 'Data API:',
      text: 'Response contains all requested fields with correct types',
    },
    {
      lead: 'Payment processing:',
      text: 'Successful charges actually create orders in the order system',
    },
    {
      lead: 'Recommendation engine:',
      text: 'Recommendations are personalized, not fallback defaults',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Real-time (< 1 second)',
      text: 'Trading systems, chat, live dashboards',
    },
    {
      lead: 'Near real-time (< 1 minute)',
      text: 'Activity feeds, notification counts, monitoring dashboards',
    },
    {
      lead: 'Periodic (< 1 hour)',
      text: 'Reports, analytics, batch-processed aggregations',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      text: 'Measure current state for 2-4 weeks',
    },
    {
      text: 'Set initial SLO at or slightly below current performance',
    },
    {
      text: 'Run for one quarter',
    },
    {
      text: 'Adjust based on what you learn',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Error budget = 100% - 99.9% = 0.1%',
      color: 'bg-secondary',
    },
    {
      lead: 'Minutes in month = 30 days × 24 hours × 60 minutes = 43,200',
      color: 'bg-info',
    },
    {
      lead: 'Allowed downtime = 43,200 × 0.001 = 43.2 minutes',
      color: 'bg-success',
    },
  ]}
/>
...
<Diagram
  ...
  size="full"
  title="From SLI measurement to error budget decision flow."
  description="A left-to-right flowchart showing how reliability measurements become deployment decisions. The flow starts with SLI: Actual Measurements, which feed into SLO: Target Level. The SLO then determines the Error Budget, defined as 100 percent minus the SLO target. Actual errors consume that budget, producing a decision point labeled Budget Remaining? If the answer is yes, the flow continues to Ship Features. If the answer is no, the flow moves to Focus on Reliability. The diagram illustrates that teams use measured service performance and remaining budget to decide whether to prioritize velocity or reliability work."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Common SLO types and their error budgets.',
    thead: {
      th: ['SLO Type', 'Target', 'Monthly Budget', 'What Consumes It'],
    },
    tbody: {
      tr: [
        {
          th: 'Availability',
          td: ['99.9%', '43 minutes downtime', 'Outages, bad deployments, maintenance windows'],
        },
        {
          th: 'Latency (P95)',
          td: ['< 200ms', '5% slow requests allowed', 'Traffic spikes, database issues, cold starts'],
        },
        {
          th: 'Error Rate',
          td: ['< 0.1%', '1 in 1000 requests can fail', 'Bugs, dependency failures, malformed input'],
        },
      ],
    },
  }}
...
<Table
  variant="criteria-highlight-table"
  content={{
    figure: 'Error budget policy thresholds and corresponding actions.',
    thead: {
      th: ['Budget Remaining', 'Status', 'What Changes'],
    },
    tbody: {
      tr: [
        {
          th: '> 50%',
          td: ['Green', 'Normal velocity. Feature work prioritized. Deploy at will.'],
        },
        {
          th: '25% - 50%',
          td: ['Yellow', 'Increased scrutiny. Reliability work gets equal priority. Review risky changes.'],
        },
        {
          th: '10% - 25%',
          td: ['Orange', 'Feature freeze for high-risk changes. SRE approval required for deployments.'],
        },
        {
          th: '0% - 10%',
          td: ['Red', 'Complete feature freeze. Only reliability improvements and critical fixes deployed.'],
        },
        {
          th: 'Exhausted',
          td: ['Black', 'Emergency measures. VP approval required for any deployment. Incident review mandatory.'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      title: 'Product',
      text: 'Wants features shipped fast',
      icon: "clipboard",
      color: "warning",
      bgColor: "warning-inverse",
    },
    {
      title: 'Engineering',
      text: 'Wants to try new things (which sometimes break)',
      icon: "dns",
      color: "info",
    ...
    {
      title: 'SRE/Ops',
      text: 'Wants stability',
      icon: "chart-line",
      color: "success",
      bgColor: "success-inverse",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Feature launches:',
      text: 'Up to 20% of monthly budget. Features are why the product exists. Spending budget to ship value to users is the point.',
    },
    {
      lead: 'Infrastructure migrations:',
      text: 'Up to 30%, scheduled when budget is healthy. Migrations reduce tech debt and often improve reliability long-term.',
    },
    {
      lead: 'Dependency upgrades:',
      text: 'Up to 15%. Staying current prevents security vulnerabilities and keeps you off unsupported versions.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Untested deployments:',
      text: 'Unpredictable budget impact',
    },
    {
      lead: 'Friday deployments:',
      text: 'Limited recovery time if issues arise',
    },
    {
      lead: 'Large batch deployments:',
      text: 'High risk, split into smaller changes',
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Weekly error budget status for payment-api (30-day rolling window).',
    thead: {
      th: ['Metric', 'Target', 'Current', 'Budget Used', 'Status'],
    },
    tbody: {
      tr: [
        {
          th: 'Availability',
          td: ['99.9%', '99.95%', '15%', 'Healthy'],
        },
        {
          th: 'Latency P99',
          td: ['200ms', '180ms', '10%', 'Healthy'],
        },
      ],
    },
  }}
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Common existing data sources and the SLIs they enable.',
    thead: {
      th: ['Source', 'What It Provides', 'SLIs Possible'],
    },
    tbody: {
      tr: [
        {
          th: 'Access logs (nginx, ALB)',
          td: ['Request count by status, response time', 'Availability, Latency'],
        },
        {
          th: 'Application logs',
          td: ['Error messages, business logic failures', 'Error rate, Quality'],
        },
        {
          th: 'Cloud provider metrics',
          td: ['Request counts, latency percentiles', 'Availability, Latency'],
        },
        {
          th: 'Database slow query log',
          td: ['Queries exceeding threshold', 'Latency contribution'],
        },
        {
          th: 'CDN logs (CloudFront, Fastly)',
          td: ['Edge response times, cache hit rates', 'End-user latency'],
        },
        {
          th: 'APM tools (if present)',
          td: ['Transaction traces, error rates', 'All categories'],
        },
      ],
    },
  }}
...
<Download
  resource="slo-error-budget-practical-guide"
  title="SLIs, SLOs, and Error Budgets: A Practical Guide"
  description="From choosing user-centric metrics to negotiating reliability tradeoffs with stakeholders — the complete framework for service level management."
  listItems={[
    {
      text: "SLO target setting worksheet",
    },
    {
      text: "Error budget policy templates",
    },
    {
      text: "Burn rate alert examples",
    },
    {
      text: "Stakeholder alignment playbook",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'SLIs must reflect user experience, not internal metrics',
    },
    {
      lead: 'SLOs should be achievable with current architecture (start with reality, improve over time)',
    },
    {
      lead: 'Error budgets convert reliability into a spendable resource',
    },
    {
      lead: 'Budget policies remove politics from reliability decisions',
    },
    {
      lead: 'When budget is healthy, spend it on velocity; when depleted, invest in reliability',
    },
  ]}
/>
