...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "The distinction that separates real chaos engineering from breaking things randomly.",
    thead: {
    ...
    tbody: {
      tr: [
        ...
        {
          th: "Proactive resilience work",
          td: ["Reactive incident response"],
        },
      ],
    },
  }}
...
<Diagram
  src={theScientificCycleDiagram}
  magnifyButtonPosition="bottom-left"
  size="full"
  title="The scientific cycle of chaos engineering"
  description="Flowchart showing the chaos engineering loop: form a hypothesis, define steady state, plan the experiment, contain blast radius, run the experiment, observe behavior, and learn and improve before repeating the cycle."
/>
...
Code: Hypothesis template with steady state definition and expected outcomes._
...
<Diagram
  src={progressiveChaosDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Progressive chaos engineering maturity levels"
  description="Flowchart showing four maturity levels progressing from non-production experiments to low-risk production experiments, production experiments with live traffic, and finally automated chaos, with each level annotated by the kind of learning it enables."
/>
...
<Table
  ...
  content={{
    ...
    thead: {
      th: ["Tool", "Complexity", "Best For", "Kubernetes Required"],
    },
    tbody: {
      tr: [
        {
          th: "kubectl",
          td: ["Low", "First experiments, simple pod/node chaos", "Yes"],
        },
        {
          th: "tc/iptables",
          td: ["Medium", "Network latency and packet loss", "No (Linux only)"],
        },
        {
          th: "toxiproxy",
          td: ["Medium", "Surgical dependency failure injection", "No"],
        },
        {
          th: "Chaos Toolkit",
          td: ["Medium", "Structured experiments with hypothesis validation", "No (has K8s extension)"],
        },
        {
          th: "LitmusChaos",
          td: ["High", "GitOps-managed, Kubernetes-native chaos", "Yes"],
        },
      ],
    },
  }}
...
<Diagram
  src={threeDimensionsDiagram}
  magnifyButtonPosition="top-left"
  title="Three dimensions of blast radius control"
  description="Flowchart showing blast radius control divided into scope limiting, time limiting, and impact limiting, with examples such as targeting a single pod, setting maximum duration and automatic aborts, and starting with low-traffic non-critical paths."
/>
  ...
  items={[
    {
      ...
      text: "If your container takes 45 seconds to start, that's 45 seconds of degraded capacity. Consider optimizing image size, adding startup probes, or adjusting replica counts.",
    },
    {
      lead: "Missing readiness probes cause traffic to unhealthy pods.",
      text: "Without a readiness probe, Kubernetes sends traffic to pods that aren't ready, causing errors during startup.",
    },
    {
...
<List
  ...
  items={[
    {
      lead: "Missing timeouts cause request pileup.",
      text: "Without timeouts, requests queue indefinitely waiting for the slow database, exhausting threads and memory.",
    },
    {
      lead: "Connection pool exhaustion.",
      text: "Under latency, connections are held longer, and the pool runs dry before requests complete.",
    },
    {
      lead: "No circuit breaker means failures cascade.",
      text: "Without a circuit breaker, every request tries the slow path, and the latency spreads to callers of your service.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Hard dependency on cache, no fallback.",
      text: "The code assumes cache is always available and throws exceptions on connection failure instead of falling back.",
    },
    {
      lead: "Timeouts too long, requests queue.",
      text: "Connection timeouts of 30 seconds mean requests pile up waiting for a cache that will never respond.",
    },
    {
      lead: "Cache reconnection logic is buggy.",
      text: "After the partition heals, the service doesn't reconnect properly and continues failing.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "No memory limits set.",
      text: "Without limits, one pod's memory pressure affects the entire node — the classic noisy neighbor problem.",
    },
    {
      lead: "OOM kill with no graceful handling.",
      text: "The application gets killed abruptly with no opportunity to drain connections or save state.",
    },
    {
      lead: "Memory leaks exposed under pressure.",
      text: "Baseline memory usage creeps up over time, and the stress test just accelerates the inevitable OOM.",
    },
  ]}
/>
...
<Diagram
  ...
  magnifyButtonPosition="top-left"
  title="Evolution from ad-hoc experiments to mature chaos practice"
  description="Flowchart showing a progression from ad-hoc experiments to regular experiments, then game days, and finally automated chaos, with each stage annotated by the learning or organizational capability it adds."
/>
...
<Table
  ...
  content={{
    ...
    thead: {
      th: ["Focus", "Quarter", "Activities", "Success Criteria"],
    },
    tbody: {
      tr: [
        {
