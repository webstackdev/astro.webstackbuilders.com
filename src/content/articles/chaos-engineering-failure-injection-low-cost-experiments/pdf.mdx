...
<Table
  ...
  content={{
    ...
    thead: { th: ["Chaos Engineering IS", "Chaos Engineering IS NOT"] },
    tbody: {
      tr: [
        { th: "Hypothesis-driven experimentation", td: ["Random destruction"] },
        { th: "Controlled failure injection", td: ["Breaking things for fun"] },
        { th: "Learning about system behavior", td: ["Proving you can cause outages"] },
        { th: "Building confidence", td: ["Testing in production blindly"] },
        { th: "Proactive resilience work", td: ["Reactive incident response"] },
      ],
    },
  }}
/>
...
<List
  ...
  items={[
    { lead: "_Slow container startup delays recovery._", text: "If your container takes 45 seconds to start, that's 45 seconds of degraded capacity. Consider optimizing image size, adding startup probes, or adjusting replica counts." },
    { lead: "_Missing readiness probes cause traffic to unhealthy pods._", text: "Without a readiness probe, Kubernetes sends traffic to pods that aren't ready, causing errors during startup." },
    { lead: "_No alerts fired despite degradation._", text: "This is the \"aha\" moment—your monitoring gap was invisible until you tested it." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "_Missing timeouts cause request pileup._", text: "Without timeouts, requests queue indefinitely waiting for the slow database, exhausting threads and memory." },
    { lead: "_Connection pool exhaustion._", text: "Under latency, connections are held longer, and the pool runs dry before requests complete." },
    { lead: "_No circuit breaker means failures cascade._", text: "Without a circuit breaker, every request tries the slow path, and the latency spreads to callers of your service." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "_Hard dependency on cache, no fallback._", text: "The code assumes cache is always available and throws exceptions on connection failure instead of falling back." },
    { lead: "_Timeouts too long, requests queue._", text: "Connection timeouts of 30 seconds mean requests pile up waiting for a cache that will never respond." },
    { lead: "_Cache reconnection logic is buggy._", text: "After the partition heals, the service doesn't reconnect properly and continues failing." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "_No memory limits set._", text: "Without limits, one pod's memory pressure affects the entire node—the classic noisy neighbor problem." },
    { lead: "_OOM kill with no graceful handling._", text: "The application gets killed abruptly with no opportunity to drain connections or save state." },
    { lead: "_Memory leaks exposed under pressure._", text: "Baseline memory usage creeps up over time, and the stress test just accelerates the inevitable OOM." },
  ]}
/>
