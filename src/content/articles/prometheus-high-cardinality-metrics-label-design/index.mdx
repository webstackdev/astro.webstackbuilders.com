...
_"Just add a label for debugging"_ is the most dangerous sentence in observability.
...
<Table
  ...
  content={{
    thead: {
      th: ['Design', 'Labels', 'Values per Label', 'Total Series', 'Approx. Memory'],
    },
    tbody: {
      tr: [
        {
          th: 'Minimal',
          td: ['method, handler, status_class', '5 × 20 × 4', '400', '~4 MB'],
        },
        {
          th: 'Reasonable',
          td: ['+ environment, region', '× 3 × 4', '4,800', '~48 MB'],
        },
        {
          th: 'Dangerous',
          td: ['+ user_id', '× 100,000', '480,000,000', '~4.8 TB'],
        },
      ],
    },
    figure: 'Cardinality explosion by label design.',
  }}
/>
...
<Diagram
  src={singleUnboundedLabelDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="A single unbounded label transforms manageable metrics into infrastructure-killing cardinality."
  description="Top-to-bottom flowchart showing how metric cardinality multiplies across labels. In the grouped section Label Multiplication, a metric named http_requests is followed by labels method with 4 values, endpoint with 10 values, status with 5 values, and user_id with 100 thousand values. One path stops before user_id and enters the grouped section Without user_id, producing 200 series from 4 times 10 times 5 and an estimated memory cost of about 2 megabytes. A second path continues through the unbounded user_id label into the grouped section With user_id, producing 20 million series from 4 times 10 times 5 times 100 thousand and an estimated memory cost of about 200 gigabytes. The manageable series count and memory estimate are highlighted positively, while the exploded series count and memory estimate are highlighted negatively. The diagram illustrates how adding a single high-cardinality label can turn a safe metric design into one that overwhelms Prometheus memory."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Failure Mode', 'Symptoms', 'Root Cause'],
    },
    tbody: {
      tr: [
        {
          th: 'Gradual OOM',
          td: [
            'Memory steadily rising, restarts under load',
            'Cardinality growing over time as more label values become active',
          ],
        },
        {
          th: 'Sudden OOM',
          td: ['Memory spike during incident, crash, monitoring blackout', 'Traffic spike activating previously dormant series'],
        },
        {
          th: 'Query OOM',
          td: ['Specific dashboard or query crashes Prometheus', 'Query touching high-cardinality metric with aggregation'],
        },
        {
          th: 'Slow startup',
          td: ['Minutes to start, high CPU during WAL replay', 'Rebuilding index for millions of series'],
        },
      ],
    },
    figure: 'Cardinality failure modes.',
  }}
/>
...
<Table
  ...
  content={{
    figure: 'Good label examples with bounded cardinality.',
    thead: {
      th: ['Label', 'Values', 'Cardinality', 'Why It Works'],
    },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">http_method</span>',
          td: [
            '<span class="font-mono">GET</span>, <span class="font-mono">POST</span>, <span class="font-mono">PUT</span>, <span class="font-mono">DELETE</span>, <span class="font-mono">PATCH</span>',
            '~7',
            'Fixed set defined by HTTP spec',
          ],
        },
        {
          th: '<span class="font-mono">environment</span>',
          td: [
            '<span class="font-mono">production</span>, <span class="font-mono">staging</span>, <span class="font-mono">development</span>',
            '3',
            'You control how many environments exist',
          ],
        },
        {
          th: '<span class="font-mono">status_class</span>',
          td: ['<span class="font-mono">2xx</span>, <span class="font-mono">3xx</span>, <span class="font-mono">4xx</span>, <span class="font-mono">5xx</span>',
          '4',
          'Bucketed from individual codes',
          ],
        },
        {
          th: '<span class="font-mono">region</span>',
          td: [
            '<span class="font-mono">us-east-1</span>, <span class="font-mono">us-west-2</span>, <span class="font-mono">eu-west-1</span>',
            '~10',
        ...
        },
        {
          ...
          td: [
            '<span class="font-mono">api</span>, <span class="font-mono">worker</span>, <span class="font-mono">scheduler</span>',
            '~20',
...
<Table
  variant="numbered-review-table"
  content={{
    figure: 'Bad label examples to avoid.',
    thead: {
      th: ['Label', 'Problem', 'Cardinality', 'Alternative'],
    },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">user_id</span>',
          td: ['Grows with user base', 'Millions', 'Traces, logs'],
        },
        {
          th: '<span class="font-mono">request_id</span>',
          td: ['Unique per request', 'Infinite', 'Exemplars'],
        },
        {
          th: '<span class="font-mono">client_ip</span>',
          td: ['Huge address space', 'Billions', 'Log analysis'],
        },
        {
          th: '<span class="font-mono">error_message</span>',
          td: ['Free-form strings', 'Unbounded', 'Error codes (bucketed)'],
        },
        {
          th: '<span class="font-mono">email</span>',
          td: ['Unique per user, PII risk', 'Millions', "Don't store in metrics"],
        },
        {
          th: '<span class="font-mono">query</span>',
      ...
      tr: [
        {
          th: 'SDK wrapper',
          td: [
            'Lowest cardinality (bucketed at source), consistent behavior across apps, type-safe interfaces',
            'Requires managing wrapper versions, upgrade coordination, transition period with mixed versions',
          ],
        },
        {
          th: 'Server-side relabeling',
          td: [
            'Single source of truth, no app changes needed, works with any exporter',
            'Config complexity grows with edge cases, relabeling regex can incur high memory and CPU usage',
          ],
        },
        {
          th: 'Raw data + recording rules',
          td: [
            'Maximum flexibility, easy to change definitions, preserves raw data for debugging',
            'Higher storage costs, more memory usage, recording rule maintenance',
          ],
        },
      ],
    },
    figure: 'Label standardization approaches compared.',
  }}
/>
...
Finding __which__ metrics are eating your cardinality budget requires a different query. The `count by (__name__)` aggregation groups all series by metric name, letting you identify the top offenders. More often than not, one or two metrics dominate — fixing those gives you most of your headroom back.
...
Finally, per-scrape alerts help you identify __which__ target is causing the problem. When a single scrape returns 50,000 samples, that target deserves investigation — either it's genuinely high-cardinality, or something is misconfigured.
...
Everything we've covered so far focuses on preventing high cardinality at the source. But what about metrics that are __legitimately__ high-cardinality? Recording rules let you pre-compute aggregations, reducing cardinality at query time without losing the ability to drill down when needed.
...
<Table
  ...
  content={{
    thead: {
      th: ['Strategy', 'Use Case', 'Cardinality Impact'],
    },
    tbody: {
      tr: [
        {
          th: 'Recording rules',
          td: ['Pre-aggregate for dashboards/alerts', 'Reduces query-time cardinality'],
        },
        {
          th: 'Federation',
          td: ['Multi-cluster aggregation', 'Central sees only aggregates'],
        },
        {
          th: 'Remote write filtering',
          td: ['Long-term storage', 'Only ships low-cardinality metrics'],
        },
        {
          th: 'Hierarchical aggregation',
          td: ['Large-scale deployments', 'Prevents linear scaling'],
        },
      ],
    },
    figure: 'Aggregation strategy comparison.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: '1. Code review',
      text: 'Add cardinality review to your PR checklist. Require justification for every new label: "what bounded set of values will this have?"',
    },
    {
      lead: '2. CI/CD gates',
      text: 'Lint metric definitions for unbounded patterns. Flag any label that looks like an ID, UUID, email, or IP address.',
    },
    {
      lead: '3. Metric unit testing',
      text: 'Write unit tests that exercise metric instrumentation. If a test generates label combinations in a loop, fail the build when unique series exceed a threshold. This catches cardinality bugs before they reach production.',
    },
    {
      lead: '4. Staging validation',
      text: 'Deploy to staging first and check cardinality growth before promoting to production.',
    },
    {
      lead: '5. Monitoring',
      text: 'Alert on series count approaching limits _and_ on growth rate. A 10% hourly increase that goes unnoticed for a day means you\'ve doubled.',
    },
    {
      lead: '6. Documentation',
      text: 'Maintain an approved label schema. Document cardinality budgets per service.',
    },
  ]}
/>
