---
title: "High-Cardinality Metrics: Prometheus Label Horror"
description: "What happens when unbounded label values explode your metrics storage, and how to design around it."
author: "kevin-brown"
cover: "./download.jpg"
coverAlt: "Prometheus pressure cooker with memory gauge spiking to red zone from unbounded labels, showing bounded versus unbounded label impact"
publishDate: 2022-09-18
isDraft: false
fileType: "PDF"
fileSize: "1.6 MB"
pages: 33
fileName: "prometheus-high-cardinality-metrics-label-design.pdf"
---

Every unique combination of metric name and label values creates a separate time series in Prometheus. Add three labels each with 100 values and you've created up to one million time series. A team added user_id to their API response time histogram—with 50 endpoints, 10 status codes, and 100,000 users, they created 50 million potential time series. Initial memory usage was fine (only active users generated metrics), but over months more users became active, and a traffic spike caused Prometheus to OOM during an incident. The fix: remove user_id, add it to traces, implement cardinality limits. Prometheus stabilized at 50,000 series.

Labels are multiplicative, not additive. A single unbounded label can destroy your Prometheus deployment.

This complete guide teaches you:

<List
  variant="check-icons-list"
  items={[
    { text: "Time series math: how label combinations create cardinality explosions" },
    { text: "Cardinality limits: setting hard boundaries in Prometheus and scrape configs" },
    { text: "Label design patterns: structuring labels for debugging without cardinality risk" },
    { text: "Histograms and cardinality: why they're especially dangerous with high-cardinality labels" },
    { text: "Bounded versus unbounded labels: user IDs, request IDs, and what not to label" },
    { text: "Prometheus internals: head blocks, indexing, and memory overhead" },
    { text: "Monitoring cardinality: tsdb info queries and cardinality tracking" },
    { text: "Cardinality troubleshooting: identifying the worst offenders" },
    { text: "Alternatives to high-cardinality labels: traces, logs, and out-of-band metadata" }
  ]}
/>

Download Your Prometheus Cardinality Guide now to design labels that don't explode your metrics storage.
