  ...
  content={{
    ...
    tbody: {
      tr: [
        ...
        {
          th: 'Connection reset, packets dropped',
          td: [
...
<List
  ...
  items={[
    ...
    {
      lead: 'SNI issues',
      text: 'occur when multiple services share a load balancer IP. The server uses SNI to decide which certificate to return. If SNI isn\'t sent (older clients, misconfigured tools), you get the default certificate, which might not match the hostname you requested.',
    },
    {
      lead: 'Expired certificates',
      text: "hit internal services harder than public ones. Let's Encrypt certificates auto-renew. Internal certificates often don't, and they're not monitored as closely. A certificate expires at 2am, and suddenly half your services are down.",
    },
  ]}
/>
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Linux system',
          td: ['Copy to `/etc/ssl/certs/` and run `update-ca-certificates`'],
        },
        {
          th: 'macOS',
          td: [
            'Run `security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ca.crt`',
          ],
        },
        {
          th: 'Node.js',
          td: ['Set `NODE_EXTRA_CA_CERTS=/path/to/ca.crt`'],
        },
        {
          th: 'Python',
          td: ['Set `REQUESTS_CA_BUNDLE=/path/to/ca.crt`'],
        },
        {
          th: 'Java',
          td: [
            'Import with `keytool -import -trustcacerts -file ca.crt -keystore truststore.jks`',
          ],
        },
        {
          th: 'Go',
          td: ['Set `SSL_CERT_FILE=/path/to/ca-bundle.crt`'],
        },
      ],
    },
...
<List
  ...
  items={[
    ...
    {
      text: 'Disable or restrict the public endpoint',
    },
    {
      text: 'Update monitoring and runbooks for the new architecture',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Peering',
      text: 'Routes missing on one side, DNS resolution not enabled, zone not associated with both VPCs',
    },
    {
      lead: 'Transit Gateway',
      text: 'Blackhole routes (attachment deleted but route remains), wrong route table association, asymmetric routing on return path',
    },
    {
      lead: 'Private Link',
      text: 'Connection stuck in "pending" (provider must accept), endpoint unavailable (check NLB health targets), private DNS not working (verify it\'s enabled on the endpoint)',
    },
  ]}
/>
