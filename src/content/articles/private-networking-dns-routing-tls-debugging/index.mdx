...
tags: ["cloud-platforms", "aws", "azure"]
featured: true
---

import dnsResolutionDiagram from "./diagrams/dns-resolution-flow.jpg"
import tlsHandshakeDiagram from "./diagrams/tls-handshake-with-failure-points.jpg"
...
<Table
  ...
  content={{
    thead: {
      th: ['Symptom', 'Likely Cause', 'Fix'],
    },
    tbody: {
      tr: [
        {
          th: 'Resolves to public IP instead of private',
          td: ['Using public DNS (8.8.8.8) instead of VPC resolver', 'Configure VPC-provided resolver'],
        },
        {
          th: 'NXDOMAIN for private endpoint',
          td: ['Private hosted zone not associated with VPC', 'Associate zone with VPC'],
        },
        {
          th: 'Same name resolves differently in different locations',
          td: ['Split-horizon DNS misconfiguration', 'Verify zone associations, compare dig results'],
        },
        {
          th: 'Old IP returned after endpoint change',
          td: ['DNS caching with high TTL', 'Flush cache, reduce TTL before migrations'],
        },
        {
          th: "Short names don't resolve",
          td: ['Missing search domain in resolv.conf', 'Add search domain or use FQDN'],
        },
      ],
    },
    figure: 'Private DNS failure modes.',
  }}
/>

<Diagram
  src={dnsResolutionDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="DNS resolution flow."
  description="Top-to-bottom flowchart with three grouped sections: Application, Resolution Chain, and Results. In Application, an app requests the hostname database.internal. The request enters the Resolution Chain, starting with a decision on whether the name exists in /etc/hosts. If it is found there, the flow goes directly to a successful result showing the private IP 10.0.1.50. If it is not found, the flow continues to the System Resolver, then to a Local Cache decision. If the answer is cached, it again returns the private IP. If it is not cached, the query goes to the VPC DNS Resolver, then to a Private Zone decision. If the private zone exists, the result is the private IP 10.0.1.50. If no private zone exists, the query falls through to Public DNS. Public DNS either returns a public IP 203.0.113.50 or returns NXDOMAIN if the name is not found. The diagram highlights the private IP result as the intended safe outcome, the public IP as a warning state, and NXDOMAIN as a failure state."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['DNS Pattern', 'Use Case', 'Configuration'],
    },
    tbody: {
      tr: [
        {
          th: 'Private hosted zone',
          td: ['Internal service discovery', 'Route 53 zone + VPC association'],
        },
        {
          th: 'VPC endpoint private DNS',
          td: ['AWS service private access', 'Enable on endpoint creation'],
        },
        {
          th: 'Resolver inbound',
          td: ['On-prem → AWS resolution', 'Inbound endpoint + security group'],
        },
        {
          th: 'Resolver outbound',
          td: ['AWS → on-prem resolution', 'Outbound endpoint + forwarding rules'],
        },
        {
          th: 'Split-horizon',
          td: ['Same name, different audiences', 'Separate public/private zones'],
        },
      ],
    },
    figure: 'Private DNS patterns.',
  }}
/>
...
<List variant="check-icons-list">
  <ListItem lead="Internet Gateway:">
    Routes <span class="font-mono">0.0.0.0/0</span> to the internet (requires public IP or NAT)
  </ListItem>
  <ListItem lead="NAT Gateway:">
    Outbound internet from private subnets (the NAT gateway itself must be in a public subnet)
  </ListItem>
  <ListItem lead="VPC Peering:">
    Direct connection to another VPC's CIDR (requires routes on both sides, no CIDR overlap)[^peering]
  </ListItem>
  <ListItem lead="Transit Gateway:">
    Hub-and-spoke for multiple VPCs (handles transitive routing)[^tgw]
  </ListItem>
  <ListItem lead="VPC Endpoint:">
    Routes to AWS services via prefix lists[^prefix]
  </ListItem>
</List>
...
<Table
  ...
  content={{
    figure: 'Routing failure modes.',
    thead: {
      th: ['Symptom', 'Cause', 'Fix'],
    },
    tbody: {
      tr: [
        {
          th: { label: 'Timeout (no error)', color: 'warning' },
          td: ['No route to destination', 'Add route for destination CIDR'],
        },
        {
          th: { label: 'Connection reset, packets dropped', color: 'warning' },
          td: [
        ...
        {
          th: { label: 'Packets silently dropped', color: 'danger' },
          td: ['Blackhole route (target deleted but route remains)', 'Remove stale routes'],
        },
        {
          th: { label: 'Traffic goes to wrong destination', color: 'danger' },
          td: [
            'Overlapping CIDRs across connected networks',
            'Redesign addressing to avoid overlaps',
          ],
        },
      ],
    },
...
<Table
  ...
  content={{
    thead: {
      th: ['Layer', 'Check', 'Command', 'If Fails'],
    },
    tbody: {
      tr: [
        {
          th: 'L3 (Network): Routing',
          td: ['Route exists?', '<span class="font-mono">ip route get \<ip\></span>', 'Add route for destination CIDR'],
        },
        {
          th: 'L3: Routing',
        ...
        },
        {
          ...
          td: [
            'Port reachable?',
            '<span class="font-mono">nc -zv -w 5 \<host\> \<port\></span>',
        ...
        },
        {
          ...
          td: [
            'Handshake succeeds?',
            '<span class="font-mono">openssl s_client -connect \<host\>:\<port\></span>',
    ...
    figure: 'Connectivity debugging checklist.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Check', 'Security Group', 'NACL'],
    },
    tbody: {
      tr: [
        {
          th: 'Scope',
          td: ['ENI/Instance', 'Subnet'],
        },
        {
          th: 'State',
          td: ['Stateful', 'Stateless'],
        },
        {
          th: 'Rules',
          td: ['Allow only', 'Allow + Deny'],
        },
        {
          th: 'Return traffic',
          td: ['Automatic', 'Must allow ephemeral ports'],
        },
        {
          th: 'Evaluation',
          td: ['All rules (any match)', 'Ordered (first match)'],
        },
        {
          th: 'Cross-VPC reference',
          td: ['Limited', 'CIDR only'],
        },
      ],
    },
    figure: 'Security group vs NACL comparison.',
  }}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: 'SNI issues',
      text: 'Occurs when multiple services share a load balancer IP. The server uses SNI to decide which certificate to return. If SNI isn\'t sent (older clients, misconfigured tools), you get the default certificate, which might not match the hostname you requested.',
    },
    {
      lead: 'Expired certificates',
      text: "Hits internal services harder than public ones. Let's Encrypt certificates auto-renew. Internal certificates often don't, and they're not monitored as closely. A certificate expires at 2am, and suddenly half your services are down.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Issue', 'Error Message', 'Solution'],
    },
    tbody: {
      tr: [
        {
          th: 'Hostname mismatch',
          td: ['"hostname doesn\'t match"', 'Add hostname to SAN, or use existing SAN name'],
        },
        {
          th: 'Untrusted CA',
          td: ['"unable to verify certificate"', 'Add CA to trust store'],
        },
        {
          th: 'Expired',
          td: ['"certificate has expired"', 'Renew certificate'],
        },
        {
          th: 'Wrong cert (SNI)',
          td: ['Certificate for wrong domain', 'Fix SNI or LB config'],
        },
        {
          th: 'Self-signed',
          td: ['"self signed certificate"', 'Add cert to trust store or use CA'],
        },
      ],
    },
    figure: 'Common TLS errors and solutions.',
  }}
/>
...
<Diagram
  src={tlsHandshakeDiagram}
  size="full"
  title="TLS handshake with failure points."
  description="Sequence diagram with two participants: Client and Server. A note marks the TLS Handshake. The client first sends ClientHello containing supported versions, cipher suites, and SNI. One alternate branch shows an SNI mismatch: the server returns the wrong certificate because no SNI match was found, the client performs hostname verification, and that verification fails with a hostname mismatch error. In the normal path, the server responds with ServerHello plus Certificate. Another alternate branch then shows certificate issues: the client verifies the certificate and checks whether the CA is trusted, whether the certificate is expired, and whether the hostname appears in the SAN list. If any of those checks fail, the client sends an alert indicating certificate_unknown. If verification succeeds, the client sends Key Exchange, the server sends Finished, the client sends Finished, and a note marks that the TLS session is established. Finally, the client sends encrypted application data to the server. The diagram shows both the successful handshake sequence and the common failure points around SNI and certificate validation."
/>
...
<Table
  variant="sticky-header-table"
  content={{
    figure: 'CA trust configuration by runtime.',
    thead: {
    ...
    tbody: {
      tr: [
        {
          th: 'Linux system',
          td: ['Copy to <span class="font-mono">/etc/ssl/certs/</span> and run `update-ca-certificates</span>'],
        },
        {
          th: 'macOS',
          td: [
            'Run <span class="font-mono">security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ca.crt</span>',
          ],
        },
        {
          th: 'Node.js',
          td: ['Set <span class="font-mono">NODE_EXTRA_CA_CERTS=/path/to/ca.crt</span>'],
        },
        {
          th: 'Python',
          td: ['Set <span class="font-mono">REQUESTS_CA_BUNDLE=/path/to/ca.crt</span>'],
        },
        {
          th: 'Java',
          td: [
            'Import with <span class="font-mono">keytool -import -trustcacerts -file ca.crt -keystore truststore.jks</span>',
          ],
        },
        {
          th: 'Go',
          td: ['Set <span class="font-mono">SSL_CERT_FILE=/path/to/ca-bundle.crt</span>'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Step 1: DNS',
      text: 'Does the name resolve to the expected IP? Use <span class="text-mono">getent hosts \<hostname\></span> (which respects system configuration) rather than just <span class="text-mono">dig</span>. Verify the IP is in the expected private range. Compare results from the VPC resolver versus public DNS to detect split-horizon issues.',
    },
    {
      lead: 'Step 2: Routing',
      text: 'Can the kernel find a path to that IP? Run <span class="text-mono">ip route get \<ip\></span> to see the routing decision. Check the VPC route table in the console for the source subnet. Look for blackhole routes where the target was deleted but the route remains.',
    },
    {
      lead: 'Step 3: Connectivity',
      text: 'Is the port reachable? Use <span class="text-mono">nc -zv -w 5 \<ip\> \<port\></span>. A timeout suggests routing or security group issues. "Connection refused" means packets arrive but nothing\'s listening — the service is down or on a different port.',
    },
    {
      lead: 'Step 4: TLS',
      text: 'Does the handshake succeed? Run <span class="text-mono">openssl s_client -connect \<ip\>:\<port\> -servername \<hostname\></span>. Check that the certificate isn\'t expired, the hostname appears in the SAN, and the CA is trusted.',
    },
    {
      lead: 'Step 5: Application',
      text: 'Does the application-level connection work? At this point, networking is fine. Check authentication credentials, authorization rules, and application-level allowlists (some services restrict by source IP).',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'DNS',
      text: 'Private hosted zone exists, associated with all source VPCs, records created for the private endpoint, TTL lowered (60 seconds or less) to enable quick rollback',
    },
    {
      lead: 'Routing',
      text: 'Routes exist from source subnets to the private endpoint subnet, VPC peering or transit gateway configured if cross-VPC, no CIDR conflicts',
    },
    {
      lead: 'Security',
      text: 'Security groups allow inbound from all source IPs/security groups, NACLs allow bidirectional traffic including ephemeral ports, endpoint policies permit required actions (for AWS VPC endpoints)',
    },
    {
      lead: 'TLS',
      text: 'Certificate includes the private DNS name in its SAN, CA trusted by all clients, certificate not expiring during the migration window',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Confirm all source IPs can reach the private endpoint',
    },
    {
      lead: 'Check that latency improved (it should — no internet hops)',
    },
    {
      lead: 'Verify VPC flow logs show no public IP traffic to the service',
    },
    {
      lead: 'Disable or restrict the public endpoint',
    },
    {
      lead: 'Update monitoring and runbooks for the new architecture',
    },
  ]}
/>
...
>
  <ListItem lead="VPC Peering">
    Works for simple topologies with 2-3 VPCs. It's a direct connection — fast and cheap. The
    limitations: no transitive routing (if A peers with B and B peers with C, A can't reach C
    through B), CIDRs can't overlap, and cross-region peering adds latency. Both sides need routes
    added to their route tables. For DNS, enable resolution in the peering connection settings and
    associate private hosted zones with both VPCs.
  </ListItem>
  <ListItem lead="Transit Gateway">
    Is the right choice when you have many VPCs or need transitive routing. Each VPC connects once
    to the gateway, which acts as a central router. You can also attach VPN connections or Direct
    Connect[^directconnect], giving on-premises networks access to all VPCs. The complexity is in
    route table management — transit gateway has its own route tables separate from VPC route
    tables, and misconfiguration causes blackholes.
  </ListItem>
  <ListItem lead="Private Link">
    (AWS PrivateLink or equivalent) exposes a specific service to other VPCs without any routing
    changes. The provider creates a Network Load Balancer and an endpoint service. Consumers create
    interface endpoints that appear as ENIs for AWS or the equivalent on other cloud providers in
    their VPC. This approach handles CIDR overlaps gracefully — the consumer never sees the
    provider's IP space. The tradeoff: it's service-by-service rather than network-wide
    connectivity.
  </ListItem>
  <ListItem lead="VPN">
    Provides encrypted connectivity over the public internet — useful when dedicated connections
    aren't available or cost-justified. Site-to-site VPN connects on-premises networks to cloud
    VPCs; client VPN gives individual users access. VPN's main drawbacks are latency (traffic still
    traverses the internet) and bandwidth limits. Whether VPN supports transitive routing depends on
    your setup: a VPN attached to a transit gateway gets transitive access to all attached VPCs,
    but a VPN attached directly to a single VPC doesn't.
  </ListItem>
</List>
...
<Table
  ...
  content={{
    thead: {
      th: ['Pattern', 'CIDR Overlap', 'Transitive', 'DNS Complexity', 'Use When'],
    },
    tbody: {
      tr: [
        {
          th: 'VPC Peering',
          td: ['Not allowed', 'No', 'Medium', '2-3 VPCs, simple topology'],
        },
        {
          th: 'Transit Gateway',
          td: ['Not allowed', 'Yes', 'Medium', 'Many VPCs, hub-spoke'],
        },
        {
          th: 'Private Link',
          td: ['Allowed', 'N/A', 'Low', 'Service exposure, overlapping CIDRs'],
        },
        {
          th: 'VPN',
          td: ['Not allowed', 'Depends', 'High', 'Hybrid cloud'],
        },
      ],
    },
    figure: 'Cross-VPC connectivity comparison.',
  }}
/>
...
<List
  ...
  items={[
    {
      title: 'Peering:',
      lead: 'Routes missing on one side, DNS resolution not enabled, zone not associated with both VPCs',
    },
    {
      title: 'Transit Gateway:',
      lead: 'Blackhole routes (attachment deleted but route remains), wrong route table association, asymmetric routing on return path',
    },
    {
      title: 'Private Link:',
      lead: 'Connection stuck in "pending" (provider must accept), endpoint unavailable (check NLB health targets), private DNS not working (verify it\'s enabled on the endpoint)',
    },
  ]}
/>
...
<Download
  resource="private-networking-dns-routing-tls-debugging"
  title="Private Networking: DNS, Routing, and TLS Failures"
  description="Debugging the networking issues that appear when services move to private connectivity."
  listItems={[
    {
      text: "Private DNS verification checklist",
    },
    {
      text: "Routing and NACL debug flow",
    },
    {
      text: "TLS handshake troubleshooting guide",
    },
    {
      text: "Endpoint migration preflight template",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Diagnostic tooling',
      text: "Scripts that run through the checklist automatically. When you're tired and stressed, you'll skip steps. The script won't.",
    },
    {
      lead: 'Architecture documentation',
      text: "Private networks have more moving parts — VPC CIDRs, route tables, peering connections, transit gateway attachments, private hosted zones. Document them so debugging isn't archaeology.",
    },
    {
      lead: 'Pre-migration testing',
      text: 'Before cutting over to private endpoints, verify every step of the path works. The migration itself should be boring.',
    },
  ]}
/>
