...
Since Future Me is often one of the intended users of TS libraries I author, providing an outstanding <span className="font-semibold text-blue-600">developer experience</span> (DX) is something I pay attention to. I've use a *lot* of third party libraries over time. Some make me feel like a braniac, and give me confidence in my abilities as a developer. Their APIs make sense and are clear in how I'm supposed to use them. If I have to dig into the code base for some reason, I can make sense of what I'm looking at. Other libraries make me feel like a bonehead. I hate that, and don't want to be <span className="italic text-red-600">That Person™</span> with libraries I author.
...
### Prompt 1: Architect's Blueprint of an API
Overhead view of an architect's drafting table with blueprints showing API function signatures, type hierarchies, and method chains rendered as technical drawings. Pencils, rulers, and a compass scattered around. Warm lighting, professional craftsmanship aesthetic. The blueprints show clean, well-organized structure.

### Prompt 2: Building Blocks with Type Annotations
Colorful 3D building blocks (like premium wooden toys) arranged in an elegant structure. Each block is labeled with TypeScript types—`string`, `number`, `interface`, `generic<T>`. Some blocks interlock perfectly (good API design) while others nearby don't fit (anti-patterns). Soft studio lighting, minimal background.

### Prompt 3: Two Developers Mirror Image
Split image: left side shows a frustrated developer squinting at confusing documentation with tangled code on screen; right side shows the same developer confidently coding with clear IntelliSense tooltips and organized imports. The transition between chaos and clarity represents good vs poor library DX.

### Prompt 4: Chain Links Forming a Fluent Interface
Close-up photograph of elegant chain links connecting smoothly, representing method chaining in a fluent API. Each link is polished metal with subtle engravings suggesting method names. The chain flows in a graceful curve. Industrial jewelry aesthetic, dramatic lighting.

### Prompt 5: Library as a Precision Tool
Photograph of a master craftsman's toolkit—precision instruments, calipers, specialized tools each in their designated spot in a wooden case. Each tool represents a well-designed library function: focused, purposeful, ergonomic. Represents the care and intentionality of good API design. Warm workshop lighting.
