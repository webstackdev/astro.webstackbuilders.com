...
<Diagram
  ...
  description={
    `Sequence diagram with three participants: Application, Postgres Process, and Backend Process. The application starts a TCP handshake by sending TCP SYN to the Postgres process, which responds with TCP SYN-ACK, followed by the application sending TCP ACK. The application then sends a startup message to the Postgres process. The Postgres process forks a backend process and hands the connection off to that backend. The backend requests authentication from the application, the application sends password or credentials, the backend verifies those credentials, and then responds with Authentication OK. A note marks that the connection is now established in roughly 10 to 200 milliseconds. After establishment, the application sends a query to the backend and receives results. Finally, the application sends Terminate, and the backend performs cleanup and exits. The diagram shows the full overhead involved in creating a fresh PostgreSQL connection before useful query work begins.`
  }
>
```mermaid
sequenceDiagram
...
flowchart TD
  ...
  subgraph withoutPgBouncer[Without PgBouncer]
    B[PostgreSQL<br/>N × 20 connections]
  end

  subgraph withPgBouncer[With PgBouncer]
