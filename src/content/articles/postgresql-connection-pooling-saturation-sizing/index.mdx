...
tags: ["systems-and-development", "typescript", "kubernetes"]
featured: true
---

import connectionEstablishmentDiagram from "./diagrams/connection-establishment-sequence.jpg"
import connectionSaturationDiagram from "./diagrams/connection-saturation-progression.jpg"
import pgbouncerDiagram from "./diagrams/pgbouncer-connection-multiplexing.jpg"
...
<Diagram
  src={connectionEstablishmentDiagram}
  size="full"
  title="Connection establishment sequence."
  description="Sequence diagram with three participants: Application, Postgres Process, and Backend Process. The application starts a TCP handshake by sending TCP SYN to the Postgres process, which responds with TCP SYN-ACK, followed by the application sending TCP ACK. The application then sends a startup message to the Postgres process. The Postgres process forks a backend process and hands the connection off to that backend. The backend requests authentication from the application, the application sends password or credentials, the backend verifies those credentials, and then responds with Authentication OK. A note marks that the connection is now established in roughly 10 to 200 milliseconds. After establishment, the application sends a query to the backend and receives results. Finally, the application sends Terminate, and the backend performs cleanup and exits. The diagram shows the full overhead involved in creating a fresh PostgreSQL connection before useful query work begins."
/>
...
<Table
  ...
  content={{
    figure: '<span class="text-mono">max_connections</span> settings and their implications.',
    thead: {
      th: ['<span class="text-mono">max_connections</span>', 'Use Case', 'Memory Overhead', 'Notes'],
    },
    tbody: {
      tr: [
        {
          th: '50-100',
          td: ['Small deployments with pooler', '~500MB-1GB', 'Recommended baseline'],
        },
        {
          th: '100-300',
          td: ['Medium deployments', '~1-3GB', 'Watch for contention'],
        },
        {
          th: '500-1000',
          td: ['Large deployments', '~5-10GB', 'Usually wrong; use pooler instead'],
        },
      ],
    },
  }}
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Connection states and their implications.',
    thead: {
      th: ['Connection State', 'Description', 'Concern Level'],
    },
    tbody: {
      tr: [
        {
          th: 'active',
          td: ['Executing query', 'Normal if short'],
        },
        {
          th: 'idle',
          td: ['Waiting for query', 'Normal (pooled)'],
        },
        {
          th: 'idle in transaction',
          td: ['Transaction open, no query', 'High (blocks vacuum)'],
        },
        {
          th: 'idle in transaction (aborted)',
          td: ['Failed transaction, not rolled back', 'Critical'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Application-side pools',
      text: 'Live within your application process. HikariCP (Java), node-postgres pool, SQLAlchemy (Python), and Go\'s <span class="text-mono">database/sql</span> all provide built-in pooling. The advantage is simplicity: no additional infrastructure, per-application configuration, and full support for prepared statements. The disadvantage is that scaling your application scales your connection count. Five application instances with pools of 20 connections each means 100 database connections. Twenty instances means 400. This couples your horizontal scaling to your database connection limits.',
    },
    {
...
<Table
  ...
  content={{
    figure: 'Pooling approaches compared.',
    thead: {
      th: ['Approach', 'Examples', 'Best For', 'Connection Scaling'],
    },
    tbody: {
      tr: [
        {
          th: 'Application-side',
          td: ['HikariCP, pg-pool, SQLAlchemy', 'Single app, prepared statements', 'instances × pool_size'],
        },
        {
          th: 'External pooler',
          td: ['PgBouncer, RDS Proxy', 'Multi-app, auto-scaling', 'Fixed, independent of apps'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Session mode',
      text: 'Client gets a dedicated server connection for its entire session. No multiplexing benefit, but full prepared statement support. Use for long-running sessions or heavy prepared statement workloads.',
    },
    {
      lead: 'Transaction mode',
      text: 'Connection returned after each transaction completes. High multiplexing, works for most web applications. Prepared statements work only with server-side preparation.',
    },
    {
      lead: 'Statement mode',
      text: 'Connection returned after each statement. Maximum multiplexing but no multi-statement transactions. Use only for simple, stateless read workloads.',
    },
  ]}
/>

<Diagram
  src={pgbouncerDiagram}
  title="PgBouncer connection multiplexing."
  description="Top-to-bottom flowchart comparing direct PostgreSQL connections with pooled connections through PgBouncer. In the Application Layer group, App Instance 1, App Instance 2, and App Instance N each have a pool of 20 connections. In the Without PgBouncer group, each application instance connects directly to PostgreSQL, causing the database to accumulate N times 20 connections as application instances scale out. In the With PgBouncer group, the same application instances send their 20 connections each to PgBouncer, which accepts all incoming client connections and multiplexes them onto a fixed pool of 50 PostgreSQL connections. The direct PostgreSQL target is highlighted negatively, while the fixed 50-connection PostgreSQL target behind PgBouncer is highlighted positively. The diagram shows how PgBouncer decouples application scaling from raw database connection counts."
/>
...
<List
  ...
  items={[
    {
      lead: 'Pool sizing',
      text: 'Follows a simple formula: <span class="font-mono">default_pool_size = PostgreSQL max_connections / number_of_pools</span>. If PostgreSQL allows 100 connections and you have 5 user + database combinations (pools), each pool gets 20 connections. The <span class="font-mono">reserve_pool</span> handles burst traffic — set it to 10-25% of <span class="font-mono">default_pool_size</span>.',
    },
    {
      lead: '<span class="font-mono">max_client_conn</span>',
      text: 'Determines how many application connections PgBouncer accepts. This can be much higher than PostgreSQL\'s <span class="font-mono">max_connections</span> because PgBouncer queues requests when all server connections are busy. Set this based on peak concurrent application connections with headroom for spikes.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: 'PgBouncer sizing by deployment scale.',
    thead: {
      th: ['Scenario', '<span class="font-mono">max_client_conn</span>', '<span class="font-mono">default_pool_size</span>', 'PostgreSQL <span class="font-mono">max_connections</span>'],
    },
    tbody: {
      tr: [
        {
          th: 'Small web app (5 instances)',
          td: ['200', '15', '100'],
        },
        {
          th: 'Medium service (20 instances)',
          td: ['500', '25', '200'],
        },
        {
          th: 'High scale (100+ instances)',
          td: ['2000', '50', '400'],
        },
      ],
    },
...
<Inset
  classes={{
    layout: "text-content-inverse text-center",
  }}
  color="content"
>
...
<Table
  variant="numbered-review-table"
  content={{
    figure: 'Pool sizing factors.',
    thead: {
      th: ['Factor', 'What to Measure', 'Impact'],
    },
    tbody: {
      tr: [
        {
          th: 'Query duration',
          td: ['P99 latency, not average', 'Longer queries need more connections'],
        },
        {
          th: 'Request rate',
          td: ['Peak QPS, not average', 'Higher rate needs more connections'],
        },
        {
          th: 'Variance',
          td: ['Standard deviation / mean', 'High variance needs more headroom'],
        },
        {
          th: 'Burst factor',
          td: ['Peak / baseline ratio', 'Typically 2-3× for web traffic'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      title: 'Oversized pools:',
      lead: 'Setting <span class="font-mono">pool_size = 100</span> "just in case" means 10 instances create 1000 connections. Size based on actual need.',
    },
    {
      title: 'No max lifetime:',
      lead: 'Connections held forever accumulate memory leaks and stale state. Recycle every 30 minutes.',
    },
    {
      title: 'No idle timeout:',
      lead: 'Idle connections waste resources and hide leaks. Timeout after 10 minutes of inactivity.',
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Pool configuration guidelines.',
    thead: {
      th: ['Setting', 'Recommended Value', 'Rationale'],
    },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">max_pool_size</span>',
          td: ['10-30 per instance', "Based on Little's Law calculation"],
        },
        {
          th: '<span class="font-mono">min_pool_size</span>',
          td: ['50% of max', 'Keep pool warm, reduce latency'],
        },
        {
          th: '<span class="font-mono">connection_timeout</span>',
          td: ['10s', 'Fail fast if pool exhausted'],
        },
        {
          th: '<span class="font-mono">idle_timeout</span>',
          td: ['10-30 min', 'Reclaim unused connections'],
        },
        {
          th: '<span class="font-mono">max_lifetime</span>',
          td: ['30 min', 'Prevent stale connections'],
        },
        {
          th: '<span class="font-mono">leak_detection</span>',
...
<List
  ...
  items={[
    {
      lead: 'Connection utilization',
      text: 'The primary signal. Query <span class="font-mono">pg_stat_activity</span> to compare current connections against <span class="font-mono">max_connections</span>. Alert at 70% utilization, investigate at 85%. By 95%, you\'re one slow query away from exhaustion.',
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: 'Queue depth',
      text: 'Counts requests waiting for connections. In PgBouncer, check <span class="font-mono">cl_waiting</span>. In application pools, check <span class="font-mono">waitingCount</span> or equivalent. Any sustained queue indicates saturation — the pool can\'t keep up with demand.',
    },
    {
...
<Table
  ...
  classes={{
    tbody:
      '[&_td:nth-child(3)]:text-warning [&_td:nth-child(4)]:text-danger [&_td:nth-child(3)]:text-center [&_td:nth-child(4)]:text-center',
  }}
  content={{
    figure: 'Saturation metric thresholds.',
    thead: {
      th: ['Metric', 'Warning Threshold', 'Critical Threshold', 'Response'],
    },
    tbody: {
      tr: [
        {
          th: 'Connection utilization',
          td: ['> 70%', '> 85%', 'Add pooler or reduce pool sizes'],
        },
        {
          th: 'Pool wait time (P99)',
          td: ['> 100ms', '> 1s', 'Increase pool size or optimize queries'],
        },
        {
          th: 'Queue depth',
          td: ['> 10', '> 50', 'Pool undersized or query bottleneck'],
        },
        {
          th: 'Idle in transaction',
          td: ['> 5 connections', '> 20 connections', 'Find and fix leaking transactions'],
        },
        {
          th: 'Connection errors',
          td: ['Any occurrence', 'Sustained errors', 'Immediate investigation'],
        },
      ],
    },
  }}
...
<Diagram
  src={connectionSaturationDiagram}
  size="full"
  title="Connection saturation progression."
  description="Top-to-bottom flowchart showing three connection saturation states. The Healthy state shows 50 out of 200 connections in use, no queue, and 15 idle connections. A traffic increase moves the system from Healthy to Warning. The Warning state shows 160 out of 200 connections in use, a queue of 5, and 2 idle connections. From Warning, if no action is taken, the system progresses to Critical. The Critical state shows 198 out of 200 connections in use, a queue of 50, and 0 idle connections. There is also a recovery path from Warning back to Healthy labeled Scale or optimize. The healthy state is highlighted positively, the warning state is highlighted as cautionary, and the critical state is highlighted negatively. The diagram explains how connection exhaustion develops gradually and where intervention can restore normal operation before the database reaches saturation."
/>
  ...
  items={[
    {
      ...
      text: 'Scale application instances to zero or block traffic at the load balancer. This stops new connection attempts.',
    },
    {
      lead: 'Clear stuck connections',
      text: 'On PostgreSQL, run <span class="font-mono">SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state = \'idle in transaction\' AND state_change < now() - interval \'5 minutes\';</span> to kill stuck sessions.',
    },
    {
    ...
    },
    {
      ...
      text: 'Watch <span class="font-mono">pg_stat_activity</span> connection count as you add instances. Stop scaling if you approach 80% utilization.',
    },
  ]}
/>

Recovery gets you back to operational, but it doesn't prevent the next incident. The cascade pattern repeats because the underlying architecture lacks defensive layers — there's nothing between "working fine" and "total collapse." Building resilience means adding checkpoints where the system can absorb pressure instead of transmitting it. Each layer below addresses a different failure mode, and they compound: connection limits cap the blast radius, circuit breakers stop the bleeding, backpressure prevents the bleeding from starting, and graceful degradation keeps users served even when the database is under strain.

<List
  variant="zebra-list"
  items={[
    {
      lead: 'Connection limits',
      text: 'Set maximums at application pools, PgBouncer, and PostgreSQL. The principle is "fail fast at the application, not the database."',
    },
    {
      lead: 'Circuit breakers',
      text: 'Detect saturation and return fast errors instead of queueing. Let the system recover.',
    },
    {
      lead: 'Backpressure',
      text: 'Rate limit at the ingress. Use bounded queues. Shed load during saturation.',
    },
    {
      lead: 'Graceful degradation',
      text: 'Disable non-critical features under stress. Serve cached responses. Return partial results.',
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Failure scenarios and responses.',
    thead: {
      th: ['Scenario', 'Detection', 'Response', 'Recovery'],
    },
    tbody: {
      tr: [
        {
          th: 'Slow queries',
          td: ['Wait time > threshold', 'Add query timeout', 'Optimize queries'],
        },
        {
          th: 'Connection leak',
          td: ['Idle-in-txn > limit', 'Kill sessions', 'Fix application code'],
        },
        {
          th: 'Traffic spike',
          td: ['Utilization > 80%', 'Shed load', 'Scale or add pooler'],
        },
        {
          th: 'Database overload',
          td: ['Connection errors', 'Circuit break', 'Scale database'],
        },
      ],
    },
  }}
...
<Download
  resource="postgresql-connection-pooling-saturation-sizing"
  title="PostgreSQL Connections: Pooling and Saturation"
  description="Connection pool sizing, timeout configuration, and the saturation signals that predict database problems."
  listItems={[
    {
      text: "Pool sizing calculation worksheet",
    },
    {
      text: "PgBouncer tuning starter config",
    },
    {
      text: "Saturation alert threshold pack",
    },
    {
      text: "Connection incident recovery runbook",
    },
  ]}
/>
...
<List
  ...
  classes={{
    wrapper: "sm:ml-6 sm:mr-12",
  }}
>
  <ListItem lead="Connections are expensive.">
    Each connection costs 5-10MB of memory, consumes CPU through context switching, and competes for
    shared resources. The counterintuitive truth: fewer connections often means better performance.
    Keep `max_connections` low and use pooling.
  </ListItem>
  <ListItem lead="Pooling decouples application scaling from database limits.">
    PgBouncer (or similar external poolers) lets you scale application instances without
    proportionally increasing database connections. Transaction pooling mode handles most workloads
    well.
  </ListItem>
  <ListItem lead="Size pools with math, not intuition.">
    Little's Law ($$L = \lambda \times W$$) gives you a starting point. Account for variance with a
    burst factor. Anti-patterns like oversized pools ("just in case") create more problems than they
    solve.
  </ListItem>
  <ListItem lead="Monitor saturation, not just errors.">
    By the time you see "too many connections" errors, you're already in a cascade. Set alerts on
    the utilization thresholds discussed earlier. Watch for idle-in-transaction sessions — they're
    connection leaks waiting to bite you.
  </ListItem>
  <ListItem lead="Design for graceful degradation.">
    Circuit breakers, retry budgets, and backpressure turn connection exhaustion from a hard failure
    into a soft degradation. Build these patterns before you need them.
  </ListItem>
</List>
