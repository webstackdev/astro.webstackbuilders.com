...
<List
  variant="check-icons-list"
  items={[
    { text: "\"This alert fired 47 times last quarter. How many of those were real problems?\" (Usually: zero or one.)" },
    { text: "\"Would you notice if this alert stopped firing tomorrow?\" (Usually: no.)" },
    { text: "\"If we delete this, what's the worst case?\" (Usually: we'd notice the problem some other way, slightly later.)" },
  ]}
/>
...
<List
  ...
  items={[
    { lead: "Something is broken right now.", text: "Not \"might break soon\" or \"is trending toward a problem.\" Actually broken." },
    { lead: "Users are affected.", text: "Either external customers or internal users who can't do their jobs." },
    { lead: "Human judgment is required.", text: "Automated remediation can't handle it, or hasn't been implemented yet." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "SLO target: 99.9% (0.1% error budget)" },
    { text: "Current error rate: 0.5%" },
    { text: "Burn rate: 0.5% / 0.1% = 5x" },
  ]}
/>
...
<List
  variant="numbered-with-background-list"
  items={[
    { text: "Check deployment status: `kubectl get deployments -n checkout`" },
    { text: "Verify database connectivity: `psql -h $DB_HOST -c \"SELECT 1\"`" },
    { text: "Check downstream dependencies: [Dependency Dashboard](/dashboards/checkout-deps)" },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "Check recent deployments: `kubectl rollout history deployment/checkout-api`" },
    { text: "Review commits: [Last 24h commits](https://github.com/org/checkout/commits)" },
    { text: "Check infra changes: [Terraform runs](https://app.terraform.io/org/runs)" },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "End-of-month batch processing (1st-3rd of each month) causes latency spikes" },
    { text: "Payment provider maintenance windows (check StatusPage first)" },
    { text: "Expected during large promotional events—verify with marketing calendar" },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { text: "If database issue: Page DBA on-call" },
    { text: "If payment provider: Check [StatusPage](https://status.stripe.com)" },
  ]}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "Firing Review", td: ["Monthly", "Which alerts fired most? What was the action rate?"] },
        { th: "Usefulness Review", td: ["Quarterly", "Do we still need this alert? Is the threshold still valid?"] },
        { th: "Runbook Review", td: ["Quarterly", "Is the runbook current? Did it help in recent incidents?"] },
        { th: "Owner Review", td: ["Annually", "Does the owning team still exist? Do they still own this service?"] },
      ],
    },
  }}
/>
