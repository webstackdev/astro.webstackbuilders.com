...
import mtlsHandshakeDiagram from "./diagrams/mtls-handshake-between-client-and-server-services.jpg"
...
<Inset
  figure="Decoded certificate showing fields relevant to mTLS"
  fullWidth={false}
  color="note"
  classes={{
    layout: "text-note-inverse",
  }}
>
Certificate:
...
<Table
  variant="soft-header-card-table"
  content={{
    figure: 'SPIFFE identity formats and certificate issuers across service meshes.',
    thead: {
    ...
    tbody: {
      tr: [
        {
          th: 'Istio',
          td: ['<span class="font-mono">spiffe://cluster.local/ns/{namespace}/sa/{service-account}</span>', 'istiod'],
        },
        {
          th: 'Linkerd',
          td: ['<span class="font-mono">spiffe://identity.linkerd.cluster.local/...</span>', 'linkerd-identity'],
        },
        {
          th: 'Consul Connect',
          td: ['<span class="font-mono">spiffe://cluster.consul/ns/{namespace}/dc/{datacenter}/svc/{service}</span>', 'Consul CA or Vault'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Add the new CA to all trust bundles (services now trust both old and new)',
    },
    {
      lead: 'Verify all services have received the updated trust bundle',
    },
    {
      lead: 'Start issuing certificates from the new CA',
    },
    {
      lead: 'Wait for all workload certificates to rotate to the new CA',
    },
    {
      lead: 'Remove the old CA from trust bundles',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      title: 'STRICT:',
      lead: 'Require mTLS. Reject any plaintext connection.',
    },
    {
      title: 'PERMISSIVE:',
      lead: 'Accept both mTLS and plaintext. Useful during migration.',
    },
    {
      title: 'DISABLE:',
      lead: 'No mTLS requirement (not recommended for production).',
    },
  ]}
/>
  ...
  items={[
    {
      ...
      text: 'The most common cause of mTLS outages. Error messages include <span class="font-mono">x509: certificate has expired</span> or <span class="font-mono">TLS handshake error: certificate verify failed</span>. Check expiration with <span class="font-mono">openssl x509 -enddate -noout -in cert.pem</span>. Fix by forcing rotation or restarting the workload to trigger certificate renewal.',
    },
    {
      lead: 'Trust chain broken',
      text: 'The certificate is valid but the CA that signed it isn\'t in the trust store. You\'ll see <span class="font-mono">x509: certificate signed by unknown authority</span>. This happens during CA rotation if trust bundles aren\'t updated before new certificates are issued. Verify the chain with <span class="font-mono">openssl verify -CAfile root.pem cert.pem</span>.',
    },
    {
      lead: 'SAN mismatch',
      text: 'The certificate is valid but doesn\'t include the hostname being used. Error: <span class="font-mono">x509: certificate is valid for X, not Y</span>. Check SANs with <span class="font-mono">openssl x509 -text -noout | grep -A1 \'Subject Alternative\'</span>. This commonly happens when DNS names change or when certificates are issued with incomplete SAN lists.',
    },
    {
      lead: 'Wrong key usage',
      text: 'The certificate exists but wasn\'t issued for mTLS. If it only has <span class="font-mono">serverAuth</span> in Extended Key Usage, it can\'t be used as a client certificate. Error: <span class="font-mono">x509: certificate specifies an incompatible key usage</span>. Reissue with both <span class="font-mono">serverAuth</span> and <span class="font-mono">clientAuth</span>.',
    },
  ]}
/>
      ...
      tr: [
        {
          ...
          td: ['Service not listening', '<span class="font-mono">netstat -tlnp</span>'],
        },
        {
          th: 'Connection reset',
          td: ['TLS version mismatch or expired cert', '<span class="font-mono">openssl x509 -enddate</span>'],
        },
        {
          th: '"Unknown authority"',
          td: ['Trust bundle missing CA', '<span class="font-mono">openssl verify -CAfile</span>'],
        },
        {
...
<List
  ...
  items={[
    {
      lead: 'Verify current certificate status — all certificates should be Ready:',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Check for ongoing incidents (don\'t rotate during an outage).',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'All services report healthy',
    },
    {
      lead: 'No TLS errors in logs',
    },
    {
      lead: 'Monitor error rates for 30 minutes',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Step 1: Assess scope.',
      text: 'Determine whether the failure affects the entire mesh or only a subset of services. Check pod status and Istio telemetry before changing policy.',
    },
    {
      lead: 'Step 2: Enable permissive mode (if needed).',
      text: 'Use this only as the emergency valve. It allows plaintext traffic so services can communicate while you fix the underlying issue.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Step 3: Diagnose root cause.',
      text: 'Check certificate expiry, CA chain validity, and istiod logs before deciding on remediation.',
    },
    {
      lead: 'Step 4: Apply fix based on diagnosis:',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Step 5: Restore strict mode:',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Post-incident:',
      text: 'Document the timeline, add monitoring for this failure mode, update automation, and schedule a post-mortem.',
    },
  ]}
/>
