...
<Table
  ...
  content={{
    ...
    thead: { th: ["Factor", "Cacheable", "Not Cacheable"] },
    tbody: {
      tr: [
        { th: "HTTP Method", td: ["GET, HEAD", "POST, PUT, DELETE, PATCH"] },
        { th: "Status Code", td: ["200, 301, 404 (configurable)", "500, 503"] },
        { th: "Cache-Control", td: ["public, max-age>0", "private, no-store, no-cache"] },
...
<Table
  ...
  content={{
    ...
    thead: { th: ["Pattern", "Category", "Risk or Benefit", "Recommended Action"] },
    tbody: {
      tr: [
        { th: "Vary on session cookie", td: ["Dangerous", "Creates entry per user, still risks cross-contamination", "Do not cache personalized content at edge"] },
        { th: "Cache without checking auth header", td: ["Dangerous", "Unauthenticated cache serves to authenticated users", "Different cache keys or no caching for auth content"] },
        { th: "Origin returns Cache-Control for personalized API", td: ["Dangerous", "CDN caches response, serves to wrong user", "Always return `private, no-store` for personalized responses"] },
        { th: "Edge-side includes for personalization", td: ["Safe", "Cache page shell, personalize at edge", "Use ESI or edge compute for fragments"] },
        { th: "Client-side personalization", td: ["Safe", "Cache generic content, personalize in browser", "Keep personalized data out of edge cache"] },
        { th: "Separate cacheable and non-cacheable endpoints", td: ["Safe", "Clear separation of concerns", "Split endpoints by cacheability"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    thead: { th: ["Step", "User A (Germany)", "User B (USA)"] },
    tbody: {
      tr: [
        { th: "1. Request", td: ["`GET /products`", "`GET /products`"] },
        { th: "2. Cache status", td: ["MISS", "HIT (User A's response)"] },
        { th: "3. Response", td: ["Prices in EUR ✓", "Prices in EUR ✗"] },
        { th: "4. Outcome", td: ["Correct experience", "Checkout fails—wrong currency"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    thead: { th: ["Solution", "How It Works", "Pros", "Cons"] },
    tbody: {
      tr: [
        { th: "Vary on country header", td: ["Add `CF-IPCountry` (or equivalent) to cache key", "Automatic, per-country caching", "Many cache variants, cold caches per region"] },
        { th: "Separate endpoints", td: ["Use explicit country param: `/api/products?country=DE`", "Explicit, highly cacheable", "Client must know country"] },
        { th: "Client-side localization", td: ["Return all currencies, JavaScript selects", "Single cache entry, maximum cacheability", "Larger response, client complexity"] },
        { th: "Edge compute", td: ["Edge function transforms cached response per-country", "Best of both worlds", "Complexity, edge compute costs"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    thead: { th: ["Strategy", "Impact", "Effort", "Actions"] },
    tbody: {
      tr: [
        { th: "Query string normalization", td: ["High", "Low", "Sort query parameters; remove tracking params (utm_*, fbclid, gclid); remove empty parameters"] },
        { th: "Cache key simplification", td: ["High", "Medium", "Audit cache key components; remove unnecessary headers; normalize Vary values"] },
        { th: "Content negotiation reduction", td: ["Medium", "Medium", "Reduce language variants; limit Accept-Encoding values; avoid User-Agent variations"] },
        { th: "TTL extension", td: ["High", "Low", "Use immutable for versioned assets; extend TTL with stale-while-revalidate; implement proper invalidation"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    thead: { th: ["Metric", "Good", "Warning", "Action If Poor"] },
    tbody: {
      tr: [
        { th: "Cache Hit Ratio", td: [">90%", "70-90%", "Analyze cache keys, extend TTL"] },
...
<List
  ...
  items={[
    { lead: "**Hit ratio varies by content type.**", text: "Static assets should exceed 95%. API responses might reasonably be 70-80% depending on query parameter diversity. Personalized endpoints will be 0% (correctly)." },
    { lead: "**Sudden hit ratio drops**", text: "usually indicate a configuration change, new URL patterns, or cache key pollution from marketing campaigns adding tracking parameters." },
    { lead: "**High origin load with good hit ratio**", text: "suggests your cache is working but TTLs are too short, causing frequent revalidation." },
    { lead: "**Per-PoP analysis**", text: "reveals geographic patterns. A single PoP with poor performance might indicate regional traffic spikes overwhelming that edge's cache capacity." },
  ]}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "**Cache keys determine correctness.**", text: "Every attribute that affects your response must be in the cache key. Miss one, and users get wrong content. Include unnecessary attributes, and your hit ratio suffers." },
    { lead: "**Vary headers are powerful but dangerous.**", text: "They let you cache content that legitimately varies, but high-cardinality Vary values destroy cache effectiveness. Normalize headers at the edge before they reach your origin." },
    { lead: "**Cache invalidation requires explicit strategy.**", text: "TTL expiration alone is rarely sufficient. Implement cache tags for complex content relationships, use stale-while-revalidate for graceful updates, and integrate purging into your deployment pipeline." },
    { lead: "**Prioritize correctness over hit ratio.**", text: "A correctly-configured cache with 70% hit ratio beats an incorrectly-configured cache with 95% hit ratio. The latter is serving wrong content to 95% of users." },
  ]}
/>
