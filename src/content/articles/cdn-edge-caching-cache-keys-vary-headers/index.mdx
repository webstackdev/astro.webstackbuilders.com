...
<Diagram
  src={cdnEdgeRequestDiagram}
  title="CDN edge request flow showing cache hit and miss paths"
  description="Sequence diagram showing a user request reaching a CDN edge point of presence, the edge checking the cache, returning immediately on a cache hit, or forwarding the request to the origin on a cache miss before storing the response and returning it to the user."
/>
...
<Table
  ...
  content={{
    figure: "Factors determining response cacheability at the edge.",
    thead: {
      th: ["Factor", "Cacheable", "Not Cacheable"],
    },
    tbody: {
      tr: [
        {
...
<Diagram
  src={tagBasedPurgingDiagram}
  title="Tag-based purging invalidating all related content"
  description="Flowchart showing a product update triggering a purge of the product tag, which invalidates every cached URL associated with that tag, including the product page, API endpoint, category listing, and brand listing."
/>
...
<Diagram
  src={staleWhileRevalidateDiagram}
  title="Stale-while-revalidate serving stale content while refreshing in background"
  description="Sequence diagram showing a stale cached response still inside the stale-while-revalidate window, where the first user receives the stale response immediately, the edge revalidates against the origin in the background, updates the cache, and the next user receives the refreshed response."
/>
...
<Table
  ...
  content={{
    figure: "Dangerous and safe patterns for personalized content caching.",
    thead: {
      th: ["Pattern", "Category", "Risk or Benefit", "Recommended Action"],
    },
    tbody: {
      tr: [
        {
          ...
          td: [
            "Dangerous",
            "Creates entry per user, still risks cross-contamination",
            "Do not cache personalized content at edge",
          ],
        },
        {
          ...
          td: [
            "Dangerous",
            "Unauthenticated cache serves to authenticated users",
            "Different cache keys or no caching for auth content",
          ],
        },
        {
          th: "Origin returns Cache-Control for personalized API",
          td: [
            "Dangerous",
            "CDN caches response, serves to wrong user",
            "Always return `private, no-store` for personalized responses",
          ],
        },
        {
          ...
          td: [
            "Safe",
            "Cache page shell, personalize at edge",
            "Use ESI or edge compute for fragments",
          ],
        },
        {
          ...
          td: [
            "Safe",
            "Cache generic content, personalize in browser",
            "Keep personalized data out of edge cache",
          ],
        },
        {
          th: "Separate cacheable and non-cacheable endpoints",
          td: [
            "Safe",
            "Clear separation of concerns",
            "Split endpoints by cacheability",
          ],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      text: "Attacker sends request with malicious header:",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      text: "Vulnerable origin reflects header in response:",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      text: "Response is cached (`X-Forwarded-Host` not in cache key)",
    },
    {
      text: "Legitimate users get poisoned cached response",
    },
  ]}
/>
...
<List
  ...
  classes={{
    wrapper: "sm:ml-8 sm:mr-12 mb-6",
  }}
  items={[
    {
      text: "Include attack vectors in cache key, OR",
    },
    {
      text: "Normalize/validate headers at edge before forwarding",
    },
    {
      text: "Do not reflect untrusted headers in responses",
    },
  ]}
/>
...
<Table
  variant="numbered-review-table"
  content={{
    figure: "Geographic cache mismatch scenario.",
    thead: {
      th: ["Step", "User A (Germany)", "User B (USA)"],
    },
    tbody: {
      tr: [
        {
          th: "Request",
          td: ['<span class="text-mono">GET /products</span>', '<span class="text-mono">GET /products</span>'],
        },
        {
          th: "Cache status",
          td: ['MISS', 'HIT (User A\'s response)'],
        },
        {
          th: "Response",
          td: ['Prices in EUR <span class="text-success">✓</span>', 'Prices in EUR <span class="text-danger">✗</span>'],
        },
        {
          th: "Outcome",
          td: ['Correct experience', 'Checkout fails — wrong currency'],
        },
      ],
    },
  }}
...
<Table
  variant="grid-and-accent-header-table"
  content={{
    figure: "Solutions for geographic and currency caching challenges.",
    thead: {
      th: ["Solution", "How It Works", "Pros", "Cons"],
    },
    tbody: {
      tr: [
        {
          th: "Vary on country header",
          td: [
            'Add <span class="text-mono">CF-IPCountry</span> (or equivalent) to cache key',
            "Automatic, per-country caching",
            "Many cache variants, cold caches per region",
          ],
        },
        {
          th: "Separate endpoints",
          td: [
            'Use explicit country param: <span class="text-mono">/api/products?country=DE</span>',
            "Explicit, highly cacheable",
            "Client must know country",
          ],
        },
        {
          th: "Client-side localization",
          td: [
            "Return all currencies, JavaScript selects",
            "Single cache entry, maximum cacheability",
            "Larger response, client complexity",
          ],
        },
        {
          th: "Edge compute",
          td: [
            "Edge function transforms cached response per-country",
            "Best of both worlds",
            "Complexity, edge compute costs",
          ],
        },
      ],
    },
...
<Table
  variant="criteria-highlight-table"
  content={{
    figure: "Strategies to improve Cache Hit Ratio (CHR) with estimated impact and effort.",
    thead: {
      th: ["Strategy", "Impact", "Effort", "Actions"],
    },
    tbody: {
      tr: [
        {
          th: "Query string normalization",
          td: [
            "High",
            "Low",
            'Sort query parameters; remove tracking params (<span class="text-mono">utm_*</span>, <span class="text-mono">fbclid</span>, <span class="text-mono">gclid</span>); remove empty parameters',
          ],
        },
        {
          th: "Cache key simplification",
          td: [
            "High",
            "Medium",
            "Audit cache key components; remove unnecessary headers; normalize Vary values",
          ],
        },
        {
          th: "Content negotiation reduction",
          td: [
            "Medium",
            "Medium",
            'Reduce language variants; limit <span class="text-mono">Accept-Encoding</span> values; avoid <span class="text-mono">User-Agent</span> variations',
          ],
        },
        {
          th: "TTL extension",
          td: [
            "High",
            "Low",
            'Use immutable for versioned assets; extend TTL with <span class="text-mono">stale-while-revalidate</span>; implement proper invalidation',
          ],
        },
      ],
    },
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Cache performance metrics and targets.",
    thead: {
      th: ["Metric", "Good", "Warning", "Action If Poor"],
    },
    tbody: {
      tr: [
        {
...
<List
  ...
  items={[
    {
      lead: "Hit ratio varies by content type",
      text: "Static assets should exceed 95%. API responses might reasonably be 70-80% depending on query parameter diversity. Personalized endpoints will be 0% (correctly).",
    },
    {
      lead: "Sudden hit ratio drops",
      text: "usually indicate a configuration change, new URL patterns, or cache key pollution from marketing campaigns adding tracking parameters.",
    },
    {
      lead: "High origin load with good hit ratio",
      text: "suggests your cache is working but TTLs are too short, causing frequent revalidation.",
    },
    {
      lead: "Per-PoP analysis",
      text: "reveals geographic patterns. A single PoP with poor performance might indicate regional traffic spikes overwhelming that edge's cache capacity.",
    },
  ]}
/>
...
<Download
  resource="cdn-edge-caching-cache-keys-vary-headers"
  title="Download the Edge Caching Guide"
  description="Get the complete CDN caching framework for safe cache keys, Vary controls, and high-hit-ratio correctness."
  listItems={[
    {
      text: "Cache key design checklist",
    },
    {
      text: "Vary header normalization patterns",
    },
    {
      text: "Personalization safety guardrails",
    },
    {
      text: "Poisoning prevention hardening guide",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Cache keys determine correctness.",
      text: "Every attribute that affects your response must be in the cache key. Miss one, and users get wrong content. Include unnecessary attributes, and your hit ratio suffers.",
    },
    {
      lead: "Vary headers are powerful but dangerous.",
      text: "They let you cache content that legitimately varies, but high-cardinality Vary values destroy cache effectiveness. Normalize headers at the edge before they reach your origin.",
    },
    {
      lead: "Cache invalidation requires explicit strategy.",
      text: "TTL expiration alone is rarely sufficient. Implement cache tags for complex content relationships, use stale-while-revalidate for graceful updates, and integrate purging into your deployment pipeline.",
    },
    {
      lead: "Prioritize correctness over hit ratio.",
      text: "A correctly-configured cache with 70% hit ratio beats an incorrectly-configured cache with 95% hit ratio. The latter is serving wrong content to 95% of users.",
    },
  ]}
/>
