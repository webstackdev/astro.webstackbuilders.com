...
tags: ["system-modernization", "ruby", "typescript", "dotnet"]
featured: true
---

import legacyTestingDiagram from "./diagrams/legacy-testing-strategy.jpg"
import seamTypeDiagram from "./diagrams/seam-type-decision.jpg"
...
Within two weeks, using the techniques in this article, they have 80% of the critical paths under test __without changing production behavior__. The "untestable" code is now testable. The difference isn't magic — it's specific techniques: characterization tests to capture existing behavior, seam identification to find injection points, and surgical dependency breaking that isolates without altering.
  ...
  content={{
    ...
    tbody: {
      tr: [
        ...
        {
          th: 'C# / Java',
          td: ['Constructor overloads or optional parameters'],
        },
        {
          th: 'PHP',
          td: ['Default parameter values'],
        },
        {
          th: 'TypeScript',
...
<Diagram
  ...
  description="Flowchart showing three seam categories and how to choose between them for legacy code testing. In the grouped section labeled Seam Types, Object Seam branches to Inject via constructor, Inject via setter, and Inject via method parameter. Link Seam branches to Module mocking and Import substitution. Subclass Seam branches to Extract to protected method and Override in test class. In a separate grouped section labeled Decision, a decision node asks Which seam to use. The Cleanest path points to Object Seam, the Quick fix path points to Link Seam, and the No constructor access path points to Subclass Seam. The diagram summarizes both the available seam types and the practical decision logic for choosing the most suitable one based on constraints in the legacy codebase."
/>
...
<Table
  ...
  classes={{
    tbody:
      '[&_td:nth-child(4)]:text-success [&_td:nth-child(5)]:text-danger',
  }}
  content={{
    thead: {
      th: ['Technique', 'When to Use', 'Pros', 'Cons'],
    },
    tbody: {
      tr: [
        {
          th: 'Extract and Override',
          td: ['Quick wins, no API change', 'Minimal code change', 'Test class inheritance'],
        },
        {
          th: 'Parameterize Constructor',
          td: ['Long-term solution', 'Clean DI pattern', 'May require interface'],
        },
        {
          th: 'Instance Delegator',
          td: ['Static method calls', 'Enables testing', 'Adds indirection'],
        },
        {
          th: 'Link Seam (mocking)',
          td: ["Can't modify code", 'No production changes', 'Fragile to refactoring'],
        },
      ],
    },
    figure: 'Dependency breaking techniques.',
  }}
/>
...
<Diagram
  src={legacyTestingDiagram}
  magnifyButtonPosition="bottom-left"
  size="full"
  title="Legacy testing strategy."
  description="Flowchart summarizing three aspects of a legacy testing strategy. In the grouped section labeled Test Pyramid for Legacy, Golden Master Tests flow to Integration Tests with the label Broad coverage, Integration Tests flow to Characterization Tests with the label Critical paths, and Characterization Tests flow to Unit Tests with the label Extracted code. In the grouped section labeled Coverage Priority, Code changing soon points to 80%+ coverage with the label High, Bug-prone code points to 60%+ coverage with the label Medium, and Stable, untouched code points to Characterization only with the label Low. In the grouped section labeled Test Types by Purpose, Characterization points to Safety net for refactoring with the label Document behavior, Golden Master points to Catch regressions with the label Complex outputs, and Unit Tests point to Design feedback with the label New code. The diagram shows that legacy code testing starts with broad behavior-capturing tests and gradually grows more targeted unit coverage as code is extracted and refactored."
/>
...
<Download
  ...
  listItems={[
    {
      text: "Characterization test starter kit",
    },
    {
      text: "Seam identification quick checklist",
    },
    {
      text: "Dependency breaking pattern catalog",
    },
    {
      text: "Legacy testing rollout plan",
    },
  ]}
/>
