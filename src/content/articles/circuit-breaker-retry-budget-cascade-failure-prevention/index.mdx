...
tags: ["apis-and-gateways", "kubernetes", "prometheus", "typescript", "go", "python"]
featured: true
---
...
<List
  ...
  classes={{
    wrapper: "sm:ml-12 sm:mr-12",
  }}
  items={[
    {
      lead: "T+0:",
      text: "Database slow (100ms → 2s response time)",
    },
    {
      lead: "T+10s:",
      text: "Service C queues grow, some requests timeout",
    },
    {
      lead: "T+20s:",
      text: "Service B sees C timeouts, starts retrying",
    },
    {
      lead: "T+30s:",
      text: "Service A sees B timeouts, starts retrying",
    },
    {
      lead: "T+45s:",
      text: "All services at maximum queue depth",
    },
    {
      lead: "T+60s:",
      text: "Database recovers, but services cannot — queues too deep",
    },
    {
      lead: "T+2min:",
    ...
    {
      lead: "T+5min:",
      text: "Services restarted, queues cleared",
    },
    {
      lead: "T+10min:",
      text: "Full recovery",
...
The timeline above shows the domino effect, but the summary tells the real story:
...
<List
  ...
  items={[
    {
      lead: "Root cause:",
      text: "30 second database slowdown",
    },
    {
      lead: "Actual outage:",
      text: "10 minutes",
...
<Diagram
  ...
  title="Circuit breaker state machine with three states"
  description="State diagram showing a circuit breaker starting closed, opening when the failure threshold is exceeded, moving to half-open after the reset timeout, closing when a probe request succeeds, or reopening when a probe request fails. Notes explain that closed allows all requests while tracking failures, open fails fast without calling the downstream service, and half-open allows limited probes to test recovery."
/>
...
Each strategy fits a different failure mode. The table below maps them to the right context and the cost you accept when using them:
...
<List
  variant="zebra-list"
  classes={{
    wrapper: "sm:ml-18 sm:mr-36 mb-6",
  }}
  items={[
    {
      lead: "Each request can retry up to N times",
    },
    {
      lead: "Problem: Under failure, ALL requests retry",
    },
    {
      lead: "Result: N+1 times the load",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Total retries limited to X% of successful requests",
    },
    {
      lead: "Example: 10% retry budget",
    },
    {
      lead: "If 1000 successful requests, allow 100 retries total",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Budget depletes quickly",
    },
    {
      lead: "Result: System stabilizes instead of amplifying",
    },
  ]}
/>
...
<Inset
  figure="Circuit Breaker Dashboard Layout"
  fullWidth={false}
...
<Table
  ...
  content={{
    thead: {
      th: ["Cause", "Remediation"],
    },
    tbody: {
      tr: [
        {
          th: "Threshold too sensitive",
          td: ['Review <span class="font-mono">failure_threshold</span> and <span class="font-mono">minimum_requests</span>'],
        },
        {
          th: "Timeouts causing false failures",
          td: ["Compare timeout settings to actual P99 response times"],
        },
        {
          th: "Window too short",
          td: ['Increase <span class="font-mono">window_duration</span> for bursty traffic patterns'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: "Failures not counted",
          td: ['Verify <span class="font-mono">failure_conditions</span> includes the error types you expect'],
        },
        {
          th: '<span class="font-mono">minimum_requests</span> too high',
          td: ["Lower it for low-traffic services"],
        },
        {
          th: "Window too long",
          td: ['Decrease <span class="font-mono">window_duration</span> to detect failures faster'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: "Downstream not actually recovered",
          td: ["Verify downstream health directly, outside the breaker"],
        },
        {
          th: "Probe requests failing",
          td: ["Review half-open probe behavior and endpoint health"],
        },
        {
          th: '<span class="font-mono">success_threshold</span> too high',
          td: ['Reduce <span class="font-mono">half_open.success_threshold</span>'],
        },
      ],
    },
  }}
...
The decreasing-timeout pattern above is necessary but not sufficient on its own — you also need to think about what happens when a downstream service is completely unreachable rather than just slow.
...
<List
  ...
  items={[
    {
      lead: "JavaScript/TypeScript:",
      text: "Opossum is the most popular choice for Node.js. It provides a straightforward circuit breaker with built-in Prometheus metrics, event hooks for observability, and a clean promise-based API. The examples in this article use opossum. For more comprehensive needs, cockatiel bundles circuit breakers with retries, timeouts, bulkheads, and fallbacks in a single package with good TypeScript support.",
    },
    {
      lead: "Java:",
      text: "Resilience4j is the successor to Netflix Hystrix (now in maintenance mode). It's designed for Java 8+ with functional programming patterns and integrates well with Spring Boot. Beyond circuit breakers, it includes rate limiters, bulkheads, retries, and time limiters — all composable. If you're on the JVM and need resilience patterns, resilience4j is the default choice.",
    },
    {
      lead: "Go:",
      text: "Gobreaker from Sony is minimal and focused — just a circuit breaker, nothing else. It's easy to understand and integrate. If you need more features, hystrix-go ports Netflix's Hystrix patterns to Go, though it's less actively maintained than gobreaker.",
    },
    {
      lead: "Python:",
      text: "Pybreaker provides a simple circuit breaker implementation. For async Python, aiobreaker offers the same patterns for asyncio-based applications.",
    },
  ]}
/>
...
<Download
  resource="circuit-breaker-retry-budget-cascade-failure-prevention"
  title="Download the Circuit Breaker Guide"
  description="Get the complete resilience playbook for retry budgets, circuit states, and cascade-failure prevention across service chains."
  listItems={[
    {
      text: "Retry amplification math workbook",
    },
    {
      text: "Circuit threshold tuning checklist",
    },
    {
      text: "Fallback design pattern catalog",
    },
    {
      text: "On-call debugging runbook",
    },
  ]}
/>
