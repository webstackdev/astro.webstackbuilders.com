...
<Diagram
  ...
  description={
    `State diagram showing the lifecycle of a preview environment from creation through cleanup. The process starts when a PR is opened, moving to Requested, then Provisioning when CI is triggered, then Ready once resources are created. From Ready the environment becomes Active while the developer is testing. Active can transition to Idle when there is no activity, and Idle can return to Active when new activity occurs. Idle can also transition to Expiring once the TTL is exceeded, followed by Destroyed when the cleanup job runs. Active and Ready can both transition directly to Destroyed when the PR is merged or closed. A note attached to the Idle state highlights that cost accumulates there without delivering value. The diagram emphasizes that the main waste comes from environments that linger idle between active testing and final destruction.`
  }
>
```mermaid
stateDiagram-v2
...
flowchart LR
  ...
  subgraph cost["Cost Comparison (db.t3.small)"]
    C1["3 x $30/mo = $90/mo"]
    C2["1 x $30/mo = $30/mo"]
  end
...
flowchart LR
  ...
  B -->|under $15| C[Normal]
  B -->|$15 to $30| D[Notify Author]
  B -->|over $30| E[Notify Author + Manager]

    A --> F{Team Budget}
  F -->|under 80%| G[Normal]
  F -->|80% to 100%| H[Warning to Lead]
  F -->|over 100%| I[Pause New Envs]
```
</Diagram>
