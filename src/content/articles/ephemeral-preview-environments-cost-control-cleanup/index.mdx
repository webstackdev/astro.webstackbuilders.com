...
<Diagram
  src={ephemeralEnvironmentDiagram}
  size="full"
  magnifyButtonPosition="bottom-left"
  title="Ephemeral environment lifecycle states."
  description="State diagram showing the lifecycle of a preview environment from creation through cleanup. The process starts when a PR is opened, moving to Requested, then Provisioning when CI is triggered, then Ready once resources are created. From Ready the environment becomes Active while the developer is testing. Active can transition to Idle when there is no activity, and Idle can return to Active when new activity occurs. Idle can also transition to Expiring once the TTL is exceeded, followed by Destroyed when the cleanup job runs. Active and Ready can both transition directly to Destroyed when the PR is merged or closed. A note attached to the Idle state highlights that cost accumulates there without delivering value. The diagram emphasizes that the main waste comes from environments that linger idle between active testing and final destruction."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Resource types and their cleanup characteristics.",
    thead: { th: ["Resource Type", "Cost Driver", "Cleanup Difficulty", "Orphan Risk"] },
    tbody: {
      tr: [
        {
          th: "Containers/Pods",
          td: ["CPU/Memory hours", "Easy (K8s namespace delete)", "Low"],
        },
        {
          th: "Databases",
...
<Diagram
  src={sharedVsDedicatedDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Shared vs dedicated resource cost comparison."
  description="Flowchart comparing dedicated preview databases with a shared database model. In the Dedicated (Expensive) group are three separate databases labeled PR-123 DB, PR-124 DB, and PR-125 DB. In the Shared (Cost-Effective) group, one Shared DB branches into three isolated schemas: schema_pr_123, schema_pr_124, and schema_pr_125. A third group labeled Cost Comparison (db.t3.small) shows that three dedicated databases cost 3 x $30/mo = $90/mo, while one shared database costs 1 x $30/mo = $30/mo. Dotted arrows connect the dedicated databases to the higher cost and the shared database to the lower cost. The diagram shows how logical isolation in one shared service can preserve separation while reducing monthly infrastructure spend."
/>
...
<Table
  ...
  content={{
    figure: "TTL policy tiers.",
    thead: { th: ["Policy", "Max Lifetime", "Idle Timeout", "Use Case"] },
    tbody: {
      tr: [
        {
          th: "Default",
          td: ["72 hours", "8 hours", "Standard PR development"],
        },
        {
          th: "Extended",
          td: ["1 week", "24 hours", "Long feature branches"],
        },
        {
          th: "Critical",
          td: ["2 weeks", "48 hours", "Demo environments, requires approval"],
        },
      ],
    },
  }}
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Common resource hourly costs for preview environments.",
...
<Diagram
  src={costMonitoringDiagram}
  magnifyButtonPosition="top-left"
  title="Cost monitoring and escalation flow."
  description="Flowchart showing two parallel escalation paths driven by cost monitoring. The first path evaluates Environment Cost: if cost is under $15 the environment remains in a Normal state, from $15 to $30 it triggers Notify Author, and above $30 it triggers Notify Author + Manager. The second path evaluates Team Budget: under 80 percent remains Normal, from 80 to 100 percent sends a Warning to Lead, and above 100 percent pauses the creation of new environments. The diagram communicates that both individual environment cost and total team budget should drive escalating notifications and enforcement decisions."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Standard tags for preview environment resources.",
    thead: { th: ["Tag", "Purpose", "Example"] },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">preview/pr-number</span>',
          td: ["Links resource to PR", '<span class="font-mono">123</span>'],
        },
        {
          th: '<span class="font-mono">preview/repository</span>',
          td: ["Source repo for cross-repo queries", '<span class="font-mono">myorg/myapp</span>'],
        },
        {
          th: '<span class="font-mono">preview/author</span>',
          td: ["Cost attribution", '<span class="font-mono">jsmith</span>'],
        },
        {
          th: '<span class="font-mono">preview/team</span>',
          td: ["Budget tracking", '<span class="font-mono">platform</span>'],
        },
        {
          th: '<span class="font-mono">preview/created-at</span>',
          td: ["TTL calculation", '<span class="font-mono">2024-01-15T10:30:00Z</span>'],
        },
        {
          th: '<span class="font-mono">preview/managed-by</span>',
          td: ["Identifies automation-created resources", '<span class="font-mono">preview-controller</span>'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: "CI/CD pipeline",
      text: "Cleanup job triggered by PR close event.",
    },
    {
      lead: "Webhooks",
      text: "Direct GitHub webhook to your cleanup service.",
    },
    {
      lead: "TTL enforcement",
      text: "CronJob that deletes expired resources.",
    },
    {
      lead: "Orphan detection",
      text: "Weekly scan for resources whose PRs no longer exist.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: "Hibernation triggers and their cost impact.",
    thead: { th: ["Trigger", "Use Case", "Typical Savings"] },
    tbody: {
      tr: [
        {
          th: "Idle timeout (4h)",
          td: ["Pause inactive environments", "40-60%"],
        },
        {
          th: "Schedule (nights/weekends)",
          td: ["Known non-working hours", "50-70%"],
        },
        {
          th: { label: "Budget pressure", color: "info" },
          td: ["Team approaching limit", "Variable"],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    figure:
      "Resource profiles with costs and recommended use cases. Hourly costs are estimates for compute resources only (Fargate or equivalent); dedicated database profiles add $0.02-0.05/hour depending on instance size.",
    ...
    tbody: {
      tr: [
        ...
        {
          th: "Enhanced",
          td: ["1000m", "2Gi", "Dedicated small", "~$0.50", "Performance testing"],
        },
        {
          th: "Production-like",
          td: ["2000m", "4Gi", "Dedicated medium", "~$1.50", "Release validation"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: "Minimal",
      text: 'Changes only in <span class="font-mono">docs/</span>, <span class="font-mono">README.md</span>, or <span class="font-mono">.md</span> files',
    },
    {
      lead: "Minimal or Standard",
      text: "Changes in frontend code with no backend changes",
    },
    {
      lead: "Standard",
      text: "Standard backend changes",
    },
    {
      lead: "Enhanced",
      text: 'PR labeled <span class="font-mono">performance-test</span> or <span class="font-mono">load-test</span>',
    },
    {
      lead: "Production-like (with approval)",
      text: 'PR labeled <span class="font-mono">release-candidate</span>',
    },
  ]}
/>
  ...
  items={[
    {
      ...
      text: "Active environments, weekly cost, projected monthly cost, budget utilization",
    },
    {
      lead: "Team breakdown",
      text: "Table showing each team's environments, cost, and budget status",
    },
    {
      lead: "Longest running",
      text: "Environments over 7 days old, prompting review",
    },
    {
      lead: "Savings achieved",
      text: "How much was saved through hibernation, TTL cleanup, and orphan detection",
    },
    {
      lead: "Recommendations",
      text: "Auto-generated suggestions like \"Consider closing PR #456, running for 12 days with no activity\"",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Treat environments as ephemeral",
      text: "Aggressive TTLs with automatic cleanup. If it hasn't been used in 72 hours, it probably won't be.",
    },
    {
      lead: "Share infrastructure where possible",
      text: "Shared databases with schema isolation, shared caches with namespace prefixes. Dedicated resources only when testing actually requires them.",
    },
    {
      lead: "Hibernate instead of delete",
      text: "Wake-on-access preserves the illusion of always-available environments while eliminating idle compute costs.",
    },
    {
      lead: "Tag everything",
      text: "Consistent tagging enables orphan detection, cost attribution, and automated cleanup.",
    },
    {
      lead: "Make costs visible",
      text: "PR comments, dashboards, and weekly reports create accountability without bureaucracy.",
    },
  ]}
/>

<Download
  resource="ephemeral-preview-environments-cost-control-cleanup"
  title="Download the Ephemeral Environments Guide"
  description="Get the complete cost-control playbook for preview environments, including cleanup automation and infrastructure-sharing strategies."
  listItems={[
    {
      text: "TTL policy implementation templates",
    },
    {
      text: "Shared infrastructure isolation patterns",
    },
    {
      text: "Hibernation and wake workflows",
    },
    {
      text: "Preview cost dashboard blueprint",
    },
  ]}
/>
