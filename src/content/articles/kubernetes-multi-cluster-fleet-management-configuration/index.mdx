...
tags: ["cloud-platforms","kubernetes","argo-cd","helm"]
featured: true
---
...
<List
  ...
  items={[
    {
      lead: "Environment-based",
      text: "Clusters are the most common pattern: dev, staging, and production running as separate clusters. The configuration model here is straightforward — core add-ons, security policies, and RBAC structure should be identical, while resource quotas, replica counts, and feature flags differ by environment. You want staging to behave like production, just smaller.",
    },
    {
      lead: "Regional",
      text: "Clusters serve global applications or data sovereignty requirements. US-East, US-West, EU-West, and AP-Southeast might all run the same application versions and security policies, but with region-specific endpoints, compliance settings, and capacity. The challenge here is keeping application deployments synchronized while allowing regional customization.",
    },
    {
      lead: "Workload isolation",
      text: "Clusters separate concerns by team or workload type: frontend, backend, data platform, ML training. Each cluster shares network policies and security baselines, but node types, add-ons, and resource profiles differ dramatically. An ML training cluster needs GPUs and tolerates batch job failures; a frontend cluster needs fast scaling and zero-downtime deployments.",
    },
    {
      lead: "Hybrid cloud",
      text: "Clusters span providers: AWS primary, GCP disaster recovery, Azure edge, on-premises for sensitive workloads. Application manifests and security policies stay consistent, but cloud-specific integrations (load balancers, storage classes, IAM bindings) differ by necessity.",
    },
  ]}
/>

<Diagram
  src={multiClusterDiagram}
  size="full"
  title="Multi-cluster topology patterns."
  description="Flowchart showing three common multi-cluster topology patterns. In the first grouped section, labeled Environment-Based, a Git Repository points to three clusters: dev-cluster, staging-cluster, and prod-cluster. In the second grouped section, labeled Regional, a Git Repository points to four regional clusters: us-east, us-west, eu-west, and ap-southeast. In the third grouped section, labeled Hybrid, a Git Repository points to four infrastructure targets: AWS, GCP, Azure, and On-Prem. The diagram emphasizes that the same GitOps source model can support different organizational reasons for multi-cluster deployment: separate environments, geographic regions, or mixed cloud and on-premises platforms."
/>
...
<List variant="numbered-with-background-list">
  <ListItem lead="Fully consistent">
    Means all clusters get exactly the same configuration. This is simple to reason about and guarantees
    reproducibility, but it's inflexible. You end up over-provisioning dev and staging to match
    production, or you can't tune production for its actual load patterns. This approach only works for
    small fleets with simple applications.
  </ListItem>
  <ListItem lead="Layered configuration">
    Uses a base configuration with environment-specific overrides. This is the sweet spot for most
    organizations. You define what's shared (security policies, monitoring stack, core add-ons) and
    what varies (replica counts, resource limits, feature flags). Kustomize overlays and Helm values
    files are the typical implementation. The risk is "override explosion"—so many layers of patches
    that nobody can trace what a cluster actually runs.
  </ListItem>
  <ListItem lead="Policy-governed">
    Configuration sets guardrails rather than identical configs. Teams can customize their clusters
    within bounds enforced by OPA/Gatekeeper[^opa-gatekeeper] or Kyverno[^kyverno]. This enables
    innovation and team autonomy, but drift happens within the policy bounds. Debugging becomes harder
    because clusters are intentionally different.
  </ListItem>
  <ListItem lead="Fully flexible">
    Means each cluster is independent, with its own GitOps repo and configuration. Maximum autonomy,
    maximum chaos. Security risks emerge when teams deviate from baselines. This only works with truly
    autonomous teams who don't need cross-cluster consistency.
  </ListItem>
</List>
  ...
  items={[
    {
      ...
      text: "Extends ArgoCD's Application concept with generators that create Applications dynamically based on clusters, Git directories, or custom lists. If you're already running ArgoCD, ApplicationSets are the natural choice. The generator patterns (cluster, list, git, matrix, merge) handle most multi-cluster scenarios elegantly. The limitation is scale: beyond 500 clusters, the ApplicationSet controller can struggle, and you'll need sharding or multiple ArgoCD instances.",
    },
    {
      lead: "Flux",
    ...
    {
      lead: "Google Config Sync (part of Anthos)",
      text: "Offers deep GKE integration with built-in policy controller and namespace hierarchy management. If your fleet is GKE-centric, Config Sync provides tight integration with Google Cloud's IAM and audit logging. For multi-cloud or on-prem clusters, it's less flexible.",
    },
    {
      lead: "Crossplane",
...
<FileExplorer
  fullWidth={false}
  classes={{
    wrapper: "sm:ml-24 sm:mr-24 mb-6",
  }}
  ...
  data={[
    {
      ...
      children: [
          ...
          children: [
            {
              ...
              name: 'platform',
              children: [
                { type: 'file', name: 'kustomization.yaml', comment: 'Assembles shared platform manifests' },
                { type: 'file', name: 'namespace.yaml', comment: 'Creates shared target namespace' },
                { type: 'file', name: 'deployment.yaml', comment: 'Defines platform service deployment' },
                { type: 'file', name: 'service.yaml', comment: 'Exposes platform service internally' },
              ],
            },
          ],
        },
        {
          ...
          children: [
            ...
            { type: 'folder', name: 'staging' },
            {
              ...
              name: 'production',
              children: [
                { type: 'file', name: 'kustomization.yaml', comment: 'Applies production-specific overrides' },
              ],
            },
          ],
        },
        {
          ...
          name: 'clusters',
          children: [
            { type: 'folder', name: 'us-east-prod' },
            { type: 'folder', name: 'us-west-prod' },
            {
              type: 'folder',
              name: 'eu-west-prod',
              children: [
                { type: 'file', name: 'kustomization.yaml', comment: 'Sets cluster-specific resources' },
              ],
            },
          ],
        },
...
<Diagram
  src={argocdDiagram}
  size="full"
  title="ArgoCD generates Applications centrally - Flux controllers pull independently."
  description="Flowchart comparing the ArgoCD and Flux multi-cluster operating models. In the grouped section labeled ArgoCD Model, ApplicationSet points to Generator, which then fans out to Cluster 1 App, Cluster 2 App, and Cluster 3 App. Each of those generated applications points to a corresponding cluster: Cluster 1, Cluster 2, and Cluster 3. In the grouped section labeled Flux Model, a shared Git Repository points directly to Cluster 1 Flux, Cluster 2 Flux, and Cluster 3 Flux. Each cluster-specific Flux controller then points to its own layered configuration composed of base plus overlay plus cluster-specific settings. The diagram highlights that ArgoCD centralizes application generation before deployment, while Flux distributes reconciliation to controllers running inside each cluster, all pulling from the same repository."
/>
...
<Diagram
  src={configurationLayeringDiagram}
  size="full"
  title="Configuration layering from fleet-wide to cluster-specific."
  description="Flowchart showing a three-layer configuration model for multi-cluster fleets. Inside a grouped section labeled Configuration Layering, Base Config points to Security policies, Monitoring stack, and Core add-ons. Environment Overlay points to Replica counts, Resource limits, and Feature flags. Cluster-Specific points to Regional endpoints, Cloud integrations, and Compliance settings. Outside those branch relationships, Base Config points to Environment Overlay, and Environment Overlay points to Cluster-Specific, showing that the layers build on one another from shared fleet-wide defaults to environment-level overrides and finally to per-cluster customization. The diagram illustrates how organizations can preserve consistency while still allowing necessary differences between environments and individual clusters."
/>
...
The production overlay references the base and layers on deployment-specific settings — replica counts, pinned image tags, resource requests and limits, and merged ConfigMap values. Kustomize's `patches` field uses JSON Patch operations to surgically modify individual fields without rewriting the entire manifest.
...
The final layer targets a specific cluster. It references the production overlay and adds region-aware patches — injecting the correct database endpoint and constraining pod scheduling to the matching availability zone via `nodeSelector`.
...
The production values file overrides the base defaults with deployment-grade settings — higher replica counts, a pinned image tag, explicit resource limits, and a PodDisruptionBudget to maintain availability during rolling updates.
...
The cluster-specific file adds the final layer of customization — scaling replicas for a high-traffic region, wiring in the regional database endpoint, and using `nodeSelector` and `tolerations` to pin workloads to dedicated node pools.
...
The key thing to get right with layered values is merge order. Helm performs a shallow merge by default, so nested maps in a later values file replace — rather than merge with — the same key from an earlier file. If your base defines `resources.requests.cpu` and a cluster file only sets `resources.requests.memory`, you lose the CPU request entirely. Use Helmfile's `mergeStrategy: strategic` or restructure values to keep overrides at leaf keys to avoid silent misconfigurations that only surface under load.

<Table
  variant="grid-and-accent-header-table"
  content={{
    thead: {
      th: ['Approach', 'Pros', 'Cons', 'Best For'],
    },
    tbody: {
      tr: [
        {
          th: 'Kustomize overlays',
          td: ['Native K8s, no templating', 'Patch complexity grows', 'Simple environment overrides'],
        },
        {
          th: 'Helm values hierarchy',
          td: ['Powerful templating', 'Values file explosion', 'Complex apps with many variants'],
        },
        {
          th: 'Jsonnet',
          td: ['Programmable, DRY', 'Steep learning curve', 'Large-scale platforms'],
        },
        {
          th: 'CUE',
          td: ['Type-safe, validation', 'Newer, less ecosystem adoption', 'Configuration validation focus'],
        },
      ],
    },
    figure: 'Configuration templating approaches.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Require fleet management labels',
      text: 'Reject resources that lack labels indicating they\'re fleet-managed. This catches resources created outside the GitOps flow.',
    },
    {
      lead: 'Require GitOps annotations',
      text: 'Reject modifications to resources that should only be changed through Git. ArgoCD adds tracking annotations; policies can require their presence.',
    },
    {
      lead: 'Restrict manual changes in production',
      text: 'Allow direct kubectl access in dev/staging but block it in production namespaces.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Canary cluster',
      text: 'A single non-critical cluster (or a dedicated canary) that receives changes first. If something breaks, the blast radius is minimal.',
    },
    {
      lead: 'Non-critical production',
      text: 'Secondary regions, lower-traffic clusters, or less critical workloads. These validate the change works at production scale without risking your primary clusters.',
    },
    {
      lead: 'Primary production',
      text: 'The clusters that matter most. By the time changes reach here, they\'ve been validated in real production conditions.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Order by criticality',
      text: 'Operate on canary/dev clusters first, production last. If something fails, you find out before touching critical infrastructure.',
    },
    {
      lead: 'Verify between clusters',
      text: 'Don\'t blindly proceed. Check health, validate the operation succeeded, wait for stabilization.',
    },
    {
      lead: 'Have a rollback plan',
      text: 'Know how to undo the operation. For some operations (like config changes), rollback is straightforward. For others (like data migrations), it\'s not.',
    },
    {
      lead: 'Respect rate limits',
      text: 'Don\'t hammer your clusters. Add delays between operations, especially in production.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Operation Type', 'Strategy', 'Verification', 'Rollback'],
    },
    tbody: {
      tr: [
        {
          th: 'Security patch',
          td: ['Parallel (all at once)', 'Basic health', 'Per-cluster'],
        },
        {
          th: 'Feature rollout',
          td: ['Progressive waves', 'Canary metrics', 'Full fleet'],
        },
        {
          th: 'Config change',
          td: ['Sequential', 'Config validation', 'Previous config'],
        },
        {
          th: 'Add-on upgrade',
          td: ['Progressive', 'Integration tests', 'Previous version'],
        },
      ],
    },
    figure: 'Fleet operation strategies by type.',
  }}
/>
...
<Download
  resource="kubernetes-multi-cluster-fleet-management-configuration"
  title="Download the Multi-Cluster Fleet Guide"
  description="Get the complete fleet-operations playbook for GitOps consistency, drift prevention, and cluster-wide change coordination."
  listItems={[
    {
      text: "ArgoCD versus Flux matrix",
    },
    {
      text: "Fleet drift detection runbook",
    },
    {
      text: "Policy enforcement design templates",
    },
    {
      text: "Progressive rollout wave strategy",
    },
  ]}
/>
