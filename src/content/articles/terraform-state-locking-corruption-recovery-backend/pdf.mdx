...
import pathBasedDiagram from "./diagrams/path-based-state-organization-hierarchy.png"
import stateLockingDiagram from "./diagrams/state-locking-prevents-concurrent-modifications.png"
...
<List
  ...
  items={[
    ...
    {
      lead: 'resources',
      text: 'The mapping between your config (`aws_vpc.main`) and real infrastructure (`vpc-0123456789abcdef0`), including all the attributes Terraform knows about.',
    },
  ]}
/>
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'S3 + DynamoDB',
          td: ['DynamoDB conditional writes', 'Infinite', '`terraform force-unlock LOCK_ID`'],
        },
        {
          th: 'Azure Blob',
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'If you find yourself opening a state file in an editor, stop. Use `terraform state mv`, `terraform state rm`, or `terraform import` instead. Manual edits bypass validation and corrupt checksums.',
    },
    {
