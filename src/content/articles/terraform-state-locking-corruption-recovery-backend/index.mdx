...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'occur despite locking, usually because someone ran Terraform locally while CI was running, or two CI jobs targeted the same state due to misconfigured triggers. Look for "state serial mismatch" errors, or changes from one apply silently missing after another completes.',
    },
    {
      lead: 'Manual state edits',
      text: 'are tempting when you need to fix something quickly, but JSON is unforgiving. A missing comma, wrong type, or malformed resource address can corrupt the entire file. Any Terraform command will fail with parse errors.',
    },
    {
      lead: 'Provider version mismatches',
      text: "happen when you upgrade providers without considering state compatibility. The provider's internal schema version might change, and state written by the old provider might not be readable by the new one. Watch for warnings about schema versions or unexpected replacement plans.",
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Set `cancel-in-progress: false` in GitHub Actions — never cancel a running apply.',
    },
    {
    ...
    },
    {
      ...
      text: 'Run `terraform plan -out=tfplan`, then `terraform apply tfplan`. This prevents drift between plan and apply.',
    },
    {
      lead: 'Never edit state manually.',
      text: 'Use `terraform state` commands instead. Manual JSON edits bypass validation and corrupt checksums.',
    },
    {
