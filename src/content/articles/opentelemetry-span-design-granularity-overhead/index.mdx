...
<Highlighter>The lesson: granularity without readability is noise, not observability.</Highlighter> More spans don't mean better visibility — they often mean worse. The goal is _enough_ spans to debug problems, not so many that you create new ones.
      ...
      tr: [
        {
          ...
          td: ['Yes', '---', 'Service boundary; stitches distributed trace together'],
        },
        {
          th: 'Database query',
          td: ['Yes', '---', 'I/O operation; where latency typically lives'],
        },
        {
          th: 'JSON parsing',
          td: ['No', 'Attribute', 'Pure computation; no I/O involved'],
        },
        {
          th: 'Each loop iteration',
          td: ['No', 'Wrap loop', 'Creates overhead; one span for batch is sufficient'],
        },
        {
          th: 'External API call',
          td: ['Yes', '---', 'Dependency visibility; external services cause most incidents'],
        },
        {
          th: 'Field validation',
          td: ['No', 'Attribute', 'Too granular; timing rarely needed'],
        },
        {
          th: 'Error handling',
          td: ['No', 'Event', 'Within parent span; events capture error context'],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      lead: 'Spans',
      text: 'Represents operations with duration. They have start and end times, can have child spans, and appear as bars in your waterfall. Use them for I/O operations and significant boundaries. Cost: highest (memory allocation, context propagation, serialization).',
    },
    {
      lead: 'Events',
...
Notice that `validateOrder()` doesn't get a span — it's pure computation. The validation timing is captured as events if you need it. The database insert __does__ get a span because it's I/O where latency matters.
...
<List
  ...
  items={[
    ...
    {
      lead: 'Flat hierarchy.',
      text: 'All spans at the same level, no nesting. You can\'t tell which operations are contained within others, or whether operations ran sequentially or in parallel. The fix is ensuring child spans inherit from their parents using <span class="font-mono">startActiveSpan</span>.',
    },
    {
      lead: 'Missing gaps.',
    ...
    {
      lead: 'Cryptic names.',
      text: 'Span names like "span," "operation," "handler," or "process" that don\'t explain what\'s happening. Auto-instrumentation often produces these. The fix is customizing span names to follow the pattern "operation resource" (<span class="font-mono">HTTP GET /api/orders</span>, <span class="font-mono">db.query orders</span>).',
    },
    {
      lead: 'Attribute explosion.',
      text: 'Hundreds of attributes per span because someone dumped all available context. The trace viewer loads slowly, important attributes are buried in noise, and storage costs balloon. The fix is selecting relevant attributes deliberately.',
    },
  ]}
/>
