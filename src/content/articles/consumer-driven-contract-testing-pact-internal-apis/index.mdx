...
<Table
  ...
  content={{
    figure: "Comparison of testing approaches for API compatibility.",
    thead: {
      th: ["Testing Approach", "Speed", "Reliability", "Isolation", "Catches Breaking Changes"],
    },
    tbody: {
      tr: [
        {
          th: "Unit tests",
          td: ["Fast", "High", "Complete", "No (mocks hide reality)"],
        },
        {
          th: "Integration tests",
          td: ["Slow", "Low", "None", "Yes (but late)"],
        },
        {
          th: "E2E tests",
          td: ["Very slow", "Very low", "None", "Yes (in production-like)"],
        },
        {
          th: "Contract tests",
          td: ["Fast", "High", "Complete", "Yes (at PR time)"],
        },
      ],
    },
  }}
...
<Diagram
  src={traditionalDiagram}
  title="Traditional vs consumer-driven contract model"
  description="Flowchart comparing the traditional model, where a provider defines a schema that the consumer uses and hopes stays accurate, with the consumer-driven model, where the consumer defines a contract that the provider verifies before deployment."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Contract scope boundaries.",
    thead: {
      th: ["In Scope", "Out of Scope"],
    },
    tbody: {
      tr: [
        {
          th: "Request path and method",
          td: ["Specific data values"],
        },
        {
          th: "Required headers",
          td: ["Database state"],
        ...
        {
          th: "Response status codes",
          td: ["Exact error messages"],
        },
        {
          th: "Response body structure",
          td: ["Performance characteristics"],
        },
        {
          th: "Field types (string, number, array)",
          td: ["Security rules"],
        },
      ],
    },
...
<List
  ...
  classes={{
    wrapper: "sm:ml-12 sm:mr-30 mb-6",
  }}
  items={[
    {
      lead: 'Provider team updates their code and runs contract verification locally',
    },
    {
      lead: 'Verification fails for consumers using the affected fields',
    },
    {
      lead: 'Pact Broker shows exactly which consumers depend on the changing contract',
    },
    {
      lead: 'Provider team coordinates with affected consumer teams',
    },
    {
      lead: 'Consumer teams update their code and contracts',
    },
    {
      lead: 'Once all consumers have migrated, the provider deploys',
    },
  ]}
/>
...
<Diagram
  src={breakingChangeDiagram}
  title="Breaking change coordination workflow with Pact"
  description="Sequence diagram showing a provider team making a local change, failing contract verification, querying the Pact Broker for affected consumers, coordinating with consumer teams to update code and contracts, then rerunning verification successfully before deployment."
/>
...
<Diagram
  src={pactWorkflowDiagram}
  title="Pact workflow from consumer test to provider verification"
  description="Sequence diagram showing the consumer test defining expectations against the Pact mock server, publishing the generated contract to the Pact Broker, and the provider test fetching that contract, replaying interactions against the real provider service, verifying responses, and publishing verification results back to the broker."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Common Pact matchers and their use cases.",
    thead: {
      th: ["Matcher", "Use Case", "Contract Meaning"],
    },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">like(example)</span>',
          td: ["Most fields", "Same type as example"],
        },
        {
          th: '<span class="font-mono">eachLike(example)</span>',
          td: ["Arrays", "Array with items of same type"],
        },
        {
          th: '<span class="font-mono">regex(pattern, example)</span>',
          td: ["Enums, formats", "Must match regex"],
        },
        {
          th: '<span class="font-mono">datetime(format)</span>',
          td: ["Timestamps", "Valid datetime in format"],
        },
        {
          th: '<span class="font-mono">integer(example)</span>',
          td: ["Counts, IDs", "Integer number"],
        },
        {
          th: '<span class="font-mono">decimal(example)</span>',
          td: ["Prices, rates", "Decimal number"],
        },
        {
          th: '<span class="font-mono">uuid()</span>',
          td: ["Identifiers", "Valid UUID format"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: "Prism (from Stoplight)",
      text: "Acts as a mock server and validation proxy. Point it at your OpenAPI spec and it validates requests/responses in real-time.",
    },
    {
      lead: "Dredd",
      text: "Generates and runs tests from your spec, comparing actual API responses to documented examples.",
    },
    {
      lead: "Schemathesis",
      text: "Takes a property-based testing approach, generating hundreds of test cases from your schema to find edge cases.",
    },
  ]}
/>
      ...
      tr: [
        {
          ...
          td: [
            "Consumer tests",
            "Tests actual consumer needs; fine-grained matchers; built-in broker",
            "Requires buy-in from all teams; learning curve",
          ],
        },
        {
          th: "OpenAPI tools",
          td: [
            ...
            "Uses existing specs; simpler mental model",
            "Specs drift from reality; tests provider, not consumer needs",
          ],
        },
        {
          th: "GraphQL Schema",
          td: ["GraphQL schema", "Schema is the contract; breaking change detection", "GraphQL-specific"],
        },
      ],
    },
  }}
...
<Diagram
  ...
  title="Decision tree for choosing contract testing approach"
  description="Flowchart guiding teams through contract testing choices based on the number of consumers, whether consumers are known, and whether an OpenAPI specification already exists, ending with recommendations such as Pact CDC, schema plus versioning, Prism or Dredd, or the full Pact workflow."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Contract testing approach by scenario.",
    thead: {
      th: ["Scenario", "Recommended Approach"],
    },
    tbody: {
      tr: [
        {
          th: "Internal microservices, known consumers",
          td: ["Pact CDC"],
        },
        {
          th: "Public API, unknown consumers",
          td: ["OpenAPI + semantic versioning"],
        },
        {
          th: "GraphQL API",
          td: ["GraphQL schema + Apollo Studio"],
        },
        {
          th: "Small team, few services",
          td: ["Pact or schema-based both work"],
        },
        {
          th: "Large org, many teams",
          td: ["Pact with Pact Broker"],
        },
        {
          th: "Legacy API, can't change provider",
          td: ["Schema-based validation only"],
        },
      ],
    },
  }}
...
<Diagram
  src={canIDeployDiagram}
  title="Can-I-Deploy flow preventing incompatible deployments"
  description="Sequence diagram showing a developer pushing code to CI, CI running tests, publishing contracts or verification results to the Pact Broker, and querying can-i-deploy. If compatibility checks pass, CI deploys to production and records the deployment; if they fail, the build stops and the developer gets a breaking change notice."
/>
...
<FileExplorer
  ...
  figure="Example project structure for consumer and provider services, showing where contract tests, provider states, Pact configuration, and generated pact files typically live."
  data={[
    {
      type: 'folder',
      ...
      children: [
          ...
          children: [
            {
              ...
              name: 'clients',
              children: [
                { type: 'file', name: 'user-service-client.ts' }
              ]
            }
          ]
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              name: 'contract',
              children: [
                { type: 'file', name: 'pact.setup.ts', comment: 'Shared Pact configuration' },
                { type: 'file', name: 'user-service.pact.ts', comment: 'Contracts for UserService' },
                { type: 'file', name: 'order-service.pact.ts', comment: 'Contracts for OrderService' }
              ]
            }
          ]
        },
        {
          type: 'folder',
          name: 'pacts',
          comment: 'Generated pact files (gitignored)',
          children: [
            { type: 'file', name: 'consumerservice-userservice.json' }
          ]
        },
        { type: 'file', name: 'pact.config.js', comment: 'Pact configuration' }
      ]
    },
    {
      type: 'folder',
        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              name: 'contract',
              children: [
                { type: 'file', name: 'provider-states.ts', comment: 'State handlers' },
                { type: 'file', name: 'provider-verification.ts', comment: 'Verification tests' }
              ]
            }
          ]
        },
        { type: 'file', name: 'pact.config.js' }
      ]
    }
  ]}
/>
...
<Table
  variant="grid-and-accent-header-table"
  classes={{
    figure: "sm:ml-24 sm:mr-24 mb-6",
  }}
  content={{
    figure: "Provider state complexity pitfalls.",
    thead: {
      th: ["", "State Description", "Problem / Implementation"],
    },
    tbody: {
      tr: [
        {
          th: '❌',
          td: [
            '"production-like database with 1000 users"',
            "Slow, brittle, not what contract tests are for",
          ],
        },
        {
          th: '❌',
          td: [
            '"user 123 with orders 456, 789 and payment method ending 4242"',
            "Too specific, couples consumer to provider data model",
          ],
        },
        {
          th: '✅',
          td: [
            '"a user exists"',
            "Create one user with auto-generated data",
          ],
        },
        {
          th: '✅',
          td: [
            '"user has orders"',
            "Create user with 2 generic orders",
          ],
        },
        {
          th: '✅',
          td: [
            '"unauthorized"',
            "No setup needed, just don't auth",
          ],
        },
      ],
    },
...
<Diagram
  src={contractTestingAtScaleDiagram}
  size="2xl"
  title="Contract testing at scale with Pact Broker as central registry"
  description="Flowchart showing multiple consumer services publishing contracts into a central Pact Broker registry, which then serves those contracts to multiple provider services for verification and compatibility tracking."
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: "Environment management",
          td: [
            '<span class="font-mono">record-deployment</span> tracks which versions run where, so <span class="font-mono">can-i-deploy</span> knows what to check against',
          ],
        },
        {
          th: "Webhook triggers",
        ...
        {
          th: "Pending pacts",
          td: ["New contracts don't fail provider builds until providers explicitly support them"],
        },
        {
          th: "WIP pacts",
          td: ["Include work-in-progress contracts in verification for early feedback"],
        },
        {
          th: "Matrix view",
          td: ["Pact Broker UI visualizes compatibility across all services at a glance"],
        },
      ],
    },
  }}
...
<Download
  resource="consumer-driven-contract-testing-pact-internal-apis"
  title="Download the Contract Testing Guide"
  description="Get the complete implementation playbook for consumer-driven contracts, provider verification, and safe multi-service deployments."
  listItems={[
    {
      text: "Pact workflow setup checklist",
    },
    {
      text: "Matcher design best practices",
    },
    {
      text: "Provider state management guide",
    },
    {
      text: "Can-i-deploy policy runbook",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Contracts capture what consumers actually use",
      text: "not what providers think they provide. This focus on real dependencies eliminates false positives from unused fields.",
    },
    {
      lead: "The Pact Broker is your safety net.",
      text: 'The <span class="font-mono">can-i-deploy</span> command answers "will this deployment break anything?" with certainty, not hope.',
    },
    {
      lead: "Contract tests verify structure, not business logic.",
      text: "Use matchers to specify types and patterns; leave exact values to integration tests.",
    },
    {
      lead: "The investment scales well.",
      text: "While integration test complexity grows quadratically with service count, contract testing grows linearly.",
    },
  ]}
/>
...
<Callout type="info">
  ...
  <List
    ...
    items={[
      {
        text: "Pick one consumer-provider pair with a stable, well-understood integration",
      },
      {
        text: "Write 2-3 consumer contract tests covering your most critical interactions",
      },
      {
        text: "Deploy Pact Broker (Docker Compose for dev, Kubernetes for production)",
      },
      {
        text: "Add provider verification tests with state handlers",
      },
      {
        text: "Wire both into CI: publish on consumer builds, verify on provider builds",
      },
      {
        text: "Add `can-i-deploy` checks before deployments",
      },
      {
        text: "Expand to additional service pairs",
      },
    ]}
  />
</Callout>
