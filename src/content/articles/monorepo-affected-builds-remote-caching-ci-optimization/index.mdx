...
tags: ["build-and-deploy", "typescript", "aws", "azure", "docker", "kubernetes"]
featured: true
---
...
<Table
  ...
  content={{
    figure: 'Base reference strategies by CI scenario.',
    thead: {
      th: ['Scenario', 'Base Reference', 'What Gets Rebuilt'],
    },
    tbody: {
      tr: [
        {
          th: 'PR to main',
          td: ['<span class="font-mono">origin/main</span>', 'All changes in the PR'],
        },
        {
          th: 'Push to main',
          td: ['<span class="font-mono">HEAD~1</span> or last successful CI', 'Just the pushed commit(s)'],
        },
        {
          th: 'Release build',
          td: ['Last release tag', 'Everything since previous release'],
        },
        {
          th: 'Nightly build',
          td: ['Last successful CI', 'Minimal (only failures to retry)'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Task name and package name',
    },
    {
      lead: 'Hash of all input files',
    },
    {
      lead: 'Hash of dependency outputs (transitive)',
    },
    {
      lead: 'Relevant environment variables',
    },
    {
      lead: 'Runtime versions (Node, Ruby, Python, JDK, .NET SDK)',
    },
    {
      lead: 'Command arguments',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Provider', 'Setup Complexity', 'Cost', 'Self-Hosted Option'],
    },
    tbody: {
      tr: [
        {
          th: 'Nx Cloud',
          td: ['Low', 'Free tier, paid for teams', 'Yes (enterprise)'],
        },
        {
          th: 'Vercel (Turbo)',
          td: ['Low', 'Free tier, paid for teams', 'No'],
        },
        {
          th: 'Self-hosted Turbo',
          td: ['Medium', 'Infrastructure only', 'Yes'],
        },
        {
          th: 'Custom S3',
          td: ['High', 'S3 costs only', 'Yes'],
        },
      ],
    },
    figure: 'Remote caching provider comparison.',
  }}
/>
...
<Diagram
  src={ciOptimizationDiagram}
  size="full"
  title="CI optimization progression from sequential to distributed execution."
  description="Flowchart comparing three stages of CI optimization in a monorepo. In the grouped section labeled Traditional CI, a single Runner performs Build pkg1, then Build pkg2, then Build pkg3, then Test all, ending at 60 min total. In the grouped section labeled Affected plus Cache, a Runner first checks a decision node labeled Affected. One branch indicates pkg1 only, leading to Build pkg1 and then Test pkg1, ending at 5 min total. Another branch indicates a Cache hit, leading to Skip pkg2, pkg3. In the grouped section labeled Distributed, a Coordinator dispatches work to Agent 1 for pkg1, Agent 2 for pkg2, and Agent 3 for pkg3, and all three converge on 3 min total. The diagram shows the progression from slow sequential execution, to skipping unaffected or cached work, to coordinating parallel work across multiple agents for the fastest feedback."
/>
...
<List
  ...
  items={[
    {
      lead: 'Input hash changes',
      text: 'The primary invalidation mechanism. When any file included in a target\'s inputs changes, the hash changes, and the cache misses. This is usually what you want: source code changed, so rebuild. The configuration challenge is ensuring only relevant files are included in inputs.',
    },
    {
      lead: 'Dependency updates',
      text: 'Cascades through the graph. When you update a package in <span class="font-mono">package.json</span> or change the lock file, packages that depend on that change need to rebuild. Both Nx and Turborepo handle this automatically through their global dependencies configuration.',
    },
    {
      lead: 'Manual invalidation',
      text: 'Occasionally necessary when something outside the tracked inputs changes — a CI environment update, a bug in the caching system, or when you need to verify that builds still work without caching. In Nx, <span class="font-mono">npx nx reset</span> clears the local cache. Adding <span class="font-mono">--skip-nx-cache</span> bypasses caching for a single run. In Turborepo, <span class="font-mono">--force</span> achieves the same.',
    },
    {
      lead: 'Time-based expiration (TTL)',
      text: 'Prevents caches from growing unboundedly and ensures stale artifacts eventually get cleaned up. Most remote cache providers let you configure retention periods—7 to 30 days is typical for active projects.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Environment inconsistency',
      text: 'The most common culprit. If your CI runners use Node 20.10 and developers use Node 20.11, the hashes won\'t match. Pin exact versions everywhere: <span class="font-mono">.nvmrc</span> or <span class="font-mono">.node-version</span> for Node, <span class="font-mono">Gemfile.lock</span> with <span class="font-mono">.ruby-version</span> for Ruby, <span class="font-mono">packages.lock.json</span> for NuGet, <span class="font-mono">composer.lock</span> for PHP, <span class="font-mono">requirements.txt</span> with pinned versions (or <span class="font-mono">pip-tools</span>) for Python. Ensure CI reads these version files, and consider Docker images with fixed runtimes for complete consistency.',
    },
    {
      lead: 'Non-deterministic output',
      text: 'Poisons the cache silently. If your build includes a timestamp, a random identifier, or output that varies based on execution order, the outputs will differ even with identical inputs. Look for build timestamps in generated files, unsorted imports or exports, and random IDs in bundles. The <span class="font-mono">SOURCE_DATE_EPOCH</span> environment variable helps with timestamp-based non-determinism.',
    },
    {
      lead: 'Overly broad input specifications',
      text: 'Causes unnecessary invalidation. If your build target includes <span class="font-mono">{projectRoot}/**/*</span> as inputs, then changing a test file invalidates the production build cache — even though tests don\'t affect the build output. Use named input sets (like Nx\'s "production" pattern) that exclude test files, documentation, and other non-production artifacts.',
    },
    {
      lead: 'Global dependencies included incorrectly',
      text: 'Causes cascading invalidation. If you list <span class="font-mono">package-lock.json</span> in every target\'s inputs, then every dependency update invalidates every cache. Be selective: only include lock files in inputs for targets that actually care about dependency versions (usually everything, but some lint tasks might not).',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Issue', 'Symptom', 'Solution'],
    },
    tbody: {
      tr: [
        {
          th: 'Node version mismatch',
          td: ['0% cache hits in CI', 'Pin exact version with <span class="font-mono">.nvmrc</span>'],
        },
        {
          th: 'Test files in build inputs',
          td: ['Cache miss on test changes', 'Use "production" named input'],
        },
        {
          th: 'Timestamps in output',
          td: ['Cache never hits twice', 'Set <span class="font-mono">SOURCE_DATE_EPOCH</span>'],
        },
        {
          th: 'Lock file in all inputs',
          td: ['Every dep update invalidates all', 'Be selective about global inputs'],
        },
      ],
    },
    figure: 'Common cache issues and solutions.',
  }}
/>
...
The key metrics for monorepo CI fall into three categories: __timing__ (how long builds take), __affected scope__ (how much of the repo rebuilds), and __cache efficiency__ (how much work caching eliminates).
...
For __PR builds__, target under 10 minutes with a 70%+ cache hit rate. Affected percentage should typically be under 20%—most PRs touch a small portion of the codebase. If you're consistently seeing 50%+ affected, either your PRs are too large or your dependency structure creates excessive coupling.

For __main branch builds__, target under 5 minutes with an 85%+ cache hit rate. Main branch builds typically include just one commit's changes, so affected percentage should be minimal. High cache hit rates are expected because the PR already primed the cache.

For __release builds__, targets depend on your release strategy. If you're rebuilding from a tag that's far behind main, expect lower cache hit rates and longer durations. For frequent releases close to main, you should still see good caching.
...
<Table
  ...
  content={{
    thead: {
      th: ['Scenario', 'Target Duration', 'Target Cache Hit Rate', 'Typical Affected %'],
    },
    tbody: {
      tr: [
        {
        ...
        {
          th: 'Release builds',
          td: ['&lt; 30 minutes', '&gt; 50%', 'Varies'],
        },
      ],
    },
    figure: 'Performance baseline targets by scenario.',
  }}
/>
...
<Download
  resource="monorepo-affected-builds-remote-caching-ci-optimization"
  title="Monorepo Release Engineering: Affected Builds"
  description="Building only what changed with affected-based builds and remote caching that actually speeds up CI."
  listItems={[
    {
      text: "Affected build setup checklist",
    },
    {
      text: "Remote cache hardening guide",
    },
    {
      text: "Cache miss debugging playbook",
    },
    {
      text: "CI metrics tracking templates",
    },
  ]}
/>
