...
<Table
  ...
  content={{
    thead: {
      th: ['Resource', 'Type', 'Enforcement', 'Exceeded Behavior', 'Recovery'],
    },
    tbody: {
      tr: [
        {
          th: 'CPU',
          td: ['Compressible', 'CFS throttling', 'Slowed execution', 'Automatic'],
        },
        {
          th: 'Memory',
          td: ['Incompressible', 'OOM kill', 'Pod termination', 'Restart'],
        },
      ],
    },
    figure: 'CPU vs memory enforcement.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Guaranteed',
      text: 'The highest priority class. To get it, every container in the pod must have requests and limits set for both CPU and memory, and the requests must equal the limits. These pods are evicted last.',
    },
    {
      lead: 'Burstable',
      text: 'The middle tier. Any pod with at least one resource request or limit, but that doesn\'t qualify for Guaranteed, falls here. This includes pods with requests only, limits only, or requests that don\'t match limits. Most production workloads end up Burstable.',
    },
    {
      lead: 'BestEffort',
      text: 'The lowest priority. Pods with no resource specifications at all — no requests, no limits on any container — get this class. They\'re evicted first under pressure.',
    },
  ]}
/>
...
The QoS class assignment is entirely mechanical — there's no annotation or field you can set to override it. Kubernetes inspects every container in the pod (including init containers for resource accounting) and applies a strict decision tree. This means a multi-container pod where one sidecar is missing its limits will drop the entire pod from Guaranteed to Burstable, even if the primary container is perfectly configured. Auditing QoS classes across a namespace with `kubectl get pods -o custom-columns=NAME:.metadata.name,QOS:.status.qosClass` is worth doing periodically, especially after adding sidecars or updating Helm charts that modify resource blocks.

<Diagram
  src={qosClassDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="QoS class determination flow."
  description="Flowchart showing how Kubernetes derives a pod's Quality of Service class from its resource specification. The process starts at Pod Resource Spec and asks whether all containers have both requests and limits. If no, the chart asks whether any container has either a request or a limit. If that answer is no, the pod is classified as BestEffort and then labeled Evicted First. If that answer is yes, the pod is classified as Burstable and then labeled Evicted Second. If all containers do have both requests and limits, the chart asks whether requests equal limits for all containers. If no, the pod is classified as Burstable and then labeled Evicted Second. If yes, the pod is classified as Guaranteed and then labeled Evicted Last. The diagram visually emphasizes that BestEffort is the lowest-priority class, Burstable is the middle class, and Guaranteed is the highest-priority class for eviction order."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['QoS Class', 'Eviction Priority', 'Memory Enforcement', 'CPU Enforcement', 'Use Case'],
    },
    tbody: {
      tr: [
        {
          th: 'BestEffort',
          td: ['First (highest)', 'None', 'None', 'Dev/batch'],
        },
        {
          th: 'Burstable',
          td: ['Middle', 'Limit (OOM)', 'Limit (throttle)', 'Most workloads'],
        },
        {
          th: 'Guaranteed',
          td: ['Last (lowest)', 'Request=Limit', 'Request=Limit', 'Critical services'],
        },
      ],
    },
    figure: 'QoS class behavior summary.',
  }}
/>
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'This creates Burstable pods that can consume unbounded memory or CPU. A memory leak in one pod can starve its neighbors or trigger node-level OOM. The pod gets scheduled based on its request, but nothing stops it from using 10x that amount.',
    },
    {
...
<Diagram
  src={resourceSizingDiagram}
  size="full"
  title="Resource sizing decision tree."
  description="Flowchart guiding the reader through pod resource sizing choices. The process starts at Size Resources and asks for the workload type. If the workload is Critical or Latency-sensitive, the chart recommends Guaranteed QoS, then requests = limits, then proceeds to Validate Config. If the workload is Variable or Web, the chart recommends Burstable QoS, then limits = 2-4x requests, then proceeds to Validate Config. If the workload is Batch or Dev, the chart recommends BestEffort or Burstable, then Minimal or no resources, then Lowest priority, cheap. From Validate Config, the chart asks whether memory limit is greater than peak usage. If no, the result is Risk: OOMKilled. If yes, it asks whether CPU limit is greater than burst need. If no, the result is Risk: Throttling. If yes, the result is Good Config. The diagram shows that workload type drives the initial sizing strategy, but every path still needs validation against observed memory peaks and CPU burst behavior to avoid runtime failures."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Metric', 'Use For', 'Calculation'],
    },
    tbody: {
      tr: [
        {
          th: 'P50 CPU',
          td: ['Baseline', 'Median usage'],
        },
        {
          th: 'P95 CPU',
          td: ['Requests', 'Normal operation ceiling'],
        },
        {
          th: 'Max CPU + 20%',
          td: ['Limits', 'Peak + buffer (throttling recoverable)'],
        },
        {
          th: 'P95 Memory',
          td: ['Requests', 'Normal operation'],
        },
        {
          th: 'Max Memory + 50%',
          td: ['Limits', 'Peak + buffer (OOM not recoverable)'],
        },
      ],
    },
    figure:
      'Metrics for right-sizing. See the efficiency table in Cost Attribution for interpreting utilization percentages.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Control', 'Scope', 'Purpose', 'Enforcement'],
    },
    tbody: {
      tr: [
        {
          th: 'LimitRange',
          td: ['Container/Pod', 'Defaults and bounds', 'Pod creation'],
        },
        {
          th: 'ResourceQuota',
          td: ['Namespace', 'Total capacity', 'Object creation'],
        },
        {
          th: 'PriorityClass',
          td: ['Cluster', 'Eviction order', 'Scheduling/eviction'],
        },
      ],
    },
    figure: 'Resource control mechanisms.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Efficiency', 'Interpretation', 'Action'],
    },
    tbody: {
      tr: [
        {
          th: '&lt; 20%',
          td: ['Severely over-requested', 'Reduce requests by 50% or more'],
        },
        {
          th: '20-50%',
          td: ['Over-requested', 'Right-size using VPA recommendations'],
        },
        {
          th: '50-80%',
          td: ['Well-sized', 'Maintain current configuration'],
        },
        {
          th: '&gt; 80%',
          td: ['Running hot', 'Increase headroom for stability'],
        },
      ],
    },
    figure: 'Efficiency interpretation and actions.',
  }}
/>
...
<Download
  resource="kubernetes-pod-resource-requests-limits-qos-classes"
  title="Download the Pod Sizing Guide"
  description="Get the complete resource-sizing playbook for requests, limits, QoS behavior, and node-pressure resilience."
  listItems={[
    {
      text: "QoS class decision matrix",
    },
    {
      text: "Request limit sizing worksheet",
    },
    {
      text: "Eviction risk reduction checklist",
    },
    {
      text: "Resource tuning query pack",
    },
  ]}
/>
