...
import qosClassDiagram from "./diagrams/qos-class-determination-flow.png"
import resourceSizingDiagram from "./diagrams/resource-sizing-decision-tree.png"
...
<List
  ...
  items={[
    {
      lead: 'Guaranteed',
      text: 'is the highest priority class. To get it, every container in the pod must have requests and limits set for both CPU and memory, and the requests must equal the limits. These pods are evicted last.',
    },
    {
      lead: 'Burstable',
      text: 'is the middle tier. Any pod with at least one resource request or limit, but that doesn\'t qualify for Guaranteed, falls here. This includes pods with requests only, limits only, or requests that don\'t match limits. Most production workloads end up Burstable.',
    },
    {
      lead: 'BestEffort',
      text: 'is the lowest priority. Pods with no resource specifications at all — no requests, no limits on any container — get this class. They\'re evicted first under pressure.',
    },
  ]}
/>
...
You can check a pod's QoS class with `kubectl get pod <name> -o yaml | grep qosClass`. If you're surprised by the result, remember that __all__ containers in the pod must meet the Guaranteed criteria — one container without limits drops the whole pod to Burstable.
