...
<List
  ...
  content={{
    items: [
      {
        lead: 'Guaranteed',
        text: 'is the highest priority class. To get it, every container in the pod must have requests _and_ limits set for both CPU and memory, and the requests must equal the limits. These pods are evicted last.',
      },
      {
        lead: 'Burstable',
