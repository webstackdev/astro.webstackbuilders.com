...
<Table
  ...
  content={{
    figure: 'Big-bang vs. strangler fig tradeoffs',
    thead: {
      th: ['Approach', 'Blast Radius', 'Rollback', 'Time to Value', 'Observability'],
    },
    tbody: {
      tr: [
        {
          th: 'Big-bang',
          td: ['100% of traffic', 'Full rollback (maybe)', 'All at the end', 'Post-migration only'],
        },
        {
          th: 'Strangler fig',
          td: ['Only migrated traffic', 'Route back to legacy', 'Incremental', 'Required at every step'],
        },
      ],
    },
  }}
...
public class LegacyInstrumentationModule : IHttpModule {
    ...
    public void Init(HttpApplication context) {
        ...
        context.EndRequest += (s, e) => {
            ...
            // Record Metrics
            RequestCounter.Add(
              1, new("method", app.Request.HttpMethod), new("status", status)
            );
            RequestDuration.Record(
              duration, new("method", app.Request.HttpMethod), new("status", status)
            );
...
histogram_quantile(
  0.99, sum(rate(http_request_duration_ms_bucket{system="legacy"}[5m])) by (le, path)
)
```
...
<Diagram
  src={shadowTrafficDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Shadow traffic architecture with async mirroring"
  description="A left-to-right flowchart showing how shadow traffic is mirrored during migration. A client request goes to a load balancer, which sends the primary request to the legacy system. The legacy system returns the response to the client. At the same time, the load balancer sends an asynchronous copy of the request to the new system. Both the new system and the legacy system send their results to a comparison and logging component. The diagram shows that users continue receiving responses only from the legacy system while the new system is evaluated in parallel."
/>
...
public class ShadowTrafficHandler : DelegatingHandler {
    ...
    public ShadowTrafficHandler(
      string newServiceBaseUrl, IComparisonService comparisonService
    ) {
    ...
    private async Task SendShadowRequest(
      HttpRequestMessage originalRequest,
      string requestBody,
      HttpStatusCode legacyStatus,
      string legacyBody,
      long legacyLatencyMs
        ...
        if (requestBody != null) {
            shadowRequest.Content = new StringContent(
...
async def compare_response_signatures(signatures: func.DocumentList):
    ...
    container = cosmos.get_database_client(
      "migration"
    ).get_container_client("signatures")
...
<List
  ...
  items={[
    {
      lead: "Mismatch rate below 0.1% (or whatever threshold you've defined as acceptable)",
    },
    {
      lead: 'No systematic errors-random noise is fine, but patterns indicate bugs',
    },
    {
      lead: 'Latency within 20% of legacy system (the new system being faster is fine; much slower is a problem)',
    },
    {
      lead: 'No memory leaks or connection pool exhaustion under sustained load',
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: 'Code: Auth service endpoint contract',
    thead: {
      th: ['Endpoint', 'Method', 'Purpose', 'SLO'],
    },
    tbody: {
      tr: [
        {
          th: '<span class="text-mono">/auth/login</span>',
          td: ['POST', 'Authenticate user, return tokens', 'P99 < 200ms, 99.9% available'],
        },
        {
          th: '<span class="text-mono">/auth/logout</span>',
          td: ['POST', 'Invalidate tokens', 'P99 < 50ms, 99.9% available'],
        },
        {
          th: '<span class="text-mono">/auth/refresh</span>',
          td: ['POST', 'Exchange refresh token for new access', 'P99 < 50ms, 99.9% available'],
        },
        {
          th: '<span class="text-mono">/auth/password/reset-request</span>',
          td: ['POST', 'Initiate password reset', 'P99 < 100ms, 99.9% available'],
        },
        {
          th: '<span class="text-mono">/auth/password/reset</span>',
          td: ['POST', 'Complete password reset', 'P99 < 100ms, 99.9% available'],
        },
        {
          th: '<span class="text-mono">/internal/validate</span>',
    ...
    public async Task<ValidationResult> ValidateTokenAsync(string token) {
        try {
            ...
            var issuedAt = long.Parse(
              principal.FindFirst(JwtRegisteredClaimNames.Iat)?.Value ?? "0"
            );
    ...
    public async Task<User> CreateUserAsync(CreateUserInput input) {
        ...
        if (_flags.IsEnabled("auth_dual_write")) {
            try {
                ...
                _logger.LogError(
                  ex,
                  "Failed to sync user {UserId} to auth service",
                  user.Id
                );
    ...
    public async Task UpdatePasswordAsync(string userId, string newPassword) {
        ...
        if (_flags.IsEnabled("auth_dual_write")) {
            try {
                ...
                _logger.LogError(
                  ex,
                  "Failed to sync password for user {UserId}",
                  userId
                );
                await _syncQueue.EnqueueAsync(
                  new SyncPasswordJob { UserId = userId, Hash = hash }
                );
            }
        }
    }
        ...
        while (true) {
            ...
            foreach (var user in users) {
                ...
                try {
                    ...
                    report.Migrated++;
                    // Be gentle with the new service
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Progressive traffic shift schedule',
    thead: {
      th: ['Day', 'New System %', 'Gate Criteria'],
    },
    tbody: {
      tr: [
        {
          th: '0',
          td: ['1%', 'Manual verification of first requests'],
        },
        {
          th: '1',
          td: ['5%', '24h at 1% with &lt;0.1% error rate increase'],
        },
        {
          th: '3',
          td: ['10%', '48h at 5% with latency within 10%'],
        },
        {
          th: '5',
          td: ['25%', '48h at 10% with no incidents'],
        },
        {
          th: '7',
          td: ['50%', '48h at 25% with no incidents'],
        },
        {
          th: '10',
          td: ['75%', '72h at 50% with no incidents'],
        },
        {
          th: '14',
          td: ['100%', '96h at 75% with no incidents'],
        },
      ],
    },
  }}
  ...
  items={[
    {
      ...
      text: 'Your own team uses the new system first. They can report issues directly.',
    },
    {
      lead: 'Beta opt-ins',
      text: 'Users who explicitly want new features and understand the tradeoffs.',
    },
    {
      lead: 'Low-tier accounts',
      text: 'Free users or small organizations where an issue has limited blast radius.',
    },
    {
      lead: 'New signups',
      text: 'Users with no history in the legacy system avoid data migration issues.',
    },
  ]}
/>
        ...
        if (!string.IsNullOrEmpty(authHeader) && authHeader.StartsWith(
          ...
          StringComparison.OrdinalIgnoreCase
            ...
            if (validation.Valid) {
                ...
                claims.AddRange(
                  validation.Permissions.Select(p => new Claim("permission", p))
                );
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Automatic rollback trigger thresholds',
    thead: {
      th: ['Trigger', 'Threshold', 'Rationale'],
    },
    tbody: {
      tr: [
        {
          th: 'Error rate',
          td: ['>1% absolute or >0.5% above baseline', 'Catches both spikes and regressions'],
        },
        {
          th: 'Latency (P99)',
          td: ['>2x legacy baseline', "New system shouldn't be slower"],
        },
        {
          th: 'Availability',
          td: ['&lt;99.5% successful responses', 'Users notice at this level'],
        },
        {
          th: 'Evaluation window',
          td: ['5 minutes with 2-minute sustained violation', 'Filters brief spikes from real issues'],
        },
      ],
    },
  }}
...
<Table
  variant="numbered-review-table"
  content={{
    figure: 'Migration completion checklist',
    thead: {
    ...
    tbody: {
      tr: [
        {
          th: 'Traffic',
          td: ['100% on new system; no legacy traffic for 7+ days; all endpoints including jobs'],
        },
        {
          th: 'Stability',
          td: ['7 days at 100% with no rollbacks; error rate at/below baseline; latency within 10%'],
        },
        {
          th: 'Observability',
          td: ['Dashboards complete; alerting configured and tested; runbooks updated'],
        },
        {
          th: 'Data',
          td: ['Migration verified; no legacy DB dependencies; reconciliation shows zero drift'],
        },
        {
          th: 'Organizational',
          td: ['Team trained; documentation updated; stakeholder sign-off obtained'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    figure: 'Legacy system sunset phases',
    thead: {
      th: ['Phase', 'Duration', 'Actions'],
    },
    tbody: {
      tr: [
        {
          th: 'Traffic removal',
          td: ['Complete', '100% traffic to new system; legacy running but idle'],
        },
        {
          th: 'Monitoring',
          td: ['2 weeks', 'Watch for unexpected legacy traffic, hardcoded references, background jobs'],
        },
        {
          th: 'Read-only mode',
          td: ['1 week', 'Disable writes on legacy; return 503 with helpful message; log all requests'],
        },
        {
          th: 'Scream test',
          td: ['1 week', 'Stop legacy service entirely; monitor dependent systems; keep infrastructure for restart'],
        },
        {
          th: 'Decommission',
          td: ['1 day', 'Remove infrastructure, archive code, delete databases (after backup)'],
        },
      ],
    },
  }}
    ...
    public async Task<ReconciliationReport> ReconcileAsync() {
        ...
        foreach (var monolithUser in monolithUsers) {
            ...
            if (differences.Any()) {
                ...
                // Legacy system is source of truth
                await _authService.SyncUserAsync(monolithUser);
                report.Fixed++;
...
<List
  ...
  items={[
    {
      lead: 'Duration:',
      text: 'Planned vs. actual (aim for < 50% overrun)',
    },
    {
      lead: 'Rollbacks:',
      text: 'Number of automatic rollbacks during traffic shifting (aim for < 3)',
    },
    {
      lead: 'Incidents:',
      text: 'User-facing incidents during migration (aim for zero)',
    },
    {
      lead: 'Downtime:',
      text: 'Total downtime attributable to the migration (aim for zero)',
    },
  ]}
/>
