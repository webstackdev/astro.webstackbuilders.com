...
The strangler fig pattern offers a different approach: incremental replacement with continuous validation. But the pattern alone isn't enough. <Highlighter>The real power comes from treating migration as an observability problem first and a development problem second.</Highlighter> When you can prove that your new service behaves identically to the legacy system _before_ any traffic shifts, migration becomes a series of low-risk deployments rather than a high-stakes gamble.
...
<Table
  ...
  content={{
    figure: 'Migration approach comparison',
    thead: {
      th: ['Aspect', 'Big-Bang Rewrite', 'Strangler Fig Migration'],
    },
    tbody: {
      tr: [
        {
          th: 'Risk profile',
          td: ['All-or-nothing cutover', 'Incremental, reversible changes'],
        },
        {
          th: 'Validation approach',
          td: ['Testing in isolation', 'Continuous production validation'],
        },
        {
          th: 'Rollback capability',
          td: ['Extremely difficult', 'Built into every step'],
        },
        {
          th: 'Business continuity',
          td: ['Extended freeze periods', 'Continuous feature delivery'],
        },
        {
          th: 'Knowledge transfer',
          td: ['Assumptions about legacy behavior', 'Documented through shadow testing'],
        },
      ],
    },
  }}
...
<Diagram
  ...
  title="Shadow traffic architecture for parallel validation"
  description="A left-to-right flowchart showing shadow traffic during migration validation. An incoming request goes to an API Gateway. The gateway sends the primary request to the Legacy System, which returns the response to the user. The gateway also sends a shadow copy of the request to the New Service. The New Service sends its output to a Comparison Engine, and the Legacy System also sends a copy of its response to that same engine. The Comparison Engine then writes any differences to a Discrepancy Log. The diagram shows that users continue receiving responses only from the legacy system while the new service is validated in parallel."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Phase', 'New Service Traffic', 'Duration', 'Success Criteria'],
    },
    tbody: {
      tr: [
        {
          th: 'Canary',
          td: ['1%', '24 hours', 'Error rate &lt; 0.1%, p99 latency within 10%'],
        },
        {
          th: 'Early adopters',
          td: ['10%', '48 hours', 'No increase in support tickets'],
        },
        {
          th: 'Partial rollout',
          td: ['50%', '72 hours', 'All SLOs maintained'],
        },
        {
          th: 'Majority',
          td: ['90%', '48 hours', 'Business metrics stable'],
        },
        {
          th: 'Complete',
          td: ['100%', 'Ongoing', 'Legacy decommission criteria met'],
        },
      ],
    },
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Error rate spike',
          td: ['&gt;2x baseline for 5 minutes', 'Catches systematic failures quickly'],
        },
        {
          th: 'Latency degradation',
          td: ['p99 &gt;150% baseline for 10 minutes', 'Identifies performance regressions'],
        },
        {
          th: 'Circuit breaker trips',
          td: ['&gt;3 trips in 15 minutes', 'Responds to downstream failures'],
        },
        {
          th: 'Business metric drop',
          td: ['&gt;5% conversion decrease for 30 minutes', 'Catches user-facing impact'],
        },
      ],
    },
    figure: 'Automatic rollback trigger configuration',
  }}
/>
