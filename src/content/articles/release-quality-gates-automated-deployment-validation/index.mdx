...
<Table
  ...
  content={{
    thead: {
      th: ['Category', 'Behavior', 'Criteria', 'Examples'],
    },
    tbody: {
      tr: [
        {
          th: 'Required blocking',
          td: [
            'Must pass; deployment blocked on failure',
            'High precision, detects critical failures, fast execution',
            'Unit tests, build success, CVE 9+ vulnerabilities, auth tests',
          ],
        },
        {
          th: 'Required advisory',
          td: [
            "Must run; failure alerts but doesn't block",
            'Important but may have false positives; trends matter more than absolute values',
            'Integration tests, performance baselines, code coverage',
          ],
        },
        {
          th: 'Optional',
          td: [
            ...
            'Nice-to-have insights, team-specific preferences',
            'Code style beyond linting, documentation coverage, complexity metrics',
          ],
        },
      ],
    },
    figure: 'Gate categories.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Dependency vulnerability scanning',
      text: '(Snyk, Dependabot, npm audit, or similar) checks your dependencies against known vulnerability databases. The key is severity-based thresholds:',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['CVSS Score', 'Severity', 'Gate Behavior'],
    },
    tbody: {
      tr: [
        {
          th: '9.0+',
          td: ['Critical', 'Block deployment — actively exploited or trivially exploitable'],
        },
        {
          th: '7.0-8.9',
          td: ['High', 'Block production, allow staging for assessment'],
        },
        {
          th: '4.0-6.9',
          td: ['Medium', "Advisory — track in backlog, don't block"],
        },
        {
          th: 'Below 4.0',
          td: ['Low', 'Log only — fix opportunistically'],
        },
      ],
    },
    figure: 'Vulnerability severity thresholds.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Health checks',
      text: 'The most basic gate: can the service respond to a health endpoint? Does it report all dependencies as reachable? Is it avoiding crash loops? This gate should be blocking with automatic rollback. If the service can\'t pass a basic health check within 60 seconds, roll back immediately.',
    },
    {
      lead: 'Smoke tests',
      text: 'Runs slightly later, 30-120 seconds post-deploy, and verify that critical paths work: authentication, core API endpoints, essential business flows. Keep the scope small — five to ten operations maximum — so the gate completes quickly. These should also trigger automatic rollback on failure.',
    },
    {
      lead: 'Startup metrics',
      text: 'Watch for resource problems during initialization: OOM kills, CPU pegged at 100%, or an explosion of error logs. These are advisory rather than blocking because some variance is expected during startup, but significant anomalies should alert for investigation.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Error rate',
      text: 'Compares <span class="text-mono">5xx</span> responses between canary and stable traffic. Fail if the canary error rate exceeds the baseline by more than 1 percentage point, or if it exceeds 5% absolute. The observation window should be at least five minutes to gather enough data.',
    },
    {
      lead: 'Latency',
...
<Table
  ...
  content={{
    thead: {
      th: ['Stage', 'Duration', 'Required Gates'],
    },
    tbody: {
      tr: [
        {
          th: '1% canary',
          td: ['5 minutes', 'Health check, error rate, latency'],
        },
        {
          th: '10% canary',
          td: ['10 minutes', 'Error rate, latency, saturation'],
        },
        {
          th: '50% canary',
          td: ['15 minutes', 'Error rate, latency, business metrics'],
        },
        {
          th: 'Full rollout',
          td: ['15 minute bake', 'All metrics stable'],
        },
      ],
    },
    figure: 'Progressive rollout gate stages.',
  }}
/>
...
<Diagram
  src={postDeploymentDiagram}
  size="full"
  title="Post-deployment validation flow."
  description="Sequence diagram with four participants: Deployment, Gate Orchestrator, Prometheus, and Alerting. Deployment begins by sending Start validation with canary deployed to the Gate Orchestrator. A loop then repeats every 30 seconds for 5 minutes. During each iteration, the Gate Orchestrator queries Prometheus for error rate, latency, and availability, and Prometheus returns metric values. The Gate Orchestrator then compares those values to configured thresholds. Inside an alternate failure branch, if any blocking gate fails, the Gate Orchestrator sends FAIL and Rollback to Deployment, sends a Notify on-call message to Alerting, and the Deployment rolls back to the previous version. If the loop completes without triggering a blocking failure, the Gate Orchestrator sends PASS and Promote to Deployment, and the Deployment increases the canary percentage. The diagram shows the observe, evaluate, rollback, or promote cycle used for automated post-deployment quality gates."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Gate Type', 'Timing', 'Blocking', 'Rollback'],
    },
    tbody: {
      tr: [
        {
          th: 'Build',
          td: ['Pre-deploy', 'Yes', 'N/A'],
        },
        {
          th: 'Unit tests',
          td: ['Pre-deploy', 'Yes', 'N/A'],
        ...
        {
          th: 'Security scan',
          td: ['Pre-deploy', 'Conditional', 'N/A'],
        },
        {
          th: 'Health check',
          td: ['Post-deploy (immediate)', 'Yes', 'Automatic'],
        },
        {
          th: 'Error rate',
          td: ['Post-deploy (5 min)', 'Yes', 'Automatic'],
        },
        {
          th: 'Latency',
          td: ['Post-deploy (5 min)', 'Yes', 'Automatic'],
        ...
        {
          th: 'Business metrics',
          td: ['Post-deploy (15 min)', 'Advisory', 'Manual'],
        },
      ],
    },
    figure: 'Gate timing and behavior.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Rate limiting',
      text: 'Set a threshold like three bypasses per team per week. Beyond that, require director approval. This creates friction that encourages fixing the underlying problem rather than repeatedly bypassing.',
    },
    {
      lead: 'Weekly review',
      text: 'Engineering leadership should review all bypasses weekly. Look for patterns: same gate bypassed repeatedly (fix the gate), same team bypassing frequently (investigate why), bypasses followed by incidents (gates were right).',
    },
    {
      lead: 'Automatic escalation',
      text: 'If the same gate is bypassed three or more times, auto-create an issue assigned to the platform team. The gate either has a precision problem or teams don\'t understand what it\'s catching.',
    },
    {
      lead: 'Transparency',
      text: 'Make bypass rates visible on a dashboard accessible to all engineers. Social pressure helps — no one wants to be the team with the highest bypass rate.',
    },
  ]}
/>
...
<Download
  resource="release-quality-gates-automated-deployment-validation"
  title="Release Health Gates Without Blocking Everything"
  description="Automated quality gates that catch real failures without becoming a bottleneck or a source of false positives."
  listItems={[
    {
      text: "Gate categorization decision framework",
    },
    {
      text: "Blocking gate precision checklist",
    },
    {
      text: "Advisory gate tuning playbook",
    },
    {
      text: "Bypass audit trail template",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Design for precision over recall',
      text: 'A gate that blocks one real problem and ten false positives is worse than no gate at all. Engineers will route around it, and trust in the whole system erodes. Better to catch fewer problems with high confidence.',
    },
    {
      lead: 'Categorize ruthlessly',
      text: 'Not every gate belongs in the critical path. Compile failures must block. Security vulnerabilities must block. But a 2% coverage regression? A 50ms latency increase? Those deserve investigation, not pipeline failure. Advisory gates surface the signal without creating friction.',
    },
    {
      lead: 'Combine threshold types',
      text: 'Absolute thresholds catch catastrophic failures (error rate above 5%, latency above 2 seconds). Relative thresholds catch regressions (20% worse than baseline). Used together, they\'re robust against both disaster and slow degradation.',
    },
    {
      lead: 'Automate the hard part',
      text: 'Manual approval gates don\'t scale. Progressive delivery tools like Argo Rollouts and Flagger shift the burden from humans to metrics. Define what "healthy" means, and let automation enforce it.',
    },
    {
