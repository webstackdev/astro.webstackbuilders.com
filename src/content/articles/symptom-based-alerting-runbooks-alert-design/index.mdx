...
<Diagram
  src={theAlertFatigueCycleDiagram}
  magnifyButtonPosition="bottom-right"
  size="full"
  title="The alert fatigue cycle."
  description="Top-to-bottom flowchart showing a reinforcing loop of alert fatigue. The sequence begins with Incident occurs, which leads to Add an alert for that. That causes Alert volume increases, which leads to More false positives. More false positives lead to Team starts ignoring alerts, which then leads to Real incident missed. Missing the real incident triggers the response We need more alerts, which loops back to Add an alert for that. The diagram illustrates how adding alerts after every incident without removing low-value alerts creates a self-reinforcing spiral of noise, distrust, and missed real problems."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Metric', 'Definition', 'Healthy Target', 'Warning Sign'],
    },
    tbody: {
      tr: [
        {
          th: 'Alert volume',
          td: ['Alerts per on-call shift', '< 5 pages/day', '> 10 pages/day'],
        },
        {
          th: 'Actionable rate',
          td: ['% alerts requiring human action', '> 80%', '< 50%'],
        },
        {
          th: 'False positive rate',
          td: ['% alerts with no real impact', '< 10%', '> 30%'],
        },
        {
          th: 'MTTA',
          td: ['Time to acknowledge', '< 5 min', '> 15 min'],
        },
        {
          th: 'MTTR',
          td: ['Time from alert to resolution', '< 30 min', '> 2 hours'],
        },
        {
          th: 'Duplicate rate',
          td: ['% alerts for same incident', '< 10%', '> 25%'],
        },
        {
          th: 'Snooze rate',
          td: ['% alerts snoozed without action', '< 5%', '> 20%'],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      lead: 'High CPU usage',
      text: 'Fires when <span class="mono-text">cpu_usage</span> > 90%. But CPU can spike during legitimate load, garbage collection, or batch jobs without any user impact. The symptom-based alternative-elevated request latency-only fires when users are actually affected. Many different causes lead to latency; one symptom alert catches them all.',
    },
    {
      lead: 'Low disk space',
      text: 'Fires when <span class="mono-text">disk_free</span> < 10%. But this triggers on expected growth, archival systems, and cold storage that\'s supposed to be full. The symptom-based alternative-write failures increasing-only fires when disk space is causing actual failures. Keep disk space as a ticket for business hours, not a 2 AM page.',
    },
    {
...
<Diagram
  src={theAlertingPyramidDiagram}
  size="full"
  title="The alerting pyramid."
  description="Top-to-bottom flowchart organized into three grouped tiers. The first tier, Page Alerts User Impact, contains Error rate greater than SLO burn rate, Latency greater than SLO target, and Availability below threshold. This tier points to User impact NOW with the label Wake someone up. The second tier, Ticket Alerts Degradation, contains CPU sustained high, Disk filling toward capacity, and Connection pool trending hot. This tier points to Potential future impact with the label Fix during business hours. The third tier, Dashboards Awareness, contains All metrics visible, Trends and rates, and Capacity projections. This tier points to No action required with the label Context for investigation. The diagram shows a prioritization model where urgent symptom-based alerts page responders, cause-based degradation alerts become tickets, and the remaining metrics stay visible on dashboards for context."
/>
...
<Table
  ...
  fullWidth={false}
  classes={{
    thead: '[&_th:nth-child(1)]:text-center [&_th:nth-child(2)]:text-center',
    tbody: '[&_th:nth-child(1)]:text-center [&_td:nth-child(2)]:text-center',
  }}
  content={{
    thead: {
      th: ['Burn Rate', 'Budget Exhaustion', 'Appropriate Response'],
    },
    tbody: {
      tr: [
        {
          th: '14.4x',
          td: ['2 hours', 'Page immediately'],
        },
        {
          th: '6x',
          td: ['5 hours', 'Page during waking hours'],
        },
        {
          th: '3x',
          td: ['10 days', 'Create ticket'],
        },
        {
          th: '1x',
          td: ['30 days', 'Dashboard metric only'],
        },
      ],
    },
...
<Table
  ...
  fullWidth={false}
  classes={{
    thead: '[&_th]:text-center',
    tbody: '[&_td]:text-center',
  }}
  content={{
    thead: {
      th: ['Short Window', 'Long Window', 'Burn Rate', 'Severity'],
    },
    tbody: {
      tr: [
        {
          th: { label: '5m', color: 'danger' },
          td: ['1h', '14.4x', 'Critical'],
        },
        {
          th: { label: '30m', color: 'danger' },
          td: ['6h', '6x', 'Critical'],
        },
        {
          th: { label: '2h', color: 'warning' },
          td: ['24h', '3x', 'Warning'],
        },
      ],
    },
    figure: 'Multi-window burn rate configurations.',
  }}
/>
...
<List variant="numbered-with-background-list">
  <ListItem lead="Latency">
    Your primary symptom signal. Alert on P99 latency[^1] exceeding your SLO, not average latency
    (which hides tail issues affecting your worst-served users). Page when P99 exceeds 2x your SLO
    for more than 5 minutes; create a ticket when it exceeds 1.5x for more than 30 minutes.
  </ListItem>
  <ListItem lead="Traffic">
    Unusual — you typically alert on _drops_, not increases. A 50% traffic drop
    compared to predictions suggests an outage that's preventing users from reaching your service.
    High traffic is usually good news; capacity concerns belong on dashboards, not alerts.
  </ListItem>
  <ListItem lead="Errors">
    Should trigger based on burn rate against your error budget, not absolute counts. A single error
    isn't actionable; an error rate that will exhaust your monthly budget in 2 hours is. Use the
    burn rate thresholds from the previous section.
  </ListItem>
  <ListItem lead="Saturation">
    The trickiest signal. High CPU or memory usage alone doesn't warrant a page — many systems
    run hot by design. Alert on saturation only when it's _causing_ symptoms: when
    queue depth correlates with latency spikes, or when memory pressure triggers OOM kills. Keep raw
    saturation metrics on dashboards for diagnosis, not as alert triggers.
  </ListItem>
</List>
...
<Table
  ...
  content={{
    thead: {
      th: ['Signal', 'Alert On', "Don't Alert On", 'Page Threshold', 'Ticket Threshold'],
    },
    tbody: {
      tr: [
        {
          th: 'Latency',
          td: ['P99 exceeding SLO', 'Average latency', '> 2x SLO for 5min', '> 1.5x SLO for 30min'],
        },
        {
          th: 'Traffic',
          td: ['Drops > 50% from prediction', 'High traffic', '< 50% for 10min', '< 70% for 1 hour'],
        },
        {
          th: 'Errors',
          td: ['Burn rate against budget', 'Individual errors', '14x burn rate', '3x burn rate'],
        },
        {
          th: 'Saturation',
          td: ['When causing symptoms', 'High usage alone', 'Correlation with symptoms', 'Disk > 85% growing 1%/day'],
        },
      ],
    },
    figure: 'Golden signals as alert candidates.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Too sensitive:',
      text: 'Alert fires multiple times per day without correlating to incidents. Raise the threshold or extend the for duration.',
    },
    {
      lead: 'Too insensitive:',
      text: 'Incidents occur without the alert firing first. Lower the threshold or shorten the for duration.',
    },
    {
      lead: 'Just right:',
      text: 'More than 80% of alerts correlate with actual user-impacting incidents. Document the threshold and monitor for drift.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Threshold Type', 'Best For', 'Weakness'],
    },
    tbody: {
      tr: [
        {
          th: 'Static',
          td: ['Stable metrics with known good values', "Doesn't adapt to growth"],
        },
        {
          th: 'Stddev-based',
          td: ['Metrics with normal distribution', 'Sensitive to outliers'],
        },
        {
          th: 'Percentile-based',
          td: ['Metrics with skewed distribution', 'Slow to adapt to new baseline'],
        },
        {
          th: 'Time-aware',
          td: ['Metrics with daily/weekly patterns', 'Requires sufficient history'],
        },
        {
          th: 'ML-based',
          td: ['Complex patterns', 'Black box, hard to debug'],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      lead: 'The <span class="text-mono">runbook_url</span> annotation must exist',
    },
    {
      lead: 'The URL must return a 200 (the runbook exists)',
    },
    {
      lead: 'The runbook must have been updated within the last 90 days',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Group related alerts:',
      text: '<span class="font-mono">group_by: [\'alertname\', \'service\']</span> prevents a cascading failure from generating 50 separate pages. One page with context is better than an inbox flood.',
    },
    {
      lead: 'Set appropriate repeat intervals:',
      text: 'Critical alerts should repeat every 15 minutes until acknowledged; warnings can wait 4 hours. Repeating too frequently causes fatigue; too infrequently risks forgotten incidents.',
    },
    {
      lead: 'Include context in notifications:',
      text: "Every page should include the runbook URL and dashboard link. The responder shouldn't have to search for these.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'T+0:',
      text: 'Page the primary on-call engineer',
    },
    {
      lead: 'T+10 min (no acknowledgment):',
      text: 'Page the secondary on-call and the engineering manager',
    },
    {
      lead: 'T+20 min (still no acknowledgment):',
      text: 'Page the engineering leadership team',
    },
    {
      lead: 'T+30 min (still no acknowledgment):',
      text: 'Page the CTO (final level)',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Was it actionable?',
      text: 'Did the alert require human intervention to resolve, or did it auto-recover or require no action?',
    },
    {
      lead: 'Was the runbook helpful?',
      text: 'Did the responder follow it, or did they have to figure things out from scratch?',
    },
    {
      lead: 'Is the threshold still appropriate?',
      text: 'Has the system changed in ways that make the threshold too sensitive or too lax?',
    },
    {
      lead: 'Should this be a page or a ticket?',
      text: 'Some alerts start as pages but should be demoted to tickets once the failure mode is better understood.',
    },
    {
      lead: 'Are there duplicates?',
      text: 'Does another alert fire for the same underlying issue?',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Metrics review (10 min):',
      text: 'Total alerts last month, actionable rate, false positive rate, top 5 noisiest alerts',
    },
    {
      lead: 'Individual alert review (30 min):',
      text: 'Walk through each alert that fired, answer the five questions, decide on disposition',
    },
    {
      lead: 'New alert proposals (10 min):',
      text: 'Review any proposed new alerts, ensure runbooks exist, verify symptom-based design',
    },
    {
      lead: 'Action items (10 min):',
      text: 'Assign threshold modifications, runbook updates, deletions',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Phase', 'Activities', 'Owner'],
    },
    tbody: {
      tr: [
        {
          th: 'Proposal',
          td: ['RFC with justification, runbook draft, threshold analysis', 'Developer'],
        },
        {
          th: 'Review',
          td: ['Team review for symptom-based design, noise assessment', 'Tech Lead'],
        },
        {
          th: 'Testing',
          td: ['Deploy to staging, validate fires correctly', 'Developer'],
        },
        {
          th: 'Deployment',
          td: ['Merge alert rule, publish runbook', 'Developer'],
        },
        {
          th: 'Monitoring',
          td: ['Track actionable rate, false positives', 'On-call team'],
        },
        {
          th: 'Tuning',
          td: ['Adjust thresholds based on data', 'Alert owner'],
        },
        {
          th: 'Review',
          td: ['Quarterly assessment, update or retire', 'Team'],
        },
        {
          th: 'Retirement',
          td: ['Remove alert, archive runbook', 'Alert owner'],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      lead: "Hasn't fired in 6 months:",
      text: 'Either the failure mode no longer occurs, or the threshold is too high to catch it. If the failure mode is still relevant, lower the threshold. If not, delete the alert.',
    },
    {
      lead: 'Less than 50% actionable rate:',
      text: 'The alert fires but does not require action more than half the time. Either raise the threshold, extend the for duration, or convert to a dashboard metric.',
    },
  ]}
/>
...
<Download
  resource="symptom-based-alerting-runbooks-alert-design"
  title="Alert Fatigue: Symptom-Based Alerting That Works"
  description="Designing alerts that wake people up for real problems and include runbooks for resolution."
  listItems={[
    {
      text: "Symptom alert design checklist",
    },
    {
      text: "Burn rate threshold calculator",
    },
    {
      text: "Runbook template starter pack",
    },
    {
      text: "Monthly alert hygiene rubric",
    },
  ]}
/>
