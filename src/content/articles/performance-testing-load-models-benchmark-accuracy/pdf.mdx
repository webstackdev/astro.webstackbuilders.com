...
tags: ["reliability-and-testing", "typescript", "python"]
featured: true
---

import environmentParityDiagram from "./diagrams/environment-parity-strategies-ordered-by-increasing-realism-and-cost.jpg"
import theFourComponentsDiagram from "./diagrams/the-four-components-of-a-complete-load-model.jpg"
...
A load model defines __what__ you're testing. It's the specification that turns "test our API" into something concrete and reproducible. Without a well-defined load model, you're just throwing requests at a server and hoping the numbers mean something.
...
<List
  ...
  items={[
    {
      lead: "Throughput",
      text: "is the simplest dimension — how many requests per second. But even this has nuance. Constant RPS is useful for baseline capacity testing but unrealistic. Ramping load helps find breaking points. Stepped increases identify specific thresholds. Variable patterns that follow time-of-day curves match production reality.",
    },
    {
      lead: "Distribution",
      text: "determines how requests arrive within each second. Uniform distribution spaces requests evenly — one request every 10ms at 100 RPS. Real traffic doesn't work that way. Production traffic follows Poisson distributions with random arrivals, or bursty patterns where requests cluster together. A system that handles 1,000 evenly-spaced RPS might fall over when those same 1,000 requests arrive in bursts of 50.",
    },
    {
      lead: "Workload mix",
      text: "specifies what operations you're testing. Most APIs have a power-law distribution: a few endpoints handle most traffic. E-commerce sites see 80% of requests on product browsing, 15% on cart operations, and 5% on checkout. If your benchmark hits the checkout endpoint at 100% weight, you're testing a scenario that never happens.",
    },
    {
      lead: "User behavior",
      text: "is often overlooked but critical. Real users have think time — they read content, fill out forms, compare options. A benchmark that fires requests as fast as possible tests maximum theoretical throughput, not realistic capacity. Think time also affects connection pooling, session state, and resource contention.",
    },
  ]}
/>

<Diagram
  src={theFourComponentsDiagram}
  title="The four components of a complete load model."
  description="Left-to-right flowchart showing four inputs that combine into a realistic load model. Throughput, Distribution, Workload Mix, and User Behavior each feed into a central node labeled Realistic Load. That central node then flows into Accurate Predictions. The diagram communicates that no single metric defines a useful benchmark: accurate performance predictions come only when request rate, arrival distribution, workload composition, and human interaction patterns are modeled together."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Pattern', 'Description', 'When to Use'],
    },
    tbody: {
      tr: [
        {
          th: 'Constant',
          td: ['Fixed RPS for duration', 'Baseline capacity measurement'],
        },
        {
          th: 'Ramping',
          td: ['Gradual increase/decrease', 'Find breaking points and saturation'],
        },
        {
          th: 'Stepped',
          td: ['Discrete jumps', 'Identify specific capacity thresholds'],
        },
        {
          th: 'Spike',
          td: ['Sudden burst', 'Test auto-scaling response time'],
        },
        {
          th: 'Soak',
          td: ['Long duration at steady load', 'Memory leaks, connection exhaustion, degradation'],
        },
      ],
    },
    figure: 'Load patterns and their use cases.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Metric', 'What It Tells You', 'Watch For'],
    },
    tbody: {
      tr: [
        {
          th: 'P50 (Median)',
          td: ['Typical user experience', 'Large gap between mean and median indicates skew'],
        },
        {
          th: 'P95',
          td: ["Most users' worst case", 'Common SLA compliance target'],
        },
        {
          th: 'P99',
          td: ['Tail latency', '1 in 100 requests; often where problems hide'],
        },
        {
          th: 'Std Dev',
          td: ['Consistency', 'High variance suggests system instability'],
        },
        {
          th: 'CV',
          td: ['Relative spread', 'Above 0.2 indicates concerning variability'],
        },
        {
          th: 'Throughput',
          td: ['Achieved capacity', 'Compare against target; watch for saturation'],
        },
      ],
    },
    figure: 'Key performance metrics and interpretation.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Factor', 'Test Environment Difference', 'Impact on Results'],
    },
    tbody: {
      tr: [
        {
          th: 'Database size',
          td: ['1% of production', '10x faster queries (all in index/cache)'],
        },
        {
          th: 'Instance type',
          td: ['Half the CPU/memory', 'Earlier saturation, different bottlenecks'],
        },
        {
          th: 'Mocked APIs',
          td: ['Instant responses', '2-3x higher apparent throughput'],
        },
        {
          th: 'Cold caches',
          td: ['Empty at test start', 'Warmup not representative'],
        },
        {
          th: 'Network',
          td: ['Same-zone or localhost', 'Missing 1-5ms latency per hop'],
        },
      ],
    },
    figure: 'Common environment differences and their impact.',
  }}
/>
...
<Diagram
  src={environmentParityDiagram}
  title="Environment parity strategies ordered by increasing realism and cost."
  description="Top-to-bottom flowchart inside a grouped section labeled Choose Based on Workload. Four strategy nodes feed into a single decision node labeled Cost vs Accuracy. Infrastructure as Code connects with the note Identical configs. Scaled-Down Production connects with the note Proportional load. Production Traffic Replay connects with the note Real patterns. Shadow Traffic connects with the note Live mirroring. The diagram shows that different environment parity strategies trade off realism against cost, and the appropriate choice depends on the workload characteristics and how much accuracy is required."
/>
...
<List
  ...
  items={[
    {
      lead: "Data volume",
      text: "Should be the same order of magnitude as production. A 100GB test database won't reveal the problems you'll hit with 10TB in production. Query planners make different decisions, indexes have different depths, and data that fits in memory in test requires disk I/O in production.",
    },
    {
      lead: "Data distribution",
      text: "Matters as much as volume. If production has 80% of orders from 5% of customers, your test data should have similar skew. Uniform test data produces uniform query plans that don't match production behavior.",
    },
    {
      lead: "Index statistics",
      text: "Must be current. After loading test data, run `ANALYZE` (PostgreSQL) or equivalent to update statistics. Without this, the query planner works from stale information and may choose suboptimal plans.",
    },
    {
      lead: "Buffer pool warmth",
      text: "Affects every query. A cold database hits disk; a warm one serves from memory. Pre-warm the buffer pool by running representative queries before starting measurement. Some databases have explicit warmup features (`pg_prewarm` in PostgreSQL).",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Issue', 'Symptom', 'Fix'],
    },
    tbody: {
      tr: [
        {
          th: 'Coordinated omission',
          td: ['Load drops under stress', 'Open-loop generation (arrival-rate executor)'],
        },
        {
          th: 'Client bottleneck',
          td: ['Client CPU at 100%', 'Distributed load generation'],
        },
        {
          th: 'Single run variance',
          td: ['Results differ 20% between runs', 'Multiple runs with statistical analysis'],
        },
        {
          th: 'Short test duration',
          td: ['No GC patterns, cold caches', 'Longer tests (30+ minutes)'],
        },
        {
          th: 'Wrong percentiles',
          td: ['P99 from averaged windows', 'Calculate once from all raw data'],
        },
      ],
    },
    figure: 'Common pitfalls and their fixes.',
  }}
/>
