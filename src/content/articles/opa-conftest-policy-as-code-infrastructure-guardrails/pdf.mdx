...
import conftestDiagram from "./diagrams/conftest-evaluation-flow.png"
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Required field',
          td: ['Ensure configuration exists', '`not input.spec.selector`'],
        },
        {
          th: 'Allowed values',
          td: ['Whitelist valid options', '`input.type in {"ClusterIP", "NodePort"}`'],
        },
        {
          th: 'Regex matching',
          td: ['Pattern validation', '`regex.match("^[a-z][a-z0-9-]*$", name)`'],
        },
        {
          th: 'Numeric constraints',
          td: ['Range validation', '`input.replicas > 0; input.replicas <= 10`'],
        },
        {
          th: 'Cross-resource',
          td: ['Reference related objects', '`data.namespaces[input.metadata.namespace]`'],
        },
        {
          th: 'Conditional',
          td: ['Environment-specific rules', '`is_production; ... production-only rule ...`'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: '"Policy violation" tells the developer nothing. "Container missing resource limits" is better. "Container \'nginx\' missing resource limits. Add `spec.containers[].resources.limits.cpu` and `memory`" is what they actually need. Include the resource name, the violation, and the fix path.',
    },
    {
...
<List
  ...
  items={[
    {
      text: "`Deployment 'api-server': Missing required label 'team'. Fix: Add metadata.labels.team with your team name.`",
    },
    {
      text: "`Pod 'worker': Container 'app' uses image from unauthorized registry 'docker.io'. Fix: Use images from 'gcr.io/company-project' or 'artifactory.company.com'.`",
    },
  ]}
/>
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Python',
          td: ['pre-commit', '`pip install pre-commit && pre-commit install`'],
        },
        {
          th: 'Node.js',
          td: ['Husky + lint-staged', '`npx husky init` (works for any project with package.json)'],
        },
        {
          th: 'Ruby/Rails',
          td: ['Overcommit', '`gem install overcommit && overcommit --install`'],
        },
        {
          th: 'PHP/Laravel',
          td: ['GrumPHP', '`composer require --dev phpro/grumphp`'],
        },
        {
          th: '.NET/C#',
          td: ['Husky.Net', '`dotnet tool install husky`'],
        },
        {
          th: 'Go',
          td: ['pre-commit or lefthook', '`go install github.com/evilmartians/lefthook`'],
        },
      ],
    },
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Speed',
          td: ['Fast (< 1 second)', 'Slow (requires `terraform plan`)'],
        },
        {
          th: 'Credentials',
...
<List
  ...
  items={[
    {
      text: 'Developer adds exception annotation with reason',
    },
    {
      text: 'PR triggers policy check — warns about exception usage',
    },
    {
      text: 'Security team reviews exception request',
    },
    {
      text: 'If approved, merge with exception and expiration date',
    },
    {
      text: 'Exception logged in audit system',
    },
  ]}
/>
