...
tags: ["cloud-platforms", "terraform", "kubernetes", "aws", "docker"]
featured: true
---
...
<List
  ...
  items={[
    {
      lead: 'Library mode',
      text: 'Embed OPA directly in applications for authorization decisions. Go applications can import OPA as a package; other languages use WASM or REST APIs.',
    },
    {
      lead: 'Daemon mode',
    ...
    {
      lead: 'CLI mode',
      text: 'Evaluate policies from the command line. This is where Conftest enters: it wraps OPA\'s CLI with ergonomic defaults for infrastructure files.',
    },
  ]}
/>
...
<Diagram
  src={conftestDiagram}
  title="Conftest evaluation flow."
  description="Left-to-right flowchart with three grouped sections. The Input section contains Terraform HCL, Kubernetes YAML, and Dockerfile as separate sources. All three inputs flow into a Parser node inside the Conftest section. The Conftest section also contains OPA Engine, Rego Policies, and External Data. The Parser sends normalized input into the OPA Engine, while Rego Policies and External Data also feed into the engine as policy logic and contextual data. The Output section contains three results: Pass or Fail, Violation Messages, and JSON Report. The OPA Engine sends results to all three outputs. The diagram shows that Conftest converts infrastructure files into structured data, evaluates them through OPA using Rego policies plus optional external context, and then emits human-readable and machine-readable results."
/>
...
<Table
  ...
  content={{
    figure: 'Common Rego validation patterns and representative examples.',
    thead: {
      th: ['Pattern', 'Use Case', 'Example'],
    },
    tbody: {
      tr: [
        {
          th: 'Required field',
          td: ['Ensure configuration exists', '<span class="font-mono">not input.spec.selector</span>'],
        },
        {
          th: 'Allowed values',
          td: ['Whitelist valid options', '<span class="font-mono">input.type in {"ClusterIP", "NodePort"}</span>'],
        },
        {
          th: 'Regex matching',
          td: ['Pattern validation', '<span class="font-mono">regex.match("^[a-z][a-z0-9-]*$", name)</span>'],
        },
        {
          th: 'Numeric constraints',
          td: ['Range validation', '<span class="font-mono">input.replicas > 0; input.replicas <= 10</span>'],
        },
        {
          th: 'Cross-resource',
          td: ['Reference related objects', '<span class="font-mono">data.namespaces[input.metadata.namespace]</span>'],
        },
        {
          th: 'Conditional',
          td: ['Environment-specific rules', '<span class="font-mono">is_production; ... production-only rule ...</span>'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Single responsibility.',
      text: 'Each rule should check one thing. A 50-line rule that validates images, resources, and labels produces confusing violations and resists modification. Split concerns into separate rules: <span class="font-mono">deny_unauthorized_image</span>, <span class="font-mono">deny_missing_resources</span>, <span class="font-mono">deny_missing_labels</span>. When a violation fires, developers know exactly what to fix.',
    },
    {
      lead: 'Actionable messages.',
      text: '"Policy violation" tells the developer nothing. "Container missing resource limits" is better. "Container \'nginx\' missing resource limits. Add <span class="font-mono">spec.containers[].resources.limits.cpu</span> and <span class="font-mono">memory</span>" is what they actually need. Include the resource name, the violation, and the fix path.',
    },
    {
...
<List
  ...
  classes={{
    wrapper: "sm:ml-6 sm:mr-12 mb-6",
    em: "!font-normal !text-content text-base",
  }}
  items={[
    {
      lead: '<span class="font-mono">Deployment \'api-server\': Missing required label \'team\'. Fix: Add metadata.labels.team with your team name.</span>',
      color: "border-danger",
    },
    {
      lead: '<span class="font-mono">Pod \'worker\': Container \'app\' uses image from unauthorized registry \'docker.io\'. Fix: Use images from \'gcr.io/company-project\' or \'artifactory.company.com\'.</span>',
      color: "border-danger",
    },
  ]}
/>
...
<FileExplorer
  figure="Policy directory structure."
  data={[
    {
      type: 'folder',
      name: 'policies',
      children: [
          ...
          children: [
            {
              ...
              name: 'pods',
              children: [
                { type: 'file', name: 'privileged.rego', comment: 'Blocks privileged pod settings' },
                { type: 'file', name: 'resources.rego', comment: 'Checks pod resource requirements' },
                { type: 'file', name: 'images.rego', comment: 'Restricts approved image registries' },
              ],
            },
            {
              type: 'folder',
              name: 'deployments',
              children: [
                { type: 'file', name: 'replicas.rego', comment: 'Validates deployment replica counts' },
                { type: 'file', name: 'labels.rego', comment: 'Requires deployment metadata labels' },
              ],
            },
            {
              type: 'folder',
              name: 'services',
              children: [
                { type: 'file', name: 'types.rego', comment: 'Restricts allowed service types' },
              ],
            },
            {
              type: 'folder',
              name: 'common',
              children: [
                { type: 'file', name: 'labels.rego', comment: 'Shared label validation rules' },
                { type: 'file', name: 'helpers.rego', comment: 'Shared helpers for policy reuse' },
              ],
            },
          ],
        },
          ...
          children: [
            {
              ...
              name: 'aws',
              children: [
                { type: 'file', name: 's3.rego', comment: 'Checks S3 bucket guardrails' },
                { type: 'file', name: 'iam.rego', comment: 'Checks unsafe IAM permissions' },
                { type: 'file', name: 'security_groups.rego', comment: 'Checks security group exposure' },
              ],
            },
            {
              type: 'folder',
              name: 'common',
              children: [
                { type: 'file', name: 'tags.rego', comment: 'Requires standard resource tags' },
              ],
            },
          ],
        },
        {
          ...
          name: 'docker',
          children: [
            { type: 'file', name: 'base_images.rego', comment: 'Restricts approved base images' },
            { type: 'file', name: 'best_practices.rego', comment: 'Checks Dockerfile best practices' },
          ],
        },
        {
          type: 'folder',
          name: 'data',
          children: [
            { type: 'file', name: 'allowed_registries.json', comment: 'Approved container registry allowlist' },
            { type: 'file', name: 'required_labels.json', comment: 'Required metadata label definitions' },
            { type: 'file', name: 'team_mappings.json', comment: 'Maps teams to ownership data' },
          ],
        },
        {
          type: 'folder',
          name: 'lib',
          children: [
            { type: 'file', name: 'helpers.rego', comment: 'Reusable utility policy functions' },
            { type: 'file', name: 'constants.rego', comment: 'Shared policy constants' },
          ],
        },
      ],
    },
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Pre-commit tools by ecosystem.',
    thead: {
      th: ['Ecosystem', 'Pre-commit Tool', 'Installation'],
    },
    tbody: {
      tr: [
        {
          th: 'Python',
          td: ['pre-commit', '<span class="font-mono">pip install pre-commit && pre-commit install</span>'],
        },
        {
          th: 'Node.js',
          td: ['Husky + lint-staged', '<span class="font-mono">npx husky init</span>'],
        },
        {
          th: 'Ruby/Rails',
          td: ['Overcommit', '<span class="font-mono">gem install overcommit && overcommit --install</span>'],
        },
        {
          th: 'PHP/Laravel',
          td: ['GrumPHP', '<span class="font-mono">composer require --dev phpro/grumphp</span>'],
        },
        {
          th: '.NET/C#',
          td: ['Husky.Net', '<span class="font-mono">dotnet tool install husky</span>'],
        },
        {
          th: 'Go',
          td: ['pre-commit or lefthook', '<span class="font-mono">go install github.com/evilmartians/lefthook</span>'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    thead: {
      th: ['Platform', 'Protection Mechanism', 'Configuration Location'],
    },
    tbody: {
      tr: [
        {
          th: 'GitHub',
          td: ['Branch protection rules', 'Settings → Branches → Add rule'],
        },
        {
          th: 'GitLab',
          td: ['Protected branches + merge request approvals', 'Settings → Repository → Protected branches'],
        },
        {
          th: 'Bitbucket',
          td: ['Branch permissions + merge checks', 'Repository settings → Branch permissions'],
        },
        {
          th: 'Azure DevOps',
          td: ['Branch policies', 'Repos → Branches → Branch policies'],
        },
        {
          th: 'AWS CodePipeline',
          td: ['Approval actions + CodeBuild gates', 'Pipeline definition (no native branch protection)'],
        },
      ],
    },
    figure: 'Branch protection across CI platforms.',
  }}
/>
...
<Table
  ...
  fullWidth={false}
  classes={{
    tbody: '[&_td:nth-child(2)]:text-center [&_td:nth-child(3)]:text-center [&_td:nth-child(4)]:text-center',
  }}
  content={{
    thead: {
      th: ['Optimization', 'Speedup', 'Example (50 policies, 100 files)', 'Complexity'],
    },
    tbody: {
      tr: [
        {
          th: 'Namespace filtering',
          td: ['2-4x', '8s → 2s', 'Low'],
        },
        {
          th: 'Parallel execution',
          td: ['2-4x', '8s → 2s', 'Low'],
        },
        {
          th: 'Policy bundling',
          td: ['1.5-2x', '8s → 5s', 'Medium'],
        },
        {
          th: 'Selective file patterns',
          td: ['2-5x', '8s → 2s', 'Low'],
        },
        {
          th: 'Combined',
          td: ['5-15x', '8s → 0.6s', 'Medium'],
        },
      ],
    },
    figure: 'Performance optimization impact.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Aspect', 'HCL Parsing', 'Plan JSON'],
    },
    tbody: {
      tr: [
        {
          th: 'Speed',
          td: ['Fast (< 1 second)', 'Slow (requires <span class="font-mono">terraform plan</span>)'],
        },
        {
          th: 'Credentials',
          td: ['None required', 'Cloud credentials needed'],
        },
        {
          th: 'Variable resolution',
          td: ['No', 'Yes'],
        },
        {
          th: 'Module expansion',
        ...
        {
          th: 'Data source values',
          td: ['No', 'Yes'],
        },
      ],
    },
    figure: 'HCL vs Plan JSON comparison.',
  }}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Developer machine',
          td: ['HCL static analysis', '< 5 seconds'],
        },
        {
          th: 'PR opened',
          td: ['HCL + basic plan', '< 2 minutes'],
        },
        {
          th: 'PR approved',
          td: ['Full plan analysis', '< 5 minutes'],
        },
      ],
    },
    figure: 'Layered evaluation stages.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Policy Type', 'Production', 'Non-Production'],
    },
    tbody: {
      tr: [
        {
          th: 'Security (privileged, root)',
          td: ['deny', 'deny'],
        },
        {
          th: 'Resource limits',
        ...
        {
          th: 'Naming conventions',
          td: ['warn', 'info'],
        },
      ],
    },
    figure: 'Severity mapping by environment.',
  }}
/>
...
__Annotation-based exceptions__ let developers request exceptions inline with the resource. The policy checks for an exception annotation before denying:
...
__Expiring exceptions__ prevent permanent workarounds. The policy parses an expiration date and blocks if the exception has lapsed:
...
__Data-driven exceptions__ centralize exception management. Instead of annotations scattered across manifests, a JSON file defines which resources are excepted:
...
<List
  ...
  items={[
    {
      lead: 'Developer adds exception annotation with reason',
    },
    {
      lead: 'PR triggers policy check — warns about exception usage',
    },
    {
      lead: 'Security team reviews exception request',
    },
    {
      lead: 'If approved, merge with exception and expiration date',
    },
    {
      lead: 'Exception logged in audit system',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Exception Type', 'Max Duration', 'Review Frequency', 'Approval Required'],
    },
    tbody: {
      tr: [
        {
          th: 'Privileged container',
          td: ['30 days', 'Weekly', 'Security team'],
        },
        {
          th: 'Host network',
          td: ['30 days', 'Weekly', 'Security team'],
        ...
        {
          th: 'Missing limits',
          td: ['90 days', 'Monthly', 'Team lead'],
        },
        {
          th: 'Non-standard image',
          td: ['90 days', 'Monthly', 'Team lead'],
        },
        {
          th: 'Deprecated API',
          td: ['180 days', 'Quarterly', 'Platform team'],
        },
      ],
    },
    figure: 'Exception governance by type.',
  }}
/>
...
<Download
  resource="opa-conftest-policy-as-code-infrastructure-guardrails"
  title="Policy as Code: OPA Guardrails With Fast Feedback"
  description="Implementing infrastructure policies with OPA and Conftest that catch violations before they reach production."
  listItems={[
    {
      text: "Rego policy starter patterns",
    },
    {
      text: "Pre-commit hook setup guide",
    },
    {
      text: "Terraform guardrail rule pack",
    },
    {
      text: "Policy rollout governance checklist",
    },
  ]}
/>
