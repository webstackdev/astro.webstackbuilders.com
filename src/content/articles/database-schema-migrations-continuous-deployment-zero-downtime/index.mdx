...
<List
  ...
  items={[
    {
      lead: 'Database acquires ACCESS EXCLUSIVE lock on <span class="font-mono">users</span>',
    },
    {
      lead: "Database rewrites entire table to add the column with default value",
    },
    {
      lead: "Database rebuilds indexes",
    },
    {
      lead: "Lock releases",
    },
...
<Diagram
  ...
  description="Flowchart showing the progression of a blocking migration and its impact on the application. The main path runs from Migration Starts to Acquire Table Lock, then to Rewrite Table Data, Rebuild Indexes, and Release Lock. A separate grouped region labeled Blocked Application Threads contains four representative threads: Thread 1: Waiting..., Thread 2: Waiting..., Thread 3: Timeout, and Thread 4: Timeout. A dotted arrow labeled blocks points from Acquire Table Lock to that blocked-thread group. The diagram shows that once the table lock is acquired, normal application work stalls behind the migration, some requests remain queued, and others eventually fail before the lock is released."
/>
...
<Diagram
  src={compatibilityWindowDiagram}
  title="Compatibility window during rolling deployment."
  description="Sequence diagram showing how old and new application versions interact with the same database during a rolling deployment. The participants are Database, Old Code (v1), and New Code (v2). A note indicates that the rolling deployment starts while both versions are running. Old Code queries the database for email, while New Code queries for both email and phone. Another note explains that if the phone column does not exist yet, the new code fails with an error stating that the phone column does not exist. A later note shows the opposite compatibility problem: if the phone column has been added while old code is still running, old code inserts only email, leaving phone as NULL and raising the question of whether that state is acceptable. The diagram illustrates why schema changes must remain compatible with both versions during the deployment overlap."
/>
...
<List
  ...
  items={[
    {
      text: "Add column after deploying new code: new code errors because column doesn't exist",
      color: "bg-danger",
    },
    {
      text: "Add column_before_old code terminates: old code might fail if it doesn't expect the column",
      color: "bg-danger",
    },
    {
      text: "Remove column while old code still runs: old code errors immediately",
      color: "bg-danger",
    },
  ]}
/>
...
<Table
  ...
  content={{
    ...
    thead: { th: ["Failure Mode", "Cause", "Symptom", "Prevention"] },
    tbody: {
      tr: [
        {
          ...
          td: [
            ...
            "Non-locking DDL tools (gh-ost, pt-osc)",
          ],
        },
        {
          th: "Column not found",
...
<List
  ...
  items={[
    ...
    {
      lead: "Contract:",
      text: "Remove the old structure. Deploy code that only uses the new structure, then drop the old column/table.",
    },
  ]}
/>
...
<Diagram
  ...
  description="Flowchart showing the three phases of a safe schema migration. In Phase 1: Expand, Add new column leads to Deploy code that writes to both, then to Backfill existing data. The flow then continues into Phase 2: Migrate, where All data in new column leads to Deploy code that reads from new, then to Stop writing to old column. From there the process moves into Phase 3: Contract, where Old column unused leads to Remove old column. Connecting arrows between the phases show that each stage must complete before the next begins. The diagram communicates that a breaking schema change is decomposed into compatible steps rather than performed as one immediate rename or drop."
/>
...
<List
  ...
  items={[
    {
      lead: "Week 1 (Expand):",
      text: 'Run a migration to add the <span class="font-mono">emailaddress</span> column. Deploy code that writes to both columns on every insert and update, but still reads from <span class="font-mono">email</span>. At this point, new rows have both columns populated; old rows only have <span class="font-mono">email</span>.',
    },
    {
      lead: "Week 2 (Migrate):",
      text:'Run a backfill job to copy <span class="font-mono">email</span> values to <span class="font-mono">email_address</span> for existing rows. Once complete, deploy code that reads from <span class="font-mono">email_address</span> instead of <span class="font-mono">email</span>. Keep writing to both columns — old pods might still be running.',
    },
    {
      lead: "Week 3 (Contract):",
      text: 'After all old pods are gone and you\'ve verified nothing reads <span class="font-mono">email</span>, deploy code that only uses <span class="font-mono">email_address</span>. Then run a migration to drop the <span class="font-mono">email</span> column.',
    },
  ]}
/>
...
-- ❌️ DANGEROUS: Direct drop while code might use it
...
-- ❌️ DANGEROUS: Direct type change
...
-- ❌️ DANGEROUS: Standard index creation
...
<Table
  ...
  content={{
    figure: "Native PostgreSQL DDL operations and external tool requirements",
    thead: { th: ["Operation", "PostgreSQL v11+ Behavior", "External Tool Needed?"] },
    tbody: {
      tr: [
        {
          th: 'Add <span class="font-mono">nullable</span> column',
        ...
        {
          th: "Create index",
          td: ['<span class="font-mono">CONCURRENTLY</span> option available', "No"],
        },
        {
          th: "Add foreign key",
          td: ['<span class="font-mono">NOT VALID</span> then <span class="font-mono">VALIDATE</span>', "No"],
        },
        {
        ...
        {
          th: 'Add <span class="font-mono">NOT NULL</span> to existing column',
...
<Table
  ...
  content={{
    figure: "Comparison of external MySQL schema migration tools",
    thead: { th: ["Tool", "Vendor", "Approach", "Key Advantage", "Key Limitation"] },
    tbody: {
      tr: [
        {
          th: "pt-online-schema-change",
          td: [
            "Percona",
            "Shadow table + triggers",
            "Battle-tested, works with replication",
        ...
        },
        {
          ...
          td: [
            "GitHub",
            "Shadow table + binlog streaming",
            "No triggers, pausable/resumable",
...
<List
  ...
  items={[
    {
      lead: "Create an empty shadow table with the new schema",
      color: "border-secondary",
    },
    {
      lead: "Copy existing rows in chunks (throttled to avoid overloading the database)",
      color: "border-accent",
    },
    {
      lead: "Capture ongoing writes (via triggers or binlog) and replay them to the shadow table",
      color: "border-success-offset",
    },
    {
      lead: "Once caught up, atomically rename tables to swap old and new",
      color: "border-warning-offset",
    },
  ]}
/>
...
<Diagram
  src={migrationAwareDiagram}
  size="full"
  title="Migration-aware deployment pipeline."
  description="Flowchart showing how a CI/CD pipeline treats database migrations differently from normal code deployments. The flow starts at Code Push and asks whether the change has migrations. If no, it goes to Normal Deploy Pipeline. If yes, it goes to Migration Safety Check, then asks whether the migration is safe. Safe migrations go to Run Migration, while unsafe ones go to Human Review Required, then Manual Approval, and back to Run Migration. After migration execution, the pipeline asks whether the migration succeeded. Success leads to Deploy Application and then Health Check. Failure leads to Rollback Migration and then Alert Team. The diagram emphasizes that migrations introduce separate review, execution, and rollback paths before application deployment continues."
/>
...
def main() -> int:
  ...
  if needs_review:
    print("❌️ Migration requires human review before deployment")
    return 1
...
<List
  ...
  items={[
    {
      lead: "Reversible migrations",
      text: 'Have straightforward inverses: <span class="font-mono">ADD COLUMN</span> reverses with <span class="font-mono">DROP COLUMN</span>, <span class="font-mono">CREATE INDEX</span> reverses with <span class="font-mono">DROP INDEX</span>, and <span class="font-mono">ADD CONSTRAINT</span> reverses with <span class="font-mono">DROP CONSTRAINT</span>. Most migration frameworks generate these down migrations automatically. When something goes wrong, you run a rollback command with the project\'s CLI tool and you\'re back to the previous state.',
    },
    {
      lead: "Irreversible migrations",
      text: 'The dangerous ones. Once you <span class="font-mono">DROP COLUMN</span>, that data is gone. Once you <span class="font-mono">TRUNCATE</span>, there\'s no undo. For these operations, your safety net is backups — take one immediately before the migration, verify it\'s restorable, and document the restoration steps. If you\'re running a backfill that transforms data, keep the original data until you\'ve verified the migration succeeded.',
    },
    {
      lead: "Forward fixes",
      text: "Your last resort when rollback isn't an option. Maybe the migration already ran in production. Maybe rolling back would lose data you can't afford to lose. Maybe the rollback would take longer than fixing the problem. In these cases, you deploy code that handles both the old and new schema states, then run a corrective migration to get to a clean state. It's messy, but sometimes it's the only path forward.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Schema-diff tools (Prisma, Entity Framework, Django)",
      text: 'These tools compare your model definitions against the current database state and generate migrations automatically. This is convenient for rapid development but requires careful review: the generated SQL might not follow safe migration patterns. A model change that looks like a rename might generate a destructive <span class="font-mono">DROP COLUMN</span> followed by <span class="font-mono">ADD COLUMN</span> instead of the expand-contract approach.',
    },
    {
      lead: "SQL-first tools (Flyway, Knex, raw migration files)",
      text: "These tools put you in control of the exact SQL that runs. You write the migration manually, which means more work but also more predictability. There's no magic diff algorithm making decisions about your production database.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: "Common migration CLI patterns across frameworks.",
    thead: { th: ["Framework", "Generate", "Apply", "Rollback", "Notes"] },
    tbody: {
      tr: [
        {
          th: "Rails",
          td: [
            '<span class="font-mono">rails generate migration AddPhone</span>',
            '<span class="font-mono">rails db:migrate</span>',
            '<span class="font-mono">rails db:rollback</span>',
            "Wraps each migration in transaction, auto-rollback",
          ],
        },
        {
          th: "Django",
          td: [
            '<span class="font-mono">python manage.py makemigrations</span>',
            '<span class="font-mono">python manage.py migrate</span>',
            '<span class="font-mono">python manage.py migrate app 0001</span>',
            "Atomic by default, good dependency tracking",
          ],
        },
        {
          th: "Laravel",
          td: [
            '<span class="font-mono">php artisan make:migration add_phone</span>',
            '<span class="font-mono">php artisan migrate</span>',
            '<span class="font-mono">php artisan migrate:rollback</span>',
            "Transaction per migration, clean rollback",
          ],
        },
        {
          th: ".NET EF Core",
          td: [
            '<span class="font-mono">PS> Add-Migration AddPhone</span>',
            '<span class="font-mono">PS> Update-Database</span>',
            '<span class="font-mono">PS> Update-Database PreviousMigration</span>',
            "No transaction by default, manual control",
          ],
        },
        {
          th: "RedwoodJS",
          td: [
            '<span class="font-mono">yarn rw prisma migrate dev</span>',
            '<span class="font-mono">yarn rw prisma migrate deploy</span>',
            "Manual",
            "Uses Prisma; no auto-rollback generation",
          ],
        },
        {
          th: "Knex.js",
          td: [
            '<span class="font-mono">npx knex migrate:make add_phone</span>',
            '<span class="font-mono">npx knex migrate:latest</span>',
            '<span class="font-mono">npx knex migrate:rollback</span>',
            "Transaction optional, SQL-first control",
          ],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      lead: "Staging verification:",
      text: "The migration ran successfully in staging with production-like data volumes. The application works with the new schema. You've tested the rollback path.",
    },
    {
      lead: "Database health:",
      text: "No long-running transactions that might conflict with your migration. Replication lag is minimal. Sufficient disk space exists (shadow-table tools like gh-ost temporarily double table size). A recent backup exists and you've verified it's restorable.",
    },
    {
      lead: "Timing and staffing:",
      text: "You're not deploying during peak traffic. Team members are available to monitor the migration and respond to problems. No other deployments are in progress that might confuse incident response.",
    },
    {
      lead: "Rollback readiness:",
      text: "The rollback script is tested. You know how long rollback will take. You've defined clear criteria for when to rollback versus wait it out.",
    },
  ]}
/>
