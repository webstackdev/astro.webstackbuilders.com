...
<List
  ...
  items={[
    { lead: "_Week 1 (Expand)_:", text: "Run a migration to add the `email_address` column. Deploy code that writes to both columns on every insert and update, but still reads from `email`. At this point, new rows have both columns populated; old rows only have `email`." },
    { lead: "_Week 2 (Migrate)_:", text: "Run a backfill job to copy `email` values to `email_address` for existing rows. Once complete, deploy code that reads from `email_address` instead of `email`. Keep writing to both columns—old pods might still be running." },
    { lead: "_Week 3 (Contract)_:", text: "After all old pods are gone and you've verified nothing reads `email`, deploy code that only uses `email_address`. Then run a migration to drop the `email` column." },
  ]}
/>
