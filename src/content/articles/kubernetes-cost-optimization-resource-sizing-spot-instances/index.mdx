...
tags: ["cloud-platforms","kubernetes","prometheus","aws","azure"]
featured: true
---
...
<Table
  ...
  content={{
    thead: {
      th: ['Aspect', 'Request', 'Limit'],
    },
    tbody: {
      tr: [
        {
          th: 'Scheduling',
          td: ['Used for placement decisions', 'Not considered'],
        },
        {
          th: 'CPU behavior',
          td: ['Guaranteed minimum', 'Throttled if exceeded'],
        },
        {
          th: 'Memory behavior',
          td: ['Guaranteed minimum', 'OOM killed if exceeded'],
        },
        {
          th: 'Cluster capacity',
          td: ['Sum of requests = schedulable capacity', 'Sum of limits can exceed node capacity'],
        },
        {
          th: 'Cost impact',
          td: ['Directly determines node count', 'Indirectly affects density'],
        },
      ],
    },
    figure: 'Requests vs limits comparison.',
  }}
/>
...
Spot works well for __stateless workloads__ (web servers, API servers, workers), __batch processing__ (data pipelines, ML training), __development environments__, and __CI/CD runners__. It's a poor fit for __databases__, __stateful services without replication__, __long-running jobs that can't checkpoint__, and __single-replica critical services__. The rule of thumb: if your workload can survive losing a node at any moment, it's a spot candidate.
...
A workload needs three things to run safely on spot: __multiple replicas__ spread across availability zones, __tolerations__ for the spot taint, and __graceful shutdown handling__ to drain cleanly when interrupted.
...
<Table
  ...
  content={{
    thead: {
      th: ['Provider', 'Warning Time', 'Detection Method', 'Handler'],
    },
    tbody: {
      tr: [
        {
          th: 'AWS',
          td: ['2 minutes', 'IMDS + EventBridge', 'aws-node-termination-handler'],
        },
        {
          th: 'GCP',
          td: ['30 seconds', 'Metadata server', 'Built-in GKE handling'],
        },
        {
          th: 'Azure',
          td: ['30 seconds', 'Scheduled Events API', 'Node drain handler'],
        },
      ],
    },
    figure: 'Spot interruption handling by provider.',
  }}
/>
...
The most impactful cost dashboard shows teams their __waste__, not their spend. "You spent $5,000" doesn't tell anyone what to do differently. "You're using 30% of what you requested" does.
...
<Table
  ...
  content={{
    thead: {
      th: ['Setting', 'Cost Optimization', 'Stability', 'Recommendation'],
    },
    tbody: {
      tr: [
        {
          th: 'scale-down-utilization-threshold',
          td: ['0.5 (aggressive)', 'Lower', 'Start at 0.5, increase if too much churn'],
        },
        {
          th: 'scale-down-unneeded-time',
          td: ['5m (aggressive)', 'Lower', '10m is balanced default'],
        },
        {
          th: 'scale-down-delay-after-add',
          td: ['10m (balanced)', 'Higher', 'Prevents thrashing'],
        },
        {
          th: 'expander',
          td: ['priority or least-waste', 'N/A', 'Use priority for spot preference'],
        },
      ],
    },
    figure: 'Autoscaler tuning recommendations.',
  }}
/>
...
<Download
  resource="kubernetes-cost-optimization-resource-sizing-spot-instances"
  title="Download the Kubernetes Cost Optimization Guide"
  description="Get the complete cost-efficiency playbook for right-sizing workloads, leveraging spot capacity, and sustaining savings over time."
  listItems={[
    {
      text: "Resource right-sizing workflow guide",
    },
    {
      text: "Spot adoption readiness checklist",
    },
    {
      text: "Autoscaler tuning strategy matrix",
    },
    {
      text: "Cost visibility dashboard templates",
    },
  ]}
/>
