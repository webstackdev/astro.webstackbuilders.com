...
<List
  ...
  items={[
    ...
    {
      lead: 'Output values',
      text: 'The data consumers reference. When someone writes <span class="font-mono">module.vpc.private_subnet_ids[0]</span>, they\'re depending on that output existing, being a list, and having at least one element. Change the output name or type and their code breaks.',
    },
    {
    ...
    },
    {
      ...
      text: 'The undocumented assumptions. Maybe your module names resources with a specific pattern (<span class="font-mono">{environment}-{name}-{resource_type}</span>), applies a standard set of tags, or assumes the VPC has DNS hostnames enabled. Perhaps it expects certain IAM permissions to exist, or assumes subnets have internet access through a NAT gateway. These are the trickiest because they\'re easy to break without realizing it — you rename your tagging convention and suddenly a downstream cost allocation dashboard stops working.',
    },
  ]}
/>

<Diagram
  src={moduleInterfaceDiagram}
  magnifyButtonPosition="bottom-right"
  size="full"
  title="Module interface separates what consumers depend on from implementation details."
  description="Top-to-bottom flowchart with three grouped sections: Module Consumer, Interface Contract, and Implementation not part of contract. In Module Consumer, one node shows a sample module block calling the vpc module with source company slash vpc slash aws, version approximately greater than 2.0, cidr_block 10.0.0.0 slash 16, and environment production. A second consumer node shows downstream usage where subnet_id is assigned from module.vpc.private_subnet_ids[0]. The consumer group connects to the Interface Contract group, which contains three contract elements: Inputs cidr_block and environment, Outputs vpc_id and private_subnet_ids, and Behavior creates VPC, subnets, and NAT. The Interface Contract group then connects to the Implementation group, which contains details such as how subnets are calculated, which availability zones are used, and internal resource naming. The diagram shows that consumers should depend on the published contract layer rather than on internal implementation details, allowing the module to evolve safely behind a stable interface."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Change Type', 'Breaking?', 'Example', 'Migration Path'],
    },
    tbody: {
      tr: [
        {
          th: 'Add optional input with default',
          td: ['No', '<span class="text-mono">variable "tags" { default = {} }</span>', 'None needed'],
        },
        {
          th: 'Remove input variable',
          td: ['Yes', 'Delete <span class="text-mono">variable "legacy_flag"</span>', 'Deprecate first; document in changelog'],
        },
        {
          th: 'Change input type',
          td: ['Yes', '<span class="text-mono">string</span> → <span class="text-mono">list(string)</span>', 'Provide migration example in changelog'],
        },
        {
          th: 'Add new output',
          td: ['No', '<span class="text-mono">output "new_arn" { ... }</span>', 'None needed'],
        },
        {
          th: 'Remove output',
          td: ['Yes', 'Delete <span class="text-mono">output "old_id"</span>', 'Deprecate for one major version first'],
        },
        {
          th: 'Change output type',
          td: ['Yes', '<span class="text-mono">string</span> → <span class="text-mono">object</span>', 'Add new output; deprecate old one'],
        },
        {
          th: 'Add new resource',
          td: ['Usually no', 'Add <span class="text-mono">aws_cloudwatch_log_group</span>', 'None if no side effects'],
        },
        {
          th: 'Remove resource',
          td: ['Yes', 'Delete <span class="text-mono">aws_s3_bucket</span>', 'Document state rm commands'],
        },
        {
          th: 'Rename resource',
          td: ['Yes', '<span class="text-mono">aws_instance.main</span> → <span class="text-mono">aws_instance.primary</span>', 'Requires <span class="text-mono">moved</span> block'],
        },
        {
          th: 'Change provider version',
          td: ['Maybe', '<span class="text-mono">>= 4.0</span> → <span class="text-mono">>= 5.0</span>', 'Depends on provider changes'],
        },
      ],
    },
    figure: 'Breaking vs non-breaking changes in module interfaces.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Default to the safer option.',
      text: "If a setting affects data durability or security, default to the more protective choice. Deletion protection should default to `true`. Encryption should default to enabled. Backup retention should be non-zero. Users can opt out for development environments; they shouldn't have to opt in for production safety.",
    },
    {
      lead: "Default to the cheaper option when safety isn't at stake.",
      text: 'Instance types, storage sizes, and replica counts can default to smaller values. Users will scale up when they need to; defaulting to expensive configurations just creates surprise bills.',
    },
    {
      lead: 'Use environment-aware defaults when behavior should differ.',
      text: 'Some settings legitimately differ between environments — deletion protection makes sense in production but blocks rapid iteration in development. You can use `null` as a sentinel value and compute the actual default in a local:',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Remove or rename an input variable',
    },
    {
      lead: "Change an input variable's type",
    },
    {
      lead: 'Remove an output',
    },
    {
      lead: "Change an output's type or structure",
    },
    {
      lead: 'Remove a resource (causes state issues for consumers)',
    },
    {
      lead: 'Bump a provider minimum version that has breaking changes',
    },
  ]}
/>

__Minor version bumps__ are for backward-compatible additions:
...
<List
  ...
  items={[
    {
      lead: 'Add a new input variable with a default value',
    },
    {
      lead: 'Add a new output',
    },
    {
      lead: 'Add a new resource (assuming no side effects)',
    },
    {
      lead: 'Deprecate (but not remove) an input or output',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Fix a bug without changing behavior consumers depend on",
    },
    {
      lead: 'Update documentation',
    },
    {
      lead: "Internal refactoring that doesn't affect state",
    },
    {
      lead: 'Update provider constraints within compatible ranges',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Strategy', 'Constraint', 'Use Case', 'Trade-off'],
    },
    tbody: {
      tr: [
        {
          th: 'Pessimistic',
          td: ['`~> 2.0`', 'Most modules', 'Gets patches automatically, no breaking changes'],
        },
        {
          th: 'Exact',
          td: ['`2.3.1`', 'Critical infrastructure', 'Maximum stability, requires manual updates'],
        },
        {
          th: 'Range',
          td: ['`>= 2.0, < 3.0`', 'Same as pessimistic', 'More explicit about intent'],
        },
        {
          th: 'Minimum',
          td: ['`>= 2.0`', 'Development only', 'Gets latest, may break unexpectedly'],
        },
      ],
    },
    figure: 'Version constraint strategies and their trade-offs.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Added',
      text: 'Side-by-side-list new features',
    },
    {
      lead: 'Changed',
...
<FileExplorer
  figure="Recommended module repository structure."
  data={[
    { type: 'file', name: 'main.tf', comment: 'Primary resource logic' },
    { type: 'file', name: 'variables.tf', comment: 'Input variable definitions' },
    { type: 'file', name: 'outputs.tf', comment: 'Output declarations' },
    { type: 'file', name: 'versions.tf', comment: 'Terraform and provider version constraints' },
    { type: 'file', name: 'README.md', comment: 'Usage documentation' },
    { type: 'file', name: 'CHANGELOG.md', comment: 'Version history and migration guides' },
    {
      type: 'folder',
      name: 'examples',
      comment: 'Working usage examples',
      children: [
        { type: 'folder', name: 'simple', comment: 'Minimal configuration' },
        { type: 'folder', name: 'complete', comment: 'All features demonstrated' },
      ],
    },
    {
      type: 'folder',
      name: 'modules',
      comment: 'Internal sub-modules (if needed)',
      children: [
        { type: 'folder', name: 'networking', comment: 'Encapsulated sub-component' },
      ],
    },
    {
      type: 'folder',
      ...
      comment: 'Test files',
      children: [
        { type: 'file', name: 'contract.tftest.hcl', comment: 'Input/output contract tests' },
        { type: 'file', name: 'validation.tftest.hcl', comment: 'Validation rule tests' },
        { type: 'file', name: 'defaults.tftest.hcl', comment: 'Default value tests' },
        { type: 'file', name: 'upgrade_v1_to_v2.tftest.hcl', comment: 'Version migration tests' },
      ],
    },
  ]}
/>
...
<Download
  resource="terraform-module-design-defaults-versioning-interfaces"
  title="Terraform Module Interfaces: Defaults and Versioning"
  description="Designing module interfaces that are easy to use, hard to misuse, and can evolve without breaking consumers."
  listItems={[
    {
      text: "Interface design decision framework",
    },
    {
      text: "Versioning and migration checklist",
    },
    {
      text: "Validation pattern code recipes",
    },
    {
      text: "Module contract test starter",
    },
  ]}
/>
