...
import kubernetesDiagram from "./diagrams/kubernetes-workload-identity-token-exchange-flow.png"
import longLivedKeyDiagram from "./diagrams/long-lived-key-authentication-model.png"
import tokenExchangeDiagram from "./diagrams/token-exchange-sequence-from-workload-to-cloud-credentials.png"
import workloadIdentityDiagram from "./diagrams/workload-identity-federation-authentication-flow.png"
...
<List
  ...
  items={[
    { text: "Day 0: Key created for CI pipeline deployment" },
    { text: "Day 30: Key copied to a second repo because someone needed the same access" },
    { text: "Day 90: Developer stores key in `.env` file for local testing" },
    { text: "Day 180: `.env` file accidentally committed to git" },
    { text: "Day 181: Git history rewritten, but the key remains in the reflog" },
    { text: "Day 365: Repository made public for an open source release" },
    { text: "Day 366: Automated scanners find the key in git history" },
    { text: "Day 367: Cryptomining instances spinning up in your account" },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "iss (issuer):",
      text: "Where the token came from — GitHub's OIDC endpoint in this case",
    },
    {
      lead: "sub (subject):",
      text: "The unique identifier for the workload, which varies based on context (branch, environment, pull request)",
    },
    {
      lead: "aud (audience):",
    ...
    {
      lead: "repository and repository_owner:",
...
<List
  ...
  items={[
    { text: "Your workflow requests an OIDC token from GitHub, specifying the intended audience (your cloud provider)" },
    { text: "GitHub signs a JWT containing claims about the workflow context" },
    { text: "Your workflow sends this JWT to your cloud provider's STS endpoint" },
    { text: "STS fetches (or retrieves from cache) GitHub's public keys to verify the signature" },
    { text: "STS validates everything: signature, issuer is trusted, claims match your policy, token isn't expired" },
    { text: "STS looks up which service account this workload should assume based on attribute mappings" },
    { text: "STS returns short-lived credentials scoped to that service account's permissions" },
  ]}
/>
