...
<List
  ...
  items={[
    { lead: "Day 0: Key created for CI pipeline deployment" },
    { lead: "Day 30: Key copied to a second repo because someone needed the same access" },
    { lead: "Day 90: Developer stores key in `.env` file for local testing" },
    { lead: "Day 180: `.env` file accidentally committed to git" },
    { lead: "Day 181: Git history rewritten, but the key remains in the reflog" },
    { lead: "Day 365: Repository made public for an open source release" },
    { lead: "Day 366: Automated scanners find the key in git history" },
    { lead: "Day 367: Cryptomining instances spinning up in your account" },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Risk', 'Impact', 'Detection Difficulty'],
    },
    tbody: {
      tr: [
        {
    ...
    figure: 'Common service account key risks and their detectability.',
  }}
/>
...
<Diagram
  src={longLivedKeyDiagram}
  size="xl"
  title="Long-lived key authentication model."
  description="Left-to-right flowchart showing a CI Runner sending a Service Account Key directly to a Cloud API for authentication. Beneath that flow is a grouped section labeled Key Properties listing three risks: the key never expires, it must be stored somewhere, and it is unclear who may have copies of it. The diagram illustrates the core weakness of static key-based authentication: access depends on a reusable secret that can persist indefinitely and spread beyond its intended owner."
/>

With federation, your CI runner requests a token from its platform, then exchanges that token with your cloud provider:

<Diagram
  src={workloadIdentityDiagram}
  size="full"
  title="Workload identity federation authentication flow."
  description="Top-to-bottom flowchart showing keyless authentication through workload identity federation. A CI Runner first requests an OIDC token from the CI Platform identity provider. The identity provider then issues a JWT containing claims to the Cloud STS service. Inside the Cloud STS node, three validation steps are shown: verify the JWT signature, check issuer trust, and validate claims against policy. If validation succeeds, Cloud STS issues short-lived credentials valid for one hour. Those credentials are then used to access the Cloud API. The diagram shows the replacement of static secrets with a signed identity assertion followed by temporary credentials."
/>
...
<List
  ...
  items={[
    {
      lead: '<span class="font-mono">iss</span> (issuer)',
      text: 'Where the token came from — GitHub\'s OIDC endpoint in this case (<span class="font-mono">https://token.actions.githubusercontent.com</span>)',
    },
    {
      lead: '<span class="font-mono">sub</span> (subject)',
      text: 'The unique identifier for the workload, which varies based on context (branch, environment, pull request) — <span class="font-mono">repo:myorg/myrepo:ref:refs/heads/main</span>',
    },
    {
      lead: '<span class="font-mono">aud</span> (audience)',
      text: "Who the token is intended for — your cloud provider's token exchange endpoint",
    },
    {
      lead: '<span class="font-mono">repository</span> and <span class="font-mono">repository_owner:</span>',
      text: "Which repo triggered the workflow",
    },
    {
      lead: '<span class="font-mono">ref:</span>',
      text: "The git reference (branch or tag) being built",
    },
    {
      lead: '<span class="font-mono">environment:</span>',
      text: "The GitHub Environment, if you're using environment protection rules",
    },
  ]}
/>
...
<Diagram
  src={tokenExchangeDiagram}
  title="Token exchange sequence from workload to cloud credentials."
  description="Sequence diagram with four participants: Workload, Identity Provider, Cloud STS, and Cloud IAM. The Workload requests an OIDC token from the Identity Provider, which returns a JWT signed with the provider's key. The Workload then sends that JWT to Cloud STS to exchange it for credentials. Cloud STS fetches the identity provider's JWKS, typically from cache, and verifies the token's signature, issuer, claims, and expiry. Cloud STS then queries Cloud IAM to look up the service account and retrieve its permissions. Finally, Cloud STS returns a short-lived access token valid for one hour back to the Workload. The diagram shows the full trust chain from workload-issued identity proof to temporary cloud credentials."
/>
...
<List
  ...
  items={[
    {
      lead: "Your workflow requests an OIDC token from GitHub, specifying the intended audience (your cloud provider)",
    },
    {
      lead: "GitHub signs a JWT containing claims about the workflow context",
    },
    {
      lead: "Your workflow sends this JWT to your cloud provider's STS endpoint",
    },
    {
      lead: "STS fetches (or retrieves from cache) GitHub's public keys to verify the signature",
    },
    {
      lead: "STS validates everything: signature, issuer is trusted, claims match your policy, token isn't expired",
    },
    {
      lead: "STS looks up which service account this workload should assume based on attribute mappings",
    },
    {
      lead: "STS returns short-lived credentials scoped to that service account's permissions",
    },
  ]}
/>
...
<Diagram
  src={kubernetesDiagram}
  size="full"
  title="Kubernetes workload identity token exchange flow."
  description="Top-to-bottom flowchart showing workload identity inside a Kubernetes cluster. In the Kubernetes Cluster group, a Pod contains a Container that requests credentials from an SDK or client library. The SDK reads a projected token from a token volume mount located at /var/run/secrets/tokens/. That token is then exchanged with Cloud IAM outside the cluster. Within Cloud IAM, the token's signature is verified and the workload binding is checked. If validation succeeds, Cloud IAM returns credentials to the SDK or client library. The diagram shows how a pod uses a projected service account token as the proof of identity, then exchanges it for temporary cloud credentials without storing static keys."
/>
...
<List
  ...
  items={[
    {
      lead: "What workload uses it?",
      text: "CI pipeline, Kubernetes pod, application server, developer laptop?",
    },
    {
      lead: "Which platform?",
      text: "GitHub Actions, GitLab CI, Jenkins, EKS, GKE, a VM somewhere?",
    },
    {
      lead: "Can it migrate?",
      text: "Does the platform support OIDC federation? Some older CI systems don't.",
    },
    {
      lead: "What's the risk?",
      text: "How old is the key? How broad are its permissions? Is it in production?",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Phase', 'Duration', 'Activities', 'Success Criteria'],
    },
    tbody: {
      tr: [
        {
          th: 'Discovery',
          td: ['1-2 weeks', 'Inventory all keys, identify usage, assess risk', 'Complete inventory with owner/usage mapping'],
        },
        {
          th: 'Foundation',
          td: ['2-4 weeks', 'Configure OIDC providers, create federated identities', 'Federation infrastructure deployed and tested'],
        },
        {
          th: 'Pilot',
          td: ['2-4 weeks', 'Migrate low-risk, non-production workloads', '3-5 workloads using federation successfully'],
        },
        {
          th: 'Production',
          td: ['4-8 weeks', 'Migrate production workloads systematically', 'All production workloads using federation'],
        },
        {
          th: 'Retirement',
          td: ['2-4 weeks', 'Disable then delete old keys', 'Zero active long-lived keys'],
        },
      ],
    },
    figure: 'Migration phases and timeline for workload identity adoption.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: "Verify no recent usage:",
      text: "Check CloudTrail or equivalent for any key usage in the last 30 days",
    },
    {
      lead: "Disable the key:",
      text: "This makes it unusable but recoverable if something breaks",
    },
    {
      lead: "Monitor for 7-14 days:",
      text: "Watch for authentication failures in your workloads",
    },
    {
      lead: "Delete the key:",
      text: "Only after the monitoring period passes without issues",
    },
    {
      lead: "Document the retirement:",
      text: "Record what was retired, when, and by whom for audit purposes",
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: 'Feature branch build',
      text: 'You\'re building from a feature branch but the policy only allows <span class="text-mono">main</span>',
      },
    {
      lead: 'Repository renamed',
      text: 'Someone renamed the repository' },
    {
      lead: 'Pull request context',
      text: 'You\'re running in a pull request context but the policy expects a branch ref',
      },
    {
      lead: 'Environment name typo',
      text: 'The GitHub Environment name has a typo',
      },
    {
      lead: 'Audience mismatch',
      text: 'Means the <span class="text-mono">aud</span> claim in your token doesn\'t match the <span class="text-mono">client_id_list</span> in your OIDC provider configuration. This typically means the workflow is requesting a token for a different audience than what you configured. Check that your workflow\'s audience parameter matches what\'s in your cloud provider\'s OIDC provider setup.',
    },
    {
...
<Table
  ...
  content={{
    thead: {
      th: ['Error', 'Likely Cause', 'Quick Fix'],
    },
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">InvalidIdentityToken</span>',
          td: ['Token expired or malformed', 'Retry the workflow; check for clock skew'],
        },
        {
          th: '<span class="font-mono">AccessDenied: Not authorized</span>',
          td: ['Trust policy condition mismatch', 'Decode token, compare `sub` to policy'],
        },
        {
          th: '<span class="font-mono">Audience does not match</span>',
          td: ['Wrong audience in token request', 'Check workflow audience vs provider config'],
        },
        {
          th: '<span class="font-mono">WebIdentityErr: failed to retrieve</span>',
          td: ['OIDC endpoint unreachable', 'Check GitHub status; retry later'],
        },
        {
          th: '<span class="font-mono">Service account does not exist</span>',
          td: ['Deleted or wrong SA binding', 'Verify SA exists and binding is correct'],
        },
      ],
    },
    figure: 'Common federation errors and their resolutions.',
  }}
/>
