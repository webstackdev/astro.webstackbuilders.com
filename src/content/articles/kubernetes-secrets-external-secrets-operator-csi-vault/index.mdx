  ...
  items={[
    {
      ...
      lead: 'Syncs secrets on a schedule and caches them as native Kubernetes secrets.',
    },
    {
      title: 'Secrets Store CSI Driver',
      lead: 'Fetches secrets on-demand when pods start.',
    },
    {
      title: 'Init containers',
      lead: 'Gives you complete control but require you to implement everything yourself.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Pattern', 'Existing Pods', 'New Pods', 'Recovery'],
    },
    tbody: {
      tr: [
        {
          th: 'ESO',
          td: ['✅ Running', '✅ Start (cached)', 'Automatic'],
        },
        {
          th: 'CSI Driver',
          td: ['✅ Running', '❌ Blocked', 'May need intervention'],
        },
        {
          th: 'Init Container',
          td: ['✅ Running', '⚠️ Depends', 'Custom logic'],
        },
      ],
    },
    figure: 'Behavior during Vault outage.',
  }}
/>
...
<Diagram
  src={threeSecretDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Three secret injection patterns."
  description="Flowchart comparing three ways Kubernetes workloads receive secrets from an external secret manager. At the top, an external group labeled External Secret Manager contains Vault, AWS Secrets Manager, or another provider. Inside a grouped Kubernetes Cluster section, the first pattern labeled Pattern 1: ESO shows ESO Controller writing to a Kubernetes Secret, which is then consumed by a pod through envFrom or volumeMount. The second pattern labeled Pattern 2: CSI Driver shows the CSI Driver DaemonSet providing a pod volume mount. The third pattern labeled Pattern 3: Init Container shows an Init Container writing secret data into a shared emptyDir, which the Main Container then reads. The external manager connects to ESO through a path labeled Periodic sync and connects to both the CSI Driver and Init Container through paths labeled On pod start. The diagram emphasizes that ESO caches into native Kubernetes Secrets, CSI fetches during pod startup, and init containers implement a custom startup-time retrieval path."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Aspect', 'Native Secrets', 'External Manager'],
    },
    tbody: {
      tr: [
        {
          th: 'Encryption at rest',
          td: ['Must configure', 'Built-in'],
        },
        {
          th: 'Access audit',
          td: ['API-level only', 'Comprehensive'],
        },
        {
          th: 'Rotation',
          td: ['Manual', 'Automatic'],
        ...
        {
          th: 'Versioning',
          td: ['None', 'Full history'],
        },
        {
          th: 'Cross-cluster',
          td: ['Manual sync', 'Centralized'],
        },
      ],
    },
    figure: 'Native vs external secret management.',
  }}
/>
...
<Diagram
  ...
  size="full"
  title="ESO sync flow."
  description="Sequence diagram showing how External Secrets Operator synchronizes secret values into Kubernetes. The participants are ExternalSecret, ESO Controller, Secret Manager, Kubernetes Secret, and Pod. The flow begins when the ESO Controller watches an ExternalSecret resource. Inside a loop that repeats every refreshInterval, the controller fetches secret values from the external Secret Manager, receives the values in response, and then creates or updates a Kubernetes Secret. Separately, a Pod mounts that Kubernetes Secret and receives the secret data. A note over the ESO Controller and Secret Manager explains that if the Secret Manager is down, the Kubernetes Secret keeps its last value. The diagram highlights that ESO fetches asynchronously on a controller schedule and that pods consume cached Kubernetes Secret data rather than making direct startup-time calls to the external manager."
/>
...
<Diagram
  src={csiDriverDiagram}
  size="full"
  title="CSI driver secret fetch flow."
  description="Sequence diagram showing how the Secrets Store CSI Driver fetches secrets during pod startup. The participants are Pod, CSI Driver, Vault Provider, and HashiCorp Vault. The flow begins when the Pod sends a mount volume request to the CSI Driver. The CSI Driver asks the Vault Provider to fetch secrets. The provider authenticates to Vault and reads the requested values. Vault returns the secret values to the provider, the provider returns secret data to the CSI Driver, and the CSI Driver mounts the secrets as files into the Pod. A note explains that the pod starts only after the secrets are mounted. An alternate failure path shows the provider attempting to authenticate to Vault when Vault is unavailable. Vault returns a connection failure, the provider returns an error to the CSI Driver, the CSI Driver reports that the mount fails, and a note states that the Pod stays Pending. The diagram emphasizes that CSI secret retrieval is synchronous and directly coupled to pod startup."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Criteria', 'ESO', 'CSI Driver', 'Init Container', 'Native'],
    },
    tbody: {
      tr: [
        {
          th: 'Sync model',
          td: ['Periodic', 'On pod start', 'On pod start', 'Manual'],
        },
        {
          th: 'Secret freshness',
          td: ['Minutes', 'At pod start', 'At pod start', 'Static'],
        },
        {
          th: 'Failure mode',
          td: ['Silent (stale)', 'Loud (blocked)', 'Configurable', 'None'],
        },
        {
          th: 'Pod startup impact',
          td: ['None', 'Adds latency', 'Adds latency', 'None'],
        },
        {
          th: 'K8s Secret created',
          td: ['Yes', 'Optional', 'No', 'Yes'],
        },
        {
          th: 'GitOps friendly',
          td: ['Yes', 'Partial', 'No', 'Yes'],
        },
        {
          th: 'Complexity',
          td: ['Medium', 'Medium', 'High', 'Low'],
        },
      ],
    },
...
The table captures raw capabilities, but choosing a pattern requires weighting those capabilities against your operational reality. A team with strong GitOps practices and tolerance for minutes of staleness will land in a different place than a team running payment infrastructure where a stale database credential means failed transactions. The deciding factor is usually the failure mode: ESO fails silently with stale data, CSI fails loudly by blocking pod startup, and init containers fail however you code them to. Each is the right answer for different risk profiles.
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Fits edge cases where ESO and CSI don\'t provide enough control. Legacy applications that need secrets in unusual formats, multi-source fallback logic, or integration with systems that don\'t have ESO/CSI providers. You\'re writing and maintaining the logic yourself.',
    },
    {
      lead: 'Native secrets',
      text: 'Fits non-sensitive configuration and development environments. With encryption at rest configured, they\'re acceptable for data that doesn\'t require rotation or audit trails.',
    },
  ]}
/>

<Diagram
  src={patternDecisionDiagram}
  size="full"
  title="Pattern decision tree."
  description="Flowchart guiding the choice between Kubernetes secret injection patterns based on failure tolerance. The process starts at Choose Secret Pattern and asks whether the application can tolerate minutes of staleness. If yes, the recommendation is ESO. If no, the chart asks whether the team accepts pod failures when the secret manager is down. If yes, the recommendation is CSI Driver. If no, the chart asks whether custom fallback logic is needed. If yes, the recommendation is Init Container. If no, the recommendation is ESO plus a short interval. The diagram highlights that the main decision points are tolerance for stale secrets, tolerance for blocked pod startup, and willingness to build custom fallback behavior."
/>
...
<List
  ...
  items={[
    {
      lead: 'Web applications and APIs',
      text: 'ESO with 15-minute refresh. These workloads tolerate brief staleness, and ESO\'s graceful degradation keeps them running through secret manager outages.',
    },
    {
      lead: 'Database connections',
      text: 'ESO with 5-minute refresh. Connection pools cache connections anyway, so the staleness window compounds. Shorter refresh reduces the risk of rotated credentials breaking pooled connections.',
    },
    {
    ...
    },
    {
      ...
      text: 'ESO with different ClusterSecretStores per cluster. Consistent pattern across environments, centralized secrets in Vault or cloud provider.',
    },
    {
      lead: 'Legacy applications',
      text: 'Init container with fallback. When you need secrets in unusual formats or custom retry logic, you control the implementation.',
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Use IAM roles scoped to specific secrets, not broad permissions. Route traffic through VPC endpoints rather than the public internet. Enable CloudTrail or equivalent audit logging.',
    },
  ]}
/>
...
<Table
  ...
  classes={{
    thead:
      '[&_th:nth-child(3)]:!text-center [&_th:nth-child(4)]:text-center',
    tbody:
      '[&_td:nth-child(3)]:!text-warning-offset [&_td:nth-child(4)]:text-danger [&_td:nth-child(3)]:!text-center [&_td:nth-child(4)]:text-center',
  }}
  content={{
    thead: {
      th: ['Metric', 'Warning', 'Critical', 'Response'],
    },
    tbody: {
      tr: [
        {
          th: 'ESO sync age',
          td: ['> 2× refreshInterval', '> 4× refreshInterval', 'Check connectivity'],
        },
        {
          th: 'CSI mount failures',
          td: ['> 1/hour', '> 5/hour', 'Check provider pods'],
        },
        {
          th: 'Vault latency',
          td: ['> 500ms', '> 2s', 'Scale Vault, check network'],
        },
        {
          th: 'Auth failures',
          td: ['Any', '> 5/hour', 'Check IAM/Vault policies'],
        },
      ],
    },
    figure: 'Monitoring thresholds.',
  }}
/>
...
<Download
  resource="kubernetes-secrets-external-secrets-operator-csi-vault"
  title="Kubernetes Secrets: ESO vs CSI vs Init Containers"
  description="Comparing secret injection patterns and their failure modes when connecting Vault or cloud secret managers."
  listItems={[
    {
      text: "Secret pattern comparison matrix",
    },
    {
      text: "ESO configuration starter templates",
    },
    {
      text: "CSI failure mode runbooks",
    },
    {
      text: "Rotation and audit checklist",
    },
  ]}
/>
