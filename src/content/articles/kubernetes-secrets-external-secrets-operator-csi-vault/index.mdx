...
<Highlighter>External secrets management in Kubernetes has consolidated around two dominant patterns.</Highlighter> External Secrets Operator runs as a controller in your cluster, periodically syncing secrets from Vault (or AWS Secrets Manager, Azure Key Vault, etc.) into native Kubernetes Secret objects. The Secrets Store CSI Driver takes a different approach: it mounts secrets directly into pods as volumes, fetching them from the external manager when pods start.
...
<Table
  ...
  content={{
    ...
    figure: 'Behavior during a 30-minute Vault outage.',
  }}
/>
...
<List
  ...
  items={[
    ...
    {
      text: 'Horizontal pod autoscaler scales up pods that immediately get stuck',
    },
    {
      text: 'A node reboot during an incident restarts pods that can\'t fetch their secrets',
    },
  ]}
/>
