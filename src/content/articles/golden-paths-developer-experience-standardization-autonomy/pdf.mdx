...
<List
  ...
  items={[
    {
      lead: "Mandated standards work for security-critical controls, legal and compliance requirements, and cross-cutting concerns like observability where inconsistency creates real operational risk",
      text: "The tradeoff is high adoption friction and stifled innovation. Use mandates sparingly.",
    },
    {
      lead: "Golden paths work for common patterns like service creation and deployment, best practices like testing and monitoring, and productivity tooling",
      text: "The tradeoff is that escape hatches can become the primary path if you're not careful, and paths can become outdated if you under-invest in maintenance.",
    },
    {
      lead: "Full self-service works for early-stage companies where standardization would slow down finding product-market fit, R&D and experimental work where the whole point is trying new things, and highly heterogeneous tech landscapes where no single path could cover the diversity",
      text: "The tradeoff is duplication of effort, knowledge silos, and operational burden distributed across every team.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      text: "A service catalog (Backstage, Port, or similar) gives developers a searchable, browsable home for all paths. Each path has a card with a description, use cases, getting-started link, and owner contact. Developers searching for \"create a new service\" or \"set up monitoring\" find the relevant path immediately.",
    },
    {
...
<List
  ...
  items={[
    {
      text: "The core journey is the sequence of steps that gets developers from \"I need a thing\" to \"I have a working thing.\" For a service creation path, this might be: scaffold the service skeleton, configure service-specific values, provision infrastructure, and verify everything works. The core journey should be fast (10 minutes or less for common cases) and should handle the common case without requiring decisions the developer isn't equipped to make.",
    },
    {
      text: "Extension points are places where teams can customize the path without forking it. These might be pre/post hooks in a pipeline, config overrides, or plugin systems. Good extension points let teams add behavior without modifying the core path.",
    },
    {
      text: "Escape hatches are documented ways to leave the path entirely when it doesn't fit. These should be visible (not hidden), have clear processes, and ideally include a migration path back to the golden path later.",
    },
  ]}
/>
