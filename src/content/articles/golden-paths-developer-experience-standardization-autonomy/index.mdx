...
<List
  ...
  items={[
    {
      lead: "Opinionated but not mandatory",
      text: "The path has strong defaults, but usage is optional. Your default CI template uses standard runners, but teams can bring their own if they have a reason.",
    },
    {
      lead: "Optimized for the common case",
      text: "The 80 / 20 rule applies. Your REST API template covers 80% of services; gRPC teams use an escape hatch. You're not trying to solve every problem — you're making the common case trivial.",
    },
    {
      lead: "Actively maintained",
      text: "The platform team owns updates, security patches, and improvements. Base images get updated monthly, and teams on the path get those updates automatically. This is the value proposition: stay on the path and maintenance is someone else's problem.",
    },
    {
      lead: "Well documented",
      text: "Clear docs, tutorials, examples, and troubleshooting guides. A getting-started guide that works, a FAQ that answers real questions, and a decision tree for when to use (and not use) the path.",
    },
    {
      lead: "Measurably better",
      text: "The path has demonstrable advantages over alternatives—50% faster deployments, 3x fewer security incidents, whatever the metrics are for your context. If you can't show the path is better, why should teams use it?",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Golden handcuffs",
      text: "No way to deviate, even with good reason. The result is shadow platforms, resentment, and eventual abandonment of the standard altogether.",
    },
    {
      lead: "Golden cobwebs",
      text: "The path exists but is unmaintained. Teams avoid it because the documentation is stale, dependencies are outdated, and security vulnerabilities pile up. An unmaintained path is worse than no path — it creates false confidence.",
    },
    {
      lead: "Golden labyrinth",
      text: "So many paths that none are clearly recommended. Teams face decision paralysis, make inconsistent choices, and you lose the network effects that make standardization valuable in the first place.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: "Comparing standardization approaches.",
    thead: {
      th: ["Approach", "Consistency", "Team Velocity", "Platform Effort", "Best For"],
    },
    tbody: {
      tr: [
        {
          th: "Mandated",
          td: ["High (enforced)", "Low (friction)", "High (enforcement)", "Compliance, security"],
        },
        {
          th: "Golden path",
          td: ["High (incentivized)", "High (paved road)", "Medium (maintenance)", "Common patterns"],
        },
        {
          th: "Self-service",
          td: ["Low", "Variable", "Low", "Innovation, edge cases"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: "Mandated standards suit high-stakes controls.",
      text: "They work for security-critical requirements, legal compliance, and cross-cutting concerns like observability where inconsistency creates real operational risk. The tradeoff is high adoption friction and stifled innovation. Use mandates sparingly.",
    },
    {
      lead: "Golden paths suit common, repeatable patterns.",
      text: "They work for service creation and deployment, best practices like testing and monitoring, and productivity tooling. The tradeoff is that escape hatches can become the primary path if you're not careful, and paths can become outdated if you under-invest in maintenance.",
    },
    {
      lead: "Full self-service suits exploration and diversity.",
      text: "It works for early-stage companies where standardization would slow down finding product-market fit, R&D work where the whole point is trying new things, and heterogeneous tech landscapes where no single path could cover the diversity. The tradeoff is duplication of effort, knowledge silos, and operational burden distributed across every team.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "A service catalog gives developers a browsable home for all paths.",
      text: "Tools like Backstage or Port let each path have a card with a description, use cases, getting-started link, and owner contact. Developers searching for \"create a new service\" or \"set up monitoring\" find the relevant path immediately.",
    },
    {
      lead: "Decision trees help when multiple paths might apply",
      text: "\"Are you building a user-facing API or an internal batch job?\" leads to different paths. These can live in the catalog or as standalone documentation. The key is reducing the cognitive load of choosing — developers shouldn't need to read five different path docs to figure out which one fits.",
    },
    {
      lead: "Search and tagging matter more as the path catalog grows",
      text: "Tags like \"kubernetes,\" \"serverless,\" \"data-pipeline,\" and \"frontend\" let developers filter to relevant options. Full-text search across path descriptions catches developers who don't know the exact terminology.",
    },
    {
      lead: "Contextual recommendations take discovery further",
      text: "When a developer creates a new repository, the platform can suggest relevant paths based on the repo name, language detection, or team membership. \"It looks like you're creating a Python service — would you like to use the Python Service Template?\" This proactive approach catches developers before they start from scratch.",
    },
  ]}
/>
...
<List
  ...
  classes={{
    wrapper: "sm:mr-6 mb-6",
  }}
  items={[
    {
      lead: "The core journey takes developers from need to working artifact.",
      text: "For a service creation path, this might be: scaffold the service skeleton, configure service-specific values, provision infrastructure, and verify everything works. The core journey should be fast (10 minutes or less for common cases) and should handle the common case without requiring decisions the developer isn't equipped to make.",
    },
    {
      lead: "Extension points let teams customize without forking.",
      text: "These might be pre/post hooks in a pipeline, config overrides, or plugin systems. Good extension points let teams add behavior without modifying the core path.",
    },
    {
      lead: "Escape hatches provide a documented way off the path.",
      text: "They should be visible (not hidden), have clear processes, and ideally include a migration path back to the golden path later.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Documented, not hidden",
      text: "Escape hatches are part of the platform, not loopholes. The alternative deployment guide lives in the main docs, not on a hidden wiki page. If teams have to discover escape hatches through word of mouth, you've created a two-tier system where connected teams have options and everyone else doesn't.",
    },
    {
      lead: "Friction proportional to risk",
      text: "Higher-risk deviations require more process. A custom Dockerfile might need only a self-review and a note in the README. Custom network configuration needs a security review. The process should match the risk, not be uniformly heavy or uniformly light.",
    },
    {
      lead: "Path back exists",
      text: "Teams should be able to return to the golden path later. If taking an escape hatch is a one-way door, teams will be reluctant to use it even when appropriate, and you'll accumulate permanent exceptions. Provide migration guides from custom to standard.",
    },
    {
      lead: "Support level is clear",
      text: "Teams know what support they'll get (or won't). Escape hatch means best-effort support, not SLA. This isn't punitive — it's honest about where the platform team's expertise and tooling apply.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: "Escape hatch tiers with escalating process and decreasing support.",
    thead: {
      th: ["Tier", "Examples", "Process", "Support Level"],
    },
    tbody: {
      tr: [
        {
          th: "Extension",
          td: ["Custom CI steps, extra K8s annotations", "Self-service, no approval", "Full support"],
        },
        {
          th: "Override",
          td: ["Custom Dockerfile, alternative test framework", "Document in README, async review", "Best-effort"],
        },
        {
          th: "Exception",
          td: ["Alternative deployment target, custom networking", "Architecture review required", "Self-maintained"],
        },
        {
          th: "Off-platform",
          td: ["Custom infrastructure, third-party managed service", "VP approval, convergence plan", "None"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      text: "CPU-only inference: 400ms latency, 8x more instances, higher cost. Rejected.",
    },
    {
      text: "Kubernetes GPU nodes: Not available; 3-month roadmap item. Rejected for timeline.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Network effects",
      text: "Value increases as more teams adopt. Early adopters contribute to a shared knowledge base of solutions. Teams can help each other in Slack because they're using the same tools. Feature requests get prioritized based on popularity. The tooling ecosystem assumes path conventions, so integrations \"just work.\"",
    },
    {
      lead: "Operational benefits",
      text: "Teams on the path get better operational support. Priority support with faster response times. Proactive monitoring where the platform team watches path services. Automatic upgrades for security patches and dependency updates. Incident support where the platform team joins on-call for path-related issues.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Organizational recognition",
      text: "Adoption is visible and valued. Adoption dashboards let teams see their status. Leadership visibility means adoption metrics appear in reports. Migration support provides dedicated help for teams converging on the path.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Shame dashboards",
      text: "Public leaderboards ranking teams by compliance create resentment and incentivize gaming metrics. Use private adoption reports with helpful context instead.",
    },
    {
      lead: "Artificial friction",
      text: "Making alternatives harder than necessary punishes legitimate use cases and breeds workarounds. Make the path easier; don't make alternatives worse.",
    },
    {
      lead: "Mandate without capability",
      text: "Requiring the path before it handles a team's use case forces bad fits and creates exceptions from day one. Expand path capabilities before mandating.",
    },
    {
      lead: "Support hostage",
      text: "Refusing to help with anything off-path alienates teams and creates an adversarial relationship. Use tiered support — less for off-path, but not zero.",
    },
    {
      lead: "Deadline without migration support",
      text: "Requiring migration by a date without helping means teams scramble, cut corners, and create technical debt. Offer migration sprints with platform team pairing.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Opportunity cost visibility",
      text: "Show teams what they're missing. \"Your service could have automatic security upgrades if you used the standard base image.\" This isn't threatening — it's informing.",
    },
    {
      lead: "Improvement collaboration",
      text: "Invite off-path teams to shape the path. \"Your use case isn't covered — let's design the extension point together.\" This turns potential adversaries into partners.",
    },
    {
      lead: "Success storytelling",
      text: "Highlight teams who migrated and benefited. \"Team X moved to the path and reduced deploy time by 60%.\" Social proof is powerful.",
    },
  ]}
/>
...
<Table
  variant="criteria-highlight-table"
  content={{
    figure: "Leading and lagging indicators for path health.",
    thead: {
      th: ["Metric", "Type", "What It Tells You", "Action Threshold"],
    },
    tbody: {
      tr: [
        {
          th: "New service adoption",
          td: ["Leading", "Path attractiveness to new projects", "< 70% investigate"],
        },
        {
          th: "Escape hatch requests",
          td: ["Leading", "Gap between path and needs", "> 20% expand path"],
        },
        {
          th: "Extension point usage",
          td: ["Leading", "Customization demands", "High usage = productize"],
        },
        {
          th: "NPS score",
          td: ["Leading", "Developer sentiment", "< 30 immediate action"],
        },
        {
          th: "Deploy frequency (path vs off)",
          td: ["Lagging", "Operational benefit realized", "Path should be higher"],
        },
        {
          th: "MTTR (path vs off)",
          td: ["Lagging", "Support model effectiveness", "Path should be lower"],
        },
        {
          th: "Migration requests",
          td: ["Lagging", "Pull toward path", "High = path improving"],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: "The what and why",
      text: "Clearly identify the path being deprecated and explain the reason. \"Service Template v1 uses Node.js 16, which reaches EOL in April. Template v2 uses Node.js 20 with improved performance and security.\" Developers are more cooperative when they understand the reasoning.",
    },
    {
      lead: "The replacement path",
      text: "Document what teams should migrate to, link to a detailed migration guide, and indicate whether automated migration tooling is available. \"Run `platform migrate service-template` to automatically upgrade\" is much better than \"figure it out yourself.\"",
    },
    {
      lead: "The impact assessment",
      text: "List all impacted services with team ownership and contact information. Teams need to know they're affected without discovering it themselves.",
    },
    {
      lead: "The timeline",
      text: "Provide clear milestones. When is deprecation announced? When does migration tooling become available? When do security patches stop? When are new deployments blocked? When is the path removed entirely? Concrete dates let teams plan.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Path owners",
      text: "Have end-to-end responsibility for specific paths — the service creation path, the CI/CD path, the observability path. They define the roadmap, gather feedback from users, prioritize improvements, own documentation, and handle exceptions and escalations. Expect path owners to spend roughly half their time on development, 30% on support, and 20% on strategy. This role requires both technical depth and product thinking — understanding not just how to build the path but what users actually need.",
    },
    {
      lead: "Path engineers",
    ...
    {
      lead: "SRE liaisons",
      text: "Ensure operational excellence for paths. They define SLOs for path infrastructure, monitor path service health, respond to incidents affecting paths, and handle capacity planning. Their focus is keeping the platform reliable so product teams can depend on it. Time splits across monitoring (40%), improvement work (30%), and incident response (30%).",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Regular touchpoints",
      text: "Include weekly office hours where anyone can ask questions, Slack support for quick answers, and comprehensive documentation for self-service. The goal is making help accessible without requiring tickets or approvals.",
    },
    {
      lead: "Escalation paths",
      text: "Handle exception requests, feature requests, and incident co-response. These need clear processes so teams know how to escalate without feeling like they're fighting bureaucracy.",
    },
    {
      lead: "Feedback loops",
      text: "Include quarterly surveys for broad sentiment, user interviews for deep understanding, and usage analytics for behavioral insights. The platform team should actively seek feedback rather than waiting for complaints.",
    },
  ]}
/>
...
<Download
  resource="golden-paths-developer-experience-standardization-autonomy"
  title="Download the Golden Paths Guide"
  description="Get the complete platform playbook for standardization with autonomy, including escape-hatch governance and adoption strategies."
  listItems={[
    {
      text: "Path design pattern library",
    },
    {
      text: "Escape hatch tier framework",
    },
    {
      text: "Adoption incentive strategy guide",
    },
    {
      text: "Platform path health metrics",
    },
  ]}
/>
