...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "-2", td: ["Namespaces, CRDs", "Infrastructure prerequisites"] },
        { th: "-1", td: ["Secrets, ConfigMaps", "Configuration dependencies"] },
        { th: "0", td: ["Deployments, Services", "Main application (default)"] },
        { th: "1", td: ["Jobs, migrations", "Post-deployment tasks"] },
        { th: "2", td: ["HPA, PodDisruptionBudgets", "Scaling configuration"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "PreSync", td: ["Before any manifests applied", "Database backups, pre-flight checks"] },
        { th: "Sync", td: ["During normal sync", "Rarely used (default behavior)"] },
        { th: "PostSync", td: ["After all manifests healthy", "Notifications, smoke tests"] },
        { th: "SyncFail", td: ["When sync fails", "Cleanup, alerting"] },
        { th: "Skip", td: ["Never (resource skipped)", "Templates, documentation"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "Hook timeout", td: ["Sync stuck at \"Running PreSync hooks\"", "Hook job takes too long"] },
        { th: "Hook crash", td: ["Sync failed, hook pod in CrashLoopBackOff", "Bug in hook script"] },
        { th: "Hook not deleted", td: ["Old hook blocks new sync", "Missing delete policy"] },
        { th: "Hook wrong phase", td: ["Resources applied before prerequisites", "Wrong hook type"] },
      ],
    },
  }}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "Mutating webhooks:", text: "that add labels or annotations to resources" },
    { lead: "Kubernetes defaulting:", text: "that adds fields your manifest doesn't specify (like `imagePullPolicy: IfNotPresent`)" },
    { lead: "Controller timestamps:", text: "like `lastTransitionTime` in status fields" },
    { lead: "Cloud provider annotations:", text: "added by load balancer controllers or ingress controllers" },
  ]}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "application-controller", td: ["Controller pod", "Sync decisions, health checks"] },
        { th: "repo-server", td: ["Repo server pod", "Manifest generation, Helm/Kustomize"] },
        { th: "server", td: ["Server pod", "API errors, authentication"] },
        { th: "redis", td: ["Redis pod", "Caching issues, state corruption"] },
      ],
    },
  }}
/>
...
<List
  variant="check-icons-list"
  items={[
    { lead: "Hook script error:", text: "Fix script, redeploy" },
    { lead: "Missing permissions:", text: "Add RBAC for hook ServiceAccount" },
    { lead: "Image pull failure:", text: "Check image name and pull secret" },
    { lead: "Resource limits:", text: "Increase memory/CPU limits" },
    { lead: "External dependency:", text: "Ensure dependency is available" },
  ]}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "`Prune=false`", td: ["Never prune this resource"] },
        { th: "`PruneLast=true`", td: ["Prune after all other resources synced"] },
        { th: "`PrunePropagationPolicy=foreground`", td: ["Wait for dependents before pruning"] },
        { th: "`ApplyOutOfSyncOnly=true`", td: ["Only apply resources that differ"] },
      ],
    },
  }}
/>
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        { th: "label", td: ["Adds ArgoCD label to resources", "Default, simple apps"] },
        { th: "annotation", td: ["Adds ArgoCD annotation", "When labels cause issues"] },
        { th: "annotation+label", td: ["Both", "Migration between methods"] },
      ],
    },
  }}
/>
