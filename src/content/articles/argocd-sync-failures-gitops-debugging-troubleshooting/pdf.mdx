...
<Table
  ...
  classes={{
    figure: "sm:ml-6 sm:mr-6 sm:mb-6",
  }}
  content={{
...
For [built-in](https://kubernetes.io/docs/concepts/workloads/) Kubernetes types, ArgoCD has hardcoded health checks that usually do the right thing. The problems start when you use Custom Resources. ArgoCD doesn't know how to assess the health of your custom CRD, so it just marks it "Healthy" immediately after creation — even if the controller hasn't reconciled it yet.
...
<List
  ...
  items={[
    {
      lead: "You refactor your manifests and forget to include something.",
    },
    {
      lead: 'You rename a resource. ArgoCD sees it as "<span class="italic">delete old, create new</span>".',
    },
    {
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: '<span class="font-mono">Prune=false</span>',
          td: ["Never prune this resource"],
        },
        {
          th: '<span class="font-mono">PruneLast=true</span>',
          td: ["Prune after all other resources synced"],
        },
        {
          th: '<span class="font-mono">PrunePropagationPolicy=foreground</span>',
          td: ["Wait for dependents before pruning"],
        },
        {
          th: '<span class="font-mono">ApplyOutOfSyncOnly=true</span>',
