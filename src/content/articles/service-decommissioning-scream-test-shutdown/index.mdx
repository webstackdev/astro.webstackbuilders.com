...
"What if someone uses it?" → No visibility into usage
  ↓
"What if it breaks something?" → Unknown dependencies
  ↓
"Who owns the decision?" → No clear authority
  ↓
"What if we need it later?" → Hoarding instinct
  ↓
"It's not hurting anything" → Cost invisibility
  ↓
  ...
  if (!config.dryRun) {
    for (const channel of config.notificationChannels) {
      await sendSlackMessage(channel, {
        text: `🔌 Service ${config.serviceId} has been shut down`,
        blocks: [
          {
            type: 'section',
          ...
          },
          {
            ...
            text: {
              type: 'mrkdwn',
              text: steps.map(s => `${s.success ? '✅' : '❌'} ${s.name}`).join('\n')
            }
          }
        ]
      })
...
async function executeRollback(config: RollbackConfig): Promise<RollbackResult> {
  console.log(`⚠️ Rolling back shutdown of ${config.serviceId}`)
...
async function verifyCleanup(serviceId: string): Promise<CleanupReport> {
  ...
  for (const v of verifications) {
    ...
    if (!passed) {
      console.warn(`⚠️ ${v.resourceType} not in expected state (${v.expectedState})`)
    }
  }
...
Decommissioning services requires more planning than building them. The scream test is the safest discovery mechanism—announce deprecation, add latency, introduce failures, then shut down, giving consumers time to identify themselves at each stage. Traffic analysis, distributed tracing, and database query logging help discover consumers before the scream test begins. Communication is critical: announce early, remind often, and provide clear migration paths. Never delete data without verified archives and retention period compliance. The ROI of decommissioning is substantial—not just in direct cost savings but in reduced security surface, compliance scope, and operational overhead. Document every decommissioning for the team that does the next one.
