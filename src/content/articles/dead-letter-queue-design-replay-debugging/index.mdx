...
Messages fail for different reasons, and those reasons determine how you should handle them. <Highlighter>Most DLQs treat all failures the same - a message failed, it goes in the queue, someone will look at it eventually.</Highlighter> That "eventually" often becomes "never" because manual triage doesn't scale.
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Context required for a debuggable DLQ.",
    thead: { th: ["What You Need for Debugging", "Why It Matters"] },
    tbody: {
      tr: [
        {
          th: "Original payload (exact bytes)",
          td: ["Replay exactly what was sent, not a re-serialized interpretation"],
        },
        {
          th: "First failure, last failure, DLQ timestamps",
          td: ["Understand the failure timeline and retry behavior"],
        },
        {
          th: "Full attempt history with timing",
          td: ["See patterns across retries, not just the final error"],
        },
        {
          th: "Error type, message, stack trace per attempt",
          td: ["Debug without guessing what went wrong"],
        },
        {
          th: "Consumer version and host",
          td: ["Know which code version failed and where"],
        },
        {
          th: "Correlation ID, trace ID",
          td: ["Connect to distributed traces and related requests"],
        },
        {
          th: "Failure classification",
          td: ["Enable automated routing and alerting"],
        },
        {
          th: "Operational state",
          td: ["Track triage progress, avoid duplicate investigation"],
        },
      ],
    },
  }}
