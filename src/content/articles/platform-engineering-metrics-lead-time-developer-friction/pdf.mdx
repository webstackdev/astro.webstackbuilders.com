...
<Table
  ...
  content={{
    thead: {
      th: ['Metric', 'Poor', 'Acceptable', 'Excellent'],
    },
    tbody: {
      tr: [
        {
          th: 'Time to first deployment',
          td: ['> 2 weeks', '1-5 days', '< 1 day'],
        },
        {
          th: 'Time to new service',
          td: ['> 1 week', '1-24 hours', '< 1 hour'],
        },
        {
          th: 'Time to new environment',
          td: ['> 3 days', '1-4 hours', '< 15 minutes'],
        },
        {
          th: 'Deployment lead time',
          td: ['> 1 day', '1-4 hours', '< 15 minutes'],
        },
      ],
    },
    figure: 'Lead time targets by maturity level.',
  }}
/>

These metrics align with DORA's[^dora] research-backed framework, which identified deployment frequency, lead time for changes, change failure rate, and time to restore service as the four key predictors of software delivery performance. DORA's annual State of DevOps reports consistently show that elite performers deploy multiple times per day with lead times under an hour — putting concrete numbers behind what "good" looks like at each maturity level.
...
<Diagram
  src={leadTimeDiagram}
  size="full"
  title="Lead time improvement visualization — 12 days reduced to 5 hours."
  description="Gantt chart titled Time to First Deployment Before versus After Platform. The chart compares two timelines. In the Before Platform section, Account setup takes 2 days, Request infrastructure takes 3 more days, Wait for provisioning takes 5 days, Configure access takes 2 days, and then First deployment appears as a milestone, for a total of roughly 12 days. In the After Platform section, Self-service onboarding takes 4 hours, Auto-provisioned environment takes 1 more hour, and then First deployment appears as a milestone, for a total of roughly 5 hours. The diagram shows how platform automation compresses a multi-day onboarding and provisioning sequence into a same-day self-service path."
/>
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Measures what percentage of common tasks developers complete without human intervention. Track portal/API completions against tickets for the same task types. If developers can provision an environment through the portal but 40% still file tickets, something\'s wrong with the self-service experience.',
    },
    {
      lead: 'Cognitive load',
...
<Table
  ...
  content={{
    thead: {
      th: ['Metric', 'What It Indicates', 'Data Source'],
    },
    tbody: {
      tr: [
        {
          th: 'Tickets/developer/month',
          td: ['Support burden', 'Ticketing system'],
        },
        {
          th: 'Self-service rate',
          td: ['Automation effectiveness', 'Portal analytics'],
        },
        {
          th: 'Time to first deploy',
          td: ['Onboarding friction', 'HR + Deployment logs'],
        },
        {
          th: 'Failed deployments',
          td: ['Platform reliability', 'CI/CD system'],
        },
        {
          th: 'Rollback frequency',
          td: ['Change confidence', 'Deployment system'],
        },
      ],
    },
    figure: 'Core friction metrics and their sources.',
  }}
/>
...
<Diagram
  src={metricsDiagram}
  size="full"
  title="Metrics data flow from source systems to dashboards."
  description="Top-to-bottom flowchart with four grouped stages: Data Sources, Collection, Storage, and Presentation. The Data Sources group contains Deployment System, Ticketing System, Platform Portal, and HR System. All four sources feed into an Event Stream in the Collection group, which then feeds into Enrichment. From Enrichment, data flows to two storage targets: Raw Events and Aggregated Metrics. Aggregated Metrics then feed three presentation outputs: Dashboards, Reports, and Alerts. The diagram shows a metrics pipeline where operational and business events are collected from multiple systems, enriched with metadata, stored in both raw and summarized forms, and then exposed for visualization, reporting, and alerting."
/>
...
<Table
  ...
  fullWidth={false}
  classes={{
    thead: "text-primary-offset",
  }}
  content={{
    thead: {
      th: ['Survey Type', 'Frequency', 'Length', 'Response Goal'],
    },
    tbody: {
      tr: [
        {
          th: 'Onboarding',
          td: ['Once (30 days)', '5 min', '> 80%'],
        },
        {
          th: 'Quarterly pulse',
          td: ['Quarterly', '3 min', '> 60%'],
        },
        {
          th: 'Feature feedback',
          td: ['After use', '2 min', '> 40%'],
        },
        {
          th: 'Annual deep-dive',
          td: ['Yearly', '15 min', '> 50%'],
    ...
    figure: 'Survey cadence and response targets.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Vanity Metric', "Why It's Misleading", 'Actionable Alternative'],
    },
    tbody: {
      tr: [
        {
          th: '"500 developers use the platform"',
          td: [
            "Doesn't indicate if they're productive",
            'Time to first deployment for new developers',
          ],
        },
        {
          th: '"10 million API calls/month"',
          td: [
            'Could be inefficient polling or errors',
            'Successful self-service completions',
          ],
        },
        {
          th: '"99.99% uptime"',
          td: [
            'Available but unusable is still failure',
            'Successful deployment rate, P99 latency',
          ],
        },
        {
          th: '"1000 deployments this month"',
          td: [
            'Includes failures, rollbacks, test envs',
            'Production deployment success rate',
          ],
        },
        {
          th: '"500 documentation pages"',
          td: ['Quantity ≠ quality or findability', 'Search success rate, ticket deflection'],
        },
      ],
    },
    figure: 'Vanity metrics and their actionable alternatives.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Report', 'Audience', 'Frequency', 'Key Question'],
    },
    tbody: {
      tr: [
        {
          th: 'Weekly digest',
          td: ['Platform team', 'Weekly', '"What happened this week?"'],
        },
        {
          th: 'Monthly review',
          td: ['Eng leadership', 'Monthly', '"Are we improving?"'],
        },
        {
          th: 'Quarterly business review',
          td: ['Executives', 'Quarterly', '"Is platform investment paying off?"'],
        },
        {
          th: 'Annual report',
          td: ['Organization', 'Yearly', '"What value did platform deliver?"'],
        },
      ],
    },
    figure: 'Reporting cadence by audience.',
  }}
/>
