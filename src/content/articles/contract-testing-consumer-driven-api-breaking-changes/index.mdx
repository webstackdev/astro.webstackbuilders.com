...
const matcherExamples = {
  ...
  userId: like('abc-123'),
  // ✅ Matches: "xyz-789", "user-1"
  // ❌ Fails: 123, null, undefined
  ...
  // ✅ Matches: "pending", "approved", "rejected"
  // ❌ Fails: "Pending", "APPROVED", "cancelled"
...
sequenceDiagram
    ...
    P->>PB: Can I deploy?
    PB->>P: ✅ Yes (all consumers compatible)
...
graph TD
    ...
    D -->|Yes| E[✅ Safe to deploy]
    D -->|No| F[❌ Breaking change detected]
...
// ❌ BAD: Over-specified - brittle contract
await provider
...
// ✅ GOOD: Properly specified - flexible contract
await provider
  .uponReceiving('get user')
  .withRequest({
    method: 'GET',
