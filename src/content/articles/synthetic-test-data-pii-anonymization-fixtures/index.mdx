...
<Table
  ...
  content={{
    figure: 'Common PII exposure scenarios in non-production',
    thead: {
      th: ['Risk', 'Consequence', 'Real-World Example'],
    },
    tbody: {
      tr: [
        {
          th: 'Data breach in staging',
          td: ['GDPR fines up to €20M or 4% revenue', 'Staging DB exposed via misconfigured security group'],
        },
        {
          th: 'Developer laptop theft',
          td: ['Customer data compromise, notification requirements', 'Laptop with local DB copy stolen from car'],
        },
        {
          th: 'Log aggregation exposure',
          td: ['PII in logs indexed by third-party service', 'Customer names in error messages sent to Datadog'],
        },
        {
          th: 'Contractor access',
          td: ['PII exposure to untrusted parties', 'Offshore QA team with full production clone'],
        },
        {
          th: 'Backup retention',
          td: ['Production data in test backups subject to retention', 'Test DB backup stored for years, breached'],
        },
        {
          th: 'Compliance audit failure',
          td: ['Failed SOC2/HIPAA audit, lost enterprise deals', 'Auditor finds real SSNs in staging environment'],
        },
      ],
    },
  }}
...
<Table
  variant="numbered-review-table"
  content={{
    figure: 'Anonymization techniques by field type',
    thead: {
      th: ['Field Type', 'Technique', 'Example', "What's Preserved"],
    },
    tbody: {
      tr: [
        {
          th: 'Email',
          td: ['Format-preserving hash', 'john.doe@company.com → user_8f3k2@example.com', 'Format validity, uniqueness'],
        },
        {
          th: 'Name',
          td: ['Consistent fake replacement', 'John Doe → Michael Smith', 'Linguistic plausibility'],
        },
        {
          th: 'Phone',
          td: ['Format-preserving randomization', '555-123-4567 → 555-987-6543', 'Format, area code patterns'],
        },
        {
          th: 'Address',
          td: ['Generalization + fake details', '123 Main St, Springfield IL → 456 Oak Ave, Springfield IL', 'Geographic region'],
        },
        {
          th: 'Date of Birth',
          td: ['Bucketing', '1985-03-15 → 1985-01-01', 'Age range, decade'],
        },
        {
          th: 'SSN/ID numbers',
          td: ['Complete replacement', '123-45-6789 → 987-65-4321', 'Format validity only'],
        },
        {
          th: 'Free text',
          td: ['NLP entity replacement', '"John called about his order" → "Customer called about their order"', 'Sentence structure'],
        },
      ],
    },
  }}
      ...
      children: [
        {
          type: 'folder',
          name: 'base',
          comment: 'Minimal data for any test run',
          children: [
        ...
        },
        {
          type: 'folder',
          name: 'scenarios',
          comment: 'Specific test case setups',
          children: [
            { type: 'folder', name: 'empty-cart' },
            { type: 'folder', name: 'checkout-flow' },
            { type: 'folder', name: 'edge-cases' },
          ],
        },
        {
          type: 'folder',
          name: 'factories',
          comment: 'Programmatic generation',
          children: [
            { type: 'file', name: 'user_factory.py' },
            { type: 'file', name: 'order_factory.py' },
          ],
        },
        {
          type: 'folder',
          name: 'seeds',
          comment: 'Environment-specific seeding',
          children: [
            { type: 'file', name: 'development.py' },
            { type: 'file', name: 'staging.py' },
            { type: 'file', name: 'e2e.py' },
          ],
        },
      ],
    },
...
<Table
  ...
  content={{
    thead: {
      th: ['Regulation', 'Test Data Requirement', 'Key Provision'],
    },
    tbody: {
      tr: [
        {
          th: 'GDPR',
          td: ['Cannot use personal data without consent', 'Article 5(1)(b) - Purpose limitation'],
        },
        {
          th: 'CCPA',
          td: ['Must honor opt-out for non-essential processing', '§1798.120 - Right to opt out'],
        },
        {
          th: 'HIPAA',
          td: ['PHI must be de-identified (Safe Harbor or Expert)', '§164.514 - De-identification standard'],
        },
        {
          th: 'PCI DSS',
          td: ['No real PANs in non-production', 'Requirement 6.4.3'],
        },
        {
          th: 'SOX',
          td: ['Data accuracy for financial testing', '§302 - Corporate responsibility'],
        },
        {
          th: 'FERPA',
          td: ['Student records must be de-identified', '§99.31 - Disclosure requirements'],
        },
      ],
    },
    figure: 'Regulatory requirements for test data',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'The policy document',
    },
    {
      lead: 'Technical controls enforcing it (scanners)',
    },
    {
      lead: 'Logs proving controls ran',
    },
    {
      lead: 'Evidence of periodic review',
    },
  ]}
/>
...
<Download
  resource="synthetic-test-data-pii-anonymization-fixtures"
  title="Test Data Without PII Leaks"
  description="Realistic fixtures for ephemeral environments that do not expose production data or violate privacy."
  listItems={[
    {
      text: "Schema-aware fixture generation workflow",
    },
    {
      text: "Deterministic anonymization implementation guide",
    },
    {
      text: "PII scanning CI policy pack",
    },
    {
      text: "Compliance audit evidence checklist",
    },
  ]}
/>
