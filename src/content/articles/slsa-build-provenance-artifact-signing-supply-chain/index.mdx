...
tags: ["build-and-deploy","docker","kubernetes"]
featured: true
---

import sbomDiagram from "./diagrams/sbom-and-provenance-relationship.jpg"
import supplyChainDiagram from "./diagrams/supply-chain-attack-points.jpg"
...
<Diagram
  src={supplyChainDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Supply chain attack points."
  description="A left-to-right flowchart showing the main stages of a software supply chain: Developer, Source Repository, Build System, Artifact Registry, and Deployment. Solid arrows show the normal progression from development through deployment. Dashed arrows mark five attack opportunities: compromised developer credentials targeting the source repository, malicious commits moving from source into the build, dependency poisoning affecting the build system itself, build tampering between the build and the artifact registry, and artifact modification between the registry and deployment. The diagram emphasizes that every transition in the chain is a trust boundary that can be attacked."
/>
...
<Table
  ...
  content={{
    ...
    thead: {
      th: ['SLSA Addresses', 'SLSA Does NOT Address'],
    },
    tbody: {
      tr: [
        {
          th: 'Unauthorized source changes',
          td: ['Bugs in legitimate code'],
        },
        {
          th: 'Build tampering',
          td: ['Vulnerabilities in dependencies'],
        },
        {
          th: 'Missing provenance',
          td: ['Social engineering'],
        },
        {
          th: 'Non-reproducible builds',
          td: ['Compromised developer machines'],
        },
        {
          th: 'Unsigned artifacts',
          td: ['Insider threats with legitimate access'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    ...
    thead: {
      th: ['Level', 'Source Requirements', 'Build Requirements', 'Provenance', 'Effort', 'Protects Against'],
    },
    tbody: {
      tr: [
        {
          th: '1',
          td: ['Version controlled', 'Scripted build', 'Exists (unsigned OK)', 'Hours', 'Manual build errors'],
        },
        {
          th: '2',
          td: ['Version controlled with history', 'Hosted build service', 'Authenticated by builder', 'Days', 'External attackers'],
        },
        {
          th: '3',
          td: ['Two-person review, retained history', 'Hardened, isolated environment', 'Non-falsifiable', 'Weeks', 'Insider threats'],
        },
        {
          th: '4',
          td: ['Two-person review, verified history', 'Hermetic, reproducible', 'Dependencies have provenance', 'Months', 'Sophisticated attacks'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    ...
    thead: {
      th: ['Method', 'Key Management', 'Verification', 'CI/CD Fit', 'Compliance'],
    },
    tbody: {
      tr: [
        {
          th: 'GPG/PGP',
          td: ['High (manual rotation, distribution)', 'Medium (keyservers, TOFU)', 'Poor', 'Medium'],
        },
        {
          th: 'Sigstore/Cosign',
          td: ['None (identity-based)', 'Low (transparency log)', 'Excellent', 'Good'],
        },
        {
          th: 'KMS-backed',
          td: ['Low (provider-managed)', 'Low (exported public key)', 'Good', 'Excellent'],
        },
      ],
    },
  }}
...
<Diagram
  src={sbomDiagram}
  size="full"
  title="SBOM and provenance relationship."
  description="A top-to-bottom flowchart showing how an image, its SBOM, and its provenance are connected. In the first grouped section, Source Code produces a Container Image. That image then leads to an SBOM, labeled as what is inside, and to Provenance, labeled as how it was built. In the second grouped section, the Container Image produces an Image Signature through the image digest, while the SBOM produces an SBOM Attestation and the Provenance produces a Provenance Attestation. All three signed outputs then flow into the Registry. The diagram shows that the image digest ties the image, SBOM, and provenance together so each can be verified independently."
/>
...
<List
  ...
  items={[
    {
      lead: 'Inventory your artifacts.',
      text: 'List every container image, binary, and package your team publishes. Include internal artifacts, not just external releases. Note which registries they live in and how they are currently tagged.',
    },
    {
      lead: 'Add basic provenance.',
      text: 'Even before formal SLSA provenance, you can embed build metadata. Include the git SHA in your image tags or labels. Add build timestamps. This gives you traceability for debugging even if it is not cryptographically verified.',
    },
    {
      lead: 'Document your build process.',
      text: 'Write down how builds work today-the scripts, the CI system, the dependencies. You will need this baseline to identify gaps and measure improvement.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Start with staging.',
      text: 'Add cosign verify to your staging deployment pipeline. Configure it to warn on failure initially-you want to catch unsigned images without blocking deployments while you are still stabilizing.',
    },
    {
      lead: 'Monitor success rates.',
      text: 'Track how often verification passes. If it is not 100%, you have signing gaps to fix before enforcing in production.',
    },
    {
      lead: 'Deploy admission control.',
      text: 'For Kubernetes, install the Sigstore Policy Controller and create a ClusterImagePolicy. Start with a non-production namespace, then expand to staging, then production.',
    },
    {
      lead: 'Add CI gates.',
      text: 'For non-Kubernetes deployments, add verification steps to your deployment workflows. Block promotion if verification fails.',
    },
  ]}
/>
...
<Table
  variant="numbered-review-table"
  content={{
    figure: 'Implementation milestones.',
    thead: {
      th: ['Milestone', 'Requirements', 'Timeline'],
    },
    tbody: {
      tr: [
        {
          th: 'SLSA Level 2 complete',
          td: ['All artifacts signed, provenance from build service, verification enforced', '6-8 weeks'],
        },
        {
          th: 'SBOM integration',
          td: ['SBOM generated for containers, attested alongside images, vulnerability scanning', '10-12 weeks'],
        },
        {
          th: 'SLSA Level 3 (optional)',
          td: ['Two-person review, isolated builds, non-falsifiable provenance', '16-20 weeks'],
        },
      ],
    },
  }}
...
<Troubleshooter
  items={{
    symptoms: {
      items: [
        {
          text: '<span class="font-mono">cosign verify</span> returns an error, <span class="font-mono">admission</span> webhook denies pods, deployments blocked.',
        },
      ],
    },
    causes: {
      items: [
        {
          text: "The image was signed, but the signing identity doesn't match your verification policy. This is the most common issue.",
        },
      ],
    },
    diagnosis: {
      lead: 'Check what identity actually signed the image. Compare the certificate identity in the output with what your policy expects. Common mismatches:',
      items: [
        {
          text: 'You signed from <span class="font-mono">refs/heads/main</span> but the policy expects <span class="font-mono">refs/tags/v*</span>',
        },
        {
          text: 'You renamed or moved your workflow file',
        },
        {
          text: 'Policy expects GitHub but you are using GitLab',
        },
      ],
    },
    fixes: {
      items: [
        {
          text: 'Update either your signing workflow or your verification policy to match.',
        },
      ],
    },
  }}
...
<Troubleshooter
  items={{
    symptoms: {
      items: [
        {
          text: '"Error: unable to get OIDC token", keyless signing fails.',
        },
      ],
    },
    causes: {
      items: [
        {
          text: 'Missing id-token: write permission in your workflow.',
        },
      ],
    },
    diagnosis: {
      lead: 'Confirm that the workflow or job permissions are actually exposing an OIDC token to the step that runs Cosign. Common checks:',
      items: [
        {
          text: 'Verify the workflow includes <span class="font-mono">id-token: write</span> and that a job-level <span class="font-mono">permissions</span> block is not overriding it.',
        },
        {
          text: 'Make sure the signing step is running in the same job that has the OIDC permission.',
        },
        {
          text: 'If you recently tightened default repository permissions, confirm the workflow still has explicit package and contents access where needed.',
        },
      ],
    },
    fixes: {
      lead: 'Add the permission block to your workflow:',
      items: [],
    },
  }}
>
...
Code: GitHub Actions OIDC permissions block.
...
<Troubleshooter
  items={{
    symptoms: {
      items: [
        {
          text: '<span class="font-mono">slsa-verifier</span> fails, attestation download returns empty.',
        },
      ],
    },
    causes: {
      items: [
        {
          text: 'The provenance workflow did not run, failed silently, or the artifact digest was not passed correctly.',
        },
      ],
    },
    diagnosis: {
      items: [
        {
          text: 'Check the GitHub Actions run-did the provenance job complete successfully?',
        },
        {
          text: 'Verify you\'re using a pinned version of <span class="font-mono">slsa-github-generator</span> (not <span class="font-mono">@main</span>)',
        },
        {
          text: 'Confirm the artifact digest passed to the generator matches what you are verifying',
        },
      ],
    },
    fixes: {
      items: [
        {
          text: 'Most often, this is a digest mismatch. The digest output from your build step must exactly match what you pass to the provenance generator.',
        },
      ],
    },
  }}
...
<Troubleshooter
  items={{
    symptoms: {
      items: [
        {
          text: 'Build times increased significantly, SBOM action times out.',
        },
      ],
    },
    causes: {
      items: [
        {
          text: 'Large images with many layers take longer to analyze.',
        },
      ],
    },
    diagnosis: {
      lead: 'Before changing the workflow, confirm where the time is going. Useful checks:',
      items: [
        {
          text: 'Compare total image size and layer count between fast and slow builds to see whether image growth is driving scan time.',
        },
        {
          text: 'Check the workflow logs to confirm whether the delay is in image download, unpacking, SBOM generation, or attestation upload.',
        },
        {
          text: 'Verify whether the SBOM job is re-building or re-pulling artifacts unnecessarily instead of working from the image digest.',
        },
      ],
    },
    fixes: {
      ...
      items: [
        {
          text: 'Use multi-stage builds to reduce final image size',
        },
        {
          text: 'Run SBOM generation in a parallel job (it only needs the image digest, not the build context)',
        },
        {
          text: 'For very large images, consider generating SBOM from lockfiles during build rather than scanning the final image',
        },
      ],
    },
  }}
...
<Download
  resource="slsa-build-provenance-artifact-signing-supply-chain"
  title="Build Provenance and Signing: A Practical Baseline"
  description="Supply chain security basics that you can implement without a dedicated security team or expensive tooling."
  listItems={[
    {
      text: "Keyless signing setup checklist",
    },
    {
      text: "Provenance verification policy examples",
    },
    {
      text: "Admission enforcement rollout plan",
    },
    {
      text: "SBOM and scan integration guide",
    },
  ]}
/>
