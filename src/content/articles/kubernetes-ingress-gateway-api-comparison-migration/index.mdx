...
<Diagram
  src={ingressDiagram}
  size="full"
  title="Ingress uses flat resources - Gateway API uses a shared Gateway."
  description="Flowchart comparing the structural models of Ingress and Gateway API. On the left, a grouped section labeled Ingress Model: Single Resource shows three separate Ingress resources, each pointing to a single Controller, and that Controller pointing to a Service. This illustrates that Ingress resources are flat and controller behavior is centralized behind one implementation. On the right, a grouped section labeled Gateway API: Layered Resources shows GatewayClass pointing to Gateway, then Gateway pointing to three route resources: HTTPRoute Team A, HTTPRoute Team B, and GRPCRoute Team C. Each route then points to its own backend service: Service A, Service B, and Service C. The diagram emphasizes that Gateway API splits responsibilities into layers and allows multiple teams and route types to share a common Gateway while still routing to separate services."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Aspect', 'Ingress', 'Gateway API'],
    },
    tbody: {
      tr: [
        {
          th: 'Resource types',
          td: ['1 (Ingress)', '3+ (GatewayClass, Gateway, *Route)'],
        },
        {
          th: 'Configuration',
          td: ['Annotations (not portable)', 'Native fields (portable)'],
        },
        {
          th: 'Namespace scope',
          td: ['Same namespace only', 'Cross-namespace supported'],
        },
        {
          th: 'Protocols',
          td: ['HTTP/HTTPS', 'HTTP, HTTPS, TCP, UDP, gRPC, TLS'],
        },
        {
          th: 'Traffic splitting',
          td: ['Annotation-dependent', 'Native support'],
        },
        {
          th: 'Header manipulation',
          td: ['Annotation-dependent', 'Native filters'],
        },
        {
          th: 'Role separation',
          td: ['No', 'Yes (infra/platform/app)'],
        },
      ],
    },
    figure: 'Ingress vs Gateway API comparison.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Capability', 'Ingress', 'Gateway API'],
    },
    tbody: {
      tr: [
        {
          th: 'Path matching',
          td: ['Exact, Prefix, ImplementationSpecific', 'Exact, PathPrefix, RegularExpression'],
        },
        {
          th: 'Header matching',
          td: ['Annotation-dependent', 'Native (`matches.headers`)'],
        },
        {
          th: 'Query parameter matching',
          td: ['Not supported', 'Native (`matches.queryParams`)'],
        },
        {
          th: 'HTTP method matching',
          td: ['Not supported', 'Native (`matches.method`)'],
        },
      ],
    },
    figure: 'Routing capability comparison.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Protocol', 'Ingress', 'Gateway API', 'Status'],
    },
    tbody: {
      tr: [
        {
          th: 'HTTP',
          td: ['<span class="text-success">✓</span> Native', '<span class="text-success">✓</span> HTTPRoute', 'GA'],
        },
        {
          th: 'HTTPS (terminate)',
          td: ['<span class="text-success">✓</span> Native', '<span class="text-success">✓</span> HTTPRoute + TLS', 'GA'],
        },
        {
          th: 'HTTPS (passthrough)',
          td: ['Annotation-dependent', '<span class="text-success">✓</span> TLSRoute', 'GA'],
        },
        {
          th: 'TCP',
          td: ['Not supported', '<span class="text-success">✓</span> TCPRoute', 'Beta'],
        },
        {
          th: 'UDP',
          td: ['Not supported', '<span class="text-success">✓</span> UDPRoute', 'Alpha'],
        },
        {
          th: 'gRPC',
          td: ['Annotation-dependent', '<span class="text-success">✓</span> GRPCRoute', 'GA'],
        },
      ],
    },
    figure:
      'Protocol support comparison. Status column reflects Gateway API specification maturity (GA/Beta/Alpha), not individual controller implementation status.',
  }}
/>
  ...
  items={[
    {
      ...
      text: 'Manages GatewayClass resources. They select and configure the controller, set cluster-wide policies, and typically have cluster-admin access. This layer changes rarely — maybe during major upgrades or when adding new controller capabilities.',
    },
    {
      lead: 'Platform team',
      text: 'Manages Gateway resources. They configure listeners, TLS certificates, and crucially, which namespaces can attach routes. They control the blast radius of application team changes through the `allowedRoutes` configuration.',
    },
    {
      lead: 'Application teams',
      text: 'Manage HTTPRoute resources in their own namespaces. They define their routing rules, but can only attach to Gateways that explicitly allow their namespace. They can\'t affect other teams\' traffic.',
    },
  ]}
/>
...
<Diagram
  src={roleBasedDiagram}
  size="full"
  title="Role-based ownership with Gateway API."
  description="Flowchart showing how Gateway API maps routing resources to different team responsibilities. A grouped section labeled Infrastructure Team contains GatewayClass pointing to Gateway Controller through a controllerName relationship. A grouped section labeled Platform Team contains Gateway pointing back to GatewayClass through gatewayClassName and also pointing to a Namespace Selector through allowedRoutes. A grouped section labeled App Team A contains HTTPRoute A pointing to Gateway through parentRef and also pointing to Service A. A grouped section labeled App Team B contains HTTPRoute B pointing to Gateway through parentRef and also pointing to Service B. The Namespace Selector points to HTTPRoute A and HTTPRoute B through label match relationships. The diagram shows that infrastructure teams choose the controller, platform teams manage the shared Gateway and define which namespaces may attach, and application teams manage their own routes and services only when their namespaces satisfy the selector rules."
/>
...
<List
  ...
  items={[
    {
      lead: 'You\'re in the middle of a major release or incident stabilization period',
    },
    {
      lead: 'The team responsible for routing config doesn\'t have capacity for learning and testing',
    },
    {
      lead: 'Your controller\'s Gateway API support is still in beta or missing route types you need',
    },
    {
      lead: 'You have a complex <span class="font-mono">configuration-snippet</span> annotation that doesn\'t have a clear Gateway API equivalent',
    },
    {
      lead: 'Your CI / CD pipeline doesn\'t have good staging coverage for routing changes',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Controller supports Gateway API at GA level for the route types you need — most major controllers (NGINX Gateway Fabric, Envoy Gateway, Istio, Contour, Kong, Traefik) have GA support',
    },
    {
      lead: 'Team has read the official Gateway API getting started guide',
    },
    {
      lead: 'Staging environment can run parallel Ingress and Gateway API resources',
    },
    {
      lead: 'Rollback plan documented (typically: delete HTTPRoute, traffic returns to Ingress)',
    },
    {
      lead: 'Monitoring covers both Ingress and Gateway API traffic paths',
    },
    {
      lead: 'At least one low-risk service identified for initial migration',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Phase 1: Parallel operation (2-4 weeks)',
      text: 'Deploy the GatewayClass and Gateway resources, but only create HTTPRoutes for new services. Your existing Ingress resources continue to handle existing traffic. This phase validates that your Gateway setup works without risking existing routes.',
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: 'Once all routes are migrated, route all traffic through Gateway. Keep Ingress resources in read-only mode (no new rules) while you monitor for issues. This is your last chance to catch problems before cleanup.',
    },
    {
...
<Table
  ...
  content={{
    thead: {
      th: ['Ingress pathType', 'HTTPRoute path.type', 'Notes'],
    },
    tbody: {
      tr: [
        {
          th: 'Exact',
          td: ['Exact', 'Direct mapping'],
        },
        {
          th: 'Prefix',
          td: ['PathPrefix', 'Direct mapping'],
        },
        {
          th: 'ImplementationSpecific',
          td: ['Varies', 'Check controller docs'],
        },
      ],
    },
    figure: 'Path type mapping between Ingress and HTTPRoute.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Rewrite annotations:',
      text: 'URLRewrite filter',
    },
    {
      lead: 'Header add/remove annotations:',
      text: 'RequestHeaderModifier / ResponseHeaderModifier filters',
    },
    {
      lead: 'Rate limiting annotations:',
      text: 'Policy attachment (controller-specific)',
    },
    {
      lead: 'Authentication annotations:',
      text: 'Policy attachment or external auth service',
    },
  ]}
/>
...
<Table
  variant="criteria-highlight-table"
  content={{
    figure: 'Controller Gateway API support.',
    thead: {
      th: ['Controller', 'Gateway API Status', 'Route Types', 'Notes'],
    },
    tbody: {
      tr: [
        {
          th: 'Envoy Gateway',
          td: ['GA', 'HTTP, gRPC, TCP, UDP, TLS', 'Purpose-built for Gateway API'],
        },
        {
          th: 'Istio',
          td: ['GA', 'HTTP, gRPC, TCP, TLS', 'Preferred over Istio Gateway'],
        },
        {
          th: 'Contour',
          td: ['GA', 'HTTP, gRPC, TLS', 'Mature implementation'],
        },
        {
          th: 'Kong',
          td: ['GA', 'HTTP, gRPC, TCP, UDP', 'Full plugin support via policy'],
        },
        {
          th: 'Traefik',
          td: ['GA', 'HTTP, gRPC, TCP, TLS', 'Middleware via policy'],
        },
        {
          th: 'NGINX Gateway Fabric',
          td: ['GA', 'HTTP, gRPC', 'Successor to nginx-ingress'],
        },
        {
          th: 'HAProxy',
          td: ['Beta', 'HTTP, TCP', 'Newer, less mature'],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Years in production',
          td: ['8+ years', '3+ years'],
        },
        {
          th: 'Stack Overflow questions',
          td: ['10,000+', '500+'],
        },
        {
          th: 'Battle-tested patterns',
          td: ['Extensive', 'Growing'],
        },
        {
          th: 'Breaking changes risk',
          td: ['Low (stable API)', 'Low (v1 is GA)'],
        },
        {
          th: 'Controller coverage',
          td: ['Universal', '~80% of major controllers'],
        },
      ],
    },
    figure: 'Ecosystem maturity comparison.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Your routing needs are simple: host-based routing, path prefixes, TLS termination',
    },
    {
      lead: 'Everything lives in one namespace (or a small number of namespaces managed by one team)',
    },
    {
      lead: 'Your current setup works and you don\'t have concrete problems to solve',
    },
    {
      lead: 'Your team doesn\'t have capacity to learn a new API right now',
    },
    {
      lead: 'Your controller doesn\'t support Gateway API yet',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'You need cross-namespace routing (app teams routing to a shared Gateway)',
    },
    {
      lead: 'You need native traffic splitting for canary or blue-green deployments',
    },
    {
      lead: 'You need TCP, UDP, or gRPC routing',
    },
    {
      lead: 'You\'re building a multi-team platform with self-service routing',
    },
    {
      lead: 'You\'re planning a controller migration anyway',
    },
    {
      lead: 'It\'s a greenfield project and you want to start with the more portable option',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Ingress',
      text: 'for legacy applications and simple internal services where migration effort isn\'t justified',
    },
    {
      lead: 'Gateway API',
      text: 'for new platform services and anything requiring advanced features',
    },
  ]}
/>
...
<Download
  resource="kubernetes-ingress-gateway-api-comparison-migration"
  title="Download the Ingress vs Gateway API Guide"
  description="Get the complete routing playbook for API selection, migration planning, and controller-compatible implementation patterns."
  listItems={[
    {
      text: "Routing capability comparison matrix",
    },
    {
      text: "Annotation migration mapping guide",
    },
    {
      text: "Controller support readiness checklist",
    },
    {
      text: "Phased migration execution plan",
    },
  ]}
/>
