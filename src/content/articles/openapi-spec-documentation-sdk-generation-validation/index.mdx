...
<List
  ...
  items={[
    {
      lead: '<span class="font-mono">info</span> for metadata',
    },
    {
      lead: '<span class="font-mono">servers</span> for environment URLs',
    },
    {
      lead: '<span class="font-mono">paths</span> for your actual endpoints',
    },
    {
      lead: '<span class="font-mono">components</span> for reusable definitions',
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: 'OpenAPI schema design patterns and when to use them',
    thead: {
      th: ['Pattern', 'When to Use'],
    },
    tbody: {
      tr: [
        {
          th: 'Separate CRUD schemas',
          td: ['Create/update/read operations have different field requirements'],
        },
        {
          th: '<span class="font-mono">allOf</span> composition',
          td: ['Multiple entities share identical base fields (audit timestamps, IDs)'],
        },
        {
          th: '<span class="font-mono">oneOf</span> polymorphism',
          td: ['A field can be one of several distinct types with different structures'],
        },
      ],
    },
  }}
...
When the spec is the source of truth, several good things happen. API design becomes a distinct activity that happens __before__ you start coding. You can share the spec with frontend teams, partner integrators, or product managers and get feedback before you've written a line of implementation code. Breaking changes become visible in the spec diff, not hidden in a code change that looks innocuous.
...
<Table
  ...
  content={{
    ...
    tbody: {
      tr: [
        {
          th: 'Swagger Editor Next',
          td: ['Browser-based editing with real-time validation; supports OpenAPI 3.1 and AsyncAPI'],
        },
        {
          th: 'Stoplight Studio',
          td: ['Visual, GUI-driven design without memorizing YAML syntax'],
        },
        {
          th: '42Crunch OpenAPI Editor (VS Code)',
          td: ['IDE-integrated editing with IntelliSense, visual navigation, and "Try it" testing'],
        },
        {
          th: 'IntelliJ OpenAPI Plugin',
          td: ['Teams already in JetBrains IDEs; can generate specs from existing controllers'],
        },
        {
          th: 'Postman',
          td: ['Teams already using Postman for testing who want spec editing in the same workspace'],
        },
      ],
    },
    figure: 'OpenAPI spec editors for design-first workflows.',
  }}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: 'Swagger UI',
      text: 'Provides interactive documentation where developers can make live requests against your API. The "Try it out" button is genuinely useful for exploration and debugging. The tradeoff: you need to think about authentication, CORS, and whether you want people hitting your production API from the docs.',
    },
    {
      lead: 'Stoplight Elements',
      text: 'A React component library for embedding documentation in an existing portal or documentation site. If you\'re building a developer portal with tutorials, guides, and API reference together, Elements lets you integrate the API docs natively rather than linking out to a separate site.',
    },
  ]}
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: 'Documentation elements that improve developer experience.',
    thead: {
      th: ['Documentation Element', 'What to Include'],
    },
    tbody: {
      tr: [
        {
          th: 'Operation description',
          td: ['Auth requirements, rate limits, caching behavior, business context'],
        },
        {
          th: 'Parameter descriptions',
          td: ['Format constraints, valid ranges, default behavior'],
        },
        {
          th: 'Response examples',
          td: ['Multiple scenarios (success variants, common errors)'],
        },
        {
          th: 'Schema descriptions',
          td: ['Field meanings, valid values, relationships to other entities'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'OpenAPI Generator (the open-source fork of Swagger Codegen)',
      text: 'Supports 50+ languages and is the default choice for teams that need SDKs in multiple languages from the same spec. The TypeScript output is decent, the Python output is usable, and you get consistent behavior across languages. The tradeoff is that "decent" and "usable" aren\'t "great"—the generated code tends toward verbosity and doesn\'t always follow language idioms.',
    },
    {
      lead: 'Orval',
      text: 'Focuses exclusively on TypeScript but produces notably better output. It generates React Query hooks, Zod validation schemas, and MSW mock handlers out of the box. If your consumers are React developers, Orval\'s output feels like code they\'d actually write.',
    },
    {
      lead: '<span class="font-mono">openapi-typescript</span>',
      text: 'Takes a different approach: it generates only TypeScript types, no runtime code. You get interfaces for your request/response bodies and use your own HTTP client. This is the lightest-weight option and works well when you want type safety without adopting a generated SDK wholesale.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Generator', 'Languages', 'Best For'],
    },
    tbody: {
      tr: [
        {
          th: 'OpenAPI Generator',
          td: ['50+ (TypeScript, Python, Java, Go, etc.)', 'Multi-language SDK needs, polyglot organizations'],
        },
        {
          th: 'Orval',
          td: ['TypeScript only', 'React apps, teams wanting React Query/Zod integration'],
        },
        {
          th: 'openapi-typescript',
          td: ['TypeScript types only', 'Type safety without generated runtime code'],
        },
        {
          th: 'AutoRest',
          td: ['TypeScript, Python, Java, C#, Go', 'Azure integrations, Microsoft ecosystem'],
        },
      ],
    },
    figure: 'SDK generators and their sweet spots.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Framework', 'Library', 'Approach'],
    },
    tbody: {
      tr: [
        {
          th: 'Rails',
          td: ['<span class="font-mono">committee</span>', 'Rack middleware that validates against OpenAPI specs'],
        },
        {
          th: 'Laravel',
          td: ['<span class="font-mono">spectator</span>', 'Request/response validation with PHPUnit integration'],
        },
        {
          th: '.NET',
          td: ['<span class="font-mono">Microsoft.AspNetCore.OpenApi</span>', 'Built-in validation in ASP.NET Core 9+; earlier versions use <span class="font-mono">NSwag</span>'],
        },
        {
          th: 'Go',
          td: ['<span class="font-mono">kin-openapi</span>', 'Middleware for Echo, Chi, Gin; validates requests and responses'],
        },
        {
          th: 'Spring Boot',
          td: ['<span class="font-mono">springdoc-openapi</span>', 'Code-first with automatic spec generation and validation'],
        },
      ],
    },
    figure: 'OpenAPI validation libraries across frameworks.',
  }}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Validation Catches', 'Validation Misses'],
    },
    tbody: {
      tr: [
        {
          th: 'Wrong types (string instead of integer)',
          td: ['Valid types with invalid business meaning'],
        },
        {
          th: 'Missing required fields',
          td: ['Fields that are conditionally required'],
        },
        {
          th: 'Format violations (invalid UUID, email)',
          td: ["Valid formats that don't exist (nonexistent user ID)"],
        },
        {
          th: 'Out-of-range values (quantity < 1)',
          td: ["Domain-specific constraints (can't order discontinued items)"],
        },
        {
          th: 'Invalid enum values',
          td: ["Contextually invalid values (can't cancel a shipped order)"],
        },
      ],
    },
    figure: 'Schema validation boundaries.',
  }}
/>
...
<List
  ...
  items={[
    ...
    {
      text: 'Removed or renamed fields',
      color: "bg-danger-offset",
    },
    {
      text: 'New required fields in request bodies',
      color: "bg-danger-offset",
    },
    {
      text: 'Changed field types',
      color: "bg-danger-offset",
    },
    {
      text: 'Narrowed enum values',
      color: "bg-danger-offset",
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: 'Schemathesis',
      text: 'Takes a property-based testing approach. It generates random valid inputs based on your schemas and looks for edge cases that break your implementation. It\'s better at finding bugs but less predictable.',
    },
  ]}
/>
...
<Download
  resource="openapi-spec-documentation-sdk-generation-validation"
  title="OpenAPI in Practice: Docs, Clients, and Validation"
  description="Generating documentation, client SDKs, and request validation from OpenAPI specs without sprawling toolchains."
  listItems={[
    {
      text: "Spec-first rollout checklist",
    },
    {
      text: "Validation middleware integration guide",
    },
    {
      text: "SDK generation pipeline template",
    },
    {
      text: "Breaking-change CI enforcement rules",
    },
  ]}
/>
