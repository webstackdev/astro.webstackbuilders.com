...
Having an efficient development setup makes spending time doing what I 💖 that much better. One side is bricks and mortar: a robust workstation, a large main monitor in a dual monitor setup, an ergonomic keyboard and mouse, a comfortable chair, and an airy and pleasant work environment.

The software side for me is focused on efficiency. I spend most of my time split between a code editor and browsers. I spent my first years writing code using Eclipse. It had good support for the languages I was using, a convenient extension API, and worked cross-platform. But a friend from my university days who worked for a Microsoft shop told me how much I was missing out by not using Visual Studio. Around that time, Redmond released VS Code.
...
For a consistent developer experience (DX), VS Code is fairly restrictive on what parts of the editor UI an extension can modify. The main Menu Bar (with drop-down items like `File`, `Edit`, and `Selection`) is mostly off limits. Extensions usually provide access to their functionality in a few ways:
