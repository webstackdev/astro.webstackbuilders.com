...
<Diagram
  src={documentationAccuracyDiagram}
  title="Documentation accuracy decay in a typical legacy system."
  description="Line chart titled Documentation Accuracy Over Time. The horizontal axis shows time points labeled Launch, 6 months, 1 year, 2 years, 3 years, 5 years, 7 years, and 10 years. The vertical axis is Accuracy percent from 0 to 100. A single line starts at 100 percent at launch, then declines to 95 percent at 6 months, 80 percent at 1 year, 65 percent at 2 years, 50 percent at 3 years, 35 percent at 5 years, 20 percent at 7 years, and 10 percent at 10 years. The chart illustrates a steady erosion in documentation accuracy as the system ages, with a notable drop after the first year and severe inaccuracy by years five through ten."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Category', 'Risk Level', 'Remediation Approach'],
    },
    tbody: {
      tr: [
        {
          th: 'Missing',
          td: ['Medium', 'Discovery through code analysis and runtime observation'],
        },
        {
          th: 'Outdated',
          td: ['High', 'Verification against running system before use'],
        },
        {
          th: 'Actively Wrong',
          td: ['Critical', 'Delete and rediscover; unlearning is harder than learning'],
        },
        {
          th: 'Tribal Knowledge',
          td: ['High', 'Structured interviews and knowledge extraction sessions'],
        },
        {
          th: 'Scattered',
          td: ['Medium', 'Consolidation and single-source-of-truth establishment'],
        },
      ],
    },
    figure: 'Documentation debt categories and remediation strategies.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Location',
      text: 'Where does this doc live? (Wiki, repo, Google Drive, someone\'s desktop)',
    },
    {
      lead: 'Last modified',
      text: 'When was the file last changed?',
    },
    {
      lead: 'Last verified',
      text: 'When did someone confirm this matches reality?',
    },
    {
      lead: 'Coverage areas',
      text: 'What does this doc claim to cover?',
    },
    {
      lead: 'Known gaps',
      text: 'What\'s explicitly missing?',
    },
    {
      lead: 'Owner',
      text: 'Who\'s responsible for keeping this current?',
    },
  ]}
/>
...
<List
  ...
  classes={{
    wrapper: "sm:ml-12 sm:mr-12 mb-6",
  }}
  items={[
    {
      text: 'What depends on this module I\'m about to change?',
      color: 'bg-secondary',
    },
    {
      text: 'Which modules have the most inbound dependencies (likely core abstractions)?',
      color: 'bg-accent',
    },
    {
      text: 'Which modules have the most outbound dependencies (likely orchestration layers)?',
      color: 'bg-info',
    },
    {
      text: 'Are there unexpected dependencies between domains that should be isolated?',
...
<List
  ...
  items={[
    {
      lead: 'Endpoint inventory',
      text: 'Which routes actually receive traffic versus which ones are documented',
    },
    {
      lead: 'Traffic patterns',
      text: 'Which endpoints are called together (often reveals workflows)',
    },
    {
      lead: 'Dependency map',
      text: 'Which downstream services get called for each inbound request',
    },
    {
      lead: 'Error patterns',
      text: 'Which endpoints fail and under what conditions',
    },
  ]}
/>
...
<Diagram
  src={eventFlowDiagram}
  magnifyButtonPosition="bottom-left"
  size="full"
  title="Event flow topology discovered through runtime observation."
  description="Left-to-right flowchart with three groups: Producers, Topics, and Consumers. In Producers, Order Service and Payment Service appear as separate source systems. In Topics, there are two event channels named order-events and payment-events. In Consumers, there are three downstream services: Notification Service, Analytics Service, and Fulfillment Service. Order Service publishes to order-events. Payment Service publishes to payment-events. The order-events topic is consumed by Notification Service, Analytics Service, and Fulfillment Service. The payment-events topic is consumed by Notification Service and Analytics Service. The diagram shows how runtime observation can reveal which services publish events, which topics carry them, and which downstream services depend on each stream."
/>
...
<Diagram
  src={serviceTopologyDiagram}
  magnifyButtonPosition="top-left"
  title="Service topology diagram generated from tracing data."
  description="Top-to-bottom flowchart divided into three groups: External, Services, and Data. In External, a Load Balancer sits at the entry point. In Services, the Load Balancer sends traffic to an API Gateway. The API Gateway then calls Auth Service, Order Service, and Inventory Service. In Data, PostgreSQL, Redis, and Kafka appear as shared infrastructure components. Order Service writes to PostgreSQL and Kafka. Inventory Service writes to PostgreSQL and Redis. Auth Service uses Redis. Kafka delivers events to Notification Service. The diagram represents a service topology reconstructed from tracing data, showing both synchronous service-to-service calls from the gateway and downstream dependencies on databases, caches, and messaging infrastructure."
/>
...
<List
  ...
  classes={{
    wrapper: "sm:ml-6 sm:mr-12 mb-6",
    em: "!font-normal !italic",
  }}
  items={[
    {
      lead: '"Walk me through what happens when a user places an order."',
    },
    {
      lead: '"What would break if the payment service went down?"',
    },
    {
      lead: '"What\'s the scariest part of this codebase to change?"',
    },
    {
      lead: '"Why did we choose Kafka instead of SQS?"',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: '"What\'s the most common thing that pages you at night?"',
    },
    {
      lead: '"What manual steps are required for deployments?"',
    },
    {
      lead: '"What\'s the recovery procedure when the database fills up?"',
    },
    {
      lead: '"Where do you look first when debugging slow API responses?"',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: '"What major changes happened in the last two years?"',
    },
    {
      lead: '"Why does that weird retry loop exist in the payment handler?"',
    },
    {
      lead: '"What features were removed, and why?"',
    },
    {
      lead: '"What\'s the oldest code that\'s still running in production?"',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: '"What business rules are in the code that aren\'t written anywhere?"',
    },
    {
      lead: '"What edge cases have special handling?"',
    },
    {
      lead: '"What compliance requirements affect this code?"',
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Interview Focus', 'Best Participants', 'Artifacts to Request'],
    },
    tbody: {
      tr: [
        {
          th: 'Architecture',
          td: ['Original architects, senior engineers', 'Design docs, ADRs, old whiteboard photos'],
        },
        {
          th: 'Operations',
          td: ['SREs, on-call engineers', 'Runbooks, incident reports, dashboards'],
        },
        {
          th: 'Business Logic',
          td: [
            'Product managers, business analysts, subject matter experts',
            'Requirements docs, user stories, email threads',
          ],
        },
        {
          th: 'History',
          td: ['Long-tenured engineers', 'Old wiki pages, Slack threads, ticket comments'],
        },
      ],
    },
    figure: 'Interview types and their ideal participants.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'Preparation (before the session)',
      text: 'Identify 3-5 people with different perspectives on the system. Prepare a blank architecture template. Get permission to record (audio and whiteboard photos).',
    },
    {
      lead: 'Architecture walkthrough (45 minutes)',
      text: 'One person draws the system on the whiteboard while others correct and add detail. The disagreements are the most valuable part — they reveal where mental models diverge.',
    },
    {
      lead: 'Scenario walkthroughs (45 minutes)',
      text: 'Pick 3-4 common operations and trace them through the system. "What happens when a user submits an order?" Follow the data through every component. Note where people hesitate or disagree.',
    },
    {
      lead: 'Edge cases and gotchas (30 minutes)',
      text: 'This is where tribal knowledge surfaces. Ask: "What\'s the thing that always trips up new team members?" and "What\'s the hack we\'re ashamed of but can\'t remove?"',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Topic',
      text: 'What\'s the knowledge about?',
    },
    {
      lead: 'Category',
      text: 'Gotcha, workaround, historical context, undocumented feature, business rule, performance tip, or debugging tip',
    },
    {
      lead: 'Description',
      text: 'The actual knowledge, in enough detail to be useful',
    },
    {
      lead: 'Source',
      text: 'Who shared this? (Important for follow-up questions)',
    },
    {
      lead: 'Affected components',
      text: 'What parts of the system does this apply to?',
    },
    {
      lead: 'Verification status',
      text: 'Unverified, verified, or outdated',
    },
  ]}
/>
...
## Source: Jane Smith (Operations), January 2024

## Affected components: order-service, wms-sync-job

## Related tickets: OPS-1234, INCIDENT-567
```
...
<Download
  resource="reverse-engineering-documentation-legacy-systems"
  title="Documenting Undocumented Systems"
  description="Reverse-engineering architecture from running code when documentation is missing or wrong."
  listItems={[
    {
      text: "Code archaeology workflow template",
    },
    {
      text: "Characterization test capture guide",
    },
    {
      text: "Tribal knowledge interview framework",
    },
    {
      text: "Living docs CI validation",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'The dangerous parts',
      text: 'Code that everyone\'s afraid to change. These need documentation because mistakes here have outsized consequences.',
    },
    {
      lead: 'The confusing parts',
      text: 'Code that requires explanation to understand. If every new team member asks the same questions, write down the answers.',
    },
    {
      lead: 'The business-critical parts',
      text: 'Code where mistakes cost money or compliance violations. These need documentation because the stakes justify the investment.',
    },
  ]}
/>
