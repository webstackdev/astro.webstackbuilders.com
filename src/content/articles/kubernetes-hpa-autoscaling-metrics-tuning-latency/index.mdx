...
<Table
  ...
  content={{
    thead: {
      th: ['Delay Component', 'Typical Duration', 'Can Reduce?', 'How'],
    },
    tbody: {
      tr: [
        {
          th: 'Metrics scrape',
          td: ['15-60s', 'Yes', '`--metric-resolution` flag on Metrics Server'],
        },
        {
          th: 'HPA sync[^hpa-sync]',
          td: ['15s', 'Yes', '`--horizontal-pod-autoscaler-sync-period` flag'],
        },
        {
          th: 'Stabilization',
          td: ['0-300s', 'Yes', '`behavior.scaleUp.stabilizationWindowSeconds`'],
        },
        {
          th: 'Pod scheduling',
          td: ['1-10s', 'Limited', 'Pre-warmed nodes, pod priority'],
        },
        {
          th: 'Pod startup',
          td: ['5-60s+', 'Yes', 'Optimize application startup time'],
        },
        {
          th: 'Readiness probe',
          td: ['5-30s', 'Yes', 'Tune probe timing'],
        },
      ],
    },
    figure: 'HPA delay components and mitigation options.',
  }}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: "T+15s",
      text: "Metrics server captures the new load",
    },
    {
      lead: "T+30s",
    ...
    {
      lead: "T+45s",
      text: "Stabilization check passes (assuming aggressive config)",
    },
    {
      lead: "T+50s",
      text: "New pods scheduled, images pulled (cached)",
    },
    {
      lead: "T+80s",
...
<Table
  ...
  content={{
    thead: {
      th: ['Metric Type', 'Pros', 'Cons', 'Best For'],
    },
    tbody: {
      tr: [
        {
          th: 'CPU utilization',
          td: [
            'Built-in, no setup; predictable relationship to compute load',
            "Lags behind actual load; doesn't reflect I/O-bound work; misleading for async processing",
            'CPU-intensive APIs, compute workers, synchronous request processing',
          ],
        },
        {
          th: 'Memory utilization',
          td: [
            'Built-in; good for memory-intensive workloads',
            "Often doesn't correlate with traffic; stable regardless of load; slow to decrease",
            'In-memory caches, batch processing with large datasets',
          ],
        },
        {
          th: 'Requests per second',
          td: [
            'Directly measures load; leading indicator; application-agnostic',
            "Requires custom metrics setup; doesn't account for request complexity",
            'Web APIs, high-traffic services',
          ],
        },
        {
          th: 'Queue depth',
          td: [
            'Perfect for async workers; directly measures backlog; leading indicator',
            'Requires custom metrics; threshold needs tuning',
            'Queue consumers, background job processors',
          ],
        },
        {
          th: 'Response latency',
          td: [
            'Reflects user experience; catches overload before failure',
            'Lagging indicator; can be noisy; requires custom metrics',
            'SLO-driven scaling, user-facing services',
          ],
        },
      ],
    },
    figure: 'Metric types for HPA scaling.',
  }}
/>
...
<List
  ...
  classes={{
    header: "text-page-inverse",
    text: "text-content",
  }}
  items={[
    {
      title: "Resource metrics",
      text: "(CPU, memory): Built-in via Metrics Server. No setup required, but limited to what kubelets report.",
      icon: "graph",
      color: "success",
    ...
    {
      title: "Pod metrics",
      text: "Custom metrics exposed by your application and scraped by Prometheus. Queried per-pod, then averaged.",
      icon: "api",
      color: "info",
...
<List
  ...
  classes={{
    wrapper: "sm:mr-18 mb-12",
  }}
  items={[
    {
      lead: "Max:",
      text: "Use the policy that results in the largest change. For scale-up, this means adding the most pods. Good for aggressive scaling.",
    },
    {
      lead: "Min:",
      text: "Use the policy that results in the smallest change. For scale-down, this means removing the fewest pods. Good for conservative scaling.",
    },
    {
      lead: "Disabled:",
      text: "Don't scale in this direction at all.",
    },
  ]}
/>

<Diagram
  src={hpaPolicyDiagram}
  size="full"
  title="HPA policy selection flow."
  description="Flowchart showing how HPA decides whether and how to scale after a metric exceeds its target. The process starts at Metric exceeds target and moves to a decision asking whether the stabilization window has passed. If no, the flow goes to Wait and re-evaluate. If yes, the flow moves to Calculate replicas per each policy. From there, a decision asks for the selectPolicy setting. If the value is Max, the chart goes to Use largest increase. If the value is Min, it goes to Use smallest increase. If the value is Disabled, it goes to No scaling. The Max and Min branches both lead to Apply scale-up. The diagram shows that HPA does not act on raw metrics alone: it first waits for stabilization rules, then compares scaling policies, then applies the policy selected by the configuration."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Pattern', 'Use Case', 'Scale-Up', 'Scale-Down', 'Tradeoff'],
    },
    tbody: {
      tr: [
        {
          ...
          td: [
            ...
            '0s stabilization, 200%/15s, Max',
            '300s stabilization, 10%/60s, Min',
            'May over-provision, higher cost',
          ],
        },
        {
          th: 'Stable traffic',
          td: [
            'Business hours, scheduled jobs',
            '60s stabilization, 50%/60s, Max',
            '600s stabilization, 20%/120s, Min',
        ...
        },
        {
          th: 'Cost optimized',
          td: [
            'Batch processing, internal tools',
            '60s stabilization, 2 pods/60s, Min',
            '120s stabilization, 50%/60s, Max',
            'May degrade during spikes, lower cost',
          ],
        },
        {
          th: 'Never scale down',
          td: [
            'Always-on services',
            'Normal policies',
            '0 pods/60s (effectively disabled)',
            'Maximum availability, maximum cost',
          ],
        },
      ],
    },
    figure: 'HPA behavior patterns by traffic type.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: "Peak-to-trough ratio",
      text: "How much does traffic vary? A 3x ratio (business hours vs night) is manageable. A 20x ratio (flash sale) requires aggressive scaling.",
    },
    {
      lead: "Ramp-up time",
      text: "How fast does traffic increase? Gradual morning ramp-up over an hour is easy. A spike in 10 seconds is hard.",
    },
    {
      lead: "Predictability",
      text: "Do you know when traffic will increase? Scheduled events can be pre-scaled. Random viral content cannot.",
    },
    {
      lead: "Spike frequency",
      text: "Rare spikes justify aggressive over-provisioning. Frequent spikes need tighter cost control.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Traffic Type', 'Peak:Trough', 'Ramp-Up', 'Predictable', 'Recommended Strategy'],
    },
    tbody: {
      tr: [
        {
          th: 'Business hours',
          td: ['2-5x', '1-2 hours', 'Yes', 'Scheduled pre-scaling + moderate HPA'],
        },
        {
          th: 'Flash sales',
          td: ['10-50x', 'Seconds', 'Yes', 'Aggressive pre-scaling + fast HPA'],
        },
        {
          th: 'Viral content',
          td: ['5-100x', 'Minutes', 'No', 'High baseline + aggressive HPA'],
        },
        {
          th: 'Steady API',
          td: ['1.5-2x', 'Gradual', 'N/A', 'Standard HPA, optimize for cost'],
        },
      ],
    },
    figure: 'Traffic pattern characteristics and scaling strategies.',
  }}
/>
...
<List
  ...
  items={[
    {
      text: "Metrics Server not running or unhealthy",
      color: "bg-accent",
    },
    {
      text: "Pods missing resource requests (HPA can't calculate utilization without knowing the baseline)",
      color: "bg-secondary",
    },
    {
      text: "Custom metrics adapter not configured for non-CPU metrics",
      color: "bg-success",
    },
  ]}
/>
...
- __CPU metric__: 60% utilization, target 50% → $$5 \times (60/50) = 6$$ replicas
- __RPS metric__: 120 RPS/pod, target 100 → $$5 \times (120/100) = 6$$ replicas
- __Latency metric__: 400ms P99, target 500ms → $$5 \times (400/500) = 4$$ replicas
...
<List
  ...
  items={[
    ...
    },
    {
      ...
      text: "The asymmetric pattern fits most workloads: zero stabilization on scale up so you respond immediately, 5+ minute stabilization on scale down so you don't thrash. Over-provisioning costs money. Under-provisioning loses customers. When in doubt, err toward more capacity.",
    },
    {
