...
tags: ["cloud-platforms", "helm", "kubernetes", "python", "argo-cd", "prometheus"]
featured: true
---
...
<Diagram
  src={helmReleaseStateDiagram}
  size="full"
  title="Helm release state and drift flow"
  description="Flowchart showing Helm upgrade processing and where drift appears. The flow starts with helm upgrade, then renders templates, computes a diff against the cluster, and applies changes. A success decision stores either a successful or failed release in Kubernetes Secrets, which then form Helm release history with revisions v1, v2, and current v3. Separately, the cluster contains actual live resources. A dotted relationship indicates the current stored release should match the live cluster state. A manual kubectl edit points directly to the live cluster state to show how out-of-band changes introduce drift between Helm's stored revision history and the actual resources."
/>
...
<Table
  ...
  fullWidth={false}
  content={{
    figure: "Common Helm drift patterns, their frequency, severity, and recovery difficulty.",
    thead: {
      th: ["Drift Type", "Frequency", "Severity", "Recovery Difficulty"],
    },
    tbody: {
      tr: [
        {
          th: "Manual kubectl edit",
          td: ["High", "Medium", "Low"],
        },
        {
          th: "Partial upgrade failure",
          td: ["Medium", "High", "Medium"],
        },
        {
          th: "Secret storage corruption",
          td: ["Low", "Critical", "High"],
        },
        {
          th: "Hook failure orphans",
          td: ["Medium", "Medium", "Medium"],
        },
        {
          th: "Three-way merge conflicts",
          td: ["Low", "High", "High"],
        },
      ],
    },
  ...
  items={[
    {
      ...
      text: 'The most common source. Someone runs <span class="font-mono">kubectl edit deployment</span> to bump resource limits during an incident, or <span class="font-mono">kubectl patch</span> to add an annotation. The cluster state changes, but Helm doesn\'t know. The next <span class="font-mono">helm upgrade</span> may revert those changes unexpectedly, or the three-way merge may produce surprising results.',
    },
    {
      lead: "Partial upgrade failures",
      text: 'These leave you in limbo. If Helm times out or hits an error mid-upgrade, some resources may be updated while others aren\'t. You\'ll see pods running different versions, <span class="font-mono">ConfigMaps</span> updated but <span class="font-mono">Deployments</span> not, and a release status of "failed" that blocks further operations.',
    },
    {
      lead: "Secret storage corruption",
      text: 'Less common but more painful. If cleanup scripts, namespace recreation, or manual deletion removes Helm\'s release secrets, the cluster still has the resources but Helm has no record of them. <span class="font-mono">helm list</span> shows nothing, but <span class="font-mono">helm install</span> fails with "already exists."',
    },
    {
      lead: "Hook failures",
      text: "Creates orphaned state. Pre-upgrade or post-upgrade hooks that fail can leave partial resources. The release is marked failed, but the hook's job or pod may still exist, and main resources may or may not have been deployed depending on when the hook failed.",
    },
    {
      lead: "Three-way merge conflicts",
      text: "The trickiest. When Helm computes an upgrade, it compares the old manifest, new manifest, and live cluster state. If someone modified a field in the cluster that you're also changing in the new manifest, the merge can produce unexpected results — fields deleted, values unexpectedly retained, or strategic merge patches behaving counter-intuitively.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ["Metric", "Purpose", "Alert Threshold"],
    },
    tbody: {
      tr: [
        {
          th: "Release count by status",
          td: ["Track failed releases", "Any failed > 1h"],
        },
        {
          th: "Version distribution",
          td: ["Identify inconsistencies", "> 1 version per chart"],
        },
        {
          th: "Drift detected",
          td: ["Cluster/Helm divergence", "Any drift > 4h"],
        },
        {
          th: "Release age",
          td: ["Stale deployments", "> 30 days without update"],
        },
      ],
    },
  }}
...
<Diagram
  src={helmReleaseFailureDiagram}
  magnifyButtonPosition="top-left"
  size="full"
  title="Helm release failure diagnosis workflow"
  description="Flowchart for troubleshooting a failed Helm upgrade. The process begins with an upgrade failure, then checks helm status, helm history, and failed revision details before branching on the failure type. Hook failures lead to hook job/pod logs and then a decision on whether the hook is recoverable, resulting in either a fix-and-retry path or deletion of hook resources before retrying. Resource failures branch to checking resource events and whether the resource exists, followed by either kubectl describe for wrong state or admission webhook investigation for creation failures. Timeouts branch to pod status and then to image/registry checks, container log review, or node resource review depending on whether pods are in ImagePullBackOff, CrashLoopBackOff, or Pending. Validation errors go to rendering templates and running a server-side `--dry-run`."
/>
...
<Download
  resource="helm-release-management-drift-detection-debugging"
