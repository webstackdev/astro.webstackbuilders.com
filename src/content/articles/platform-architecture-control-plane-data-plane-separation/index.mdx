...
tags: ["platform-engineering", "kubernetes", "terraform", "grafana", "argo-cd", "aws", "backstage-idp"]
featured: true
---

import controlPlaneDiagram from "./diagrams/control-plane-pushes-desired-state-to-data-plane-data-plane-reports-actual-state-back_deep-dive.jpg"
import scalingProgressionDiagram from "./diagrams/scaling-progression-from-shared-infrastructure-to-regional-fleets.jpg"
  ...
  content={{
    ...
    tbody: {
      tr: [
        ...
        {
          th: 'Consistency needs',
          td: ['Strong consistency required', 'Eventually consistent often acceptable'],
        },
        {
          th: 'Scaling trigger',
        ...
        {
          th: 'Upgrade strategy',
          td: ['Careful, rolling, coordinated', 'Aggressive, canary, replaceable'],
        },
      ],
    },
...
<Diagram
  ...
  description="Top-to-bottom flowchart with two grouped sections: Control Plane and Data Plane. In the Control Plane, API Server connects to State Store and Controllers. Controllers connect to Policy Engine and Scheduler, showing that orchestration logic sits in the control layer. In the Data Plane, three worker nodes represent execution targets. Controllers send Desired State to Worker Node 1, Worker Node 2, and Worker Node 3. Each worker node sends Actual State back to Controllers. The diagram illustrates the core control loop: the control plane stores intent and policies, pushes desired state outward, and continuously receives observed state from the data plane to reconcile differences."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Layer', 'Primary Plane', 'Change Velocity', 'Scaling Factor'],
    },
    tbody: {
      tr: [
        {
          th: 'Infrastructure',
          td: ['Data', 'Slow', 'Capacity needs'],
        },
        {
          th: 'Orchestration',
          td: ['Both', 'Medium', 'Cluster count'],
        },
        {
          th: 'Platform Services',
          td: ['Control', 'Slow', 'Teams/tenants'],
        },
        {
          th: 'Developer Interface',
          td: ['Control', 'Fast', 'Feature velocity'],
        },
        {
          th: 'Applications',
          td: ['Data', 'Constant', 'Traffic/users'],
        },
      ],
    },
    figure: 'Platform layer characteristics.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: 'The infrastructure layer',
      text: 'Mostly data plane: compute, storage, and network resources that workloads consume. Cloud providers or on-premise hardware live here. Changes are slow (provisioning new capacity) and the scaling factor is raw capacity needs.',
    },
    {
      lead: 'The orchestration layer',
      text: 'Spans both planes. Kubernetes is the obvious example: the API server and controllers are control plane, while kubelets and container runtimes are data plane. This layer changes at medium velocity — cluster upgrades, new node pools — and scales with cluster count.',
    },
    {
      lead: 'The platform services layer',
      text: 'Primarily control plane: service mesh control planes, secrets management, certificate management, observability backends, CI/CD orchestration. These services manage configuration and policy for the layers below. Changes are slow (new platform capabilities) and scaling tracks team and tenant count.',
    },
    {
      lead: 'The developer interface layer',
      text: 'Pure control plane: the platform portal, self-service APIs, GitOps repositories, documentation and service catalogs. This is where developers interact with the platform. Changes are fast (feature velocity) and scaling depends on developer experience improvements.',
    },
    {
      lead: 'The application layer',
      text: 'Pure data plane: the actual workloads, databases, caches, message queues, and user-facing services that your platform exists to run. Changes are constant (deployments) and scaling tracks traffic and users.',
    },
  ]}
/>
...
>
  <ListItem lead="Network isolation">
    Prevents tenants from communicating unless explicitly allowed. Network policies default-deny
    cross-namespace traffic. Service mesh mTLS ensures traffic is encrypted and authenticated. For
    stronger isolation, separate VPCs[^vpc] or subnets provide network-level boundaries that don't
    depend on Kubernetes enforcement.
  </ListItem>

  <ListItem lead="Compute isolation">
    Prevents resource contention. Resource quotas limit how much CPU, memory, and storage a tenant can
    consume. For stronger isolation, dedicated node pools with taints ensure tenant workloads only run
    on designated nodes. At the extreme, dedicated clusters eliminate any compute sharing.
  </ListItem>

  <ListItem lead="Storage isolation">
    Protects data. Per-tenant storage classes can enforce encryption and access controls. Per-tenant
    encryption keys mean one tenant's compromise doesn't expose another's data. For the strongest
    isolation, completely separate storage backends.
  </ListItem>

  <ListItem lead="Identity isolation">
    Controls who can do what. RBAC rules scope permissions to specific namespaces. Service accounts are
    namespace-scoped by default. Integration with external identity providers (OIDC, LDAP) enables
...
<Table
  ...
  content={{
    thead: {
      th: ['Scale', 'Tenants', 'Resources', 'Architecture'],
    },
    tbody: {
      tr: [
        {
          th: 'Small',
          td: ['< 10', '< 1,000', 'Shared control plane'],
        },
        {
          th: 'Medium',
          td: ['10-100', '1,000-10,000', 'Dedicated control plane nodes, multiple controller replicas, sharded informers'],
        },
        {
          th: 'Large',
          td: ['100-1,000', '10,000-100,000', 'Regional control planes, federated configuration, per-tenant virtual clusters'],
        },
        {
          th: 'Enterprise',
          td: ['> 1,000', '> 100,000', 'Multi-cluster fleet, hierarchical management, dedicated clusters for large tenants'],
        },
      ],
    },
    figure: 'Control plane scaling progression.',
  }}
/>
...
<Diagram
  src={scalingProgressionDiagram}
  size="full"
  title="Scaling progression from shared infrastructure to regional fleets."
  description="Top-to-bottom flowchart showing three scaling stages. In the Small Scale group, a Shared Control Plane connects to a Shared Data Plane, representing a single combined environment. In the Medium Scale group, a Dedicated Control Plane connects to two separate node pools, showing how a single control layer can manage a segmented data plane within one cluster. In the Large Scale group, Regional Control Plane 1 manages Data Plane Cluster 1 and Data Plane Cluster 2, while Regional Control Plane 2 manages Data Plane Cluster 3 and Data Plane Cluster 4. The diagram shows an architectural progression from simple shared infrastructure, to segmented node pools, to multiple regional control planes managing separate fleets of data plane clusters as scale and geographic distribution increase."
/>
  ...
  items={[
    {
      ...
      text: 'Affects the workloads in that failure domain. A node crash evicts all pods on that node. A zone outage takes down everything in that zone. Unlike control plane failures, these directly impact user traffic.',
    },
  ]}
/>
  ...
  content={{
    ...
    tbody: {
      tr: [
        ...
        {
          th: 'Controller crash',
          td: ['No reconciliation', 'Restart, leader election', 'Seconds'],
        },
        {
          th: 'Node failure',
...
<Table
  ...
  content={{
    thead: {
      th: ['Factor', 'Custom Controllers', 'Backstage'],
    },
    tbody: {
      tr: [
        {
          th: 'Primary interface',
          td: ['kubectl, GitOps', 'Web UI, catalog'],
        },
        {
          th: 'Infrastructure depth',
          td: ['Deep (creates/manages resources)', 'Shallow (aggregates/displays)'],
        },
        {
          th: 'Self-service model',
          td: ['Declarative resources', 'Templates, forms'],
        },
        {
          th: 'Maintenance burden',
          td: ['Build and maintain controllers', 'Operate Backstage, maintain plugins'],
        },
        {
          th: 'Best for',
          td: ['K8s-native automation', 'Discovery, ownership, portal'],
        },
      ],
    },
    figure: 'Comparing developer interface approaches.',
  }}
/>
