...
tags: ["build-and-deploy", "docker", "python", "ruby", "go"]
featured: true
---
...
<List
  ...
  items={[
    {
      lead: "A static prefix",
      text: 'For namespacing and manual invalidation (e.g., <span class="font-mono">v1-deps-*</span>)',
    },
    {
      lead: "Dynamic components",
      text: "Captures everything affecting the cached content (e.g., lockfile hash)",
    },
    {
      lead: "Optionally, a version or OS component",
      text: 'For environment-specific caches (e.g., <span class="font-mono">runner.os</span>)',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Completeness means the cache key includes everything that affects the output.",
      text: "If your compiled artifact depends on the compiler version, the compiler version must be in the cache key. Miss something, and you'll serve stale artifacts.",
    },
    {
      lead: "Determinism means the same inputs always produce the same outputs.",
      text: "If your build embeds timestamps or downloads \"latest\" versions of tools, caching becomes unsafe — the cached artifact won't match what a fresh build would produce.",
    },
    {
      lead: "Isolation means caches don't leak between unrelated builds.",
      text: "PR builds shouldn't pollute the main branch cache. Without isolation, one build's experimental changes can corrupt another build's cache.",
    },
  ]}
/>
...
__Note: The following YAML snippets are step excerpts — they belong inside a `jobs.<job_id>.steps:` block in a complete workflow file.__
...
<List
  ...
  items={[
    {
      lead: "Floating versions",
      text: 'Using <span class="font-mono font-semibold">npm install</span> instead of <span class="font-mono font-semibold">npm ci</span> allows the package manager to resolve different versions on different runs, even with the same lockfile.',
    },
    {
      lead: "Timestamps",
      text: 'If your build embeds the current timestamp in artifacts, a cached artifact won\'t match a fresh build. Use <span class="font-mono font-semibold">SOURCE_DATE_EPOCH</span> for reproducible builds.',
    },
    {
      lead: "Parallel race conditions",
      text: "If multiple jobs write to the same cache simultaneously, you get undefined behavior. Use job-specific keys or configure jobs to only read from the cache.",
    },
    {
      lead: "Network fetches",
      text: "If your build downloads \"latest\" versions of tools or resources, different builds get different versions. Pin versions and cache the downloads.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Cache hit ratio",
      text: 'Is the percentage of builds that found a usable cache. Calculate it as <span class="font-mono font-semibold">cache_hits / (cache_hits + cache_misses)</span>. A healthy ratio is above 80%. Below 50% for more than a day means something is wrong with your cache key design.',
    },
    {
      lead: "Time saved",
      text: "Is the cumulative time you didn't spend rebuilding. Track this per workflow and per job to identify which caches are most valuable. If a cache saves 30 seconds but adds 45 seconds of overhead (downloading, extracting, uploading), it's not worth it.",
    },
    {
      lead: "Cache size",
      text: "Matters because storage isn't free, and most platforms have limits. GitHub Actions has a 10GB limit per repository, shared across all workflows. Track current size and growth rate, and alert before you hit limits.",
    },
    {
      lead: "Build time percentiles",
      text: "(p50, p95) tell you what engineers actually experience. Compare cached builds to uncached builds to quantify the improvement. If your p95 build time is still 20 minutes, caching isn't solving the right problem.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Lockfile changed (intended behavior, but verify the change was intentional)",
    },
    {
      lead: "Cache key includes something volatile (like a timestamp or random value)",
    },
    {
      lead: "Cache was evicted due to size limits or inactivity",
    },
    {
      lead: "Restore keys aren't matching due to prefix mismatch",
    },
    {
      lead: "Different runner OS or architecture than the cached build",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Cache keys must capture everything that affects build output.",
      text: "If your key doesn't include all the inputs, you'll restore stale artifacts. If your key includes too much, you'll never hit the cache. The sweet spot is a key that changes exactly when the cached content should change.",
    },
    {
      lead: "Correctness comes before speed.",
      text: "A cache that serves wrong artifacts is worse than no cache. When debugging mysterious failures, always test with a clean cache first. If the problem disappears, your cache key is incomplete.",
    },
    {
      lead: "Measure continuously.",
      text: "Cache hit ratio tells you whether caching is working. A 50% hit ratio means half your builds are paying full price. Investigate misses systematically: is the key too specific? Is something non-deterministic? Are caches being evicted?",
    },
    {
      lead: "Layer ordering matters for Docker.",
      text: "Put stable content early in your Dockerfile, volatile content late. A single misplaced `COPY . .` can invalidate the entire layer cache on every commit.",
    },
  ]}
/>

<Download
  resource="ci-pipeline-caching-docker-layers-dependency-cache"
  title="Download the CI Pipeline Caching Guide"
  description="Get the complete caching playbook for fast builds, safe artifact reuse, and deterministic dependency behavior across CI workflows."
  listItems={[
    {
      text: "Cache key design workbook",
    },
    {
      text: "Docker layer optimization patterns",
    },
    {
      text: "Branch isolation cache strategy",
    },
    {
      text: "Cache debugging runbook checklist",
    },
  ]}
/>
