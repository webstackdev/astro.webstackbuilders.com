  ...
  items={[
    {
      ...
      text: "for namespacing and manual invalidation (e.g., `v1-deps-`)",
    },
    {
      lead: "Dynamic components",
      text: "that capture everything affecting the cached content (e.g., lockfile hash)",
    },
    {
      lead: "Optionally, a version or OS component",
      text: "for environment-specific caches (e.g., `runner.os`)",
    },
  ]}
/>
  ...
  items={[
    {
      ...
      text: "If your compiled artifact depends on the compiler version, the compiler version must be in the cache key. If it depends on source files, those file hashes must be in the key. Miss something, and you'll serve stale artifacts.",
    },
    {
      lead: "Determinism means the same inputs always produce the same outputs.",
      text: "If your build embeds timestamps, downloads \"latest\" versions of tools, or has race conditions in parallel jobs, caching becomes unsafe — the cached artifact won't match what a fresh build would produce.",
    },
    {
      lead: "Isolation means caches don't leak between unrelated builds.",
      text: "PR builds shouldn't pollute the main branch cache. Different workflows should have separate cache namespaces. Without isolation, one build's experimental changes can corrupt another build's cache.",
    },
  ]}
/>
  ...
  items={[
    {
      ...
      text: "Using `npm install` instead of `npm ci` allows the package manager to resolve different versions on different runs, even with the same lockfile.",
    },
    {
      lead: "Timestamps",
      text: "If your build embeds the current timestamp in artifacts, a cached artifact won't match a fresh build. Use `SOURCE_DATE_EPOCH` for reproducible builds.",
    },
    {
  ...
  items={[
    {
      ...
      text: "Is the percentage of builds that found a usable cache. Calculate it as `cache_hits / (cache_hits + cache_misses)`. A healthy ratio is above 80%. Below 50% for more than a day means something is wrong with your cache key design.",
    },
    {
      lead: "Time saved",
...
<List
  ...
  items={[
    {
      text: "Lockfile changed (intended behavior, but verify the change was intentional)",
    },
    {
      text: "Cache key includes something volatile (like a timestamp or random value)",
    },
    {
      text: "Cache was evicted due to size limits or inactivity",
    },
    {
      text: "Restore keys aren't matching due to prefix mismatch",
    },
    {
      text: "Different runner OS or architecture than the cached build",
    },
  ]}
/>
