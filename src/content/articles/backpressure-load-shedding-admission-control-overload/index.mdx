...
<Highlighter>Most systems don't fail because they can't handle load.</Highlighter> They fail because they try to handle _all_ the load. Without explicit overload handling, a system accepts work it cannot complete, queues grow unbounded, and what starts as a traffic spike becomes a prolonged outage.
...
<Diagram
  ...
  description="Flowchart showing the feedback loop that turns overload into a full outage. A Traffic Spike leads to Queue Growth, which leads to Latency Increase. Increased latency causes Client Timeouts, which trigger Client Retries, sending more traffic back into the original spike and reinforcing the cycle. A second branch from Latency Increase leads to Resource Exhaustion, then Thread Pool Saturation, then Connection Pool Exhaustion, and finally Complete Failure. The diagram shows both the retry feedback loop and the internal resource collapse path that make overloaded systems degrade rapidly rather than recover naturally."
/>
...
<Diagram
  ...
  description="Flowchart showing an admission control decision at the start of request handling. An Incoming Request reaches a decision point labeled Admission Control. If the request is accepted, it moves to Process Request, then to a decision asking Completed. Successful completion returns a 200 Response, while failure returns a 500 Response. If the request is rejected at admission, the system immediately returns 503 plus Retry-After, which leads to Client Backs Off. The diagram emphasizes that rejecting early preserves capacity and guides clients toward backoff instead of letting requests consume resources only to fail later."
/>
...
<List
  ...
  items={[
    {
      lead: "Rate limiting:",
      text: "cap throughput at N requests per second regardless of what the system can handle. It's blunt but effective as a safety valve.",
    },
    {
      lead: "Concurrency limiting:",
      text: "smarter because it naturally adapts. Allow N requests in-flight simultaneously. If requests are fast, you serve more per second; if they're slow, you serve fewer but don't overload.",
    },
    {
      lead: "Latency-based admission:",
      text: "the most sophisticated — it directly targets the metric you care about. As P99 latency increases, admission probability decreases. The system automatically finds its sustainable capacity.",
    },
    {
      lead: "Queue depth limiting:",
      text: "rejects when the work queue exceeds a threshold. This bounds worst-case latency since you know how much work is ahead of any new request.",
    },
  ]}
/>

Of these, latency-based admission gives you the tightest feedback loop. Here's how it works as Express middleware:
  ...
  items={[
    {
      ...
      text: "When you can't serve a request, say so immediately. A 503 returned in 5ms is infinitely better than a timeout after 30 seconds. The fast rejection frees resources and gives clients useful information.",
    },
    {
      lead: "Not all work is equal — prioritize ruthlessly:",
      text: "During overload, protect what matters: health checks, payments, authenticated users. Shed the non-essential to preserve the critical.",
    },
    {
      lead: "Degrade predictably rather than fail catastrophically:",
      text: "A system that maintains 50% goodput during a 10x traffic spike is succeeding. A system that accepts everything and serves nothing has failed completely.",
    },
  ]}
/>
