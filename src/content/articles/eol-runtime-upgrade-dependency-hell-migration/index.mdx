...
import directDependenciesDiagram from "./diagrams/direct-dependencies-hide-a-much-larger-transitive-graph.jpg"
...
<Diagram
  src={directDependenciesDiagram}
  title="Direct dependencies hide a much larger transitive graph."
  description="Flowchart showing how a small set of direct dependencies expands into a much larger transitive graph. Your Application depends directly on express 4.18, prisma 5.0, and winston 3.8. Those direct dependencies branch into body-parser 1.20, cookie-parser 1.4, prisma-client 5.0, prisma-engines 5.0, and logform 2.4. The graph continues downward to raw-body 2.5, cookie 0.5, a Native Binary - Node 16+, iconv-lite 0.4, and safer-buffer 2.1. A grouped region labeled Direct 3 contains the three direct packages, and another labeled Transitive 10 plus contains the deeper dependency set. The native binary node is visually emphasized as the blocker. The diagram illustrates how a seemingly small application dependency list can conceal a much larger transitive tree where a single deep dependency can block a runtime upgrade."
/>
...
<Table
  ...
  content={{
    figure: "Common dependency blocker types and strategies.",
    thead: { th: ["Blocker Type", "Example", "Resolution Strategy"] },
    tbody: {
      tr: [
        {
          th: "Native modules",
          td: ["node-sass, bcrypt older versions", "Upgrade or replace with pure JS"],
        },
        {
          th: "Deprecated packages",
          td: ["request, node-uuid", "Replace with maintained alternative"],
        },
        {
          th: "Abandoned packages",
          td: ["No updates in 2+ years", "Fork, replace, or vendor"],
        },
        {
          th: "Internal packages",
          td: ["Company packages not updated", "Coordinate with owning team"],
        },
        {
          th: "Version constraints",
          td: ["Peer dependency conflicts", "Negotiate with upstream"],
        },
        {
          th: "Binary distributions",
          td: ["Pre-built binaries for old runtime", "Rebuild or find new source"],
        },
      ],
    },
  }}
...
<Table
  ...
  content={{
    figure: "Common .NET Framework blockers and their modern replacements.",
    thead: { th: ["Blocker", "Replacement", "Migration Effort"] },
    tbody: {
      tr: [
        {
          th: "ASP.NET Web Forms",
          td: ["Blazor or Razor Pages", "Extreme — full rewrite"],
        },
        {
          th: "WCF server-side",
          td: ["gRPC or REST APIs", "High — new service contracts"],
        },
        {
          th: "Entity Framework 6",
          td: ["EF Core", "Medium-high — query translation differences"],
        },
        {
          th: "System.Web dependencies",
          td: ["ASP.NET Core equivalents", "High — scattered throughout codebase"],
        },
        {
          th: "Crystal Reports",
          td: ["Third-party or custom", "High — report redesign"],
        },
        {
          th: "Windows-only APIs",
          td: ["Platform-specific code or alternatives", "Varies"],
        },
      ],
    },
  }}
...
<Table
  variant="grid-and-accent-header-table"
  content={{
    figure: "Framework EOL patterns and common upgrade blockers.",
    thead: { th: ["Framework", "EOL Pattern", "Common Blockers", "Key Friction"] },
    tbody: {
      tr: [
        {
          th: "Ruby on Rails",
          td: [
            "3 versions supported; older dropped aggressively",
            "Ruby version requirements, gem native extensions, database adapter changes",
            "Rails version often forces Ruby version; gems lag behind Rails releases",
          ],
        },
        {
          th: "Django",
          td: [
            "2 versions supported; ~8 month release cycle",
            "Python version requirements, third-party app compatibility, ORM query changes",
            "Aggressive deprecation warnings become errors; middleware API changes",
          ],
        },
        {
          th: "Spring Boot",
          td: [
            "12 months OSS support per minor; commercial extended",
            "Java version requirements, Spring Framework major version jumps",
            "Java 8 → 17 jump required; XML config to Java config migration",
          ],
        },
        {
          th: "Laravel",
          td: [
            "2 years bug fixes, 3 years security",
            "PHP version requirements, Composer dependency conflicts",
            "PHP 7 → 8 type system changes break packages",
          ],
        },
      ],
    },
...
<List
  ...
  items={[
    {
      lead: "Preparation",
      text: "Starts weeks before the migration. Inventory every server and its OS version. Identify applications with dependencies on OS-level packages or paths. Test applications on the new OS in dev and staging environments. Update CI/CD pipelines to build artifacts compatible with both OS versions.",
    },
    {
      lead: "Parallel operation",
      text: "Runs both OS versions simultaneously in production. Route a small percentage of traffic to new-OS instances and monitor closely for errors, latency changes, and functional issues. This is where you discover the edge cases that testing missed — the hardcoded path that changed, the system call that behaves differently, the package that is no longer available.",
    },
    {
      lead: "Cutover",
      text: "Shifts the majority of traffic to the new OS while keeping old instances warm. Do not decommission old infrastructure immediately. Give yourself a week to catch issues that only manifest under full production load or at specific times (batch jobs, month-end processing).",
    },
    {
      lead: "Cleanup",
      text: "Where you finally remove the old infrastructure, update documentation, and simplify CI/CD back to a single target OS.",
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Find a replacement.",
      text: 'Many abandoned packages have actively maintained alternatives. The <span class="font-mono">request</span> library, which powered half the Node ecosystem for years, was deprecated in 2020. Its replacements — <span class="font-mono">node-fetch</span>, <span class="font-mono">axios</span>, <span class="font-mono">got</span> — are well documented and widely adopted. Search for "[package name] alternative" or check the npm deprecation notice for suggestions.',
    },
    {
      lead: "Upgrade to a compatible version.",
      text: "Sometimes the package is not abandoned; you are just on an old version. Check whether a newer major version exists that supports your target runtime. This requires a migration, but it is often less work than replacing the package entirely.",
    },
    {
      lead: "Fork and patch.",
      text: 'If the package is abandoned and has no replacement, you can fork it to your organization, apply the minimal fixes needed for runtime compatibility, and publish it under your own scope. Update your <span class="font-mono">package.json</span> to reference the fork.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: "Vendor the code.",
      text: 'For small packages with simple functionality, copying the source code into your repository may be simpler than maintaining a fork. Put it in a <span class="font-mono">vendor/</span> directory, update your imports, and apply whatever patches you need. This removes the dependency from your <span class="font-mono">package.json</span> entirely.',
    },
    {
      lead: "Rewrite.",
      text: 'If the package\'s functionality is narrow and well-defined, writing a replacement yourself may be faster than maintaining someone else\'s code. The infamous <span class="font-mono">left-pad</span> incident aside, many utility packages are just a few functions that you could implement in an afternoon.',
    },
  ]}
/>
...
<List
  ...
  items={[
    ...
    {
      lead: "Decommission old runtime instances",
      text: "Remove the previous runtime after a soak period confirms stability.",
    },
  ]}
/>
...
<Download
  resource="eol-runtime-upgrade-dependency-hell-migration"
  title="Download the EOL Runtime Upgrade Guide"
  description="Get the complete migration playbook for runtime upgrades, transitive blocker resolution, and staged compatibility rollout plans."
  listItems={[
    {
      text: "Dependency blocker triage matrix",
    },
    {
      text: "Abandoned package fallback strategies",
    },
    {
      text: "Override and patch workflows",
    },
    {
      text: "Incremental upgrade execution runbook",
    },
  ]}
/>
