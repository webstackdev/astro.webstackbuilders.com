...
<Diagram
  ...
  size="full"
  title="External DNS lookup showing failed search domain queries before success."
  description="Sequence diagram showing how Kubernetes DNS resolution slows an external lookup when ndots is set to 5. An Application Pod sends a lookup for api.stripe.com to /etc/resolv.conf. A note over the resolver explains that the hostname has 2 dots and that ndots:5 causes search domains to be tried first. The resolver then sends three queries to CoreDNS in order: api.stripe.com.default.svc.cluster.local, api.stripe.com.svc.cluster.local, and api.stripe.com.cluster.local. CoreDNS returns NXDOMAIN for each of those internal-style names. After those failures, the resolver finally sends api.stripe.com as a fully qualified domain name to CoreDNS. CoreDNS forwards that final query to an upstream DNS server, receives the A record 93.184.216.34, and returns the successful answer back to the application. The diagram emphasizes that the delay comes from the three failed search-domain lookups that happen before the real external hostname is attempted."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Metric', 'Before Fix', 'After Fix'],
    },
    tbody: {
      tr: [
        {
          th: 'DNS resolution time',
          td: ['3.2 seconds', '85ms'],
        },
        {
          th: 'Total request latency',
          td: ['3.4 seconds', '285ms'],
        },
        {
          th: 'P99 latency',
          td: ['15+ seconds', '450ms'],
    ...
    figure: 'Latency improvement after fixing ndots.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: "Launch a debug pod",
      text: '<span class="font-mono">kubectl run dns-debug --image=nicolaka/netshoot --rm -it --restart=Never -- bash</span>',
    },
    {
      lead: "Run the ndots test",
      text: 'Compare <span class="font-mono">time nslookup api.example.com</span> vs <span class="font-mono">time nslookup api.example.com.</span>',
    },
    {
      lead: "Interpret the results",
      text: "If the FQDN (with dot) is 10x+ faster, apply one of the fixes above",
    },
  ]}
/>
