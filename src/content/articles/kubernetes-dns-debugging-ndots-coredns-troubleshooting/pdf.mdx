...
import corednsQueryDiagram from "./diagrams/coredns-query-processing-flow.png"
import dnsDebuggingDiagram from "./diagrams/dns-debugging-decision-tree.png"
import externalDnsDiagram from "./diagrams/external-dns-lookup-showing-failed-search-domain-queries-before-success.png"
import internalServiceDiagram from "./diagrams/internal-service-discovery-fast-path-with-single-query.png"
...
<Diagram
  src={internalServiceDiagram}
  title="Internal service discovery — fast path with single query."
  description="Sequence diagram showing the fast path for Kubernetes internal service discovery. An Application Pod sends a lookup for myservice to /etc/resolv.conf. A note over the resolver lists the pod DNS configuration: nameserver 10.96.0.10, search domains default.svc.cluster.local, svc.cluster.local, and cluster.local, and options ndots:5. The resolver sends the fully expanded query myservice.default.svc.cluster.local to CoreDNS. CoreDNS then queries the Kubernetes API for the Service named myservice in the default namespace. The Kubernetes API returns the Service IP 10.96.123.45 to CoreDNS, and CoreDNS returns an A record with that same IP back to the application. The diagram emphasizes that internal service discovery completes in a single query path because the search domain expansion immediately matches a cluster service."
/>
...
<Diagram
  src={externalDnsDiagram}
  title="External DNS lookup — slow path with multiple failed queries before success."
  description="Sequence diagram showing how Kubernetes DNS resolution slows an external lookup when ndots is set to 5. An Application Pod sends a lookup for api.example.com to /etc/resolv.conf. A note over the resolver explains that api.example.com has 2 dots and that ndots:5 means search domains are tried first. The resolver then sends three queries to CoreDNS in order: api.example.com.default.svc.cluster.local, api.example.com.svc.cluster.local, and api.example.com.cluster.local. CoreDNS returns NXDOMAIN for each of those names. After the failed internal-style lookups, the resolver finally sends api.example.com as a fully qualified domain name to CoreDNS. CoreDNS forwards that query to an upstream DNS server, receives an A record for 93.184.216.34, and returns the successful answer to the application. The diagram shows that the latency comes from the repeated failed search-domain queries that happen before the real external hostname is attempted."
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['DNS Option', 'Default', 'Purpose', 'Impact'],
    },
    tbody: {
      tr: [
        {
          th: 'ndots',
          td: ['5', 'Dots threshold for search domain use', 'Lower = fewer unnecessary queries'],
        },
        {
          th: 'timeout',
          td: ['5', 'Seconds before query timeout', 'Lower = faster failure, but risk of false negatives'],
        },
        {
          th: 'attempts',
          td: ['2', 'Retry count per nameserver', 'Higher = more resilient, but slower failure'],
        },
        {
          th: 'rotate',
          td: ['off', 'Round-robin nameservers', 'On = better load distribution'],
        },
        {
          th: 'single-request',
          td: ['off', 'One request at a time', 'On = fixes some UDP issues'],
        },
      ],
    },
    figure: 'DNS options in resolv.conf.',
  }}
/>
...
<Diagram
  src={corednsQueryDiagram}
  title="CoreDNS query processing flow."
  description="Flowchart showing the decision path CoreDNS follows when handling a DNS query. The chart is grouped under the heading CoreDNS Query Flow. A query first enters at Query Received, then moves to a decision asking In Cache. If yes, the flow goes directly to Return Cached Response. If no, it moves to a second decision asking whether the query is for cluster.local. If yes, the flow goes to Kubernetes Plugin, then to a decision asking Service Exists. A yes result returns Return Service IP, while a no result returns NXDOMAIN. If the query is not for cluster.local, the flow goes to a decision asking Custom Zone. A yes result forwards the query to Zone Server, while a no result forwards it to Upstream. The outcomes Return Service IP, NXDOMAIN, Forward to Zone Server, and Forward to Upstream all converge on Cache Response. The diagram emphasizes that cached answers are returned immediately, internal cluster names are handled by the Kubernetes plugin, and everything else is forwarded externally or to a custom zone handler before being cached."
/>
...
<Diagram
  src={dnsDebuggingDiagram}
  title="DNS debugging decision tree."
  description="Flowchart showing a step-by-step DNS troubleshooting path in Kubernetes. The process starts at DNS Problem Reported and first asks whether the pod can reach the CoreDNS IP. If no, the issue is categorized as Network issue and the next step is Check CNI, NetworkPolicy. If yes, the chart asks whether internal DNS is working. If no, the issue is categorized as CoreDNS issue and the next step is Check CoreDNS pods, logs, config. If internal DNS is working, the chart asks whether external DNS is working. If no, the issue is categorized as Upstream or forward issue and the next step is Check CoreDNS forward config, upstream DNS. If external DNS is working but slow, the chart asks whether lookups are faster with a trailing dot. If yes, the issue is categorized as ndots issue and the recommended action is Reduce ndots or use FQDNs. If no, the issue is categorized as Upstream DNS slow and the recommended action is Change upstream DNS or increase cache. The diagram is intended to guide the reader from symptom to likely cause in a fixed order that rules out the most common DNS failure modes."
/>
...
<List
  ...
  items={[
    {
      lead: "DNS timeouts or intermittent failures",
      text: "Check if CoreDNS pods are overloaded (`kubectl top pods -n kube-system -l k8s-app=kube-dns`), look for NetworkPolicies blocking UDP/TCP port 53, and verify the upstream DNS in CoreDNS's forward configuration is reachable.",
    },
    {
      lead: "External DNS slow, internal DNS fast",
      text: "This is almost always an ndots issue. Compare resolution time with and without a trailing dot. If the FQDN is faster, reduce ndots in pod dnsConfig or use FQDNs in your application configuration.",
    },
    {
      lead: "Service discovery fails for new services",
      text: "Services usually propagate to CoreDNS within 10 seconds. If lookups fail, verify the service exists and has endpoints (`kubectl get svc,ep -n <namespace>`). Also check that you're using the correct namespace in the FQDN.",
    },
    {
      lead: "DNS works in some pods but not others",
      text: "Compare `/etc/resolv.conf` between working and broken pods. Check each pod's `dnsPolicy` setting. If the broken pod is on a specific node, investigate node-level DNS or NetworkPolicies scoped to that namespace.",
    },
  ]}
/>
...
<Table
  ...
  content={{
    thead: {
      th: ['Policy', 'CoreDNS Used', 'Use Case'],
    },
    tbody: {
      tr: [
        {
          th: '`ClusterFirst`',
          td: ['Yes', 'Default for standard workloads'],
        },
        {
          th: '`Default`',
          td: ["No", "Pod needs the node's DNS, not cluster DNS"],
        },
        {
          th: '`ClusterFirstWithHostNet`',
          td: ['Yes', 'Pods with `hostNetwork: true` that still need cluster DNS'],
        },
        {
          th: '`None`',
          td: ['No', 'Fully custom DNS configuration via `dnsConfig`'],
        },
      ],
    },
    figure: 'DNS policy comparison.',
  }}
/>
...
<List
  ...
  items={[
    {
      lead: "Is CoreDNS running?",
      text: "`kubectl get pods -n kube-system -l k8s-app=kube-dns`",
    },
    {
      lead: "Can you resolve internal names?",
      text: "`nslookup kubernetes.default` from inside a pod",
    },
    {
      lead: "Can you resolve external names?",
      text: "`nslookup google.com` from inside a pod",
    },
    {
      lead: "Is it an ndots issue?",
      text: "Compare `time nslookup api.example.com` vs `time nslookup api.example.com.`",
    },
    {
      lead: "Check CoreDNS logs",
      text: "for errors: `kubectl logs -n kube-system -l k8s-app=kube-dns --tail=50`",
    },
  ]}
/>
