...
import observabilityArchitectureDiagram from './observability-architecture-overview.jpg'
...
Customers were often the alerting system. The company's mean time to detection was four hours—most of that time being _"we didn't know there was a problem."_ Customers would report issues, then engineers would start investigating. This was embarrassing and damaging to customer trust.
...
<Table
  ...
  content={{
    figure: 'Observability state before implementation',
    thead: {
      th: ['Signal', 'Tools in Use', 'Coverage', 'Correlation'],
    },
    tbody: {
      tr: [
        {
          td: ['Metrics', 'CloudWatch, StatsD, custom', '80% of services', 'None'],
        },
        {
          td: ['Logs', 'CloudWatch Logs, Elasticsearch, files', '95% of services', 'None'],
        },
        {
          td: ['Traces', 'X-Ray, Jaeger', '20% of services', 'Partial'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'OpenTelemetry for instrumentation',
      text: 'We needed vendor-neutral instrumentation that would work across all three languages and let us change backends later if needed. OpenTelemetry was the clear choice. It had become the industry standard, with solid SDKs for Node.js, Python, and Go.',
    },
    {
      lead: 'Grafana stack for backends',
      text: 'We evaluated Datadog, New Relic, and the Grafana stack (Prometheus, Loki, Tempo). Datadog would have cost $180K/year at their scale (60+ services, 45 engineers, approximately 500GB of logs and 2 billion metric data points per month). The Grafana stack, self-hosted on their existing Kubernetes infrastructure, cost about $30K/year in compute and storage. The feature gap was real, Datadog\'s UI is better, but not $150K/year better.',
    },
    {
      lead: 'Correlation IDs everywhere',
      text: 'Every request entering the system would get a trace ID that propagated through all services, appeared in all logs, and linked metrics to specific requests. This was the foundation that would make debugging possible.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Months 1-2: Foundation',
      text: 'We deployed the Grafana stack, Prometheus for metrics, Loki for logs, Tempo for traces, Grafana for visualization. We built the infrastructure and validated it worked before touching any application code.',
    },
    {
      lead: 'Months 3-4: Instrumentation',
      text: 'We added OpenTelemetry SDKs to all services, starting with the most critical paths. Auto-instrumentation handled HTTP and database calls. We added custom spans for business logic that needed visibility.',
    },
    {
      lead: 'Month 5: Log aggregation and compliance',
      text: 'We migrated all services to ship logs to Loki with proper formatting. We built a redaction pipeline to strip PHI before logs reached the aggregation layer.',
    },
    {
      lead: 'Month 6: Alerting and dashboards',
      text: 'We created service dashboards, built alerting rules based on SLOs, and integrated with PagerDuty. We trained the engineering team on the new tools.',
    },
  ]}
/>
...
<Diagram
  src={observabilityArchitectureDiagram}
  title="Observability architecture overview."
  description="Flowchart showing an Applications group with four sources: Node.js with OpenTelemetry, Python with OpenTelemetry, Go with OpenTelemetry, and Legacy services that emit logs only. All four send telemetry to an OpenTelemetry Collector responsible for redaction. The collector forwards metrics to Prometheus, logs to Loki, and traces to Tempo inside a Storage Backends group. Prometheus, Loki, and Tempo then feed Grafana for visualization. Grafana connects to Alertmanager, which routes alerts to PagerDuty. The diagram shows OpenTelemetry as the collection layer and the Grafana stack as the unified storage, visualization, and alerting path."
/>
...
<List
  ...
  items={[
    {
      lead: 'Error rate alerts',
      text: 'Fire when error rate exceeds SLO burn rate',
    },
    {
      lead: 'Latency alerts',
      text: 'Fire when P99 latency degrades beyond SLO',
    },
    {
      lead: 'Availability alerts',
      text: 'Fire when synthetic checks fail',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'UI polish',
      text: 'Datadog\'s interface is more intuitive. Grafana required more training.',
    },
    {
      lead: 'APM features',
      text: 'Datadog\'s automatic service maps and dependency detection are better. We built our own service map using trace data, but it required manual maintenance.',
    },
    {
      lead: 'Log search speed',
      text: 'Loki\'s query performance on large time ranges was slower than Datadog\'s. We mitigated this with better indexing and query optimization, but it remained a gap.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: 'Observability implementation outcomes',
    thead: {
    ...
    tbody: {
      tr: [
        {
          td: ['Mean time to detection', '4 hours', '8 minutes', '97% reduction'],
        },
        {
          td: ['Mean time to resolution', '6 hours', '45 minutes', '88% reduction'],
        },
        {
          td: ['Alert noise', 'Baseline', '-70%', 'Significant reduction'],
        },
        {
          td: ['Issues resolved without escalation', '~20%', '80%', '4x improvement'],
        },
        {
          td: ['HIPAA logging findings', 'N/A', 'Zero', 'Compliant'],
        },
        {
          td: ['Annual tooling costs', '$180K (Datadog est.)', '$30K', '$150K savings'],
        },
      ],
    },
  }}
...
The cultural change was significant too. Engineers started adding custom instrumentation proactively because they could see the value. _"Let me add a span for this"_ became common in code reviews. Observability shifted from afterthought to design consideration.
...
<List
  ...
  items={[
    {
      lead: 'Correlation is the foundation',
      text: 'Individual metrics, logs, and traces have limited value. Connected signals that tell a coherent story are transformative. Start with trace IDs everywhere, then build on that foundation.',
    },
    {
      lead: 'OpenTelemetry is the right bet',
      text: 'Vendor-neutral instrumentation means you can change backends without re-instrumenting. We chose the Grafana stack today, but if requirements change, we can switch without touching application code.',
    },
    {
      lead: 'Self-hosted can work',
      text: 'The conventional wisdom is that observability should be SaaS. But for cost-conscious organizations with compliance requirements, self-hosted solutions like the Grafana stack are mature enough for production. The tradeoff is operational overhead, not capability.',
    },
  ]}
/>
