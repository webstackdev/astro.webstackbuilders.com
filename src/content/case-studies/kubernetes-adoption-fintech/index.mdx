...
<List
  ...
  items={[
    {
      lead: 'EKS over self-managed Kubernetes',
      text: 'The team did not have the capacity to manage control plane operations alongside their regular work. EKS shifted that burden to AWS and gave us a clear compliance story for the control plane.',
    },
    {
      lead: 'Istio for service mesh',
      text: 'We needed mTLS between all services for PCI compliance. Istio also gave us traffic management capabilities that would make the migration safer, we could shift traffic gradually between old and new deployments.',
    },
    {
      lead: 'ArgoCD for GitOps',
      text: 'Every configuration change would go through Git. This gave us audit trails for compliance, easy rollbacks, and a deployment model the team could understand and trust.',
    },
    {
      lead: 'Karpenter over Cluster Autoscaler',
      text: 'Payment traffic has spiky patterns. Karpenter\'s faster provisioning and bin-packing would handle those spikes better and reduce costs.',
    },
  ]}
/>
...
<Table
  ...
  content={{
    figure: 'Migration phase timeline',
    thead: {
      th: ['Phase', 'Months', 'Focus', 'Risk Level'],
    },
    tbody: {
      tr: [
        {
          td: ['Platform foundation', '1-3', 'EKS cluster, networking, Istio, observability, CI/CD', 'Low'],
        },
        {
          td: ['Pilot services', '4-5', 'Three low-risk internal services', 'Low-Medium'],
        },
        {
          td: ['Batch migration', '6-8', 'Remaining services, prioritized by risk and dependency', 'Medium-High'],
        },
        {
          td: ['Compliance certification', '9', 'QSA validation, documentation, evidence collection', 'Low'],
        },
      ],
    },
  }}
...
We deployed a multi-AZ EKS cluster in `us-east-1` with node groups spread across three availability zones. The cluster ran Kubernetes 1.28 with managed add-ons for CoreDNS, `kube-proxy`, and the VPC CNI[^vpc-cni].

Karpenter[^karpenter] handled node provisioning with provisioners tuned for different workload types. Payment processing services ran on dedicated node pools with instance types selected for consistent performance (no burstable instances). Batch processing and internal tools shared a separate pool that could use spot instances.

[^vpc-cni]: The Amazon VPC CNI is EKS's default networking plugin. It assigns VPC IP addresses directly to pods so they can talk to other AWS network resources without an overlay network, which improves performance and makes native VPC controls like flow logs, routing, and security policies easier to use.

[^karpenter]: Karpenter is an open-source Kubernetes autoscaler that launches and removes right-sized nodes based on what pending workloads actually need. It is faster and more flexible than older autoscaling approaches, but it adds some operational complexity and depends heavily on cloud-provider integration.
...
<List
  ...
  items={[
    {
      lead: 'Istio\'s overhead',
      text: 'Istio adds roughly 2-3ms of latency per hop. For payment processing with a 100ms SLA, this mattered. We accepted it because the security and traffic management benefits outweighed the cost, and the services still met SLA comfortably.',
    },
    {
      lead: 'Managed node groups vs. Karpenter',
      text: 'We started with managed node groups for simplicity, then migrated to Karpenter in month 4 when we understood our workload patterns better. Karpenter\'s flexibility was worth the additional operational complexity.',
    },
    {
      lead: 'Single cluster vs. multi-cluster',
      text: 'We considered separate clusters for CDE and non-CDE workloads. The compliance team ultimately accepted namespace-level isolation with network policies, which reduced operational overhead significantly.',
    },
  ]}
/>
...
<Table
  ...
  classes={{
    thead: '[&_th:nth-child(2)]:text-center [&_th:nth-child(3)]:text-center',
    tbody: '!text-content [&_td:nth-child(2)]:text-center [&_td:nth-child(3)]:text-center',
  }}
  content={{
    figure: 'Migration outcomes summary',
    thead: {
    ...
    tbody: {
      tr: [
        {
          td: ['Deployment frequency', 'Bi-weekly', '15+ daily', '30x improvement'],
        },
        {
          td: ['Mean time to recovery', '45 minutes', '8 minutes', '82% reduction'],
        },
        {
          td: ['Infrastructure costs', 'Baseline', '-30%', '$800K annual savings'],
        },
        {
          td: ['PCI-DSS audit findings', 'N/A', 'Zero', 'Passed'],
        },
        {
          td: ['On-call pages', 'Baseline', '-60%', 'Reduced'],
        },
        {
          td: ['P99 latency', '85ms', '72ms', '15% improvement'],
        },
      ],
    },
  }}
...
<List
  ...
  items={[
    {
      lead: 'Deployment frequency increased from bi-weekly to 15+ times daily',
      text: 'Teams now deployed independently without coordinating release windows. Most deployments were fully automated with manual approval only for production.',
    },
    {
      lead: 'Mean time to recovery dropped from 45 minutes to 8 minutes',
      text: 'Kubernetes self-healing, combined with proper health checks and Istio\'s traffic management, meant most issues resolved automatically. When human intervention was needed, rollback was a single click in ArgoCD.',
    },
    {
      lead: 'Infrastructure costs reduced by 30%',
      text: 'Better bin-packing from Karpenter, automatic scaling that actually worked, and elimination of over-provisioned EC2 instances drove the savings.',
    },
    {
      lead: 'PCI-DSS audit passed with zero findings',
      text: 'The QSA specifically called out the mTLS implementation and audit logging as exemplary.',
    },
    {
      lead: 'On-call pages reduced by 60%',
      text: 'The self-healing capabilities and improved observability meant engineers were not woken up for issues the platform could handle itself.',
    },
    {
      lead: 'P99 latency improved from 85ms to 72ms',
      text: 'Despite Istio\'s overhead, the consistent performance from dedicated node pools and better resource allocation actually improved latency.',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Compliance constraints can drive better architecture',
      text: 'The PCI requirements forced us to implement mTLS, network segmentation, and comprehensive audit logging. These practices made the system more secure and more observable than it would have been otherwise.',
    },
    {
      lead: 'Migrate incrementally with traffic shifting',
      text: 'Running old and new systems in parallel with gradual traffic migration eliminated big-bang risk. When we found issues, we shifted traffic back. No customer ever noticed a migration-related problem.',
    },
    {
      lead: 'Invest in the platform before migrating workloads',
      text: 'The three months we spent building the platform foundation, observability, CI/CD, security controls, before migrating any production traffic paid off. Teams trusted the platform because it was proven before they depended on it.',
    },
  ]}
/>
