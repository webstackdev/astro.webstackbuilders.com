...
cover: "./cover.jpg"
coverAlt: "Encrypted mTLS tunnels connecting service pods with SPIFFE identity certificates at each endpoint for cryptographic verification"
client: "non-disclosed-financial-services-firm"
duration: "7 months"
...
*[SVIDs]: SPIFFE Verifiable Identity Document

import zeroTrustArchitectureDiagram from './zero-trust-architecture-with-istio-service-mesh.jpg'
...
Their upcoming SOC2 Type II audit added urgency. The auditors had flagged their network segmentation as a concern in the previous cycle. New regulatory guidance emphasized zero trust principles — _"never trust, always verify"_ — and the firm needed to demonstrate progress.
...
Before implementing anything, I needed to understand the current communication patterns. I deployed network flow analysis tools[^flow-tools] to capture which services talked to which, on what ports, at what volume.

[^flow-tools]: Common tools for network flow analysis include VPC Flow Logs (AWS) or NSG Flow Logs (Azure) for cloud-level traffic capture, Zeek (formerly Bro) for deep packet inspection and connection logging, Cilium Hubble for Kubernetes-native network observability, ntopng for real-time traffic analysis, and Kentik or ElastiFlow for flow data aggregation and visualization. In hybrid environments, a combination of cloud-native flow logs and an on-premises collector like Zeek or SiLK typically provides the broadest coverage.
...
<Table
  ...
  content={{
    figure: 'Zero trust implementation phases',
    thead: {
      th: ['Phase', 'Focus', 'Risk Level'],
    },
    tbody: {
      tr: [
        {
          td: ['Mesh Installation', 'Install Istio sidecars in permissive mode; mTLS available but not required', 'Low'],
        },
        {
          td: ['Observability', 'Gain visibility via Kiali; map which services communicate and mTLS coverage', 'Low'],
        },
        {
          td: ['Policy Definition', 'Define authorization policies based on observed patterns; audit mode first', 'Medium'],
        },
        {
          td: ['Strict Mode', 'Enable mTLS strict mode; flip policies from audit to enforce', 'High'],
        },
      ],
    },
  }}
...
<Diagram
  src={zeroTrustArchitectureDiagram}
  size="full"
  title="Zero trust architecture with Istio service mesh."
  description="Diagram of a Kubernetes cluster containing two services, Service A and Service B. Service A contains an application container and an Envoy sidecar. Service B also contains an Envoy sidecar and an application container. The two Envoy sidecars communicate with each other over an mTLS connection. A separate Istio Control Plane group contains Istiod, a SPIRE Server, and OPA or Gatekeeper. Both Envoy sidecars connect to the control plane, showing that policy, identity, and mesh configuration are distributed centrally while workload-to-workload traffic is mutually authenticated between sidecars."
/>
...
<Table
  ...
  classes={{
    ...
    tbody: '!text-content [&_td:nth-child(2)]:text-center',
  }}
  content={{
    figure: 'Zero trust implementation outcomes',
    thead: {
    ...
    tbody: {
      tr: [
        {
          td: ['Service-to-service encryption', '0%', '100% mTLS'],
        },
        {
          td: ['Firewall rules', '200+', 'Eliminated (identity-based policies)'],
        },
        {
          td: ['SOC2 Type II findings', 'Flagged', 'Zero network security findings'],
        },
        {
          td: ['Credential revocation time', 'Hours', 'Seconds'],
        },
        {
          td: ['Lateral movement surface', 'Unbounded', '95% reduction (threat model estimate)'],
        },
      ],
    },
  }}
  ...
  items={[
    {
      ...
      text: 'You cannot secure what you cannot see. Network flow analysis and service mapping must precede policy definition. We discovered communication patterns that surprised even senior engineers.',
    },
    {
      lead: 'Gradual rollout is essential',
      text: 'Permissive mode, audit policies, then enforcement. Any attempt to flip zero trust on overnight would have caused outages. The phased approach built confidence and caught issues early.',
    },
    {
      lead: 'Identity beats network location',
      text: 'SPIFFE workload identity fundamentally changes the security model. Instead of "this request came from a trusted IP range," you get "this request came from a cryptographically verified workload." That is a much stronger foundation.',
    },
  ]}
/>
