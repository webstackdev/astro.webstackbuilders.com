...
cover: "./cover.jpg"
coverAlt: "Kafka cluster as central hub with producers and consumers orbiting around it, event messages flowing through topics as packets"
client: "moloco"
duration: "6 months"
...
import eventStreamingDiagram from './event-streaming-architecture-with-kafka-as-central-hub.jpg'
...
<Table
  ...
  content={{
    figure: 'Table: Event categories and their latency requirements',
    thead: {
      th: ['Event Category', 'Producer', 'Consumers', 'Staleness Tolerance'],
    },
    tbody: {
      tr: [
        {
          td: ['Impressions', 'Ad server', 'Analytics, Billing, Fraud', 'Seconds'],
        },
        {
          td: ['Clicks/Conversions', 'Tracking service', 'Analytics, Attribution', 'Seconds'],
        },
        {
          td: ['Budget changes', 'Campaign service', 'Bidding, Pacing', 'Minutes'],
        },
        {
          td: ['User segments', 'DMP (Data Management Platform)', 'Targeting, Analytics', 'Hours'],
        },
        {
          td: ['Billing events', 'Billing service', 'Finance, Reporting', 'Minutes'],
        },
      ],
    },
  }}
...
For schema management, I chose [Confluent Schema Registry](https://docs.confluent.io/platform/current/schema-registry/index.html). When 30+ services exchange events, schema evolution is critical. The registry enforced compatibility—you couldn't deploy a producer that broke consumers.
...
<List
  ...
  items={[
    {
      lead: 'Deploy MSK cluster with appropriate capacity',
    },
    {
      lead: 'Implement Schema Registry and establish conventions',
    },
    {
      lead: 'Migrate billing events as pilot domain (lower volume, well-understood)',
    },
    {
      lead: 'Build monitoring dashboards for Kafka lag and throughput',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Migrate impression and click events to Kafka',
    },
    {
      lead: 'Implement change data capture from the analytics database',
    },
    {
      lead: 'Build real-time aggregation with ksqlDB',
    },
    {
      lead: 'Deploy real-time dashboards (beta)',
    },
  ]}
/>
...
<List
  ...
  items={[
    {
      lead: 'Migrate budget and campaign events',
    },
    {
      lead: 'Remove synchronous dependencies from critical paths',
    },
    {
      lead: 'Implement dead letter topics and error handling',
    },
    {
      lead: 'Decommission legacy batch pipelines',
    },
  ]}
/>
...
<Diagram
  src={eventStreamingDiagram}
  title="Event streaming architecture with Kafka as central hub."
  description="Flowchart showing three producer services on the left: Ad Server Producer, Tracking Producer, and Campaign Producer. At the center is a Kafka (MSK) group containing three topics: impressions, clicks, and budget-changes. On the right are three consumer services: Analytics Consumer, Billing Consumer, and Fraud Consumer. Ad Server sends events to the impressions topic, Tracking sends to clicks, and Campaign sends to budget-changes. The impressions topic fans out to Analytics, Billing, and Fraud. The clicks topic fans out to Analytics and Billing. The budget-changes topic feeds Analytics. The diagram shows Kafka acting as the decoupling layer between event producers and downstream consumers."
/>
...
For the real-time dashboards, we used [ksqlDB](https://docs.confluent.io/platform/current/ksqldb/overview.html#ksqldb-quick-start) (a streaming SQL engine for Kafka) to process streams and materialize aggregations:
...
Some events originated from database writes rather than application code. Rather than retrofitting every service to publish events, we used [Kafka Connect](https://www.redpanda.com/guides/kafka-tutorial-what-is-kafka-connect) with [Debezium](https://debezium.io/documentation/reference/stable/index.html)[^debezium] for CDC:
...
<Table
  ...
  content={{
    figure: 'Table: Event-driven architecture migration outcomes',
    thead: {
    ...
    tbody: {
      tr: [
        {
          td: ['Analytics latency', '24 hours', 'Under 5 seconds'],
        },
        {
          td: ['Cascade failures', '8 incidents/month', '0 (services decoupled)'],
        },
        {
          td: ['System throughput', 'Baseline', '3x increase (same infrastructure)'],
        },
        {
          td: ['Feature velocity', 'Weeks per analytics feature', 'Days (just add a consumer)'],
        },
        {
          td: ['Deployment coordination', 'Required across services', 'Independent deployments'],
        },
      ],
    },
  }}
...
[^debezium]: Debezium is an open-source change data capture platform that monitors database commits and emits inserts, updates, and deletes as ordered event streams for downstream consumers.
...
<List
  ...
  items={[
    {
      lead: 'Events decouple time and availability',
      text: 'When services communicate via events, a slow consumer does not slow down producers. This eliminates cascade failures and lets teams work independently. The architectural benefit was more valuable than the real-time analytics.',
    },
    {
      lead: 'Schema management is cultural change',
      text: 'In request/response, you coordinate deployments. With events, you need contractual guarantees that schemas evolve safely. Investing in Schema Registry and compatibility rules early prevented painful breaking changes later.',
    },
    {
      lead: 'Start with replay capability',
      text: 'The ability to reprocess events from the beginning is invaluable for debugging, disaster recovery, and building new consumers. SQS\'s lack of replay would have been a constant limitation. Choose infrastructure that supports your future needs, not just today\'s.',
    },
  ]}
/>
